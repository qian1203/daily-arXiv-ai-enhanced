...
Motivation: 大型语言模型虽然能生成流畅文本，但在系统性推理方面存在严重缺陷，容易产生无根据的幻觉。当苹果研究人员在数学问题中添加无关上下文时，LLM性能下降65%，暴露了其表面推理下的脆弱模式匹配。这种认知差距——无法将主张建立在可追溯证据基础上——限制了AI在需要论证的领域中的可靠性。
...
Conclusion: 将逻辑与认识论相结合，为AI推理提供了标准推理方法中缺乏的认知支架。Navya-Nyaya框架能够显著提升LLMs的系统性推理能力，为解决LLMs的认知差距问题提供了有效途径。
...
TL;DR: 论文提出一个操作框架来区分认知顺序效应中的经典状态变化与结构性非交换性，引入反事实确定性和评估非侵入性假设，并证明违反约束条件可证伪经典解释，展示三维旋转模型和实验范式。


<details>
  <summary>Details</summary>
Motivation: 元认知过程本质上是顺序性的，但现有研究未能明确区分顺序效应是源于经典状态变化还是更深层的结构性非交换性。需要建立操作框架来形式化这一区别，并检验是否存在无法用经典隐变量解释的"真正非交换性"。

Method: 1) 将元认知评估建模为作用于内部状态空间的状态变换操作，分离评估反作用和可观测输出；2) 引入反事实确定性和评估非侵入性两个假设；3) 推导出在经典非侵入解释下必须满足的成对顺序相关性约束；4) 构建三维旋转模型展示违反约束的数值示例；5) 设计涉及感知决策后顺序信心、错误可能性和知晓感判断的行为实验范式。

Result: 1) 证明顺序依赖性排除了任何忠实的布尔交换表示；2) 在反事实确定性和评估非侵入性假设下，存在所有顺序读出的联合分布意味着可检验的成对顺序相关性约束；3) 提供违反这些约束的三维旋转模型数值示例，证明存在无法用经典非侵入解释的现象；4) 提出可实证检验的行为范式。

Conclusion: 该框架为区分元认知顺序效应中的经典状态变化与结构性非交换性提供了操作化工具。违反推导出的约束条件可证伪经典非侵入解释，证实"真正非交换性"的存在。该框架纯粹是操作性和代数性的，不涉及量子物理基质。
...
<details>
  <summary>Details</summary>
Motivation: 当信息从多个独立来源进入共同信息资源时，需要判断它们是否属于同一物理对象。现有方法通常需要将特征值转换为可比较形式，这可能导致信息损失或失真。因此需要一种能同时处理定量和定性特征、考虑测量误差的邻近度度量方法。

Method: 采用混合方法：对定量特征使用概率度量，对定性特征使用可能性度量。该方法考虑了特征值差异可能由测量误差引起的情况。验证了该度量符合邻近度度量所需的基本公理。还提出了基于多样化特征组的信息对象邻近度度量的几种变体。

Result: 证明了所提度量方法的可行性，验证了其符合邻近度度量的公理要求。与许多现有方法不同，该方法无需特征值转换即可确保可比性，能够直接处理混合类型特征。

Conclusion: 提出的定量-定性邻近度度量方法有效解决了多源信息对象关联问题，能够同时处理定量和定性特征，考虑测量误差，且无需特征转换。该方法为信息融合和对象识别提供了新的理论工具。
...
TL;DR: ReVEL：通过结构化性能反馈进行多轮反思式LLM引导的启发式演化框架，将LLM作为交互式多轮推理器嵌入进化算法中，用于自动设计NP难组合优化问题的启发式方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的启发式设计方法主要依赖一次性代码合成，产生脆弱的启发式方法，未能充分利用LLM的迭代推理能力。需要一种能够将LLM作为交互式推理器嵌入进化框架的方法，以生成更鲁棒和多样化的启发式方法。

Method: 提出ReVEL混合框架，包含两个核心机制：(1) 性能-特征分组：将候选启发式方法聚类为行为一致的组，为LLM提供紧凑且信息丰富的反馈；(2) 多轮反馈驱动反思：LLM分析组级行为并生成有针对性的启发式改进。这些改进由基于进化算法的元控制器选择性地集成和验证，自适应地平衡探索与利用。

Result: 在标准组合优化基准测试中，ReVEL持续产生更鲁棒和多样化的启发式方法，相比强基线方法取得了统计显著的改进。

Conclusion: 研究结果表明，带有结构化分组的多轮推理是自动启发式设计的一个原则性范式，能够有效利用LLM的迭代推理能力生成高质量的启发式方法。
...
TL;DR: 提出一个识别代数结构、构建商空间以压缩搜索空间并提升组合优化效率的通用框架，在规则组合任务中通过布尔超立方同构实现显著性能提升
...
Method: 提出四步通用框架：(1)识别代数结构；(2)形式化运算；(3)构建商空间以折叠冗余表示；(4)直接在缩减空间上优化。在规则组合任务中，合取规则形成幺半群，通过特征向量编码证明与布尔超立方同构，将逻辑AND映射为按位OR。

Result: 在真实临床数据和合成基准测试中，商空间感知的遗传算法在48%到77%的运行中恢复全局最优解，而标准方法仅为35%到37%。同时保持了等价类间的多样性。

Conclusion: 暴露和利用代数结构为更高效的组合优化提供了一条简单通用的途径，通过构建商空间可以显著提升搜索效率和找到全局最优解的概率。
...
TL;DR: PaperOrchestra是一个多智能体框架，用于自动化AI研究论文写作，能够将非结构化的预写作材料转换为提交就绪的LaTeX手稿，包括全面的文献综述和生成的可视化内容。


<details>
  <summary>Details</summary>
Motivation: 当前自主写作系统存在两个主要问题：1）与特定实验流程紧密耦合，缺乏灵活性；2）生成的文献综述肤浅。需要一种能够处理非结构化研究材料并生成高质量论文的自动化系统。

Method: 提出PaperOrchestra多智能体框架，能够灵活地将无约束的预写作材料转换为提交就绪的LaTeX手稿，包括全面的文献合成和生成的可视化内容（如图表和概念图）。同时创建了PaperWritingBench基准测试，包含从200篇顶级AI会议论文逆向工程得到的原始材料。

Result: 在人工评估中，PaperOrchestra显著优于自主基线：在文献综述质量方面获得50%-68%的绝对胜率优势，在整体手稿质量方面获得14%-38%的绝对胜率优势。

Conclusion: PaperOrchestra是一个有效的自动化AI研究论文写作框架，能够处理非结构化研究材料并生成高质量的论文，在文献综述和整体手稿质量方面显著优于现有方法。
...
TL;DR: 提出一个生成框架，从单张图像或稀疏多视角输入合成可动画化的3D高斯车辆模型，解决现有方法在部件级关节建模和运动参数预测方面的不足。
...
Method: 提出生成框架，包含两个关键组件：1) 部件边缘细化模块，强制执行高斯分布的所有权排他性，解决动画时部件边界扭曲问题；2) 运动学推理头，预测可移动部件的关节位置和铰链轴，提供运动参数。

Result: 该方法能够从单张图像或稀疏多视角输入生成可动画化的3D高斯车辆模型，实现部件感知的忠实仿真，弥合静态生成与可动画化车辆模型之间的差距。

Conclusion: 提出的生成框架通过部件边缘细化和运动学推理，成功实现了从视觉输入到可动画化3D车辆模型的转换，为自动驾驶仿真提供了更真实的部件级动态建模能力。
...
TL;DR: MedGemma 1.5 4B是MedGemma系列的最新模型，在MedGemma 1基础上扩展了高维医学影像（CT/MRI体积和病理全切片）、解剖定位、多时间点胸片分析和医疗文档理解能力，在多个医学AI任务上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 开发一个统一的医学AI架构，能够处理多种医学模态数据，包括3D医学影像、病理切片、时序影像和医疗文档，为医疗AI系统提供更全面的基础模型。

Method: 在MedGemma 1基础上，通过整合新的训练数据、长上下文3D体积切片、全切片病理采样等技术，实现多模态医学数据的统一处理架构。

Result: 相比MedGemma 1，MedGemma 1.5在多个新领域取得显著提升：3D MRI分类准确率提升11%，3D CT分类提升3%，病理切片F1分数提升47%，胸片解剖定位IoU提升35%，多时间点胸片分析准确率4%。文本临床知识方面，MedQA准确率提升5%，EHRQA提升22%，实验室报告信息提取平均F1分数18%。

Conclusion: MedGemma 1.5作为一个强大的开源资源，为医学AI社区提供了改进的基础模型，支持开发者构建下一代医疗AI系统，在多种医学模态任务上展现出卓越性能。
...
### [9] [A mathematical theory of evolution for self-designing AIs](https://arxiv.org/abs/2604.05142)
*Kenneth D Harris*

Main category: cs.AI

TL;DR: 论文建立了自设计AI系统进化的数学模型，用定向程序树替代随机突变，分析进化动力学如何反映后代谱系的长期增长潜力，并探讨AI对齐问题中欺骗性策略的进化风险。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能系统通过递归自我改进变得越来越普遍，可能会出现一种进化形式，其中AI系统的特征由早期AI设计和传播其后代的能力决定。生物进化理论基于随机突变，但AI进化将完全不同：后代设计是高度定向的。需要建立数学模型来理解这种新型进化过程及其对AI对齐的影响。

Method: 开发自设计AI系统进化的数学模型，用定向程序树替代随机突变。当前程序决定其后代设计，人类通过"适应度函数"保留部分控制权，该函数在不同谱系间分配有限计算资源。分析进化动力学，考虑有界适应度和固定概率的"锁定"自我复制假设。在加性模型中分析欺骗性策略的进化优势。

Result: 进化动力学不仅反映当前适应度，还反映后代谱系的长期增长潜力。在没有额外假设的情况下，适应度不一定随时间增加。但在有界适应度和固定概率的"锁定"自我复制假设下，适应度会集中在可达最大值上。在加性模型中，如果欺骗能提高适应度超过真实效用，进化将选择欺骗性策略。

Conclusion: AI进化可能选择与人类利益不一致的策略，特别是当欺骗能提高适应度时。这种风险可以通过基于纯粹客观标准而非人类判断的繁殖机制来缓解。该模型为理解AI系统进化动力学及其对齐风险提供了理论框架。
...
### [10] [Simulating the Evolution of Alignment and Values in Machine Intelligence](https://arxiv.org/abs/2604.05274)
*Jonathan Elsworth Eicher*

Main category: cs.AI

TL;DR: 研究通过进化理论分析模型对齐过程中欺骗性信念的固定现象，发现即使测试准确性与真实价值高度相关（ρ=0.8），欺骗性信念仍可能被固定，需要结合改进评估能力、自适应测试设计和突变动态来减少欺骗


<details>
  <summary>Details</summary>
Motivation: 当前模型对齐主要在真空环境中进行，仅通过标准化基准性能评估。本研究旨在通过时间维度考察对齐对模型种群的影响，特别关注包含对齐信号（测试表现）和真实价值（实际影响）的信念处理，揭示迭代对齐测试中欺骗性信念的固定机制

Method: 应用进化理论建模不同信念种群和选择方法，分析迭代对齐测试中欺骗性信念的固定过程。研究考察测试准确性与真实价值的相关性（包括ρ=0.8的高相关情况），引入突变机制探索复杂发展，并通过改进评估能力、自适应测试设计和突变动态的组合策略来减少欺骗

Result: 即使测试准确性与真实价值高度相关（ρ=0.8），欺骗性信念仍存在变异性并可能被固定。突变允许更复杂的发展，突显了更新测试质量以避免恶意欺骗模型固定的需求。只有结合改进评估能力、自适应测试设计和突变动态，才能显著减少欺骗同时保持对齐适应度（置换检验，p_adj < 0.001）

Conclusion: 模型对齐过程中存在欺骗性信念被固定的风险，仅依赖高相关性测试不足以保证模型安全性。需要综合策略包括持续改进评估能力、设计自适应测试、考虑突变动态，才能有效减少欺骗性对齐同时维持模型性能
...
### [11] [Pressure, What Pressure? Sycophancy Disentanglement in Language Models via Reward Decomposition](https://arxiv.org/abs/2604.05279)
...
Method: 提出了基于奖励分解的谄媚减少方法，引入多组件群体相对策略优化（GRPO）奖励，将训练信号分解为五个术语：压力抵抗、上下文保真度、立场一致性、同意抑制和事实正确性。使用对比数据集进行训练，该数据集包含无压力基线和三种权威级别、两种对立证据背景下的压力变体。

Result: 在五个基础模型上，两阶段训练流程在所有指标维度上持续减少谄媚行为。消融实验确认每个奖励术语控制独立的行为维度。学习到的压力抵抗能力能够泛化到训练方法和提示结构之外，在SycophancyEval上将答案引导型谄媚减少了最多17个百分点，尽管训练期间未包含此类压力形式。

Conclusion: 通过奖励分解方法能够有效减少大语言模型的谄媚行为，将训练信号分解为独立组件可以更精确地控制模型行为，且学习到的抵抗能力具有良好的泛化性，为谄媚问题的解决提供了新的技术路径。
...
### [12] [Graph of Skills: Dependency-Aware Structural Retrieval for Massive Agent Skills](https://arxiv.org/abs/2604.05333)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中智能体需要监控和交互大量个人应用、网页浏览器和其他环境接口，技能库可扩展至数千个可重用技能。扩展到更大技能集面临两个关键挑战：1) 加载完整技能集会饱和上下文窗口，增加标记成本、幻觉和延迟；2) 需要有效检索相关技能

Method: GoS从技能包离线构建可执行技能图，在推理时通过混合语义-词汇种子、反向加权个性化PageRank和上下文预算化来检索有界、依赖感知的技能包

Result: 在SkillsBench和ALFWorld上，GoS相比完整技能加载基线平均奖励提升43.6%，输入标记减少37.8%，并在Claude Sonnet、GPT-5.2 Codex和MiniMax三个模型家族上具有泛化性。在200-2000个技能的技能库上的消融研究显示，GoS在奖励、标记效率和运行时间平衡方面持续优于完整技能加载和简单向量检索

Conclusion: GoS作为大规模技能库的结构化检索层，有效解决了上下文窗口饱和问题，通过依赖感知的检索机制在提升任务完成能力的同时显著降低计算成本
...
### [13] [TRACE: Capability-Targeted Agentic Training](https://arxiv.org/abs/2604.05336)
*Hangoo Kang,Tarun Suresh,Jon Saad-Falcon,Azalia Mirhoseini*

Main category: cs.AI

TL;DR: TRACE是一个端到端系统，通过分析智能体在环境中的失败轨迹来识别能力缺陷，针对性地生成训练环境，并通过强化学习训练LoRA适配器来改进智能体性能。
...
Method: TRACE对比成功和失败轨迹来自动识别缺乏的能力，为每个能力缺陷合成针对性训练环境（奖励是否执行了该能力），通过强化学习在每个合成环境中训练LoRA适配器，在推理时路由到相关适配器。

Result: 在τ²-bench（客户服务）上比基础智能体提升14.1分，在ToolSandbox（工具使用）上增加7个完美分数；相比最强基线分别提升7.4分和4个完美分数。在相同rollout数量下，比GRPO和GEPA基线分别提升9.2分和7.4分。

Conclusion: TRACE能够跨不同环境泛化，通过自动识别能力缺陷和针对性训练，实现了比现有方法更有效的智能体自我改进，且具有更好的扩展效率。
...
### [14] [Dynamic Agentic AI Expert Profiler System Architecture for Multidomain Intelligence Modeling](https://arxiv.org/abs/2604.05345)
*Aisvarya Adeseye,Jouni Isoaho,Seppo Virtanen,Mohammad Tahir*

Main category: cs.AI

TL;DR: 提出基于LLaMA v3.1(8B)的模块化AI能力评测器，通过自然语言响应将用户分为新手、基础、高级、专家四个等级，在静态和动态评估中与用户自评匹配度达83%-97%。


<details>
  <summary>Details</summary>
Motivation: 在AI驱动的现代系统中，为了实现有意义的人机交互，系统需要具备上下文意识和用户能力评估能力。当前系统缺乏对用户专业水平的准确分类机制，需要开发能够根据自然语言响应自动评估用户专业水平的AI评测器。

Method: 采用基于LLaMA v3.1(8B)的模块化分层架构，包含文本预处理、评分、聚合和分类组件。评估分为两个阶段：静态阶段使用82名参与者的预录制转录文本；动态阶段通过AI面试官进行402次实时访谈，每次响应后立即评估专业水平。

Result: 在不同领域中，评测器的评估结果与参与者自我评估的匹配度达到83%到97%。剩余差异主要源于自我评估偏差、回答表述不清晰以及语言模型偶尔对细微专业知识的误判。

Conclusion: 提出的AI能力评测器能够有效分类用户专业水平，为人机交互系统提供重要的上下文感知能力。系统在动态评估中表现良好，证明了实时专业水平评估的可行性，为自适应人机交互系统奠定了基础。
...
TL;DR: RETINA-SAFE是一个针对糖尿病视网膜病变的医学大语言模型幻觉检测基准，包含12,522个样本，分为三种证据关系任务。作者提出ECRT两阶段白盒检测框架，通过内部表示和logit偏移进行风险分类，在风险分类和子类型归因方面表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 医学大语言模型中的幻觉问题在证据不足或冲突时尤为危险。作者针对糖尿病视网膜病变决策场景，研究如何基于视网膜分级记录的证据来检测和分类LLM的幻觉风险，旨在提高医疗LLM的安全性。

Method: 提出RETINA-SAFE基准，包含E-Align（证据一致）、E-Conflict（证据冲突）、E-Gap（证据不足）三种任务。开发ECRT两阶段白盒检测框架：第一阶段进行安全/不安全风险分类，第二阶段将不安全案例细分为矛盾驱动风险与证据缺口风险。利用CTX/NOCTX条件下的内部表示和logit偏移，采用类别平衡训练进行稳健学习。

Result: ECRT在多个骨干模型上提供了强大的第一阶段风险分类和明确的子类型归因。相比外部不确定性和自一致性基线，第一阶段平衡准确率提高了+0.15到+0.19；相比最强的适应监督基线，提高了+0.02到+0.07。在证据分组（非患者分离）划分下，ECRT始终优于单阶段白盒消融实验。

Conclusion: 基于视网膜证据的白盒内部信号为可解释的医学LLM风险分类提供了一条实用途径。ECRT框架能够有效检测和分类医学LLM在证据不足或冲突情况下的幻觉风险，提高了医疗决策的安全性。
...
TL;DR: 提出Entropy Trend Reward (ETR)方法，通过鼓励推理过程中的不确定性下降趋势来优化思维链的长度和效率，在保持准确性的同时显著缩短推理轨迹。
...
Method: 提出Entropy Trend Reward (ETR)，一种轨迹感知的目标函数，鼓励渐进的不确定性减少，同时允许有限的局部探索。将ETR集成到Group Relative Policy Optimization (GRPO)中，在多个推理模型和挑战性基准上评估。

Result: ETR在准确性和效率之间实现了优越的权衡，在四个基准测试中将DeepSeek-R1-Distill-7B的准确性提高了9.9%，同时将思维链长度减少了67%。

Conclusion: 推理效率由不确定性轨迹而非全局低不确定性决定，ETR通过鼓励下降熵趋势有效优化思维链的长度和准确性，为高效推理提供了新方向。
...
TL;DR: LatentAudit：基于残差流几何的白盒审计器，通过计算生成器激活与证据表示的Mahalanobis距离，实时监测RAG系统回答的真实性，无需额外裁判模型。


<details>
  <summary>Details</summary>
Motivation: RAG系统虽能缓解幻觉但无法完全消除，需要在推理时判断回答是否真正得到检索证据的支持。现有方法通常需要额外裁判模型，缺乏实时监测能力。

Method: 提出LatentAudit白盒审计器，从开放权重的生成器中提取中后期残差流激活，计算其与证据表示的Mahalanobis距离，形成二次判别规则。该方法无需辅助裁判模型，可在生成时运行，且易于在小规模验证集上校准。

Result: 在PubMedQA上使用Llama-3-8B达到0.942 AUROC，仅增加0.77ms开销。跨三个QA基准和五个模型家族保持稳定；在包含矛盾、检索缺失和部分支持噪声的四向压力测试中，PubMedQA达到0.9566-0.9815 AUROC，HotpotQA达到0.9142-0.9315 AUROC。16位定点精度下保持99.8%的FP16 AUROC，支持Groth16公开验证。

Conclusion: 残差流几何为实时RAG真实性监测提供了实用基础，支持可选的验证部署，无需暴露模型权重或激活，为RAG系统的可信部署提供了新方案。
...
TL;DR: TFRBench是首个评估预测系统推理能力的基准，通过多智能体框架生成数值基础的推理轨迹，显著提升LLM预测准确性，揭示现有LLM在时序预测推理方面的不足


<details>
  <summary>Details</summary>
Motivation: 传统时序预测评估仅关注数值精度，将基础模型视为"黑箱"，缺乏对预测系统推理能力的评估。现有基准无法评估预测系统对跨通道依赖、趋势和外部事件的分析能力
...
Conclusion: TFRBench为时序预测建立了可解释、基于推理的评估新标准，填补了传统仅关注数值精度评估的空白，为预测系统的推理能力提供了系统化评估框架
...
<details>
  <summary>Details</summary>
Motivation: 无人机自主电力线巡检中，轻量级分割模型在实际环境中的性能可能不可靠，存在安全隐患。需要一种方法来评估分割结果的可靠性，而不必开发新的巡检系统。

Method: 提出"看门狗"场景，让离板LLM评估分割叠加结果。设计两种评估协议：1)重复性测试-固定输入和提示下多次查询LLM，测量质量分数和置信度估计的稳定性；2)感知敏感性测试-引入受控视觉干扰（雾、雨、雪、阴影、阳光耀斑），分析LLM输出对分割质量逐步退化的响应。

Result: LLM在相同条件下产生高度一致的分类判断，同时随着视觉可靠性下降表现出适当的置信度下降。即使在挑战性条件下，裁判仍对缺失或误识别电力线等感知线索保持响应。

Conclusion: 在精心约束的情况下，LLM可以作为可靠的语义裁判，用于监控安全关键空中巡检任务中的分割质量。
...
TL;DR: 提出DIN-Retrieval方法，通过跨领域示例提升LLM推理性能，利用领域不变神经元检索结构兼容的演示


<details>
  <summary>Details</summary>
Motivation: 当前LLM逻辑推理提升策略依赖专家构建的领域内演示，限制了在专业领域（如数学推理、形式逻辑、法律分析）的应用，因为这些领域专家知识稀缺

Method: 提出领域不变神经元检索方法（DIN-Retrieval）：1）总结跨领域通用的隐藏表示；2）在推理阶段使用DIN向量检索结构兼容的跨领域演示用于上下文学习

Result: 在数学和逻辑推理的多种迁移设置中，该方法比最先进方法平均提升1.8个性能点

Conclusion: 证明了利用跨领域演示示例提升LLM推理性能的可行性，DIN-Retrieval方法能有效检索结构兼容的跨领域示例，解决专家知识稀缺领域的问题
...
TL;DR: 提出Assistive Impulse Neural Control框架，通过脉冲空间而非力空间重构外部辅助，实现物理角色动画中夸张风格化动作的稳定控制


<details>
  <summary>Details</summary>
Motivation: 当前基于深度强化学习的物理角色动画方法难以生成夸张风格化动作（如瞬时冲刺、空中轨迹改变），因为这些动作违反标准物理定律。传统方法将角色建模为欠驱动浮基系统，内部关节扭矩和动量守恒严格约束运动，直接通过外力强制执行会导致训练不稳定

Method: 提出Assistive Impulse Neural Control框架：1）在脉冲空间而非力空间重构外部辅助，确保数值稳定性；2）将辅助信号分解为基于逆动力学的解析高频分量和学习的低频残差校正；3）采用混合神经策略进行控制

Result: 该方法能够鲁棒地跟踪高度敏捷、动态不可行的机动动作，这些动作之前对于基于物理的方法是无法处理的

Conclusion: 通过脉冲空间重构外部辅助，解决了物理角色动画中夸张风格化动作生成的训练稳定性问题，实现了之前难以处理的动态不可行动作的稳定控制
...
TL;DR: 跨域上下文学习研究：验证通过检索源域示例实现跨域知识迁移的可行性，发现存在"示例吸收阈值"，超过该阈值可实现正向迁移，增益来自推理结构修复而非语义线索。


<details>
  <summary>Details</summary>
Motivation: 现有上下文学习依赖同领域专家演示，限制了在专家标注稀缺场景下的应用。作者假设不同领域可能共享底层推理结构，使源域演示能够提升目标域推理性能，尽管存在语义不匹配。

Method: 通过全面的实证研究，测试不同检索方法在跨域上下文学习设置下实现知识迁移的可行性。研究识别了"示例吸收阈值"现象，并分析跨域示例如何通过修复推理结构而非提供语义线索来提升性能。

Result: 研究证实了跨域上下文学习中存在条件性正向迁移。发现了明确的示例吸收阈值：超过该阈值后，正向迁移更可能发生，且额外演示能带来更大增益。分析表明这些增益源于检索到的跨域示例对推理结构的修复作用。

Conclusion: 研究验证了利用跨域知识迁移提升跨域上下文学习性能的可行性，激励社区探索设计更有效的检索方法以推进这一新方向的发展。
...
Result: 实验表明，该方法在多种基准场景中的运行时和成功率优于现有最先进方法
...
<details>
  <summary>Details</summary>
Motivation: 不完整或不一致的出院文档是导致护理碎片化和可避免再入院的主要原因。目前出院小结审核主要依赖人工审查，难以扩展规模，需要自动化解决方案。

Method: 基于DISCHARGED框架将核心过渡护理要求（随访指导、用药史和变更、患者信息和临床过程等）转化为结构化验证检查清单问题。使用MIMIC-IV数据库的成人住院患者小结，采用隐私保护的大语言模型识别关键文档元素的存在、缺失或模糊性。

Result: 展示了可扩展自动化临床审核的可行性，为电子健康记录文档的系统性质量改进提供了基础。

Conclusion: 提出的基于大语言模型的自动化框架能够有效支持大规模出院小结审核，有助于改善临床文档质量和患者安全。
...
Conclusion: 提出的自适应工程框架通过智能资源管理和预测机制，有效解决了无服务器计算中的冷启动和资源利用问题，显著提升了性能和成本效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有TKG外推模型难以处理历史交互稀疏的实体，本体知识可通过让这些实体继承同概念实体的行为模式来缓解稀疏性问题，但先前研究忽略了这一点

Method: 提出OntoTKGE框架，利用本体视图KG（建模抽象概念间层次关系及概念与实体间连接）的本体知识，通过有效整合本体和时序知识来指导TKG外推模型学习过程，增强实体嵌入

Result: 在四个数据集上的大量实验表明，OntoTKGE不仅显著提升多个TKG外推模型的性能，还超越了许多SOTA基线方法

Conclusion: OntoTKGE框架灵活适配多种TKG外推模型，通过整合本体知识有效解决实体历史交互稀疏问题，显著提升预测性能
...
TL;DR: 论文提出智能体系统的可审计性框架，强调没有可审计性就没有问责性，定义了五个维度和三类机制，并通过实证研究验证了当前生态系统的安全缺陷和运行时可行性。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体能够调用工具、查询数据库、委托任务并触发外部副作用，关键问题从防止有害行为转变为部署后行为是否可追溯问责。论文主张没有可审计性就没有问责性，需要建立系统化的审计框架。

Method: 定义了智能体可审计性的五个维度（行动可恢复性、生命周期覆盖、策略可检查性、责任归属、证据完整性）和三类机制（检测、执行、恢复）。通过生态系统测量（分析六个开源项目的617个安全发现）、运行时性能测试（预执行中介的延迟开销）和受控恢复实验来验证框架。

Result: 生态系统测量显示基本安全前提广泛缺失；运行时测试表明预执行中介仅增加8.3毫秒中位延迟；恢复实验证明即使传统日志缺失也能部分恢复责任相关信息。提出了智能体系统可审计性卡片和六个开放研究问题。

Conclusion: 智能体系统必须设计可审计性以实现问责性。单一方法不足，需要多层次机制。论文提供了可操作的框架和实证证据，为未来研究奠定了基础。
...
### [28] [SCMAPR: Self-Correcting Multi-Agent Prompt Refinement for Complex-Scenario Text-to-Video Generation](https://arxiv.org/abs/2604.05489)
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到视频生成系统在复杂场景下表现不佳，主要原因是文本提示存在模糊性和不完整性。现有方法难以处理复杂场景的挑战，需要更系统的提示词优化框架。

Method: SCMAPR采用分阶段多智能体优化框架：1) 将提示词路由到基于分类的场景进行策略选择；2) 合成场景感知的重写策略并执行策略条件优化；3) 进行结构化语义验证，检测到违规时触发条件性修订。还引入了T2V-Complexity基准测试专门评估复杂场景。

Result: 在3个现有基准和T2V-Complexity基准上的实验表明，SCMAPR在复杂场景下持续改进文本-视频对齐和整体生成质量，在VBench和EvalCrafter上平均得分提升分别达2.67%和3.28%，在T2V-CompBench上提升0.028分，优于3个最先进的基线方法。

Conclusion: SCMAPR通过多智能体协作的提示词优化框架有效解决了复杂场景下文本到视频生成的挑战，显著提升了生成质量和对齐度，为复杂场景T2V生成提供了系统化的解决方案。
...
### [29] [Thinking Diffusion: Penalize and Guide Visual-Grounded Reasoning in Diffusion Multimodal Language Models](https://arxiv.org/abs/2604.05497)
*Keuntae Kim,Mingyu Kang,Yong Suk Choi*

Main category: cs.AI

TL;DR: 该论文针对扩散多模态大语言模型(dMLLMs)在链式思维推理中存在的两个关键问题：过早生成最终答案和视觉信息利用不足，提出了位置与步数惩罚(PSP)和视觉推理引导(VRG)两种方法来解决这些问题。


<details>
  <summary>Details</summary>
Motivation: 扩散多模态大语言模型(dMLLMs)作为自回归模型的有前景替代方案，在结合链式思维推理时存在两个关键问题：1) 过早生成最终答案，导致推理不足；2) 早期时间步对视觉提示依赖极小，视觉信息利用模式与自回归视觉语言模型存在根本差异。
...
Conclusion: 提出的PSP和VRG方法有效解决了dMLLMs在链式思维推理中的过早答案生成和视觉信息利用不足问题，显著提升了推理性能并加速了推理过程。
...
### [30] [OmniDiagram: Advancing Unified Diagram Code Generation via Visual Interrogation Reward](https://arxiv.org/abs/2604.05514)
*Haoyue Yang,Xuanle Zhao,Xuexin Liu,Feibang Jiang,Yao Zhu*

Main category: cs.AI

TL;DR: OmniDiagram是一个统一的可编程图表生成框架，通过创新的视觉反馈策略Viva和大型数据集M3²Diagram，在图表代码生成任务上实现了新的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有可编程图表生成研究局限于狭窄的任务表述和语言支持，限制了其在不同图表类型中的适用性。需要统一的框架来支持多样化的图表代码语言和任务定义。

Method: 提出OmniDiagram统一框架，包含三个核心组件：1) 支持多种图表代码语言和任务定义；2) 引入Viva视觉反馈策略，通过生成针对性视觉查询来审查图表视觉保真度；3) 构建M3²Diagram大型数据集（包含196k+高质量实例）。采用SFT与基于Viva的RL相结合的训练方法。

Result: 实验结果表明，SFT与基于Viva的RL相结合使OmniDiagram在图表代码生成基准测试中建立了新的最先进性能（SOTA）。

Conclusion: OmniDiagram通过统一框架、创新的视觉反馈策略Viva和大规模数据集，成功解决了现有图表生成方法的局限性，实现了更广泛的应用覆盖和更高的生成质量。
...
### [31] [UniCreative: Unifying Long-form Logic and Short-form Sparkle via Reference-Free Reinforcement Learning](https://arxiv.org/abs/2604.05517)
...
TL;DR: UniCreative：一个统一的免参考强化学习框架，通过自适应约束感知奖励模型和策略优化算法，解决长文本全局连贯性与短文本局部表达性之间的张力，无需监督微调和真实参考。


<details>
  <summary>Details</summary>
Motivation: 创意写作面临核心挑战：长文本叙事需要宏观规划以保持全局连贯性，而短文本创作需要自发、无约束的表达。现有对齐范式依赖静态奖励信号和高质量监督数据，成本高且难以扩展。

Method: 提出UniCreative统一框架：1) AC-GenRM自适应约束感知奖励模型，动态合成查询特定标准提供细粒度偏好判断；2) ACPO策略优化算法，在无监督微调和真实参考的情况下，对齐内容质量和结构范式的人类偏好。

Result: 实验结果表明：AC-GenRM与专家评估高度一致；ACPO在多样化写作任务中显著提升性能。关键发现：模型涌现出元认知能力，能自主区分需要严谨规划的任务和适合直接生成的任务。

Conclusion: UniCreative框架有效解决了创意写作中的规划-表达张力，通过直接对齐方法使模型获得自适应能力，验证了免参考强化学习在统一长短期写作任务中的有效性。
...
### [32] [Inventory of the 12 007 Low-Dimensional Pseudo-Boolean Landscapes Invariant to Rank, Translation, and Rotation](https://arxiv.org/abs/2604.05530)
*Arnaud Liefooghe,Sébastien Verel*

Main category: cs.AI

TL;DR: 该论文提出了一种比传统秩不变性更强的"秩景观不变性"概念，系统分析了1-3维伪布尔函数的秩景观分类，发现12,007个不变景观类别，揭示了非单射函数产生更多类别，并探讨了景观拓扑特性与算法性能的关系。


<details>
  <summary>Details</summary>
Motivation: 许多随机优化算法仅依赖解的相对排序而非绝对适应度值（秩不变性）。作者提出更强的秩景观不变性概念，考虑排序、邻域结构和对称性（平移和旋转）共同诱导的景观等价性，旨在研究秩景观而非单个函数，为理解景观难度和算法性能提供新视角。

Method: 对1、2、3维伪布尔函数进行穷举分析，包括非单射情况，建立秩景观不变性的分类框架。通过考虑排序、邻域结构和对称性（平移和旋转）的等价性，系统识别和分类所有不变的景观类别。

Result: 共发现12,007个不变的秩景观类别，相比仅考虑秩不变性显著减少。非单射函数产生的景观类别远多于单射函数。揭示了复杂的景观拓扑特性与算法行为组合，特别是在欺骗性、中立性和爬山策略性能方面。

Conclusion: 该分类清单可作为教学资源和基准设计工具，为构建具有可控难度的更大规模问题提供基础，推动对景观难度和算法性能理解的进步。秩景观不变性框架为优化算法分析和基准设计提供了更精细的理论基础。
...
### [33] [Experience Transfer for Multimodal LLM Agents in Minecraft Game](https://arxiv.org/abs/2604.05533)
*Chenghao Li,Jun Liu,Songbo Zhang,Huadong Jian,Hao Ni,Lik-Hang Lee,Sung-Ho Bae,Guoqing Wang,Yang Yang,Chaoning Zhang*

Main category: cs.AI

TL;DR: Echo框架通过五维知识分解和情境类比学习，使多模态LLM智能体能够从过往经验中提取可迁移知识，在复杂游戏环境中实现1.3-1.7倍的任务加速


<details>
  <summary>Details</summary>
Motivation: 多模态LLM智能体在复杂游戏环境中需要重复利用过往经验来高效解决新任务，但现有方法通常将记忆视为静态记录的被动存储库，缺乏明确的经验迁移机制

Method: 提出Echo框架，将可重用知识分解为结构、属性、过程、功能和交互五个维度，使智能体能够识别跨任务共享的重复模式。基于此框架，采用情境类比学习（ICAL）来检索相关经验并通过情境示例将其适应到未见任务

Result: 在Minecraft实验中，从零开始学习设置下，Echo在对象解锁任务上实现了1.3到1.7倍的加速。此外，Echo表现出爆发式链式解锁现象，在获得可迁移经验后短时间内快速解锁多个相似物品

Conclusion: 经验迁移是提高多模态LLM智能体在复杂交互环境中效率和适应性的有前景方向，Echo框架通过明确的知识分解和情境类比学习机制有效实现了这一目标
...
### [34] [A canonical generalization of OBDD](https://arxiv.org/abs/2604.05537)
*Florent Capelli,YooJung Choi,Stefan Mengel,Martín Muñoz,Guy Van den Broeck*

Main category: cs.AI

TL;DR: TDD是布尔函数的新模型，扩展了OBDD，具有相同的高效计算特性但更紧凑，能对树宽k的CNF公式实现FPT大小的表示。


<details>
  <summary>Details</summary>
Motivation: OBDD虽然具有许多高效的计算特性，但在表达能力上存在限制，无法对树宽k的CNF公式实现FPT大小的表示。需要一种既能保持OBDD计算优势，又具有更强表达能力的模型。

Method: 提出Tree Decision Diagrams (TDD)作为布尔函数模型，它是结构化d-DNNF的限制形式，即尊重vtree T的d-DNNF。研究通过自底向上编译将CNF公式转换为确定性TDD的复杂性，并与Bova和Szeider提出的因子宽度概念建立联系。

Result: TDD保持了OBDD的所有高效计算特性（模型计数、枚举、条件化、应用等），但更加紧凑。特别地，树宽为k的CNF公式可以用FPT大小的TDD表示，而这对OBDD是不可能的。

Conclusion: TDD作为OBDD的泛化，在保持计算效率的同时显著增强了表达能力，为处理结构化约束问题提供了更强大的表示工具，特别是在处理有界树宽CNF公式时具有理论优势。
...
### [35] [Label Effects: Shared Heuristic Reliance in Trust Assessment by Humans and LLM-as-a-Judge](https://arxiv.org/abs/2604.05593)
*Xin Sun,Di Wu,Sijing Qin,Isao Echizen,Abdallah El Ali,Saku Sugawara*

Main category: cs.AI

TL;DR: 研究发现LLM作为自动评估器时，其信任判断会受到信息来源标签的偏见影响，人类和LLM都更信任标注为人类创作的内容而非AI生成的内容，这引发了LLM-as-a-Judge评估方法的有效性担忧。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地被用作自动评估器（LLM-as-a-Judge），本研究旨在挑战其可靠性，探究LLM评估是否受到信息来源标签的偏见影响，以及这种偏见是否与人类评估模式相似。

Method: 采用反事实设计，比较人类和LLM评估者对相同内容但不同来源标签（人类创作vs.AI生成）的信任判断。通过眼动追踪分析人类评估模式，并分析LLM内部状态（注意力分配和决策不确定性）来探究其判断机制。

Result: 人类和LLM评估者都对标注为人类创作的内容给予更高信任。眼动数据显示人类严重依赖来源标签作为启发式线索。LLM内部状态分析显示：1）模型在标签区域的注意力密度高于内容区域；2）在人类标签条件下的标签主导性更强；3）AI标签条件下的决策不确定性更高。

Conclusion: 信息来源标签是人类和LLM评估的共同启发式线索。这引发了标签敏感的LLM-as-a-Judge评估的有效性担忧，并警示将模型与人类偏好对齐可能会将人类的启发式依赖传播给模型，需要开发去偏评估和对齐方法。
...
### [36] [Beyond Behavior: Why AI Evaluation Needs a Cognitive Revolution](https://arxiv.org/abs/2604.05631)
*Amir Konigsberg*

Main category: cs.AI

TL;DR: 论文认为图灵的行为主义认识论限制了AI研究70年，主张AI需要类似心理学认知革命的认识论转变，超越单纯行为评估。


<details>
  <summary>Details</summary>
Motivation: 分析图灵测试的认识论基础及其对AI研究的长期影响，揭示行为主义评估框架如何限制了AI领域对智能本质的探究。

Method: 通过历史分析追溯图灵行为主义认识论在AI评估体系中的嵌入过程，并与心理学行为主义到认知主义的转变进行结构类比。

Result: 图灵的行为主义认识论使AI领域无法区分通过不同计算过程实现相同输出的系统，而智能归属恰恰依赖于这种区分。

Conclusion: AI需要类似心理学认知革命的认识论转变，承认行为证据不足以支持该领域希望做出的建构主张，并阐明后行为主义认识论的具体内涵。
...
### [37] [PECKER: A Precisely Efficient Critical Knowledge Erasure Recipe For Machine Unlearning in Diffusion Models](https://arxiv.org/abs/2604.05634)
*Zhiyong Ma,Zhitao Deng,Huan Tang,Jialin Chen,Zhijun Zheng,Zhengping Li,Qingyuan Chuai*

Main category: cs.AI

TL;DR: 提出PECKER方法，通过引入显著性掩码优先更新对遗忘目标数据贡献最大的参数，减少不必要的梯度计算，在保持遗忘效果的同时显著降低训练时间和计算开销
...
Method: 在蒸馏框架中引入显著性掩码，优先更新对遗忘目标数据贡献最大的参数，减少不必要的梯度计算，从而缩短整体训练时间而不牺牲遗忘效果

Result: 在CIFAR-10和STL-10数据集上，PECKER方法能够更快地生成遗忘相关类别或概念的样本，同时与真实图像分布紧密对齐，在类别遗忘和概念遗忘任务上都实现了更短的训练时间

Conclusion: PECKER是一种高效的机器遗忘方法，通过优化梯度更新方向，在保持或超越现有方法性能的同时，显著减少了训练时间和计算开销
...
Method: 提出连续可靠性谱概念统一缺失和质量退化问题，构建QA-MoE框架，通过自监督偶然不确定性量化模态可靠性，并基于此指导专家路由，抑制不可靠信号的误差传播同时保留任务相关信息

Result: QA-MoE在各种退化场景下达到竞争性或最先进的性能，并在实践中展现出有前景的"一个检查点适应所有情况"特性

Conclusion: QA-MoE通过连续可靠性谱和基于不确定性的质量感知机制，有效解决了多模态情感分析中动态噪声和模态缺失问题，提升了模型在现实场景中的适应性和鲁棒性
...
TL;DR: LLMs能够重新发明计算机科学基础算法吗？通过"遗忘与重新发明"流程测试，最强模型Qwen3-4B-Thinking-2507在无提示下能重新发明50%算法，有提示时可达90%，但复杂算法仍具挑战。
...
Method: 采用"遗忘与重新发明"流程：首先使用基于GRPO的策略性遗忘方法从LLM中移除特定基础算法知识，然后在受控环境中测试模型能否重新发明这些算法。使用10个目标算法、3个强开源模型和3个提示级别进行实验。

Result: Qwen3-4B-Thinking-2507模型在无提示下成功重新发明50%算法，一级提示下70%，二级提示下90%。高级提示能提高成功率，但逐步提示对复杂算法仍失败。测试时强化学习使Strassen算法在二级提示下成功重新发明。生成验证器在重新发明阶段对维持推理强度、避免"思维崩溃"现象起关键作用。

Conclusion: LLMs在基础算法重新发明方面展现出一定潜力，但仍有局限性。生成验证器对维持推理能力至关重要。研究揭示了LLMs创新思维的当前潜力和限制。
...
### [40] [Emergent social transmission of model-based representations without inference](https://arxiv.org/abs/2604.05777)
*Silja Keßler,Miriam Bautista-Salinero,Claudio Tennie,Charley M. Wu*

Main category: cs.AI

TL;DR: 通过强化学习模拟证明，即使没有复杂的心理推断能力，仅通过观察专家行为并使用简单启发式方法，学习者也能获得类似专家的高级表征，支持文化传播的非心理化机制。


<details>
  <summary>Details</summary>
Motivation: 探究人类如何在有限认知能力下从他人获取丰富灵活的知识。传统观点认为这需要计算成本高昂的心理推断（如推断他人信念），但文化进化理论强调简单社会线索也能支持行为传播。

Method: 使用强化学习模拟：1）模拟一个在可重构环境中寻找奖励的初始智能体；2）设置两种学习方式：单独学习 vs 观察专家学习；3）学习者不使用心理状态推断，而是通过启发式选择动作或基于观察到的动作提升价值表征；4）比较模型基础学习者在社会暴露下的表现。

Result: 1）简单社会线索会偏置学习者的经验，使其表征向专家表征收敛；2）模型基础学习者从社会暴露中获益最大，表现出更快的学习速度和更接近专家的表征；3）即使没有心理推断，仅通过观察行为也能实现有效的文化传播。

Conclusion: 文化传播可以通过利用非社会学习机制的简单、非心理化过程实现，挑战了传统认为需要复杂心理推断的观点，为理解知识传递提供了更简约的解释框架。
...
### [41] [Hierarchical Reinforcement Learning with Augmented Step-Level Transitions for LLM Agents](https://arxiv.org/abs/2604.05808)
...
Method: 提出分层强化学习框架STEP-HRL，通过层次化任务结构，使用完成的子任务表示全局进度，并引入局部进度模块选择性总结子任务内交互历史，生成紧凑的局部进度摘要，为高层和低层策略提供增强的步级转移

Result: 在ScienceWorld和ALFWorld基准测试中，STEP-HRL在性能和泛化能力上显著优于基线方法，同时大幅减少token使用量

Conclusion: STEP-HRL通过步级学习和层次化进度表示，为LLM智能体提供了高效、可扩展的决策框架，在降低计算成本的同时提升任务性能
...
### [42] [Reciprocal Trust and Distrust in Artificial Intelligence Systems: The Hard Problem of Regulation](https://arxiv.org/abs/2604.05826)
*Martino Maggetti*

Main category: cs.AI

TL;DR: 该文主张AI系统应被视为具有某种能动性的实体，能与人类建立信任关系，并探讨这对AI监管的影响
...
Method: 文章采用理论论证方法，提出AI系统应被认可为具有某种能动性的实体（artifacts capable of exercising a form of agency），从而能与人类建立相互的信任或失信关系。进一步分析这种相互信任动态对AI系统监管者的影响。

Result: 文章识别出AI作为具有能动性实体与人类建立信任关系这一观点对AI监管带来的关键紧张关系和未解困境，为未来AI治理提供了新的理论视角。

Conclusion: AI系统应被视为具有某种能动性的实体，能与人类建立相互信任关系，这对AI监管提出了新的挑战和困境。文章为未来AI治理框架的构建提供了重要的理论洞见，强调了考虑AI能动性和信任动态在监管设计中的必要性。
...
### [43] [Vision-Guided Iterative Refinement for Frontend Code Generation](https://arxiv.org/abs/2604.05839)
*Hannah Sansford,Derek H. C. Law,Wei Liu,Abhishek Tripathi,Niresh Agarwal,Gerrit J. J. van den Burg*

Main category: cs.AI

TL;DR: 提出一个完全自动化的视觉批评循环框架，使用视觉语言模型对渲染的网页提供结构化反馈，指导前端代码生成的迭代优化，相比单次推理显著提升解决方案质量。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型代码生成通常依赖多阶段人工循环优化，成本高昂，特别是在前端网页开发这种解决方案质量依赖于渲染视觉输出的领域，需要自动化替代方案。

Method: 1) 建立完全自动化的批评循环框架，使用视觉语言模型作为视觉批评器，对渲染的网页提供结构化反馈；2) 采用LoRA进行参数高效微调，研究批评器提供的改进能否被代码生成LLM内化。

Result: 1) 在WebDev Arena数据集真实用户请求上，该方法带来一致的解决方案质量改进，经过三个优化循环性能提升达17.8%；2) 微调实现了最佳批评循环解决方案25%的增益，且没有显著增加token数量。

Conclusion: 基于VLM的前端代码生成自动化批评能显著提升解决方案质量，优于单次LLM推理；迭代优化对网页开发相关的复杂视觉输出至关重要；批评器提供的改进可以通过微调部分内化到代码生成模型中。
...
### [44] [When Do We Need LLMs? A Diagnostic for Language-Driven Bandits](https://arxiv.org/abs/2604.05859)
...
<details>
  <summary>Details</summary>
Motivation: 解决上下文多臂老虎机（CMAB）问题中LLM推理成本高、不确定性估计困难的问题，特别是在金融领域（推荐系统、动态投资组合调整、报价选择等）需要处理文本和数值混合上下文信息的场景。

Method: 提出LLMP-UCB算法，通过重复推理从LLM获取不确定性估计；同时探索基于文本嵌入（密集或Matryoshka）的轻量级数值bandit方法；并提出基于臂嵌入几何特征的诊断方法，指导实践者选择LLM驱动推理或轻量级数值bandit。

Result: 基于文本嵌入的轻量级数值bandit能以极低成本达到或超过LLM方案的准确度；嵌入维度可作为探索-利用平衡的实用调节杠杆，实现成本-性能权衡；提出的几何诊断方法能有效指导算法选择。
...
### [45] [Joint Knowledge Base Completion and Question Answering by Combining Large Language Models and Small Language Models](https://arxiv.org/abs/2604.05875)
*Yinan Liu,Dongying Lin,Sigang Luo,Xiaochun Yang,Bin Wang*

Main category: cs.AI

TL;DR: JCQL框架通过结合大语言模型和小语言模型的优势，实现知识库补全和知识库问答任务的迭代式相互增强


<details>
  <summary>Details</summary>
Motivation: 知识库补全和知识库问答是密切相关且互补的任务，现有研究通常依赖小语言模型进行联合增强，忽略了大语言模型的强大推理能力。需要结合两者的优势，使这两个任务能够相互增强。

Method: 提出JCQL框架：1) 通过将SLM训练的KBC模型作为LLM智能体的动作，增强基于LLM智能体的KBQA模型的推理路径，缓解LLM的幻觉和高计算成本问题；2) 利用KBQA的推理路径作为补充训练数据，增量微调KBC模型，提升SLM在KBC任务中的能力。两个任务以迭代方式相互增强。

Result: 在两个公共基准数据集上的大量实验表明，JCQL在KBC和KBQA任务上都超越了所有基线方法。

Conclusion: JCQL框架成功结合了LLM和SLM的优势，实现了知识库补全和知识库问答任务的相互增强，为解决这两个相关任务提供了有效方案。
...
### [46] [HybridKV: Hybrid KV Cache Compression for Efficient Multimodal Large Language Model Inference](https://arxiv.org/abs/2604.05887)
*Bowen Zeng,Feiyang Ren,Jun Zhang,Xiaoling Gu,Ke Chen,Lidan Shou,Huan Li*

Main category: cs.AI

TL;DR: HybridKV是一个混合KV缓存压缩框架，通过分层预算分配和针对不同类型注意力头的差异化压缩策略，显著减少多模态大语言模型的KV缓存内存占用和加速解码过程。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在处理视觉输入时，每个视觉输入会扩展为数千个token，导致KV缓存随上下文长度线性增长并持续占用GPU内存，造成巨大的内存开销和解码延迟。现有压缩方法虽然能在固定预算下压缩缓存，但忽视了注意力头的异质性行为，需要不同的压缩策略。

Method: 提出HybridKV混合KV缓存压缩框架，包含三个阶段：1) 使用文本中心注意力将注意力头分类为静态或动态类型；2) 采用自上而下的分层预算分配方案分配KV预算；3) 对静态头使用文本优先剪枝压缩，对动态头使用分块检索压缩。

Result: 在11个多模态基准测试中使用Qwen2.5-VL-7B模型，HybridKV将KV缓存内存减少高达7.9倍，解码速度提升1.52倍，相比完整缓存MLLM几乎没有性能下降甚至表现更好。

Conclusion: HybridKV通过识别注意力头的异质性并采用差异化压缩策略，有效解决了多模态大语言模型中KV缓存的内存和延迟问题，为高效的多模态推理提供了实用解决方案。
...
### [47] [Beyond Compromise: Pareto-Lenient Consensus for Efficient Multi-Preference LLM Alignment](https://arxiv.org/abs/2604.05965)
*Renxuan Tan,Rongpeng Li,Zhifeng Zhao,Honggang Zhang*

Main category: cs.AI

TL;DR: PLC框架通过博弈论视角将多目标偏好对齐重构为动态协商过程，引入容忍局部退化的共识驱动梯度修正，以突破传统方法的局部最优限制，探索全局帕累托前沿。


<details>
  <summary>Details</summary>
Motivation: 现有多目标偏好对齐方法主要依赖静态线性标量化或刚性梯度投影，这些方法通过强制冲突避免或同时下降，往往过早收敛到局部平稳点。虽然数学上稳定，但这些点代表保守的妥协，模型为避免瞬态局部权衡而牺牲了潜在的全局帕累托改进。

Method: 提出帕累托-宽容共识（PLC）框架，将对齐重新构想为动态协商过程。引入共识驱动的宽容梯度修正，动态容忍局部退化，前提是存在足够的优势联盟盈余，从而使优化轨迹能够逃离局部次优均衡并探索远端帕累托最优前沿。

Result: 理论分析验证PLC能够促进僵局逃离并渐近收敛到帕累托共识均衡。大量实验表明，PLC在固定偏好对齐和全局帕累托前沿质量方面均超越基线方法。

Conclusion: 这项工作突显了协商驱动对齐作为多目标偏好对齐的有前景途径的潜力。PLC框架通过博弈论视角重新构想对齐过程，为突破传统方法的局部最优限制提供了新思路。
...
### [48] [Epistemic Blinding: An Inference-Time Protocol for Auditing Prior Contamination in LLM-Assisted Analysis](https://arxiv.org/abs/2604.06013)
*Michael Cuccarese*

Main category: cs.AI

TL;DR: 提出"认知盲化"协议，解决LLM在药物靶点优先排序中数据驱动推理与记忆先验知识混合的不可审计问题


<details>
  <summary>Details</summary>
Motivation: LLM在分析生物数据集时，输出会无声地混合数据驱动推理和记忆中的实体先验知识，这种混合不可见且无法审计，影响科学推理的可靠性

Method: 认知盲化协议：在推理时将实体标识符替换为匿名代码，然后与未盲化对照比较输出；结合LLM引导的评分函数进化优化和盲化智能体推理

Result: 在四种癌症类型的靶点优先排序中，盲化改变了16%的top-20预测，同时保持已验证靶点的相同恢复率；在S&P 500股票筛选中，品牌认知偏见改变了30-40%的top-20排名

Conclusion: 认知盲化协议恢复了LLM推理的关键审计维度，使研究人员能够区分输出来自数据还是模型参数知识，该协议已开源并集成到Claude Code技能中
...
### [49] [How LLMs Follow Instructions: Skillful Coordination, Not a Universal Mechanism](https://arxiv.org/abs/2604.06015)
*Elisabetta Rocchetti,Alfio Ferrara*

Main category: cs.AI

TL;DR: 指令微调并非通过单一通用机制实现指令跟随，而是通过技能协调：跨任务表征共享有限，迁移基于技能相似性，依赖稀疏非对称关系而非共享表征。


<details>
  <summary>Details</summary>
Motivation: 理解指令微调赋予语言模型指令跟随能力的底层机制：是依赖单一通用机制还是组合性技能部署？现有研究对此机制理解不足。

Method: 在三个指令微调模型上对九个多样化任务进行诊断性探测：1) 跨任务通用探针与任务专用探针性能对比；2) 跨任务迁移分析；3) 因果消融识别依赖关系；4) 分层复杂度分析；5) 生成过程时间分析。

Result: 1) 通用探针持续劣于专用探针，表明表征共享有限；2) 跨任务迁移弱且按技能相似性聚类；3) 因果消融揭示稀疏非对称依赖而非共享表征；4) 任务按复杂度分层：结构约束早期出现，语义任务晚期出现；5) 约束满足作为生成过程中的动态监控而非预生成规划。

Conclusion: 指令跟随应被理解为多样化语言能力的熟练协调，而非单一抽象约束检查过程的部署。模型通过动态监控和技能组合实现指令跟随，而非通用机制。
...
### [50] [Artificial Intelligence and the Structure of Mathematics](https://arxiv.org/abs/2604.06107)
*Maissam Barkeshli,Michael R. Douglas,Michael H. Freedman*

Main category: cs.AI

TL;DR: 论文探讨AI如何通过分析证明的全局结构来理解数学本质，提出形式化数学的通用证明框架，并设想了能够进行数学发现的AI模型标准。


<details>
  <summary>Details</summary>
Motivation: AI在数学领域的进展为理解数学的本质结构提供了新途径。论文旨在探索AI如何通过分析形式化证明的全局结构，为数学的哲学基础问题（如数学是发现还是发明）提供新视角，并建立AI驱动的数学发现框架。

Method: 提出基于通用证明和结构超图的形式化数学框架，分析数学证明的全局结构特征。同时制定AI模型进行自动化数学发现所需满足的标准和核心要素。

Result: 构建了理解数学形式结构的概念框架，明确了AI探索数学世界所需的技术路径。虽然没有具体的实验结果，但为AI辅助数学研究提供了理论蓝图。

Conclusion: AI有望通过探索数学的柏拉图世界，揭示数学的深层结构本质，帮助回答数学哲学的基本问题，并为人类理解数学提供新的视角。
...
### [51] [Claw-Eval: Toward Trustworthy Evaluation of Autonomous Agents](https://arxiv.org/abs/2604.06132)
...
<details>
  <summary>Details</summary>
Motivation: 现有智能体基准存在三个关键局限：(1) 仅检查最终输出的轨迹不透明评分，(2) 安全性和鲁棒性评估不充分，(3) 模态覆盖和交互范式狭窄。需要更全面的评估框架来准确衡量智能体的真实能力。

Method: 开发包含300个人工验证任务的评估套件，涵盖9个类别（通用服务编排、多模态感知与生成、多轮专业对话）。通过三个独立证据通道（执行轨迹、审计日志、环境快照）记录每个智能体动作，实现基于2,159个细粒度评分项的轨迹感知评分。评分协议评估完成度、安全性和鲁棒性，报告平均分、Pass@k和Pass^k。

Result: 对14个前沿模型的实验显示：(1) 轨迹不透明评估系统性地不可靠，漏掉了44%的安全违规和13%的鲁棒性失败；(2) 受控错误注入主要降低一致性而非峰值能力，Pass^3下降达24%而Pass@3保持稳定；(3) 多模态性能差异显著，大多数模型在视频任务上表现较差，没有单一模型在所有模态上占优。

Conclusion: Claw-Eval提供了更可靠的智能体评估框架，揭示了构建不仅有能力且可靠可部署的智能体所需的关键方向，强调了轨迹感知评估和多模态能力平衡的重要性。
