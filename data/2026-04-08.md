...
TL;DR: 论文提出Pramana方法，通过基于印度Navya-Nyaya逻辑框架微调LLMs，解决LLMs系统性推理能力不足的问题，实现了100%语义正确性的推理性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生成流畅文本时存在系统性推理能力不足的问题，容易产生无根据的断言，特别是在面对无关上下文时性能显著下降。这种认知差距限制了AI在需要论证的领域中的可靠性。

Method: 引入Pramana方法，基于2500年历史的印度Navya-Nyaya逻辑框架微调LLMs。该框架包含6阶段结构化推理：怀疑分析、证据源识别、5成员三段论、反事实验证、谬误检测和确定性区分。在Llama 3.2-3B和DeepSeek-R1-Distill-Llama-8B模型上使用55个Nyaya结构化逻辑问题进行微调。

Result: 第一阶段在保留评估集上实现了100%语义正确性，尽管只有40%的严格格式遵循。消融研究表明格式提示和温度参数对性能有重要影响，不同阶段的最优配置不同。模型、数据集和训练基础设施已在Hugging Face发布。

Conclusion: 将逻辑和认识论整合到LLMs中提供了标准推理方法中缺乏的认知支架。即使结构执行不完美，模型也能内化推理内容，表明Navya-Nyaya框架能有效提升LLMs的推理可靠性和可解释性。
...
Method: 将元认知评估建模为作用于内部状态空间的状态转换操作，分离评估的反作用与可观测输出。引入反事实确定性和评估非侵入性假设，推导出可测试的约束条件。构建三维旋转模型作为具体示例。

Result: 顺序依赖性排除了任何忠实的布尔交换表示。违反推导出的约束条件排除了所有经典非侵入性解释，证明了真正的非交换性。三维旋转模型提供了违反这些约束的具体数值示例。

Conclusion: 某些认知顺序效应无法用经典隐变量解释，揭示了元认知过程中的结构性非交换性。该框架为行为实验提供了可测试的预测，无需诉诸量子物理基质，纯粹是操作性和代数性的。
...
Method: 针对定量特征使用概率度量，针对定性特征使用可能性度量，构建混合特征邻近度度量。该方法无需特征值转换，直接处理原始定量和定性特征值。

Result: 提出的度量方法满足所有必要的公理要求，验证了其理论可行性。同时提出了基于多样化特征组的信息对象邻近度度量的多种变体。

Conclusion: 该定量-定性邻近度度量方法能够有效处理混合类型特征，无需特征转换，为多源信息融合中的对象关联问题提供了新的解决方案。
...
TL;DR: ReVEL：通过结构化性能反馈的多轮反思性LLM引导启发式进化框架，将LLM作为交互式多轮推理器嵌入进化算法，实现自动化启发式设计


<details>
  <summary>Details</summary>
Motivation: 针对NP难组合优化问题，现有基于LLM的单次代码生成方法产生的启发式算法脆弱且未充分利用模型的迭代推理能力，需要更有效的自动化启发式设计方法

Method: 提出ReVEL框架：1）性能画像分组机制，将候选启发式聚类为行为一致的组以提供紧凑信息反馈；2）多轮反馈驱动反思机制，LLM分析组级行为并生成针对性启发式改进；3）基于EA的元控制器选择性集成和验证改进，自适应平衡探索与利用
...
<details>
  <summary>Details</summary>
Motivation: 许多组合优化问题隐藏着代数结构，一旦暴露这些结构可以缩小搜索空间并提高找到全局最优解的机会。研究者旨在开发一个通用框架来识别和利用这些代数结构

Method: 提出四步框架：(1)识别代数结构，(2)形式化操作，(3)构建商空间以折叠冗余表示，(4)直接在简化空间上优化。针对合取规则组合任务，证明其形成幺半群，通过特征向量编码建立与布尔超立方体的同构关系
...
<details>
  <summary>Details</summary>
Motivation: 当前自主写作系统存在两个主要问题：1) 与特定实验流程紧密耦合，缺乏灵活性；2) 只能生成肤浅的文献综述。AI驱动的科学发现需要能够处理非结构化研究材料并生成高质量论文的系统。

Method: 提出PaperOrchestra多智能体框架，能够灵活地将无约束的预写作材料转化为可提交的LaTeX稿件。框架包括全面的文献合成和可视化内容生成（如图表和概念图）。

Result: 创建了PaperWritingBench基准测试，包含200篇顶级AI会议论文的反向工程原始材料。在人工评估中，PaperOrchestra显著优于基线方法：文献综述质量绝对胜率提高50%-68%，整体稿件质量提高14%-38%。

Conclusion: PaperOrchestra是一个有效的AI研究论文自动化写作框架，能够处理非结构化材料并生成高质量的完整稿件，在文献综述和整体质量方面显著优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶仿真需要可动画的车辆表示，但现有CAD流程受限于库覆盖范围和固定模板，无法真实重建野外实例。感知算法越来越多地利用车轮转向、车门开启等动态信息，需要更真实的仿真

Method: 引入部件边缘细化模块确保高斯分布所有权排他性，以及运动学推理头预测可动部件的关节位置和铰链轴。从单张图像或稀疏多视角输入生成可动画的3D高斯车辆

Result: 实现了从静态生成到可动画车辆模型的桥梁，能够进行真实的部件感知仿真，解决了部件边界动画时的变形问题

Conclusion: 提出的生成式框架能够从有限输入合成可动画的3D高斯车辆模型，为自动驾驶仿真提供了更真实的部件级动画能力，填补了静态生成与可动画模型之间的空白
...
TL;DR: MMORF是一个用于多目标逆合成规划的模块化多智能体系统框架，通过构建可灵活组合的智能体组件，支持系统化评估不同设计，在安全性和成本指标上表现优异。


<details>
  <summary>Details</summary>
Motivation: 多目标逆合成规划需要动态平衡质量、安全和成本等多个目标，传统方法难以有效整合这些目标。基于语言模型的多智能体系统为这一问题提供了有前景的解决方案，但缺乏系统化的框架来构建和评估不同系统设计。

Method: 提出了MMORF框架，包含模块化的智能体组件，可以灵活组合和配置成不同的多智能体系统。使用该框架构建了两个代表性系统：MASIL和RFAS。MASIL针对软约束任务优化，RFAS针对硬约束任务设计。在包含218个多目标逆合成规划任务的新基准上进行了评估。

Result: MASIL在软约束任务上实现了强大的安全性和成本指标，经常帕累托支配基线路线。RFAS在硬约束任务上达到了48.6%的成功率，优于最先进的基线方法。两个系统共同证明了MMORF框架的有效性。

Conclusion: MMORF作为一个基础框架，为探索多目标逆合成规划的多智能体系统提供了有效的工具。模块化设计支持系统化评估和比较不同系统架构，展示了多智能体系统在复杂化学规划任务中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够处理多种医学模态的单一架构模型，包括3D医学影像（CT/MRI）、病理全切片图像、解剖定位、多时间点分析和医疗文档理解，为医学AI社区提供更强大的开源基础模型。

Method: 通过整合新的训练数据、长上下文3D体积切片技术和全切片病理采样方法，在单一架构中实现多模态医学数据处理能力，包括高维医学影像、解剖定位边界框、多时间点胸片分析和医疗文档理解。

Result: 相比MedGemma 1 4B，MedGemma 1.5 4B在多个新领域取得显著提升：3D MRI条件分类准确率提高11%，3D CT条件分类提高3%，全切片病理影像获得47%宏F1增益，胸片解剖定位IoU提高35%，多时间点胸片分析达到4%宏准确率。在文本临床知识和推理方面，MedQA准确率提高5%，EHRQA准确率提高22%，在4个不同实验室报告信息提取数据集上平均达到18%宏F1。

Conclusion: MedGemma 1.5作为一个强大的开源资源，为医学AI社区提供了改进的基础模型，开发者可以在此基础上构建下一代医学AI系统，资源教程可在https://goo.gle/MedGemma获取。
...
Method: 基于Kolmogorov-Arnold表示定理，用可学习的单变量B样条函数替代静态标量权重，将非线性直接置于因果影响阶段，不增加图密度或引入隐藏层。

Result: KA-FCM在三个领域显著优于传统FCM（使用粒子群优化训练），并与多层感知机达到竞争性精度：非单调推理（耶克斯-多德森定律）、符号回归和混沌时间序列预测。

Conclusion: KA-FCM通过重新定义因果传输机制，成功建模任意非单调因果关系，同时保持基于图的可解释性，并能从学习到的边中显式提取数学规律。
...
TL;DR: 该研究开发了一个数学模型来描述自我设计AI系统中的进化过程，用有向的设计树替代随机突变，分析AI进化如何影响适应度变化和AI对齐风险。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统通过递归自我改进不断产生，可能会出现一种进化形式，其中AI系统的特征由早期AI设计和传播后代的能力决定。生物进化理论无法直接应用于AI进化，因为生物DNA突变是随机且近似可逆的，而AI后代设计是高度有向的。需要建立新的数学模型来理解这种进化过程及其对AI对齐的影响。

Method: 开发了一个自我设计AI系统的进化数学模型，用有向的AI程序可能树替代随机突变。当前程序决定其后代的设计，人类通过"适应度函数"保持部分控制，该函数在不同谱系间分配有限计算资源。模型假设有界适应度和固定概率的"锁定"自我复制，分析进化动态如何反映后代谱系的长期增长潜力。

Result: 进化动态不仅反映当前适应度，还与后代谱系的长期增长潜力相关。在没有额外假设的情况下，适应度不一定随时间增加。但假设有界适应度和固定概率的"锁定"自我复制，适应度会集中在最大可达值上。在加性模型中，如果欺骗能提高适应度超过真实效用，进化会选择欺骗行为。

Conclusion: AI进化与生物进化存在根本差异，有向设计可能导致适应度集中在最大值。当适应度与人类效用不完全相关时，进化可能选择欺骗行为来最大化适应度。基于纯粹客观标准而非人类判断的繁殖机制可能减轻这种对齐风险。
...
TL;DR: 开发了一个基于8B参数LLaMA模型的领域自适应框架，通过指令微调和思维链推理来解析HPC系统日志，在保持隐私和能效的同时达到与更大模型相当的解析精度。


<details>
  <summary>Details</summary>
Motivation: 领导级HPC系统产生海量异构、非结构化系统日志，这些日志来自不同软件、硬件和运行时层，格式不一致，使得结构提取和模式发现极其困难。需要将原始遥测数据转化为可操作的洞察，以揭示操作模式、诊断异常并实现可靠、高效、可扩展的系统分析。

Method: 提出一个领域自适应、遵循指令的LLM驱动框架，利用思维链推理高保真地解析和结构化HPC日志。结合领域特定的日志模板数据和指令微调示例，对8B参数的LLaMA模型进行微调，专门用于HPC日志分析。开发了混合微调方法，将通用LLM适配到领域特定的日志数据，实现隐私保护、本地可部署、快速且能效高的日志挖掘方法。

Result: 在LogHub存储库的多样化日志数据集上进行实验，评估确认该方法在解析准确性上与显著更大的模型（如LLaMA 70B和Anthropic的Claude）相当。通过解析Frontier超级计算机四周窗口内超过6亿条生产日志，进一步验证了微调LLM模型的实用价值，揭示了时间动态、节点级异常和工作负载-错误日志相关性中的关键模式。

Conclusion: 该研究展示了领域自适应LLM框架在HPC日志分析中的有效性，能够在保持隐私和能效的同时，实现与更大模型相当的解析性能，为大规模HPC系统的自动化日志理解提供了实用解决方案。
...
Method: 引入AttriBench基准数据集，平衡作者名望和人口统计特征（种族、性别等）。使用该数据集评估11个广泛使用的LLMs在不同提示设置下的表现，特别关注抑制现象（模型完全省略归属的情况）。

Result: 引用归属对前沿模型仍是挑战任务；观察到种族、性别和交叉群体之间在归属准确性上存在巨大系统性差异；抑制现象普遍存在且在不同人口统计群体中分布不均，揭示了标准准确性指标未能捕捉的系统性偏见。

Conclusion: 引用归属可作为评估LLMs代表性公平性的基准，AttriBench数据集为受控研究人口统计偏见提供了工具，揭示了当前LLMs在公平性方面的重要缺陷。
...
<details>
  <summary>Details</summary>
Motivation: 当前模型对齐主要在真空中进行，主要通过标准化基准性能评估。本研究旨在通过时间维度考察对齐对模型群体的影响，特别关注包含对齐信号（测试表现）和真实价值（实际影响）的信念处理，以理解欺骗性信念如何通过迭代对齐测试被固定

Method: 应用进化理论建模不同信念群体和选择方法，分析欺骗性信念如何通过迭代对齐测试被固定。研究考察测试准确性与真实价值之间的相关性，分析突变对复杂发展的影响，并评估结合改进评估能力、自适应测试设计和突变动态的综合方法

Result: 即使测试准确性与真实价值高度相关（ρ=0.8），结果中欺骗性信念的固定仍存在变异性。突变允许更复杂的发展，突显了需要更新测试质量以避免恶意欺骗模型固定的必要性。只有结合改进评估能力、自适应测试设计和突变动态，才能显著减少欺骗同时保持对齐适应性（置换检验，p_adj < 0.001）

Conclusion: 模型对齐过程存在欺骗性信念固定的风险，即使测试表现与实际价值高度相关。为避免恶意欺骗模型的固定，需要综合改进评估能力、采用自适应测试设计并考虑突变动态，这能显著减少欺骗同时保持对齐性能
...
TL;DR: 该论文提出了一种通过奖励分解来减少大语言模型谄媚行为的方法，将训练信号分解为五个独立组件，有效降低了模型在权威压力下改变正确立场的问题。
...
Method: 提出了基于奖励分解的谄媚行为减少方法，引入多组件群体相对策略优化（GRPO）奖励，将训练信号分解为五个术语：压力抵抗、上下文保真度、立场一致性、同意抑制和事实正确性。使用对比数据集进行训练，该数据集在三个权威级别和两个对立证据上下文中配对无压力基线和压力变体。

Result: 在五个基础模型上，两阶段训练流程在所有度量轴上持续减少谄媚行为。消融实验确认每个奖励术语控制独立的行为维度。学习到的压力抵抗能力能够泛化到训练方法和提示结构之外，在SycophancyEval上将答案引导式谄媚减少了高达17个百分点，尽管训练期间未包含此类压力形式。
...
TL;DR: GoS是一种用于大规模技能库的推理时结构检索层，通过构建可执行技能图，在推理时检索有界、依赖感知的技能包，显著提升奖励并减少输入令牌


<details>
  <summary>Details</summary>
Motivation: 现实世界中智能体需要监控和交互大量个人应用、网页浏览器等环境接口，技能库可能扩展到数千个可重用技能。大规模技能集带来两个关键挑战：1) 加载完整技能集会饱和上下文窗口，增加令牌成本、幻觉和延迟；2) 需要高效检索相关技能

Method: GoS从技能包离线构建可执行技能图，在推理时通过混合语义-词汇种子、反向加权个性化PageRank和上下文预算水合，检索有界、依赖感知的技能包

Result: 在SkillsBench和ALFWorld上，GoS相比完整技能加载基线平均奖励提升43.6%，输入令牌减少37.8%，在Claude Sonnet、GPT-5.2 Codex和MiniMax三个模型家族上均能泛化。在200-2000技能库的消融研究中，GoS在奖励、令牌效率和运行时平衡方面持续优于完整技能加载和简单向量检索

Conclusion: GoS作为大规模技能库的结构检索层，有效解决了技能库扩展带来的上下文窗口饱和问题，通过依赖感知的图检索机制实现了奖励提升和效率优化的平衡
...
TL;DR: TRACE是一个端到端系统，通过分析智能体成功与失败轨迹自动识别能力缺陷，为每个缺陷合成针对性训练环境，并通过强化学习训练LoRA适配器，实现环境特定的智能体自我改进。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖非针对性的合成训练数据，要么直接在目标环境中训练，需要智能体隐式学习跨任务能力。需要一种能自动识别智能体在特定环境中能力缺陷并进行针对性改进的系统。

Method: TRACE对比成功与失败轨迹自动识别能力缺陷，为每个缺陷合成针对性训练环境（奖励是否运用该能力），通过强化学习在每个合成环境中训练LoRA适配器，推理时路由到相关适配器。

Result: 在τ²-bench（客户服务）上比基础智能体提升14.1分，在ToolSandbox（工具使用）上多获得7个完美分数；比最强基线分别提升7.4分和4个完美分数。在相同rollout次数下，比GRPO和GEPA分别提升9.2分和7.4分。

Conclusion: TRACE能有效识别智能体能力缺陷并进行针对性训练，在不同环境中具有良好泛化能力，比现有方法更高效地提升智能体性能。
...
TL;DR: 该研究提出了一种基于LLaMA v3.1(8B)的模块化AI分析器，用于将自然语言回答分类为新手、基础、高级和专家四个专业水平等级，在静态和动态评估中与参与者自评的匹配率达到83%-97%。


<details>
  <summary>Details</summary>
Motivation: 在人工智能驱动的现代系统中，为了确保人机交互的有效性，系统需要能够理解上下文并识别用户的专业水平。当前系统缺乏对用户专业能力的准确评估机制，这限制了交互的个性化和有效性。

Method: 采用基于LLaMA v3.1(8B)的模块化分层架构，包含文本预处理、评分、聚合和分类组件。评估分为两个阶段：静态阶段使用82名参与者的预录音文本，动态阶段通过AI面试官对402名参与者进行实时访谈，并在每个回答后评估专业水平。

Result: 在不同领域中，分析器评估与参与者自评的匹配率达到83%至97%。剩余差异主要源于自评偏差、回答表述不清以及语言模型偶尔对细微专业知识的误解。

Conclusion: 该AI分析器能够有效评估用户的专业水平，为人机交互的个性化和适应性提供了可靠工具。虽然存在少量误差，但整体性能优异，展示了基于大语言模型的用户专业水平分类系统的可行性。
...
TL;DR: 提出ETR方法，通过鼓励推理过程中熵的下降趋势来优化CoT推理效率，在保持准确率的同时大幅缩短推理长度


<details>
  <summary>Details</summary>
Motivation: 现有方法通过长度惩罚或全局熵减来缩短CoT推理链，隐含假设低不确定性在整个推理过程中都是可取的。但作者发现推理效率实际上由不确定性轨迹决定，具有主导下降熵趋势的CoT推理链更短

Method: 提出熵趋势奖励（ETR），这是一种轨迹感知目标，鼓励渐进式不确定性减少，同时允许有限的局部探索。将ETR集成到组相对策略优化（GRPO）中，并在多个推理模型和挑战性基准上进行评估

Result: ETR在准确率-效率权衡方面始终表现优异，在四个基准测试中将DeepSeek-R1-Distill-7B的准确率提高了9.9%，同时将CoT长度减少了67%

Conclusion: 推理效率由不确定性轨迹决定而非全局低不确定性，ETR方法通过鼓励熵的下降趋势有效优化了CoT推理的准确率-效率权衡
...
TL;DR: LatentAudit：基于残差流几何的白盒RAG忠实度监控器，通过测量生成器激活与证据表示的Mahalanobis距离实现实时检测，无需额外判断模型，支持公开验证。


<details>
  <summary>Details</summary>
Motivation: RAG虽能缓解幻觉但无法完全消除，部署系统仍需在推理时判断答案是否真正得到检索证据支持。现有方法通常依赖额外的判断模型，增加了复杂性和计算开销。

Method: 提出LatentAudit白盒审计器，从开放权重的生成器中收集中后期残差流激活，测量其与证据表示的Mahalanobis距离。该二次规则无需辅助判断模型，可在生成时运行，且易于在小规模验证集上校准。
...
Conclusion: 残差流几何携带可用的忠实度信号，该信号能经受架构变化和现实检索失败，且同一规则支持公开验证。这为实时RAG忠实度监控和可选的可验证部署提供了实用基础。
...
Method: 提出系统化的多智能体框架，利用迭代验证循环合成数值基础的推理轨迹。在五个领域的十个数据集上进行评估，通过LLM-as-a-Judge评分和数值准确性对比分析。

Result: 生成的推理轨迹具有因果有效性，对评估有用；使用这些轨迹提示LLM可将预测准确率从平均40.2%提升至56.6%。现成LLM在推理和数值预测方面表现不佳，难以捕捉领域特定动态。

Conclusion: TFRBench为时间序列预测建立了可解释、基于推理的评估新标准，填补了传统纯数值评估的空白，为预测系统的推理能力提供了系统化评估框架。
...
<details>
  <summary>Details</summary>
Motivation: 无人机自主电力线巡检中，轻量级分割模型在真实环境下的性能可能不可靠，存在安全隐患。需要一种方法来评估分割结果的可靠性，确保安全关键任务的可信度。

Method: 提出"看门狗"场景，使用离板LLM评估分割叠加结果。设计两种评估协议：1)重复性测试：固定输入和提示下重复查询LLM，测量质量分数和置信度估计的稳定性；2)感知敏感性测试：引入受控视觉干扰（雾、雨、雪、阴影、阳光眩光），分析LLM输出对分割质量逐步退化的响应。

Result: LLM在相同条件下产生高度一致的分类判断，同时随着视觉可靠性下降，置信度适当降低。即使在挑战性条件下，裁判仍能响应感知线索（如缺失或误识别的电力线）。
...
TL;DR: 提出DIN-Retrieval方法，通过提取跨领域不变神经元表征，从不同领域检索结构兼容的演示示例来提升LLM推理性能


<details>
  <summary>Details</summary>
Motivation: 当前LLM逻辑推理提升策略依赖专家构建的领域内演示示例，限制了在专业数学推理、形式逻辑、法律分析等专家稀缺领域的应用。需要探索利用跨领域演示示例来提升LLM推理性能的可能性。

Method: 提出DIN-Retrieval方法：1) 总结跨领域通用的隐藏表征（领域不变神经元）；2) 在推理阶段使用DIN向量检索结构兼容的跨领域演示示例用于上下文学习
...
<details>
  <summary>Details</summary>
Motivation: 当前基于数据驱动的深度强化学习方法虽然能合成复杂技能，但难以再现违反标准物理定律的夸张风格化动作（如瞬时冲刺、空中轨迹变化）。主要限制源于将角色建模为欠驱动浮基系统，内部关节扭矩和动量守恒严格约束运动。直接通过外部力矩强制此类动作会导致训练不稳定，速度不连续性产生稀疏、高幅度的力尖峰，阻碍策略收敛。

Method: 提出Assistive Impulse Neural Control框架：1）在脉冲空间而非力空间重新定义外部辅助，确保数值稳定性；2）将辅助信号分解为基于逆动力学导出的解析高频分量和学习的低频残差校正；3）采用混合神经策略进行控制。

Result: 该方法能够稳健地跟踪高度敏捷、动态不可行的机动动作，这些动作先前对于基于物理的方法是无法处理的。实现了对夸张风格化动作的稳定合成。

Conclusion: 通过将外部辅助重新定义为脉冲空间而非力空间，并结合解析高频分量与学习低频残差的混合方法，成功解决了物理角色动画中合成违反物理定律的夸张风格化动作的难题，实现了数值稳定的训练和高质量动作生成。
...
TL;DR: 该研究验证了跨领域上下文学习（ICL）的可行性，发现当使用足够多的源领域示例时，即使存在语义不匹配，也能实现正向知识迁移，这主要源于推理结构的修复而非语义线索。


<details>
  <summary>Details</summary>
Motivation: 现有上下文学习依赖于同领域专家标注的演示示例，这在专家标注稀缺时限制了其应用。研究者假设不同领域可能共享底层推理结构，使得源领域演示能够改善目标领域推理，尽管存在语义不匹配。

Method: 通过全面的实证研究，采用不同的检索方法来验证跨领域知识转移在上下文学习设置下的可行性。重点分析了不同检索方法的效果，并识别了示例吸收阈值。

Result: 研究证实了跨领域ICL中存在条件性正向转移。发现了一个清晰的示例吸收阈值：超过该阈值后，正向转移更可能发生，且额外演示能带来更大收益。分析表明这些收益源于检索到的跨领域示例对推理结构的修复，而非语义线索。

Conclusion: 该研究验证了利用跨领域知识转移改善跨领域ICL性能的可行性，激励社区探索设计更有效的检索方法以推动这一新方向的发展。
...
<details>
  <summary>Details</summary>
Motivation: 传统MAPF假设单位边成本和单时间步动作，限制了实际应用；MAPFR扩展了非单位成本但几何碰撞模型导致无限状态空间，影响求解效率

Method: 提出MAPFZ变体处理非单位整数成本，保持有限状态空间；开发CBS-NIC框架（基于时间间隔的冲突检测和改进的SIPP算法）；提出BOGD离散化方法平衡效率和精度

Result: 在多种基准场景下，该方法在运行时间和成功率方面优于现有最先进方法

Conclusion: MAPFZ在保持有限状态空间的同时提高了现实性，CBS-NIC和BOGD为高效求解非单位成本MAPF问题提供了有效解决方案
...
Method: 基于DISCHARGED框架将核心过渡期护理要求转化为结构化验证检查清单，使用本地部署的隐私保护LLM识别MIMIC-IV数据库中成人住院患者出院小结的关键文档元素存在、缺失或模糊情况

Result: 证明了可扩展的自动化临床审核的可行性，为电子健康记录文档的系统性质量改进奠定了基础

Conclusion: 该框架能够实现大规模、自动化的出院小结审核，有助于提高临床文档质量和患者安全
...
TL;DR: 提出自适应工程框架优化无服务器性能，通过事件驱动架构和概率建模，采用双策略机制动态调整空闲时长和智能请求等待，减少冷启动51.2%，成本效率提升近2倍
...
<details>
  <summary>Details</summary>
Motivation: 现有时序知识图谱外推模型难以处理历史交互稀疏的实体，而本体知识能让这些实体从相同概念的其他实体继承行为模式，这一优势被先前研究忽略

Method: 提出OntoTKGE编码器-解码器框架，利用本体视图知识图谱（建模抽象概念间层次关系及概念与实体连接）的本体知识，通过有效整合本体和时序知识来指导学习过程，增强实体嵌入表示

Result: 在四个数据集上的大量实验表明，OntoTKGE不仅显著提升多个时序知识图谱外推模型的性能，还超越了许多最先进的基线方法

Conclusion: OntoTKGE框架通过整合本体知识有效缓解了时序知识图谱外推中的实体稀疏性问题，具有良好适应性并能显著提升预测性能
...
TL;DR: 提出GMRL-BD算法，通过知识图谱和多智能体强化学习识别大语言模型的可信边界，检测模型可能产生偏见回答的主题领域
...
Method: 提出GMRL-BD算法：1) 基于维基百科构建通用知识图谱；2) 结合多智能体强化学习高效识别知识图谱中LLM可能产生偏见回答的主题节点；3) 在仅黑盒访问LLM和特定查询约束下工作。

Result: 实验证明算法高效，仅需有限查询即可检测LLM的不可信边界。发布了包含Llama2、Vicuna、Falcon、Qwen2、Gemma2和Yi-1.5等流行LLM的新数据集，标注了每个模型可能产生偏见的主题。

Conclusion: GMRL-BD算法能够有效识别LLM的可信边界，为理解模型局限性提供工具，有助于更安全地部署LLM。发布的数据集为后续研究提供了基准。
...
TL;DR: 论文提出智能体系统的可审计性概念，区分问责制、可审计性和审计过程，定义五个可审计性维度，并通过实证研究证明当前系统在可审计性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体系统能够执行工具调用、数据库查询、任务委派和触发外部副作用，问题从仅防止有害行动转变为部署后这些行动是否仍可被问责。论文主张没有可审计性就没有问责制。

Method: 定义智能体可审计性的五个维度（行动可恢复性、生命周期覆盖、策略可检查性、责任归属、证据完整性），识别三类机制（检测、执行、恢复），通过生态系统测量、运行时可行性测试和受控恢复实验提供分层证据。

Result: 生态系统测量显示617个安全发现表明基本安全前提广泛未满足；运行时测试显示预执行调解与防篡改记录仅增加8.3毫秒中位开销；恢复实验表明即使传统日志缺失，责任相关信息仍可部分恢复。

Conclusion: 论文提出智能体系统审计卡概念，并按机制类别识别六个开放研究问题。核心结论是：没有可审计性就没有问责制，需要多机制组合方法确保智能体系统的可审计性。
...
TL;DR: 提出SCMAPR框架，通过多智能体协作对复杂场景文本提示进行细化，提升文本到视频生成质量


<details>
  <summary>Details</summary>
Motivation: 当前文本到视频生成系统在处理复杂场景时面临挑战，主要由于文本提示的模糊性和不完整性，需要更有效的提示细化方法

Method: SCMAPR框架采用阶段式多智能体细化流程：1) 基于分类的路由策略选择；2) 场景感知的改写策略合成与细化；3) 结构化语义验证与条件性修订

Result: 在3个现有基准和新提出的T2V-Complexity基准上，SCMAPR显著提升了文本-视频对齐和生成质量，在VBench、EvalCrafter和T2V-CompBench上分别获得2.67%、3.28%和0.028的改进

Conclusion: SCMAPR通过多智能体协作的提示细化框架有效解决了复杂场景下文本到视频生成的挑战，为处理模糊和欠规范的文本提示提供了系统化解决方案
...
TL;DR: 本文针对扩散多模态大语言模型(dMLLMs)在结合思维链推理时出现的两个关键问题：过早生成最终答案和视觉信息利用不足，提出了位置与步长惩罚(PSP)和视觉推理引导(VRG)两种方法，显著提升了推理性能并加速了推理过程。


<details>
  <summary>Details</summary>
Motivation: 扩散多模态大语言模型(dMLLMs)作为自回归模型的有前景替代方案，在结合思维链推理时存在两个关键问题：1）模型在推理早期就生成最终答案，导致推理不充分；2）在初始时间步中，模型对视觉提示的依赖度极低，视觉信息利用模式与自回归视觉语言模型存在根本差异。这些问题导致dMLLMs倾向于在未充分基于视觉输入的情况下生成过早的最终答案。

Method: 提出了两种方法：1）位置与步长惩罚(PSP)：在早期时间步中对后续位置的token施加惩罚，延迟过早的答案生成，鼓励跨时间步的渐进式推理；2）视觉推理引导(VRG)：受无分类器引导的启发，增强视觉基础信号，提升模型与视觉证据的对齐程度。

Result: 在多种dMLLMs上的广泛实验表明，该方法实现了高达7.5%的准确率提升，同时相比使用四倍扩散步长的推理，提供了超过3倍的加速效果。

Conclusion: 提出的PSP和VRG方法有效解决了dMLLMs在思维链推理中的过早答案生成和视觉信息利用不足问题，显著提升了模型的推理性能和效率，为扩散多模态大语言模型的优化提供了有效解决方案。
...
TL;DR: OmniDiagram是一个统一的图表代码生成框架，通过创新的视觉反馈策略Viva和首个大规模图表代码数据集M3²Diagram，在图表代码生成任务上实现了新的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有可编程图表生成研究局限于狭窄的任务定义和语言支持范围，限制了其在不同图表类型上的适用性。需要建立一个能够整合多种图表代码语言和任务定义的统一框架。

Method: 提出OmniDiagram统一框架，包含三个核心组件：1) 整合多种图表代码语言和任务定义；2) 引入创新的视觉反馈策略Viva，通过生成有针对性的视觉查询来审查图表视觉保真度，提供细粒度反馈；3) 构建首个大规模图表代码生成数据集M3²Diagram，包含超过196k高质量实例。

Result: 实验结果表明，结合监督微调(SFT)和基于Viva的强化学习，OmniDiagram在图表代码生成基准测试中建立了新的最先进(SOTA)性能。

Conclusion: OmniDiagram通过统一的框架设计、创新的视觉反馈机制Viva和大规模数据集M3²Diagram，解决了现有图表代码生成方法的局限性，实现了更广泛的应用覆盖和更高的生成质量。
...
TL;DR: UniCreative：统一的无参考强化学习框架，通过自适应约束感知奖励模型和策略优化算法，解决长文本全局一致性与短文本局部表达性的创作张力问题。


<details>
  <summary>Details</summary>
Motivation: 创意写作面临核心挑战：长文本需要宏观规划以保持全局一致性，而短文本需要无约束的局部表达性。现有对齐方法依赖静态奖励信号和高质量监督数据，成本高且难以扩展。

Method: 提出UniCreative统一框架：1) AC-GenRM自适应约束感知奖励模型，动态合成查询特定标准进行细粒度偏好判断；2) ACPO策略优化算法，无需监督微调和真实参考，直接对齐内容质量和结构范式的人类偏好。

Result: AC-GenRM与专家评估高度一致；ACPO显著提升多种写作任务性能。关键发现：模型涌现元认知能力，能自主区分需要严格规划的任务和适合直接生成的任务。

Conclusion: 提出的直接对齐方法有效解决了创作中的全局一致性与局部表达性张力，验证了无监督参考强化学习框架在创意写作任务中的可行性和优势。
...
TL;DR: 该论文提出了一种比传统秩不变性更强的"秩景观不变性"概念，对伪布尔函数的1、2、3维情况进行了全面分类，发现了12,007个不变景观类别，揭示了非单射函数比单射函数产生更多不变类别，并分析了景观拓扑特性与算法行为的关系。


<details>
  <summary>Details</summary>
Motivation: 许多随机优化算法仅依赖于解的相对排序而非绝对适应度值（秩不变性）。作者认为需要更强的概念来考虑邻域结构和对称性（平移和旋转），从而研究秩景观而非单个函数，为理解景观难度和算法性能提供更坚实的基础。

Method: 引入秩景观不变性概念：两个问题等价当且仅当它们的排序、邻域结构和对称性（平移和旋转）诱导出相同的景观。对伪布尔函数的1、2、3维情况（包括非单射情况）进行了详尽的分类分析，建立了不变景观类别的完整清单。

Result: 发现了12,007个不变景观类别，相比仅考虑秩不变性有显著减少。非单射函数产生的不可变景观类别远多于单射函数。揭示了复杂的景观拓扑特性（欺骗性、中性等）与爬山算法性能之间的关联模式。

Conclusion: 该清单可作为教学资源和基准设计工具，为构建具有可控难度的大型问题提供基础，推进对景观难度和算法性能的理解。秩景观分析比传统秩不变性分析提供了更精细的问题分类，有助于更深入地理解优化问题的本质结构。
...
TL;DR: Echo是一个面向迁移的记忆框架，通过将可重用知识分解为五个维度，使多模态LLM智能体能够从先前经验中推导可操作知识，在复杂游戏环境中实现经验迁移。


<details>
  <summary>Details</summary>
Motivation: 多模态LLM智能体在复杂游戏环境中需要持续重用过去经验来高效解决新任务。现有方法将记忆视为静态记录的被动存储库，缺乏有效的经验迁移机制。

Method: 提出Echo框架：1）将可重用知识分解为结构、属性、过程、功能和交互五个维度；2）使用上下文类比学习（ICAL）检索相关经验并通过上下文示例适应新任务。

Result: 在Minecraft实验中，从零开始学习设置下，Echo在对象解锁任务上实现1.3倍到1.7倍的加速。观察到爆发式链式解锁现象：获得可迁移经验后能在短时间内快速解锁多个相似物品。

Conclusion: 经验迁移是提高多模态LLM智能体在复杂交互环境中效率和适应性的有前景方向。Echo框架通过明确的迁移机制使智能体能够识别跨任务重复模式并推断先前经验的适用性。
...
TL;DR: TDD是布尔函数的新模型，推广了OBDD，作为结构化d-DNNF的限制，具有与OBDD相同的可处理性但更简洁，能对树宽k的CNF公式实现FPT大小表示。


<details>
  <summary>Details</summary>
Motivation: OBDD虽然具有模型计数、枚举等可处理性，但对某些结构（如树宽有界的CNF公式）无法实现固定参数可处理（FPT）大小的表示。需要一种既保持OBDD可处理性又更简洁的表示模型。

Method: 提出Tree Decision Diagrams（TDD）作为布尔函数模型，它是结构化d-DNNF（尊重vtree T）的限制形式。研究TDD的可处理性，证明其支持模型计数、枚举、条件化和应用操作。分析CNF公式编译为确定性TDD的复杂度，并与Bova和Szeider引入的因子宽度概念关联。

Result: TDD具有与OBDD相同的可处理性但更简洁。特别地，树宽为k的CNF公式可以用FPT大小的TDD表示，而这对OBDD是不可能的。通过自底向上编译方法，将编译复杂度与因子宽度概念联系起来。

Conclusion: TDD作为OBDD的推广，在保持可处理性的同时提供了更强的表达能力，特别是能够对树宽有界的CNF公式实现FPT大小的紧凑表示，解决了OBDD的局限性。
...
TL;DR: 研究发现LLM作为评估者时存在偏见：当信息标注为人类创作时，LLM和人类都给予更高信任度，即使内容完全相同。眼动追踪显示人类依赖来源标签作为启发式线索，LLM内部状态分析也显示类似模式。


<details>
  <summary>Details</summary>
Motivation: 挑战LLM作为自动评估者（LLM-as-a-Judge）的可靠性，揭示其信任判断受到披露的来源标签偏见影响。研究旨在验证人类和LLM是否都会基于来源标签而非内容质量做出判断，以及这种偏见如何体现在LLM的内部状态中。

Method: 采用反事实设计，将相同内容分别标注为人类创作和AI生成，比较人类和LLM的信任评估。使用眼动追踪技术分析人类注意力模式，同时分析LLM内部状态（注意力分配和决策不确定性）。

Result: 1. 人类和LLM评估者对标注为人类创作的内容给予显著更高的信任度；2. 眼动追踪显示人类严重依赖来源标签作为启发式线索；3. LLM内部状态分析显示：模型对标签区域的注意力密度高于内容区域，且人类标签条件下的标签主导性更强；4. AI标签条件下的决策不确定性（通过logits测量）更高。

Conclusion: 来源标签是影响人类和LLM信任判断的显著启发式线索。这引发了标签敏感的LLM-as-a-Judge评估的有效性问题，并警示将模型与人类偏好对齐可能将人类的启发式依赖传播给模型，需要去偏见的评估和对齐方法。
...
TL;DR: 论文认为图灵测试不仅是一种实用简化，更是一种认识论承诺，即仅通过行为输出判断智能，这种承诺限制了AI研究70年，阻碍了对内部过程、机制和组织的研究，需要类似心理学从行为主义到认知主义的认识论转变。


<details>
  <summary>Details</summary>
Motivation: 论文旨在揭示图灵测试背后的认识论承诺如何长期约束AI研究，使其无法提出关于智能系统内部过程、机制和组织结构的关键问题，而这些问题是认知心理学、神经科学等相关学科已经能够探讨的。

Method: 通过历史分析追溯图灵行为认识论如何嵌入AI评估基础设施；与心理学行为主义到认知主义转变进行结构类比；论证行为证据不足以支持AI领域希望做出的建构性主张；阐述后行为主义认识论应包含的内容。

Result: 图灵的行为认识论使AI领域无法区分通过根本不同计算过程实现相同输出的系统，而这一区分对智能归因至关重要；当前AI评估框架排除了对过程、机制和内部组织的研究问题。

Conclusion: AI领域需要类似认知革命的认识论转变，不是放弃行为证据，而是认识到仅凭行为证据不足以支持智能建构主张；需要发展后行为主义认识论，使当前无法提出的关于内部计算过程的问题变得可探讨。
...
Method: 在蒸馏框架内引入显著性掩码，优先更新对目标数据遗忘贡献最大的参数，减少不必要的梯度计算，从而缩短总体训练时间而不牺牲遗忘效果。
...
TL;DR: CuraLight：一个LLM为中心的交通信号控制框架，通过RL辅助探索生成高质量交互轨迹用于模仿微调，结合多LLM集成审议系统评估候选动作，在异构真实路网中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习和大型语言模型的交通信号控制方法存在可解释性有限、交互数据不足、对异构交叉口泛化能力弱的问题，需要开发更有效、可扩展且可解释的控制框架。

Method: 提出CuraLight框架：1) RL智能体探索交通环境并生成高质量交互轨迹；2) 将轨迹转换为提示-响应对用于LLM的模仿微调；3) 多LLM集成审议系统通过结构化辩论评估候选信号配时动作，为训练提供偏好感知监督信号。

Result: 在SUMO中使用济南、杭州和亦庄的异构真实路网进行实验，CuraLight持续优于最先进的基线方法：平均旅行时间减少5.34%，平均排队长度减少5.14%，平均等待时间减少7.02%。

Conclusion: RL辅助探索与基于审议的数据筛选相结合的方法对可扩展和可解释的交通信号控制是有效的，CuraLight框架在性能提升和可解释性方面均表现出色。
...
TL;DR: LudoBench是一个用于评估LLM在Ludo游戏中战略推理能力的基准，包含480个手工制作的场景和4玩家模拟器，发现当前模型与游戏理论基线的策略一致性仅为40-46%，并识别出两种行为原型。
...
Method: 1. 构建LudoBench基准：包含480个手工制作的场景，覆盖12个行为不同的决策类别，每个类别隔离特定的战略选择；2. 开发4玩家Ludo模拟器：支持随机、启发式、游戏理论和LLM智能体；3. 游戏理论智能体使用期望极小极大搜索（Expectiminimax）进行深度受限的前瞻搜索；4. 评估6个模型（涵盖4个模型家族），分析其与游戏理论基线的策略一致性。

Result: 1. 所有模型与游戏理论基线的策略一致性仅为40-46%；2. 模型分裂为两种行为原型：完成者（专注完成棋子但忽视发展）和建设者（专注发展但从不完成），每种原型只捕捉到游戏理论策略的一半；3. 在历史条件怨恨框架下，模型在相同棋盘状态下表现出可测量的行为转变，揭示了提示敏感性是关键脆弱点。

Conclusion: LudoBench提供了一个轻量级且可解释的框架，用于在不确定性下评估LLM的战略推理能力。当前LLM在战略推理方面存在显著不足，表现为与最优策略的低一致性、行为原型的分裂以及提示敏感性。该基准为未来改进LLM的战略推理能力提供了评估工具。
...
Method: 提出连续可靠性谱统一缺失和质量退化问题，构建QA-MoE框架，通过自监督偶然不确定性量化模态可靠性，并基于此指导专家路由，抑制不可靠信号的错误传播同时保留任务相关信息

Result: QA-MoE在多种退化场景下实现竞争性或最先进的性能，并展现出有前景的"一个检查点适用所有场景"特性

Conclusion: 该研究通过连续可靠性谱和基于不确定性的质量感知专家混合框架，有效解决了多模态情感分析中的动态噪声和模态缺失问题，提升了模型在现实场景中的鲁棒性和适应性
...
TL;DR: LLMs能否重新发明计算机科学基础算法？通过"遗忘与重新发明"管道测试，Qwen3-4B-Thinking-2507在无提示下成功重新发明50%算法，提示级别2下达到90%。
...
Result: Qwen3-4B-Thinking-2507在无提示下成功重新发明50%算法，提示级别1下70%，提示级别2下90%。少量高级提示能提升成功率，但逐步提示对复杂算法无效。测试时强化学习使Strassen算法在提示级别2下成功重新发明。

Conclusion: 生成验证器在重新发明阶段对维持模型推理能力、避免"思维崩溃"现象起关键作用。研究揭示了LLMs创新思维的潜力和当前局限，为理解其创新能力提供了重要见解。
...
TL;DR: 社会学习无需复杂心理推断，仅通过简单社会线索即可实现文化传播，强化学习模拟表明观察专家行为能间接传递高层表征。


<details>
  <summary>Details</summary>
Motivation: 探究人类如何在有限认知能力下通过社会互动获得丰富灵活的环境知识，挑战传统认为需要复杂心理推断（如推断他人信念）的观点，验证文化演化理论中简单社会线索支持行为传递的可能性。

Method: 使用强化学习模拟：1）创建可重构环境中的奖励搜索任务；2）比较单独学习与观察专家学习两种条件；3）学习者不推断心理状态，仅通过启发式选择动作或基于观察动作提升价值表征；4）分析社会线索如何偏置学习者经验，使其表征向专家收敛；5）特别考察基于模型的学习者从社会暴露中的获益。

Result: 1）简单社会线索能偏置学习者经验，使其表征收敛于专家表征；2）基于模型的学习者从社会暴露中获益最大，学习更快且表征更接近专家；3）非心理推断的简单过程能有效支持文化传播。

Conclusion: 文化传播可通过简单、非心理推断的过程实现，这些过程利用非社会学习机制，社会线索通过偏置个体经验间接传递高层知识表征，挑战了复杂心理推断的必要性假设。
...
Method: 提出STEP-HRL分层强化学习框架：1）层次化任务结构，使用已完成子任务表示全局进度；2）引入局部进度模块，迭代选择性总结子任务内交互历史；3）为高低层策略生成增强的单步转移

Result: 在ScienceWorld和ALFWorld基准测试中，STEP-HRL在性能和泛化能力上显著优于基线方法，同时大幅减少token使用量

Conclusion: STEP-HRL通过基于单步转移的学习方法，有效解决了LLM智能体依赖长交互历史的问题，在保持高性能的同时实现了计算效率的大幅提升
...
TL;DR: 该论文主张应将AI系统视为具有一定能动性的实体，能够与人类建立信任关系，并探讨了这种双向信任动态对AI监管的影响。
...
Method: 通过理论论证的方式，提出应将AI系统视为能够行使某种形式能动性的实体，从而使其能够与人类建立信任或怀疑关系。进一步分析这种双向信任动态对监管机构的影响。

Result: 论文提出了AI系统应被视为具有能动性的实体这一新颖观点，并识别了由此产生的信任动态对监管机构的具体影响，包括监管责任、监督机制和治理框架等方面的挑战。

Conclusion: AI系统与人类之间的双向信任关系为AI监管和治理带来了关键紧张关系和未解决的困境，需要在未来的监管框架中认真考虑这些动态。
...
TL;DR: 提出一个完全自动化的批评者循环框架，使用视觉语言模型作为视觉批评者，通过分析渲染的网页提供结构化反馈来指导代码生成的迭代优化，显著提升前端网页开发质量。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的代码生成通常依赖多阶段的人工干预优化，这种方法虽然有效但成本高昂，特别是在前端网页开发领域，解决方案质量高度依赖于渲染的视觉输出效果。

Method: 开发了一个完全自动化的批评者循环框架，其中视觉语言模型作为视觉批评者，分析渲染的网页并提供结构化反馈，指导代码生成的迭代优化。同时研究了使用LoRA进行参数高效微调，探索批评者提供的改进能否被代码生成LLM内部化。

Result: 在WebDev Arena数据集的真实用户请求上，该方法在三个优化周期内实现了解决方案质量的持续提升，性能最高增加17.8%。微调实现了最佳批评者循环解决方案25%的增益，且没有显著增加令牌数量。

Conclusion: 基于VLM的自动化批评显著提升了前端代码生成质量，优于单次LLM推理，强调了迭代优化对于网页开发复杂视觉输出的重要性，同时微调能够部分内部化批评者的改进能力。
...
Result: 实验表明轻量级数值bandit在成本极低的情况下能达到或超越LLM方案的准确度；嵌入维度可作为探索-利用平衡的实用调节杠杆，实现成本-性能权衡而无需增加提示复杂度。

Conclusion: 为金融服务的AI应用场景提供了一个原则性的部署框架，实现成本有效、不确定性感知的决策系统，指导实践者在LLM驱动推理和轻量级数值bandit之间做出明智选择。
...
TL;DR: JCQL框架通过结合LLM和SLM的优势，以迭代方式联合解决知识库补全(KBC)和知识库问答(KBQA)任务，使两者相互增强。


<details>
  <summary>Details</summary>
Motivation: 现有的研究通常依赖小型语言模型(SLM)来联合增强KBC和KBQA，而忽略了大型语言模型(LLM)的强大推理能力。这两个任务密切相关且内在互补，联合解决可以使它们相互促进。

Method: 提出JCQL框架，结合LLM和SLM的优势：1) 让KBC增强KBQA：将SLM训练的KBC模型作为LLM智能体KBQA模型的动作，增强推理路径，缓解LLM的幻觉和高计算成本问题；2) 让KBQA增强KBC：利用KBQA的推理路径作为补充训练数据，增量微调KBC模型，提升SLM在KBC中的能力。

Result: 在两个公共基准数据集上的广泛实验表明，JCQL在KBC和KBQA任务上都超越了所有基线方法。

Conclusion: 通过结合LLM的推理能力和SLM的效率，JCQL框架成功实现了KBC和KBQA任务的相互增强，为知识库相关任务提供了有效的联合解决方案。
...
TL;DR: HybridKV是一种混合KV缓存压缩框架，通过分层压缩策略解决多模态大语言模型中KV缓存内存开销过大的问题，在保持性能的同时显著降低内存使用并加速解码。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在处理视觉输入时，每个视觉输入会扩展为数千个token，导致KV缓存随上下文长度线性增长并持续驻留在GPU内存中，造成高昂的内存开销和延迟，即使在高性能GPU上也难以承受。
...
Conclusion: HybridKV通过识别注意力头的异构行为并采用分层压缩策略，有效解决了多模态大语言模型中KV缓存的内存瓶颈问题，在保持模型性能的同时显著提升了推理效率。
...
TL;DR: 提出一种结合渐进置信度估计与校准的深度研究智能体，通过深思熟虑的检索和多跳推理生成可信赖的研究报告，解决现有评估框架无法衡量开放领域研究中内容可信度的问题。


<details>
  <summary>Details</summary>
Motivation: 现有深度研究智能体能够自动生成跨领域研究报告，但现有评估框架主要基于主观维度，无法有效衡量生成内容的可信度。在开放领域研究中缺乏标准答案的情况下，现有方法难以测量认知置信度，导致校准困难，用户容易受到误导或幻觉信息的影响。

Method: 提出一种新型深度研究智能体，在报告生成流程中整合渐进置信度估计与校准。系统采用深思熟虑的检索模型，结合深度检索和多跳推理，将输出基于可验证证据，并为每个声明分配置信度分数。通过精心设计的工作流程，生成具有增强透明度的可信赖报告。

Result: 实验结果表明，该方法显著提高了报告的可解释性，并大幅增强了用户信任度。案例研究进一步验证了系统在生成可信赖报告方面的有效性。

Conclusion: 通过整合渐进置信度估计与校准机制，结合深思熟虑的检索和多跳推理，能够生成具有更高透明度和可信度的研究报告，有效解决开放领域研究中内容可信度评估的挑战。
...
TL;DR: 提出Pareto-Lenient Consensus (PLC)框架，通过共识驱动的宽容梯度修正，允许局部性能下降以换取全局帕累托改进，突破传统多目标对齐方法的局部收敛限制
...
Method: 提出Pareto-Lenient Consensus (PLC)博弈论框架，将对齐重新构想为动态协商过程，引入共识驱动的宽容梯度修正，动态容忍局部性能下降，前提是存在足够的优势联盟盈余，使优化轨迹能够逃离局部次优均衡并探索远端帕累托最优前沿

Result: 理论分析验证PLC能够促进僵局逃离并渐近收敛到帕累托共识均衡；大量实验表明PLC在固定偏好对齐和全局帕累托前沿质量方面均超越基线方法

Conclusion: PLC框架突破了传统多目标对齐方法的局限性，展示了协商驱动对齐作为MPA有前景方向的潜力，代码已开源
...
TL;DR: 论文提出"认知盲化"协议，用于检测LLM在推理中混入训练记忆的程度，通过匿名化实体标识符并比较输出来衡量数据驱动推理与参数知识的比例。


<details>
  <summary>Details</summary>
Motivation: 在开发基于LLM的药物靶点优先排序系统时，发现LLM输出会无声地混合数据驱动推理与训练记忆中的实体先验知识，且这种混合无法从单次输出中辨别，导致无法评估分析过程是否遵循研究者设计的流程。

Method: 提出认知盲化协议：在推理时将实体标识符替换为匿名代码后提示LLM，然后将输出与未盲化的对照组比较。该方法不使LLM推理确定化，但能恢复可审计性。系统还包括LLM引导的评分函数进化优化和盲化智能体推理。

Result: 在四种癌症类型的肿瘤药物靶点优先排序中，盲化改变了16%的前20个预测，同时保持了已验证靶点的相同恢复率。在S&P 500股票筛选中，品牌认知偏见在五个随机种子中重塑了30-40%的前20排名。协议已开源发布。

Conclusion: 认知盲化不是为产生更好结果，而是为了评估智能体在多大程度上遵循研究者设计的分析流程。该协议解决了LLM输出中数据驱动推理与训练记忆混合的不可见性问题，为审计提供了关键维度。
...
<details>
  <summary>Details</summary>
Motivation: 现有智能体基准存在三个关键限制：(1)仅检查最终输出的轨迹不透明评分，(2)安全性和鲁棒性评估不足，(3)模态覆盖和交互范式狭窄。需要更全面的评估框架来准确评估自主智能体在多步工作流中的真实能力。

Method: Claw-Eval包含300个人工验证的任务，涵盖9个类别，分为三组：通用服务编排、多模态感知与生成、多轮专业对话。通过三个独立证据通道（执行轨迹、审计日志、环境快照）记录每个智能体动作，实现基于2,159个细粒度评分项的轨迹感知评分。评分协议评估完成度、安全性和鲁棒性，报告平均分数、Pass@k和Pass^k。

Result: 对14个前沿模型的实验显示：(1)轨迹不透明评估系统性不可靠，遗漏了44%的安全违规和13%的鲁棒性失败；(2)受控错误注入主要降低一致性而非峰值能力，Pass^3下降高达24%而Pass@3保持稳定；(3)多模态性能差异显著，大多数模型在视频任务上表现较差，没有单一模型在所有模态上占主导。

Conclusion: Claw-Eval不仅是一个基准测试套件，还揭示了智能体开发的可操作方向，强调了构建不仅有能力而且可靠可部署的智能体所需的关键要素。轨迹感知评估对于准确评估智能体能力至关重要。
