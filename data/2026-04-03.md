...
TL;DR: 本文提出结构化LLM路由本质上是一个系统层面的负载分配问题，而非单纯的提示工程问题。研究发现没有通用的最佳路由模式，性能主要由后端特定的交互效应决定，需要根据异构后端条件进行权衡。


<details>
  <summary>Details</summary>
Motivation: 当前结构化LLM路由通常被视为提示工程问题，但作者认为这本质上是系统层面的负载分配问题。随着LLM成为智能体AI系统的核心控制组件，可靠的结构化路由必须在实际部署约束下平衡正确性、延迟和实现成本。

Method: 通过全面的全因子基准测试，评估了48种部署配置和15,552个请求，覆盖OpenAI、Gemini和Llama后端。研究考察了结构工作在不同生成栈层的分配方式：模型直接输出结构、传输过程中压缩、或生成后本地重建。
...
<details>
  <summary>Details</summary>
Motivation: 传统因果推断方法依赖不可观测的假设(如可忽略性、平行趋势等)来替代缺失的反事实数据，但都无法直接产生反事实结果本身。需要一种新框架来使因果主张更具可检验性

Method: 提出数字孪生反事实框架：1) 将数字孪生模拟器形式化为潜在结果框架中的随机映射；2) 引入孪生保真度假设层次(边际保真度、联合保真度、结构保真度)；3) 建立五层验证架构将不可证伪主张转化为可检验测试；4) 形式化分解将因果量分为可边际验证和copula依赖两类；5) 提供边界、敏感性和不确定性量化工具

Result: 该框架使边际因果主张(如ATE、CATE、QTE)可通过可观测臂比较进行检验，而联合因果主张(如ITE分布、获益/损害概率、处理效应方差)则明确依赖于不可观测的个体内依赖结构。框架不解决因果推断的根本问题，但使不同因果主张的可检验性变得清晰

Conclusion: DTCF提供了一个系统框架，使边际因果主张变得可检验，联合因果主张变得假设索引化，并形式化表征了两者之间的差距。它通过模拟而非统计估计反事实，将因果推断从依赖不可验证假设转向基于模拟和分层验证的方法
...
Method: IDEA2采用半自动化工作流，集成了大语言模型和专家在环的协作过程。核心是迭代循环：从需求文档中初始LLM提取CQs，领域专家在协作平台上进行共同创建性审查和反馈，然后LLM基于反馈迭代重新表述被拒绝的CQs直至达成共识。使用溯源模型跟踪每个CQ的完整生命周期。

Result: 在2个真实世界场景（科学数据、文化遗产）中验证，表明IDEA2能够加速需求工程过程，提高最终CQs的接受度和相关性，并在领域专家中展现出高可用性和有效性。

Conclusion: IDEA2通过集成LLM和专家在环的协作过程，有效解决了本体工程中能力问题提取的瓶颈问题，提高了效率和质量，并确保了透明度和可重复性。
...
Result: 建立了WMAS的理论框架，包括世界分类维度、语义模型形式化方法，并实现了Ontobox平台作为具体实例，展示了基于共享世界模型的系统如何实现全局一致性和可验证行为。

Conclusion: 世界中心多智能体系统为结构化领域提供了一种更优的架构选择，通过共享的显式世界表示确保了语义一致性、可解释性和长期稳定性，为企业和制度系统的智能化提供了理论基础和实现路径。
...
TL;DR: AI自动化呈现为"潮汐式上升"而非"波浪式崩溃"，AI能力在广泛任务上持续提升，预计到2029年LLM能以80%-95%成功率完成大多数文本任务


<details>
  <summary>Details</summary>
Motivation: 研究AI自动化的本质特征，区分"波浪式崩溃"（AI能力在少数任务上突然爆发）和"潮汐式上升"（AI能力在广泛任务上持续提升）两种模式，通过实证数据验证哪种模式主导AI自动化进程

Method: 基于美国劳工部O*NET分类体系，构建超过3000个基于文本的LLM可处理任务评估集；通过17000多次由相关职业工作者进行的评估，测量AI模型在广泛任务上的表现；分析AI能力随时间的变化趋势

Result: 发现很少证据支持"波浪式崩溃"，但大量证据表明"潮汐式上升"是AI自动化的主要形式；2024年第二季度AI模型能以约50%成功率完成人类需3-4小时的任务，预计到2025年第三季度提升至65%；若当前趋势持续，到2029年LLM在大多数文本任务上的平均成功率将达到80%-95%

Conclusion: AI自动化主要表现为广泛而持续的"潮汐式上升"模式，而非特定任务上的突然突破；AI能力提升将对经济产生深远影响，但组织采用AI的过程可能需要更长时间
...
TL;DR: 该研究探讨了LLM中的认知偏见，发现偏见在模型激活空间中呈线性可分方向，通过激活导向技术可显著减少偏见同时保持下游能力。
...
Method: 定义了LLM认知偏见为系统性的、可复现的偏离正确答案的行为，并引入LLM CogBias基准，围绕四个认知偏见家族组织：判断、信息处理、社会和反应偏见。使用线性探针和对比设计分析偏见在激活空间中的表征，应用激活导向技术来调节偏见行为。

Result: 认知偏见在所有四个家族中系统性地出现，其程度和去偏见效果强烈依赖于偏见家族：提示级去偏见显著减少反应偏见，但对判断偏见产生反效果。偏见在模型激活空间中编码为线性可分方向，激活导向技术实现了26-32%的偏见分数减少，同时在25个基准测试中保持了下游能力。

Conclusion: 尽管不同模型间的偏见表征近乎正交，但激活导向在不同架构中以相似速率减少偏见，表明存在共享的功能组织。该研究为理解和缓解LLM中的认知偏见提供了实证基础和方法框架。
...
TL;DR: 提出RIFT分类法，系统化诊断评估量规的质量问题，包含8种失效模式，分为3大类，并通过自动化指标实现可扩展诊断。
...
Method: 使用扎根理论方法，从5个多样化基准中迭代标注量规，开发RIFT分类法；通过人工标注一致性评估分类法；提出自动化量规质量指标并与人工标注对齐。

Result: RIFT包含8种失效模式，分为可靠性失效、内容有效性失效和后果有效性失效3大类；人工标注一致性达87%配对一致性和0.64平均Cohen's kappa；自动化指标与人工失效模式标注对齐，F1最高达0.86。

Conclusion: RIFT为量规质量诊断提供了系统化框架，支持可扩展的诊断方法，有助于改进LLM评估和训练中的量规设计。
...
TL;DR: 提出基于信息价值(VOI)的POMDP规划框架VOIMCP，通过选择性忽略低VOI的观测信息来高效分配计算资源，避免不必要的观测分支。


<details>
  <summary>Details</summary>
Motivation: POMDP在状态和转移不确定性下的规划具有原则性形式，但受维度灾难和历史灾难影响，在有限规划时间内获得高性能策略仍是主要挑战。许多POMDP问题中，信息价值(VOI)在信念空间上变化，这为优化计算分配提供了机会。

Method: 提出动态规划框架，基于每个信念点的信息价值有条件地处理观测。在此基础上提出VOIMCP算法，这是一种蒙特卡洛树搜索算法，当VOI较低时选择性地忽略观测信息，避免不必要的观测分支，从而更高效地分配计算资源。

Result: 提供了VOI推理框架的近似最优性理论保证，推导了VOIMCP的非渐近收敛边界。仿真评估表明，VOIMCP在多个POMDP基准测试中优于基线方法。

Conclusion: 通过利用信息价值在信念空间中的变化特性，提出的VOIMCP框架能够更高效地分配计算资源，在有限规划时间内获得更好的性能，为大规模POMDP规划提供了有效的解决方案。
...
TL;DR: 该论文研究了AI辅助药物系统在现实医疗环境中的可靠性，重点关注系统错误类型及其临床后果，而非仅依赖传统性能指标。


<details>
  <summary>Details</summary>
Motivation: AI系统在医疗和药学领域应用日益广泛，但在高风险药物管理场景中，即使单个错误也可能导致严重患者伤害。传统评估指标无法充分反映系统在真实决策环境中的可靠性，需要关注错误发生机制及其临床影响。

Method: 通过一系列受控模拟场景（包括药物相互作用和剂量决策），分析不同类型的系统故障：漏检相互作用、错误风险标记和不适当剂量推荐。研究关注错误如何发生以及AI系统产生错误输出时的后果。

Result: 研究发现AI在药物相关场景中的错误可能导致不良反应、治疗无效或延迟护理，特别是在缺乏足够人工监督的情况下。研究还揭示了过度依赖AI推荐的风险以及决策过程透明度有限的挑战。

Conclusion: 该研究为医疗AI评估提供了可靠性视角，强调理解故障行为和现实影响的重要性。在药房实践等安全关键领域，需要将传统性能指标与风险感知评估方法相结合，并确保充分的人工监督。
...
TL;DR: Elhuyar是一个多智能体、人机协同系统，整合大语言模型、结构化AI和人类科学家，用于从科学文献中提取、分析和迭代精炼见解，加速科学发现。


<details>
  <summary>Details</summary>
Motivation: 科学发现受到碎片化文献的限制，需要大量人工努力来收集、分析和理解。现有AI工具缺乏结构化、多步骤的方法来从科学文献中提取深度见解，而大语言模型存在幻觉和不完整提取的问题。

Method: 开发了Elhuyar系统，这是一个多智能体、人机协同框架，将任务分配给专门的智能体：筛选论文、提取数据、拟合模型和总结发现，同时通过人类监督确保可靠性。系统生成结构化报告，包含提取的数据、可视化、模型方程和文本摘要，支持通过迭代精炼进行深入探究。

Result: 在材料科学领域部署该系统，分析了钨在氦离子辐照下的文献，展示了实验相关的指数型氦气泡生长与辐照剂量和温度的关系，为聚变反应堆中的等离子体面向材料提供了见解。

Conclusion: Elhuyar系统展示了AI辅助文献综述如何揭示科学模式并加速发现，通过整合大语言模型、结构化AI和人类专业知识，克服了现有工具的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM方法主要关注可行实例的解决方案生成，缺乏对不可行性检测的明确处理，这在组合优化问题中是一个重要但被忽视的方面

Method: 提出不可行性感知框架：1) 通过数学规划公式和零相位不可行性筛选构建可验证训练数据集；2) 使用监督微调训练LLM进行联合解决方案生成和不可行性检测；3) 利用LLM输出作为下游局部搜索的初始解

Result: 微调后的8B参数LLM相比GPT-5.2整体准确率提升30%；LLM引导的初始解使下游局部搜索速度提升2倍

Conclusion: 该框架成功解决了组合优化中不可行性检测的挑战，证明了LLM在优化问题中的实用价值，即使LLM输出不完美也能通过下游搜索加速优化过程
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（如提示工程、思维链）仅关注单个文档，忽略了文档间关系，导致大语言模型在从大型文本语料库中提取数据时产生幻觉。科学文献总结中，相似实验设置的论文应得出相似结论，需要利用文档间关系提高提取准确性。

Method: 提出Peer Context Outlier Detection (P-COD)方法：通过比较提取数据与语料库中已验证的同行信息，调整置信度分数，标记低置信度结果供专家审查。高置信度结果（得到同行验证支持）被视为可靠。

Result: 在6个科学领域的实验中，异常检测精度高达98%。该方法减少幻觉，增强对自动化系统的信任，使研究人员能够专注于模糊案例，简化数据提取工作流程。

Conclusion: P-COD通过利用文档间关系有效减少LLM在科学文献数据提取中的幻觉，提高提取准确性，增强系统可信度，优化研究流程。
...
TL;DR: 提出一个基于技能演化的智能体框架，用于超表面逆向设计，通过迭代优化特定求解器的策略而不修改模型权重或物理求解器，提高任务成功率和效率。


<details>
  <summary>Details</summary>
Motivation: 超表面逆向设计需要复杂的计算电磁学专业知识和特定求解器的软件工程能力，现有语言驱动系统缺乏跨任务的可复用工作流知识积累，限制了设计效率。

Method: 开发了一个包含编码智能体、演化技能构件和基于物理仿真的确定性评估器的框架，通过上下文级技能演化迭代优化特定求解器策略，保持物理求解器不变。

Result: 在训练对齐任务族中，任务成功率从38%提升到74%，标准通过率从0.510提高到0.870，平均尝试次数从4.10减少到2.30；在保留任务族中，虽然二进制成功率变化不大，但最佳裕度改进、错误构成变化和智能体行为表明工作流知识的部分迁移。

Conclusion: 技能演化的主要价值在于围绕可靠计算引擎积累可复用的特定求解器专业知识，为实现更自主和易访问的超表面逆向设计工作流提供了实用路径。
...
TL;DR: 提出基于角色的LLM框架用于健康食品政策信息提取，通过分配专家角色解决政策文档结构多样性带来的幻觉、误分类和遗漏问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在健康食品政策领域的信息提取面临挑战，包括幻觉、误分类和遗漏，主要源于政策文档的结构多样性和不一致性。需要更可靠的方法来自动化政策信息提取。

Method: 提出基于角色的LLM框架，分配三个专家角色：LLM政策分析师（元数据和机制分类）、LLM法律策略专家（识别复杂法律方法）、LLM食品系统专家（分类食品系统阶段）。框架通过角色特定提示融入结构化领域知识，包括法律机制明确定义和分类标准。
...
Conclusion: 基于角色的LLM框架通过模拟专家分析工作流程，结合结构化领域知识，显著提升了健康食品政策信息提取的准确性和可靠性，为解决政策文档结构多样性带来的挑战提供了有效方案。
...
TL;DR: 本文研究将表示自编码器（RAE）整合到连续自回归（AR）模型中的挑战，提出两种改进方法：分布归一化简化建模难度，以及高斯噪声注入缓解曝光偏差，使RAE在AR模型中达到与VAE相当的性能。


<details>
  <summary>Details</summary>
Motivation: 生成模型的潜在空间长期由VAE编码器主导，而预训练表示编码器（如DINO、SigLIP、MAE）的潜在表示被认为不适合生成建模。虽然RAE方法显示表示自编码器也能达到与VAE编码器竞争的性能，但将其整合到连续自回归（AR）模型中的研究仍然很少。本文旨在探索高维表示自编码器在AR范式中的挑战。

Method: 1. 分析AR模型的独特性质，识别两个主要障碍：复杂的token-wise分布建模和高维性放大的训练-推理差距（曝光偏差）。2. 通过分布归一化进行token简化，降低建模难度并改善收敛性。3. 在训练期间加入高斯噪声注入，增强预测鲁棒性，缓解曝光偏差问题。

Result: 实验结果表明，这些修改显著缩小了性能差距，使表示自编码器在AR模型上能够达到与传统VAE相当的结果。这为视觉理解和生成建模之间更统一的架构铺平了道路。

Conclusion: 本文成功解决了将高维表示自编码器整合到连续自回归模型中的挑战，通过token简化和噪声注入方法，使RAE-AR模型能够达到与VAE-AR相当的性能，推动了视觉理解与生成建模架构的统一化。
...
TL;DR: 研究发现LLM训练中归一化层和优化器之间存在显著交互作用：Dynamic Erf与Muon优化器组合时出现负向交互，性能差距扩大3倍，而Dynamic Tanh无此问题。问题源于erf在Muon快速谱范数增长下的饱和与尺度盲区失效模式。


<details>
  <summary>Details</summary>
Motivation: 传统上LLM训练中将归一化层和优化器视为独立设计选择，但本研究通过实验发现这种假设可能不成立，特别是Dynamic Erf与Muon优化器组合时出现严重性能下降，揭示了归一化-优化器交互作用的重要性。

Method: 采用3x2因子设计实验，在10亿参数规模和1000训练步数下，测试不同归一化层（Dynamic Erf、Dynamic Tanh、RMSNorm）与优化器（AdamW、Muon）的组合。通过分析erf在Muon下的失效模式（饱和和尺度盲区），提出两种解决方案：EMA混合重新引入运行尺度估计，以及降低Derf的alpha参数。

Result: Dynamic Erf与Muon组合时出现严重负向交互：与RMSNorm的差距从AdamW下的+0.31 nats扩大到Muon下的+0.97 nats（3倍增长）。Dynamic Tanh无此问题。EMA混合恢复约84%性能差距，降低Derf alpha从0.5到0.3恢复约80%性能。使用Derf默认alpha与Muon组合会产生0.66-nat交互惩罚，但不会导致NaN或发散，在短期试验中容易被忽略。

Conclusion: 归一化层和优化器不应被视为独立设计选择，存在显著交互作用。Dynamic Erf与Muon组合存在严重兼容性问题，源于erf在快速谱范数增长下的失效模式。通过调整参数（alpha降低）或引入EMA混合可缓解问题。研究强调了在LLM训练中系统测试归一化-优化器组合的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在线性OR问题中表现良好，但在现实世界非线性场景中因数学公式与求解器代码之间的语义不对齐以及信息提取不稳定而性能严重下降

Method: 提出NED-Tree框架：(a)采用逐句提取策略确保参数映射的鲁棒性和可追溯性；(b)使用递归树结构将复杂非线性项自适应分解为求解器兼容的子元素

Result: 在10个基准测试中平均准确率达到72.51%，建立了新的最先进水平；同时创建了NEXTOR基准测试用于复杂非线性、多约束OR问题

Conclusion: NED-Tree是首个通过元素分解驱动LLM解决非线性建模困难的框架，实现了建模语义与代码语义的对齐，为OR问题自动化建模提供了系统解决方案
...
TL;DR: ThinkTwice是一个两阶段框架，通过GRPO联合优化LLM的推理能力和答案精炼能力，无需正确性信号或批判标注，在数学推理基准上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习与验证（RLVR）方法通常需要额外的正确性信号或批判标注来训练模型进行自我精炼。本文旨在开发一种无需这些额外监督信号的方法，通过联合优化推理和精炼能力来提升模型性能。
...
Conclusion: 联合训练推理和自我精炼是RLVR的一种原则性且有效的方法论。ThinkTwice框架通过简单的两阶段优化，无需额外监督信号，显著提升了模型的推理和精炼能力，为强化学习与验证提供了新的研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 探究LLMs在教学决策时是采用基于学习者知识推理的贝叶斯最优策略，还是使用简单的启发式规则，通过受控任务比较LLMs与人类的教学策略

Method: 使用先前研究人类教学策略的受控任务，让LLMs作为模拟教师，在奖励标注的有向图中观察假设学习者的轨迹，揭示单一边缘以改善学习者路径规划。使用与人类研究相同的认知模型（贝叶斯最优教师、弱贝叶斯变体、启发式基线、非心智化效用模型）拟合LLMs的逐试次选择，并进行模型比较（BIC）

Result: 在基线实验中，大多数LLMs表现良好，策略在试次间变化小，图间表现与人类相似。模型比较显示贝叶斯最优教学最能解释大多数模型的决策。脚手架干预使模型遵循推理或奖励导向的提示，但这些干预不一定能改善在启发式不一致测试图上的教学效果，有时甚至降低性能

Conclusion: 认知模型拟合揭示了LLMs的教学策略，表明它们主要采用贝叶斯最优推理而非简单启发式，同时提示合规性不能保证更好的教学决策
...
<details>
  <summary>Details</summary>
Motivation: 现有拒绝特征选择方法依赖特征在有害提示上的激活强度，这倾向于捕捉表面信号而非拒绝决策的因果因素。需要更可靠的方法来识别因果性拒绝特征以理解LLM拒绝行为的内在机制。

Method: 提出CRaFT框架，通过使用拒绝边界附近的提示来评估特征对模型拒绝-合规决策的影响度，以此对特征进行排序。该方法基于电路影响而非激活强度来选择特征。

Result: 在Gemma-3-1B-it模型上，CRaFT将攻击成功率从6.7%提升至48.2%，在多个越狱基准测试中优于基线方法。

Conclusion: 电路影响是比激活强度更可靠的准则，用于识别因果性中介拒绝行为的特征。CRaFT框架为理解LLM拒绝机制提供了更有效的方法。
...
TL;DR: 提出Metric Freedom (F)作为技能效用的先验预测指标，揭示技能效用取决于评估指标而非任务本身，并开发自适应蒸馏框架优化多智能体系统


<details>
  <summary>Details</summary>
Motivation: 多智能体系统(MAS)虽然能处理复杂任务，但存在协调开销大、上下文碎片化和脆弱的阶段排序等问题。将MAS蒸馏为单智能体技能可以绕过这些成本，但缺乏何时蒸馏什么的原则性指导，导致实证结果不一致（技能提升从28%改善到2%退化）

Method: 1. 引入Metric Freedom (F)作为技能效用的先验预测指标，通过Mantel测试量化输出多样性与得分方差之间的耦合度，衡量评估指标评分地形的拓扑刚性；2. 提出两阶段自适应蒸馏框架：阶段1作为选择性提取机制，在"自由"指标上提取工具和知识同时丢弃限制性结构以保持探索；阶段2专门针对"刚性"指标(F ≤ 0.6)进行计算密集的迭代优化，消除轨迹局部过拟合

Result: 在4个任务、11个数据集和6个指标上的评估显示，F能强预测技能效用(ρ=-0.62, p<0.05)。相同的智能体轨迹在刚性vs自由指标下产生截然相反的技能提升，证明技能效用本质上是指标层面的属性。自适应智能体匹配或超越原始MAS，同时将成本降低8倍、延迟降低15倍

Conclusion: 技能效用主要由评估指标的拓扑特性决定而非任务本身，Metric Freedom (F)作为有效的先验预测指标，指导的自适应蒸馏框架能显著提升多智能体系统的效率和性能
...
Method: 提出GraphWalk框架，为LLM提供一组最小正交图操作工具，使其能够通过顺序图导航进行推理，每个工具调用代表可验证的执行步骤

Result: 在迷宫遍历和合成企业知识图谱上的评估显示，基于工具的遍历方法相比上下文基线在所有测试模型家族中都获得显著提升，且随着规模增大优势更明显

Conclusion: GraphWalk框架通过工具化图遍历使LLM能够处理大规模知识图谱的复杂推理任务，提供透明执行轨迹，解决了上下文窗口限制问题
...
TL;DR: 对四种AWS Bedrock大语言模型在收据项目分类任务上进行系统性成本感知评估，发现Claude 3.7 Sonnet在准确性和成本效率方面达到最佳平衡


<details>
  <summary>Details</summary>
Motivation: 在生产导向的分类框架中，需要系统评估大语言模型在收据项目分类任务上的性能与成本效益，比较不同模型在准确性、响应稳定性和token级成本方面的表现，并研究零样本和少样本提示方法在准确性和成本方面的适用性

Method: 使用AWS Bedrock平台上的四种指令调优模型（Claude 3.7 Sonnet、Claude 4 Sonnet、Mixtral 8x7B Instruct、Mistral 7B Instruct），在收据项目分类任务上进行实验，评估准确性、响应稳定性和token级成本，比较零样本和少样本提示方法的效果

Result: 实验结果表明，Claude 3.7 Sonnet在分类准确性和成本效率之间达到了最有利的平衡，为收据项目分类任务提供了最优的模型选择

Conclusion: Claude 3.7 Sonnet是收据项目分类任务中最具成本效益的模型选择，该研究为生产环境中的大语言模型应用提供了基于实际成本和性能的系统评估框架
...
Method: 提出承诺不确定性定位（commitment uncertainty localization）：给定去噪轨迹，识别交叉链熵超过无监督阈值的token位置；引入OSCAR框架：运行N条并行去噪链（随机揭示顺序），计算交叉链香农熵检测高不确定性位置，然后基于检索证据进行针对性重掩码

Result: 在TriviaQA、HotpotQA、RAGTruth和CommonsenseQA上使用LLaDA-8B和Dream-7B，OSCAR显著减少幻觉内容并提高事实准确性；其基于熵的不确定性信号优于专门的训练检测器，消融实验证实定位和校正具有互补增益

Conclusion: 扩散语言模型具有识别事实不确定性的内在能力，这在自回归模型的顺序token承诺结构中不存在；OSCAR展示了利用DLMs轨迹进行不确定性感知生成的有效性，为未来研究提供了基础
...
TL;DR: EnviSmart是一个部署在校园存储基础设施上的生产数据管理系统，通过三轨知识架构和角色分离的多智能体设计，将可靠性作为架构属性，解决了LLM驱动的智能体在环境FAIR数据管理中可能产生看似合理但错误输出的问题。


<details>
  <summary>Details</summary>
Motivation: 将LLM驱动的智能体嵌入环境FAIR数据管理具有吸引力，因为它们可以外部化操作知识并在异构数据和不断发展的规范中进行规模化策展。然而，用概率性工作流替换确定性组件改变了故障模式：LLM管道可能生成看似合理但错误的输出，这些输出通过表面检查并传播到不可逆的操作中，如DOI分配和公开发布。
...
Result: 在两个生产部署中进行了比较：大学GIS中心生态档案（849个策展数据集）作为单智能体基线；SF2Bench复合洪水基准（2,452个监测站和8,557个发布文件，跨越39年）验证了多智能体工作流。多智能体方法提高了效率（由单个操作员在两天内完成，并在部署中重复使用工件）和可靠性：审计交接检测并阻止了影响所有2,452个站点的坐标转换错误。代表性事件（ISS-004）展示了基于边界的遏制，检测延迟为10分钟，零用户暴露，解决时间为80分钟。

Conclusion: EnviSmart通过架构方法解决了LLM驱动数据管理的可靠性问题，将三轨知识架构与角色分离的多智能体设计相结合，在信任边界处恢复确定性保证，防止错误传播到不可逆操作中。该系统已在PEARC 2026被接受。
...
TL;DR: CORAL是一个基于大语言模型的多智能体自主进化框架，用于开放式发现问题，通过持久记忆、异步执行和心跳干预实现知识积累与协作，在数学、算法和系统优化任务上取得显著性能提升。
...
Result: 在多样化的数学、算法和系统优化任务评估中，CORAL在10个任务上创造了新的最先进结果，相比固定进化搜索基线，改进率提高了3-10倍且评估次数更少。在Anthropic内核工程任务中，四个协同进化智能体将最佳已知得分从1363周期提升到1103周期。

Conclusion: 机制分析表明这些收益来自知识重用和多智能体探索与通信。结果表明更大的智能体自主性和多智能体进化能显著改善开放式发现问题。代码已开源。
...
Result: 建立了包含七种模式的完整设计词汇，提供了初级保健AI系统的参考架构和糖尿病风险预测场景的完整走查

Conclusion: 为构建抗本体扭曲的临床AI系统提供了理论基础和设计框架，但缺乏实证验证，需在未来生产系统中评估
...
TL;DR: BACM-RL：一种基于强化学习的上下文预算感知管理方法，通过序列决策优化长时推理任务中的历史信息压缩策略


<details>
  <summary>Details</summary>
Motivation: LLM智能体在长时推理任务中面临上下文长度限制，需要在有限预算下平衡历史信息保留与上下文约束，现有方法在预算缩减时性能下降明显

Method: 提出预算感知上下文管理（BACM）框架，将上下文管理建模为带预算约束的序列决策问题，并开发BACM-RL——基于课程学习的端到端强化学习方法，学习不同预算下的压缩策略

Result: 在组合多目标QA和长时网页浏览基准测试中，BACM-RL在不同模型规模和任务复杂度下均优于现有方法，在高复杂度场景下获得1.6倍以上性能提升，且在预算缩减时保持优势

Conclusion: BACM-RL通过强化学习优化上下文压缩策略，有效解决了LLM智能体在有限上下文预算下的长时推理挑战，为实际部署提供了可行的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有模型融合方法主要针对同构模型（来自相同预训练骨干），但在开放模型生态系统中，有用专家模型往往基于不同架构（如Llama、Qwen、Mistral），直接权重空间融合因架构不匹配、潜在基不对齐和跨源冲突而变得不适定。

Method: HeteroFusion包含两个核心组件：1）基于拓扑的对齐，通过匹配功能模块结构而非原始张量坐标来跨异构骨干传递知识；2）冲突感知去噪，在融合过程中抑制不兼容或噪声传递信号。该方法在保持目标适配器基的同时预测结构化更新，确保稳定且良条件的传递过程。

Result: 在异构传递、多源融合、噪声源鲁棒性和跨家族泛化等设置中，HeteroFusion始终优于强大的融合、合并和集成基线方法。

Conclusion: HeteroFusion为异构语言模型融合提供了有效解决方案，通过拓扑对齐和冲突感知去噪克服了架构不匹配问题，在开放模型生态系统中实现了更好的知识整合。
...
<details>
  <summary>Details</summary>
Motivation: AIGC的快速扩散正在重构在线内容生态，需要对其行为特征和分布影响进行严谨研究，以理解AIGC与人类生成内容在创作和消费方面的差异及其对平台生态的影响。

Method: 利用中国领先视频分享平台的纵向数据集，包含数千万用户数据，对比分析AIGC与HGC的创作和消费行为特征，并深入探究算法内容分发机制的作用。

Result: 发现"规模优先于偏好"的动态：AIGC创作者通过高产量获得与HGC相当的总体参与度，尽管消费者明显偏好HGC；算法分发机制能有效调节AIGC相关的竞争利益。
...
Method: 1) 两阶段级联架构：第一阶段高召回故障检测，第二阶段异常样本细粒度分类；2) 多方法融合压缩策略：基于互信息、梯度分析和SE注意力权重减少传感器通道；3) 1+1分支LiteInception架构压缩InceptionTime参数；4) 知识蒸馏作为精确率-召回率调节机制；5) 双层可解释性框架集成四种归因方法。

Result: 在NGAFID数据集上：故障检测准确率81.92%，召回率83.24%；故障识别准确率77.00%。模型压缩：传感器通道从23减至15，参数压缩70%，CPU推理加速8倍以上，F1损失小于3%。

Conclusion: LiteInception框架在效率、准确性和可解释性之间取得了良好平衡，适用于资源受限的边缘设备部署，并能通过切换训练策略适应不同诊断场景需求。
...
<details>
  <summary>Details</summary>
Motivation: AnIML作为分析化学和生物学的XML标准，在工业研发实验室中广泛使用，但其XML模式的表达性允许不同利益相关者产生分歧解释，导致语义不一致，破坏了AnIML模式原本设计的互操作性支持。

Method: 采用专家在环方法，结合LLM辅助需求获取与协作本体工程开发AnIML本体论。通过多层验证：将真实AnIML文件转换为知识图谱的数据驱动转换、通过SPARQL验证能力问题、以及基于对抗性负面能力问题映射到已建立的本体反模式并通过SHACL约束强制执行的新型验证协议。

Result: 开发了AnIML本体论，成功形式化了AnIML语义并与Allotrope数据格式对齐，通过多层验证方法证明了本体的有效性和可靠性。

Conclusion: AnIML本体论通过形式化语义和与Allotrope对齐，解决了XML模式导致的语义不一致问题，为跨系统和跨实验室互操作性提供了坚实基础，支持数据驱动的科学发现。
...
TL;DR: SAT框架解决二维单尺寸切割库存问题，通过需求扩展、板材分配变量、非重叠约束和不可行方向消除规则，在Cui-Zhao基准测试中优于OR-Tools、CPLEX和Gurobi


<details>
  <summary>Details</summary>
Motivation: 二维单尺寸切割库存问题(2D-CSSP)是制造业中的核心任务，需要从库存板材中切割矩形物品以满足需求同时最小化浪费。该问题通过要求每种物品类型的多个副本而推广了装箱问题，这导致了强烈的组合爆炸。现有方法在处理这种组合复杂性方面存在挑战，需要更有效的解决方案。

Method: 提出了一个基于SAT的框架，其中物品类型按需求进行扩展，每个副本都有一个板材分配变量，并且非重叠约束仅在分配给同一板材的副本之间激活。还引入了不可行方向消除规则，当只有一种方向能放入板材时固定旋转变量。对于最小化板材数量，比较了三种方法：非增量SAT与二分搜索、跨迭代重用子句的增量SAT以及加权部分MaxSAT。

Result: 在Cui-Zhao基准测试套件上，最好的SAT配置证明为最优的实例数量是OR-Tools、CPLEX和Gurobi的2-3倍，并且获得了更低的优化差距。SAT方法之间的相对排名取决于旋转：无旋转时增量SAT最强，而当旋转增加公式大小时，非增量SAT更有效。

Conclusion: SAT方法为二维切割库存问题提供了有效的解决方案，在处理组合复杂性方面优于传统优化工具。不可行方向消除规则和不同SAT策略的适应性使用显著提高了性能，证明了SAT求解器在组合优化问题中的潜力。
...
TL;DR: AeroTherm-GPT：首个用于高超声速热防护系统设计的LLM智能体，采用约束闭环生成框架解决传统LLM在生成可执行仿真工件时的级联约束违反问题。
...
Result: 在HyTPS-Bench上评估并验证外部基准，AeroTherm-GPT实现88.7%端到端成功率（95%置信区间：87.5-89.9），比非CDG消融基线提高12.5个百分点。上游优先机制每次动作解决多个下游违反，根因修复效率达4.16（扁平清单修复为1.76），且在科学推理和代码生成任务上无灾难性遗忘。

Conclusion: 约束闭环生成框架通过约束依赖图指导的迭代修复，有效解决了LLM在高超声速热防护系统设计中面临的级联约束违反问题，显著提高了端到端成功率，为安全关键工程领域的LLM应用提供了新范式。
...
TL;DR: 提出领域情境化概念图（DCG）框架，将领域作为知识表示的内在组成部分而非外部注解，通过模态世界约束解决概念在不同领域中的意义漂移问题。


<details>
  <summary>Details</summary>
Motivation: 知识图谱能高效存储大量关系，但难以处理概念在不同领域中意义变化的问题。现有系统通常将领域信息作为元数据、限定符或图级组织处理，这些机制仅用于过滤和溯源，不改变断言的形式状态，导致许多知识系统的实际失败源于将领域视为断言的外部因素。

Method: 引入领域情境化概念图（DCG）框架，将领域写入关系并解释为模态世界约束。采用(C, R at D, C')形式，其中at D标识关系成立的世界。形式上，关系通过领域索引的必然性算子解释，使真值、推理和冲突检查都限定在相关世界中。开发了克里普克式语义、紧凑谓词系统、Prolog实现，以及到RDF、OWL和关系数据库的映射。

Result: DCG框架实现了三个重要结果：1) 模糊概念可以在表示点进行消歧；2) 无效断言可以根据其领域进行挑战；3) 跨领域关系可以通过显式谓词连接。为领域本身提供了结构化和可计算的角色。

Conclusion: 领域应被视为知识表示的一部分而非补充注解。DCG通过将领域作为表示内部的结构化和可计算元素，解决了知识系统中因将领域视为外部因素而导致的许多实际失败问题，实现了对领域本身的表示重新解释。
...
TL;DR: PGPO提出了一种细粒度的信用分配框架，通过量化token级别的视觉依赖性，动态重塑优势函数，增强视觉依赖token的学习信号，抑制语言先验的梯度噪声，显著提升多模态推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于可验证奖励的强化学习框架存在方法学缺陷：通过在所有生成token上分配相同的优势值，稀释了多模态推理中关键视觉基础步骤的学习信号，无法有效优化视觉依赖的推理过程。

Method: 提出Token Visual Dependency概念，通过KL散度量化视觉输入带来的因果信息增益；基于此设计Perception-Grounded Policy Optimization框架，采用阈值门控、质量守恒机制，在token级别动态重塑优势函数，放大视觉依赖token的学习信号，抑制语言先验的梯度噪声。

Result: 在Qwen2.5-VL系列模型上，基于七个具有挑战性的多模态推理基准测试，PGPO使模型性能平均提升18.7%。理论和实证分析证实PGPO有效降低梯度方差，防止训练崩溃，并作为强大的正则化器促进稳健的感知基础多模态推理。

Conclusion: PGPO通过细粒度的token级别信用分配，解决了现有RLVR框架中学习信号稀释的问题，实现了更有效的感知基础多模态推理优化，为视觉语言模型的强化学习提供了新的方法论框架。
...
TL;DR: 该研究评估了表格上下文学习在临床预测中的表现，提出了AWARE框架以改进检索质量，在极端不平衡情况下AUPRC提升达12.2%
...
Method: 构建多队列EHR基准，比较经典模型、深度表格模型和表格上下文学习模型。提出AWARE框架，使用监督嵌入学习和轻量级适配器实现任务对齐的检索。

Result: PFN-based TICL模型在低数据状态下样本效率高，但随着异质性和不平衡增加，基于朴素距离的检索性能下降。AWARE在极端不平衡情况下将AUPRC提升达12.2%，且随着数据复杂性增加，改进效果更明显。
...
TL;DR: 提出CG-iLAO*算法，通过约束生成技术避免考虑昂贵动作，显著减少Bellman备份中动作评估数量，加速随机最短路径问题求解


<details>
  <summary>Details</summary>
Motivation: 传统SSP求解算法使用启发式函数指导状态选择，但Bellman备份仍需评估所有可用动作，即使启发式表明某些动作代价过高，导致计算资源浪费

Method: 将启发式搜索重新表述为线性规划问题，引入针对SSP的高效约束生成技术，开发CG-iLAO*算法，在iLAO*基础上集成该技术以跳过昂贵动作

Result: CG-iLAO*仅考虑iLAO* 40%的动作（某些问题低至1%），平均减少3.5倍动作代价评估，比iLAO*快2.8倍，比LRTDP快3.7倍

Conclusion: 通过约束生成技术避免评估昂贵动作能显著提升SSP求解效率，CG-iLAO*在保持最优性的同时大幅减少计算开销
...
TL;DR: 提出一种基于可能性监督的多类分类学习方法，通过构造可容许概率分布集并计算KL投影来训练模型，在保持可能性分布定性结构的同时提高预测性能。


<details>
  <summary>Details</summary>
Motivation: 在多类分类任务中，传统监督通常使用硬标签或概率分布，但实际应用中可能存在不确定性或模糊性。可能性分布能更好地表达类别的分级可信度，但需要将其转化为适合概率模型训练的形式，同时保持可能性分布的定性结构（如序关系）。

Method: 1. 对每个训练实例，将归一化的可能性分布转化为可容许概率分布集：结合概率兼容性（与可能性/必然性测度一致）和线性形状约束（保持可能性分布的定性结构）。2. 对模型输出的正概率向量，计算其到可容许集的Kullback-Leibler投影（最近的可容许分布）。3. 使用Dykstra算法结合负熵Bregman投影计算投影，提供各约束集投影的显式公式。4. 通过最小化预测与其投影之间的散度来训练模型。

Result: 1. 投影算法在合成数据和真实世界自然语言推理任务（基于ChaosNLI数据集）上均表现出足够的计算效率，适合实际应用。2. 基于投影的学习目标能够提升预测性能，验证了方法的有效性。

Conclusion: 该方法成功地将可能性监督融入多类分类学习框架，通过构造可容许概率分布集和KL投影机制，在保持可能性分布定性结构的同时实现了有效的模型训练，为处理不确定性监督信息提供了新途径。
...
Conclusion: Qiana是一个强大且灵活的逻辑框架，能够在特定上下文中进行推理，支持量化表达和矛盾容忍，与现有一阶逻辑工具兼容，具有广泛的应用潜力。
...
TL;DR: 该研究创建了NeuroPose-AHM数据集，这是一个包含57种神经系统疾病中异常头部运动（AHMs）的知识库，通过多LLM框架从1430篇文献中提取了2756条患者组记录，并展示了其在颈部肌张力障碍分析中的应用价值。


<details>
  <summary>Details</summary>
Motivation: 神经系统疾病中的异常头部运动（AHMs）缺乏整合运动学测量、临床严重程度评分和患者人口统计学的多条件资源，这阻碍了AI驱动诊断工具的开发。

Method: 采用多LLM提取框架从1430篇同行评审文献中构建NeuroPose-AHM数据集，包含57种神经系统疾病的2756条患者组记录。通过四任务框架分析颈部肌张力障碍：1）多标签AHM类型分类；2）构建头颈严重程度指数（HNSI）；3）验证HNSI与真实患者数据的临床相关性；4）运动类型概率与HNSI分数的桥接分析。

Result: LLM提取可靠性高（kappa = 0.822），AHM类型分类F1分数为0.856。HNSI成功标准化了异质临床评分量表，与真实患者数据在严重程度区间（6.7%）对齐。运动类型概率与HNSI分数存在显著相关性（p < 0.001）。

Conclusion: NeuroPose-AHM是一个结构化、基于知识的资源，为神经系统AHM研究提供了分析工具，数据集已在Zenodo公开可用，展示了其在临床研究和AI诊断开发中的实用价值。
...
TL;DR: SenseMath基准测试评估LLM的数字结构敏感性推理能力，发现模型能应用计算捷径但缺乏人类数字感的结构理解


<details>
  <summary>Details</summary>
Motivation: 研究LLM是否具有类似人类的数字感——识别数值结构、在适当时应用捷径、在不适用时避免使用的能力，以理解模型数字推理的本质

Method: 引入SenseMath基准测试，包含4,800个项目，涵盖8个捷径类别和4个数字尺度，匹配强捷径、弱捷径和控制变体。评估三个认知需求递增的设置：捷径使用、适用性判断和问题生成

Result: 五个LLM（GPT-4o-mini到Llama-3.1-8B）显示一致模式：明确提示时模型能应用捷径策略（准确率提升达15%），但标准思维链提示下自发使用率低于40%。模型能力局限于使用层面，会过度泛化捷径到不适用的场景，且无法从头生成有效的捷径问题

Conclusion: 当前LLM表现出程序性捷径流畅性，但缺乏支撑人类数字感的结构理解——即理解捷径何时有效及为何有效的深层认知能力
...
<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的步态分析大多依赖有监督分类器和疾病标签数据，难以泛化到异质性病理表现。需要一种无需疾病标签、能够检测异常并校正运动学的无标签方法。
...
Result: 在10名保留的正常参与者模拟7种步态异常的验证中，准确识别了生物力学不一致的关节，显著减少了所有分析关节的角度偏差（大效应量），并保持了正常运动学特征。

Conclusion: 该方法实现了无需疾病标签的可解释、个体化步态损伤定位，为异质性病理表现的步态分析提供了有效的无标签框架。
...
### [44] [ATBench: A Diverse and Realistic Trajectory Benchmark for Long-Horizon Agent Safety](https://arxiv.org/abs/2604.02022)
...
<details>
  <summary>Details</summary>
Motivation: 现有轨迹级基准测试存在交互多样性不足、安全失败观测粗糙和长时域真实性弱等局限，无法有效评估LLM智能体在现实部署中多步交互中涌现的风险。

Method: 基于风险来源、失败模式和现实危害三维分类法构建轨迹；使用异构工具池和长上下文延迟触发协议模拟真实风险涌现；包含1000个轨迹（503安全/497不安全），平均9.01轮对话和3.95k tokens；采用规则和LLM过滤加人工审核确保数据质量。

Result: ATBench对前沿LLM、开源模型和专用防护系统都具有挑战性；支持分类分层分析、跨基准比较和长时域失败模式诊断；包含1954个调用的工具，来自2084个可用工具池。

Conclusion: ATBench提供了一个结构化、多样化和真实的轨迹级基准测试，能够全面评估LLM智能体在多步交互中的安全性，填补了现有基准测试的不足。
...
### [45] [The Latent Space: Foundation, Evolution, Mechanism, Ability, and Outlook](https://arxiv.org/abs/2604.02029)
...
TL;DR: 该论文是一篇关于语言模型中潜在空间的综述，系统性地梳理了该领域的发展脉络、技术机制、能力应用及未来展望。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型的发展，潜在空间正成为语言模型的原生计算基质。传统基于显式token生成的理解方式存在局限性，而许多关键内部过程在连续潜在空间中更为自然。该综述旨在为语言模型中的潜在空间研究提供统一且最新的全景图。

Method: 采用五维视角组织综述：基础（定义潜在空间范围）、演化（追溯发展历程）、机制（从架构、表示、计算、优化四个技术维度分析）、能力（展示潜在空间在推理、规划、建模、感知、记忆、协作、具身等广泛能力谱系中的支持作用）、展望（讨论开放挑战和未来方向）。

Result: 建立了语言模型潜在空间的系统性分析框架，识别了该领域的四大技术发展主线（架构、表示、计算、优化）和七大能力维度（推理、规划、建模、感知、记忆、协作、具身），为理解潜在空间作为下一代智能的通用计算和系统范式奠定了基础。

Conclusion: 潜在空间正成为语言模型的核心计算范式，该综述不仅为现有研究提供了参考框架，更重要的是为理解潜在空间作为下一代智能的通用计算和系统范式奠定了基础，有望推动该领域的进一步发展。
...
Result: 传统问卷风险评估准确率略高，但GPT驱动的自适应问卷所需问题更少，用户体验更流畅、更具吸引力，用户偏好明显。

Conclusion: ARQuest有潜力提升用户满意度并简化保险流程，进一步发展可能在风险评估准确性上超越传统方法，推动保险行业创新。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的传感器和通信损坏严重削弱了多智能体协同感知的优势。现有方法将损坏视为静态扰动或被动适应受损输入，无法主动恢复底层干净语义，需要一种能主动恢复语义的鲁棒协同感知方法
...
Conclusion: Diff-KD通过将扩散生成精炼与知识蒸馏结合，有效解决了多智能体协同感知中的传感器和通信损坏问题，实现了鲁棒的语义恢复和融合
...
TL;DR: 该论文研究了LLM在时间序列解释生成与评估中的表现，发现评估能力比生成能力更稳定可靠，为无参考的时间序列解释评估提供了新方法。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏评估LLM生成的时间序列解释事实正确性的通用方法，现有方法要么需要真实解释作为参考，要么无法评估自由形式的文本推理，需要开发无参考的评估框架。

Method: 使用LLM同时作为时间序列解释的生成器和评估器，构建包含350个时间序列案例的合成基准数据集，涵盖七种查询类型，每个案例配有正确、部分正确和错误的解释，通过三元正确性标签评估模式识别、数值准确性和答案忠实性。

Result: 发现明显的不对称性：生成能力高度依赖模式类型，在某些查询类型上系统性地失败（如季节性下降和波动率变化的准确率仅0.00-0.12），而结构断点的准确率达0.94-0.96；评估能力更稳定，即使模型自身输出错误，也能正确排序和评分解释。

Conclusion: 基于LLM的数据驱动评估在时间序列解释领域是可行的，LLM作为时间序列推理的评估器比作为生成器更可靠，为无参考的时间序列解释评估提供了实用框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有社交机器人检测方法存在三个主要问题：1) 过度依赖单模态信号导致检测精度有限；2) 对AI生成内容(AIGC)特定生成模式不够敏感；3) 未能充分建模语言模式与行为动态之间的相互作用。LLM驱动的社交机器人通过生成类人内容规避传统检测，对在线讨论构成日益严重的威胁。

Method: 提出TRACE-Bot统一双通道框架，从异构数据源（个人信息、交互行为、推文数据）构建细粒度表示。采用双通道架构：一个通道通过预训练语言模型捕获语言表示，另一个通道通过多维活动特征捕获行为异常，这些特征通过最先进的AIGC检测器信号增强。融合后的表示通过轻量级预测头进行分类。

Result: 在两个公开的LLM驱动社交机器人数据集上实现了最先进的性能，准确率分别为98.46%和97.50%。结果表明该方法对高级机器人策略具有强鲁棒性，验证了联合利用隐式语义表示和AIGC增强行为模式的有效性。

Conclusion: TRACE-Bot通过联合建模语言表示和行为模式，有效解决了现有LLM驱动社交机器人检测方法的局限性。该方法展示了多模态融合和AIGC特定信号增强的重要性，为新兴社交机器人检测提供了有效解决方案。
...
### [50] [From High-Dimensional Spaces to Verifiable ODD Coverage for Safety-Critical AI-based Systems](https://arxiv.org/abs/2604.02198)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI在航空等安全关键领域部署时，EASA要求证明AI/ML组件ODD的完整覆盖，但现有方法在高维参数空间中缺乏可扩展性和形式化基础，缺乏标准化工程方法来连接抽象ODD定义与可验证证据
...
### [51] [Blinded Radiologist and LLM-Based Evaluation of LLM-Generated Japanese Translations of Chest CT Reports: Comparative Study](https://arxiv.org/abs/2604.02207)
*Yosuke Yamagishi,Atsushi Takamatsu,Yasunori Hamaguchi,Tomohiro Kikuchi,Shouhei Hanaoka,Takeharu Yoshikawa,Osamu Abe*

Main category: cs.AI

TL;DR: 评估LLM生成胸部CT报告日文翻译的教育适用性，比较放射科医生评估与LLM作为评判者的表现，发现两者一致性极低
...
Method: 分析150份胸部CT报告，比较人工编辑日文翻译与DeepSeek-V3.2生成的翻译。由放射科医生和放射科住院医师进行盲法评估（术语准确性、可读性、整体质量、放射科风格真实性），同时由3个LLM（DeepSeek-V3.2、Mistral Large 3、GPT-5）作为评判者评估相同对。使用QWK和百分比一致性评估一致性

Result: 放射科医生与LLM评判者之间一致性接近零（QWK=-0.04至0.15），两位放射科医生之间一致性也很差（QWK=0.01至0.06）。放射科医生1认为59%的病例术语相当，在可读性（51%）和整体质量（51%）上更偏好LLM翻译。放射科医生2认为75%的病例可读性相当，在整体质量上更偏好人工编辑翻译（40% vs 21%）。所有3个LLM评判者强烈偏好LLM翻译（70%-99%），并在>93%的病例中认为其更具放射科风格

Conclusion: LLM生成的翻译常被认为自然流畅，但两位放射科医生评估差异显著。LLM作为评判者表现出对LLM输出的强烈偏好，与放射科医生的一致性可忽略。对于翻译放射学报告的教育用途，仅依赖自动化LLM评估不足够，专家放射科医生评审仍然重要
...
### [52] [VISTA: Visualization of Token Attribution via Efficient Analysis](https://arxiv.org/abs/2604.02217)
...
TL;DR: 提出一种轻量级、模型无关的token重要性可视化技术，通过扰动策略和三矩阵分析框架生成相关性图，无需额外计算成本
...
Method: 基于扰动的策略结合三矩阵分析框架：1) 角度偏差矩阵捕捉语义方向变化；2) 幅度偏差矩阵测量语义强度变化；3) 维度重要性矩阵评估各向量维度贡献。通过系统移除每个token并测量这三个互补维度的影响，得到综合重要性分数

Result: 开发了模型无关的token重要性可视化技术，提供数学基础扎实的token重要性度量，开源实现已发布在Infosys Responsible AI Toolkit中

Conclusion: 该方法为理解LLM如何处理提示信息提供了轻量级、模型无关的解决方案，无需额外计算成本，支持可解释AI的广泛应用
...
### [53] [When to ASK: Uncertainty-Gated Language Assistance for Reinforcement Learning](https://arxiv.org/abs/2604.02226)
...
### [54] [Answering the Wrong Question: Reasoning Trace Inversion for Abstention in LLMs](https://arxiv.org/abs/2604.02230)
*Abinitha Gourabathina,Inkit Padhi,Manish Nagireddy,Subhajit Chaudhury,Prasanna Sattigeri*

Main category: cs.AI

TL;DR: Trace Inversion方法通过分析LLM的推理轨迹来重构模型可能回答的问题，通过比较原始查询与重构查询的相似度来判断是否需要弃权，显著提升了推理模型的弃权能力。


<details>
  <summary>Details</summary>
Motivation: 推理模型在复杂任务上表现出色，但其弃权能力较差，容易产生幻觉回答。当模型回答错误时，往往是因为它实际上在回答一个与原始查询不同的问题。需要一种新的方法来检测这种查询不对齐的情况，从而提升模型的弃权能力。

Method: 提出查询不对齐框架和Trace Inversion方法：1) 生成模型的推理轨迹；2) 仅基于推理轨迹重构模型最可能回答的查询；3) 比较原始查询与重构查询的相似度。相似度低表明模型可能回答了错误的问题，需要弃权。

Result: 在4个前沿LLM和9个弃权QA数据集上的广泛实验表明，Trace Inversion在36个设置中的33个都优于竞争基线，有效提升了弃权性能。

Conclusion: Trace Inversion通过检测查询不对齐来提升LLM的弃权能力，将幻觉回答重新解释为模型回答了错误的问题而非错误地回答问题，为可靠部署LLM提供了有效方法。
...
### [55] [Do Emotions in Prompts Matter? Effects of Emotional Framing on Large Language Models](https://arxiv.org/abs/2604.02236)
...
<details>
  <summary>Details</summary>
Motivation: 探究用户查询中的第一人称情感框架如何影响大语言模型在不同基准任务上的表现，理解情感语调在LLM行为中的作用

Method: 在六个基准领域测试静态情感前缀的影响，分析情感强度变化和人类编写前缀的效果，开发自适应情感提示框架EmotionRL

Result: 静态情感前缀通常只产生小的准确率变化；情感影响在社交基础任务中更可变；自适应情感选择比固定情感提示获得更可靠的性能提升
...
### [56] [De Jure: Iterative LLM Self-Refinement for Structured Extraction of Regulatory Rules](https://arxiv.org/abs/2604.02276)
...
Method: 四阶段管道：1) 源文档规范化到结构化Markdown；2) LLM驱动的语义分解为结构化规则单元；3) 多维度LLM-as-a-judge评估（19个维度）；4) 在有限再生预算内迭代修复低分提取

Result: 在金融、医疗和AI治理三个监管领域评估，金融领域提取质量持续单调提升，3次迭代内达峰值；跨领域保持高性能；下游合规问答评估中，De Jure提取规则在73.8%-84.0%情况下优于先前工作
...
### [57] [Novel Memory Forgetting Techniques for Autonomous AI Agents: Balancing Relevance and Efficiency](https://arxiv.org/abs/2604.02280)
