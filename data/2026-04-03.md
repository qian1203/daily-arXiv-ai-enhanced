...
TL;DR: 该论文将结构化LLM路由重新定义为系统级负担分配问题，而非单纯的提示工程问题，并通过跨后端基准测试发现路由性能高度依赖于后端特定交互效应，无通用最优方案。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型成为智能体AI系统的核心控制组件，可靠的结构化路由需要在正确性、延迟和实现成本之间取得平衡。当前研究通常将结构化路由视为提示工程问题，但作者认为这本质上是系统级负担分配问题，需要在实际部署约束下进行优化。

Method: 作者提出了一个可部署的框架，用于在异构后端条件下进行结构化路由推理。通过全面的全因子基准测试，覆盖48种部署配置和15,552个请求，在OpenAI、Gemini和Llama后端上评估不同结构工作分配策略（模型直接输出、传输中压缩、生成后本地重建）的性能。

Result: 核心发现：不存在通用的最佳路由模式，后端特定的交互效应主导性能。在Gemini和OpenAI上保持高可靠性的模式，在Llama上可能出现显著的正确性下降；压缩实现的效率增益也强烈依赖于后端。性能表现呈现明显的后端依赖性。

Conclusion: 该工作贡献了一个可部署的框架，用于在生产级智能体专家系统中导航正确性-成本-延迟边界。提供了跨后端评估方法和实际部署指导，强调需要根据具体后端特性选择路由策略，而非寻找通用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统因果推断方法依赖不可观测的反事实数据，通过假设填补缺失信息，但无法直接产生反事实结果本身。需要一种新框架将不可证伪的因果主张转化为可检验的假设

Method: 提出数字孪生反事实框架：1) 将数字孪生模拟器形式化为潜在结果框架中的随机映射；2) 建立孪生保真度假设层次结构(边际保真度、联合保真度、结构保真度)；3) 设计五级验证架构将不可证伪主张转化为可检验测试；4) 形式化分解将因果量区分为可边际验证和依赖copula的部分；5) 提供边界、敏感性和不确定性量化工具

Result: 建立了一个系统框架，使边际因果主张(如ATE、CATE、QTE)可通过可观测数据检验，而联合因果主张(如ITE分布、获益/损害概率、处理效应方差)则明确依赖不可观测的个体内依赖结构，并提供了量化工具

Conclusion: DTCF不解决因果推断的根本问题，但提供了一个框架：使边际因果主张变得可检验，联合因果主张变得假设索引化，并形式化表征了两者之间的差距，为因果推断提供了更透明和可验证的方法论
...
TL;DR: IDEA2是一个集成大语言模型的半自动化工作流，通过专家在环的协作过程，从需求文档中提取能力问题，经过专家评审反馈和迭代优化，最终达成共识，解决本体工程中能力问题提取的瓶颈。
...
Method: IDEA2采用半自动化工作流，集成大语言模型和专家在环的协作过程。核心是一个迭代循环：1）从需求文档中基于LLM提取初始CQs；2）在可访问的协作平台上由领域专家进行共同创建评审和反馈；3）对拒绝的CQs进行反馈驱动的LLM迭代重新表述，直到达成共识。使用溯源模型跟踪每个CQ的完整生命周期，包括编辑谱系、匿名反馈和生成参数。

Result: 在2个真实世界场景（科学数据、文化遗产）中验证了IDEA2工作流。结果表明：IDEA2能够加速需求工程过程，提高最终CQs的接受度和相关性，并在领域专家中展现出高可用性和有效性。

Conclusion: IDEA2通过集成LLM和专家在环的协作过程，有效解决了本体工程中能力问题提取的瓶颈问题，提供了一种透明、可重复且高效的工作流，显著改进了CQ提取的质量和效率。
...
<details>
  <summary>Details</summary>
Motivation: 传统智能体中心架构在企业和制度系统等结构化领域中存在语义一致性、可解释性和长期稳定性方面的不足，需要一种基于共享显式世界表示的新架构。

Method: 提出世界中心多智能体系统概念，对世界进行分类（本体显式性、规范性等维度），使用语义模型作为数学形式化表示，并开发Ontobox平台作为实现。

Result: 建立了WMAS理论框架，提供了世界分类维度，提出了语义模型作为形式化表示方法，并实现了Ontobox平台作为具体实现。

Conclusion: 世界中心多智能体系统为结构化领域提供了一种更优的架构选择，通过共享世界模型实现全局一致性、可验证系统行为和更好的可解释性。
...
TL;DR: 该研究提出AI自动化是一个连续体，包含"崩溃浪潮"（AI能力在少数任务上突然激增）和"上升潮汐"（AI能力持续广泛提升）两种模式。基于对3000多个文本任务的评估，研究发现上升潮汐是主要的AI自动化形式，AI在广泛任务上表现良好且快速改进。


<details>
  <summary>Details</summary>
Motivation: 研究旨在理解AI自动化的本质模式，区分AI能力提升是集中在少数任务上的突然突破（崩溃浪潮），还是在广泛任务上的持续改进（上升潮汐）。这有助于预测AI对经济和劳动力市场的实际影响。

Method: 使用美国劳工部O*NET分类中的3000多个文本任务进行评估，这些任务可由大型语言模型处理。通过来自相关工作岗位的17000多次人工评估，测量AI模型在不同任务上的表现和进步趋势。

Result: 研究发现几乎没有崩溃浪潮的证据，但上升潮汐是主要的AI自动化形式。AI在广泛任务上表现良好且快速改进：2024年第二季度，AI模型能以约50%的成功率完成人类需要3-4小时的任务；到2025年第三季度，成功率预计升至约65%。如果趋势持续，到2029年LLM在大多数文本任务上的平均成功率将达到80%-95%。

Conclusion: AI自动化主要表现为广泛任务上的持续改进（上升潮汐），而非少数任务上的突然突破（崩溃浪潮）。AI能力正在快速提升，预计未来几年将在文本任务上达到高成功率。然而，AI对经济和劳动力市场的实际影响将取决于组织采用AI的时间线，这可能比技术能力发展慢得多。
...
TL;DR: 本文研究了LLM中的认知偏差，发现偏差在模型激活空间中呈现线性可分方向，并通过激活导向技术实现了26-32%的偏差减少，同时保持下游任务能力。


<details>
  <summary>Details</summary>
Motivation: LLM越来越多地应用于高风险决策场景，先前研究表明LLM在行为上表现出认知偏差，但这些偏差是否对应可识别的内部表示以及能否通过针对性干预来缓解，仍然是一个开放性问题。

Method: 1) 定义LLM认知偏差为在可计算基准任务中系统性、可复现的偏离正确答案；2) 引入LLM CogBias基准，围绕四个认知偏差家族组织：判断、信息处理、社会和反应偏差；3) 评估三个LLM；4) 使用对比设计下的线性探针分析偏差在激活空间中的表示；5) 应用激活导向技术来调节偏差行为。

Result: 1) 认知偏差在所有四个家族中系统性出现，幅度和去偏差响应强烈依赖于偏差家族：提示级去偏差显著减少反应偏差，但对判断偏差产生反效果；2) 认知偏差在模型激活空间中编码为线性可分方向；3) 激活导向实现了26-32%的偏差分数减少，同时在25个基准测试中保持下游能力（Llama：可忽略的退化；Qwen：判断偏差最多-19.0pp）；4) 尽管模型间的偏差表示接近正交（平均余弦相似度0.01），但导向在不同架构中以相似速率减少偏差，表明共享的功能组织。

Conclusion: LLM认知偏差在激活空间中具有可识别的线性表示，可以通过激活导向技术有效减少，同时保持模型能力。不同模型架构间偏差表示虽然正交，但去偏差效果相似，表明存在共享的功能组织机制。
...
TL;DR: 提出RIFT分类法，用于系统诊断评分标准的质量问题，包含8种失败模式，分为3大类，并通过自动化指标实现可扩展诊断
...
Method: 基于扎根理论开发RIFT分类法，通过迭代标注来自5个不同基准的评分标准，包含8种失败模式（可靠性、内容效度、结果效度三类）；提出自动化评分标准质量指标

Result: 观察到公平的一致性（87%成对一致性和0.64平均Cohen's kappa）；自动化指标与人工失败模式标注对齐，达到最高0.86 F1分数

Conclusion: RIFT为评分标准质量提供了系统化的诊断框架，自动化指标支持可扩展的诊断，有助于改进LLM评估和训练中的评分标准设计
...
<details>
  <summary>Details</summary>
Motivation: POMDP虽然为状态和转移不确定性下的规划提供了原则性形式化，但在有限规划时间内获得高性能策略仍然面临维数灾难和历史灾难的挑战。许多POMDP问题中，信息价值(VOI)在信念空间上变化，这为优化计算资源分配提供了机会。

Method: 提出动态规划框架，基于每个信念的信息价值有条件地处理观测。在此基础上提出VOIMCP算法，这是一种蒙特卡洛树搜索算法，当VOI较低时选择性忽略观测信息，避免不必要的观测分支扩展。

Result: 提供了VOI推理框架的近似最优性理论保证，推导了VOIMCP的非渐近收敛界。仿真评估表明，VOIMCP在多个POMDP基准测试中优于基线方法。

Conclusion: 通过利用信息价值在信念空间上的变化特性，提出的VOIMCP算法能够更高效地分配计算资源，在有限规划时间内获得更好的性能，为解决大规模POMDP规划问题提供了有效方法。
...
<details>
  <summary>Details</summary>
Motivation: AI系统在医疗和药学领域日益普及，用于药物推荐、剂量确定和药物相互作用检测等任务。尽管这些系统在标准评估指标下表现良好，但其在真实世界决策中的可靠性仍未被充分理解。在药物管理等高风险领域，即使是单个错误推荐也可能导致严重患者伤害。

Method: 通过一系列涉及药物相互作用和剂量决策的受控模拟场景，分析不同类型的系统故障，包括遗漏相互作用、错误风险标记和不适当的剂量推荐。研究关注错误发生的方式以及AI系统产生错误输出时的后果。

Result: 研究发现，在药物相关情境中，AI错误可能导致药物不良反应、治疗无效或护理延迟，特别是在没有足够人工监督的情况下使用系统时。研究还揭示了过度依赖AI推荐的风险以及决策过程透明度有限带来的挑战。

Conclusion: 本研究为医疗AI评估提供了以可靠性为中心的视角，强调理解故障行为和真实世界影响的重要性。在药学实践等安全关键领域，需要将传统性能指标与风险感知的评估方法相结合，并确保足够的人工监督和系统透明度。
...
TL;DR: Elhuyar是一个多智能体、人机协同系统，结合LLM、结构化AI和人类科学家，从科学文献中提取、分析和迭代提炼见解，加速科学发现。


<details>
  <summary>Details</summary>
Motivation: 科学文献分散且需要大量人工努力来收集、分析和理解，现有AI工具缺乏结构化、多步骤的方法来从科学文献中提取深度见解，LLM存在幻觉和不完整提取的问题。

Method: 开发多智能体、人机协同系统，将任务分配给专门的智能体进行论文筛选、数据提取、模型拟合和结果总结，通过人类监督确保可靠性，生成包含提取数据、可视化、模型方程和文本摘要的结构化报告。
...
TL;DR: 提出了一种针对组合优化问题的不可行性感知框架，结合可认证数据集构建、监督微调和LLM辅助下游搜索，在minor-embedding问题上实现了同时进行解生成和不可行性检测


<details>
  <summary>Details</summary>
Motivation: 现有LLM方法主要关注可行实例的解生成，缺乏对不可行性检测的显式处理，需要一种能够同时处理可行和不可行实例的框架

Method: 1) 提出可认证数据集构建方法，包含新的数学规划公式和可证明的零相不可行性筛选；2) 使用精确优化流水线生成训练数据；3) 对8B参数LLM进行监督微调；4) 将LLM输出作为下游局部搜索的暖启动

Result: 微调模型相比GPT-5.2整体准确率提升达30%；LLM引导的暖启动在下游局部搜索中相比从零开始提供高达2倍的加速

Conclusion: 该框架成功将LLM应用于组合优化问题，实现了同时的解生成和不可行性检测，并通过LLM辅助暖启动提供了实用的优化加速方法
...
TL;DR: P-COD是一种利用文档间关系检测异常值的新方法，通过比较提取数据与语料库中已验证的同行信息，调整置信度分数并标记低置信度结果供专家审查，从而减少大语言模型在科学文献摘要中的幻觉。


<details>
  <summary>Details</summary>
Motivation: 减少大语言模型在从大型文本语料库中提取数据时的幻觉问题，当前方法如提示工程和思维链提示仅关注单个文档，未能考虑语料库中文档间的关系。

Method: 提出Peer Context Outlier Detection (P-COD)方法，利用文档间关系提高提取准确性。在科学文献摘要领域，通过比较提取数据与语料库中已验证的同行信息，调整置信度分数并标记低置信度结果供专家审查。

Result: 在6个科学领域的实验中，异常值检测精度达到98%，表明该方法能有效减少幻觉，增强对自动化系统的信任，并让研究人员专注于模糊案例，简化数据提取工作流程。

Conclusion: P-COD通过利用文档间关系显著提高了数据提取的准确性，减少了幻觉，为科学文献摘要等领域的自动化数据提取提供了更可靠的解决方案。
...
TL;DR: 提出一个基于技能进化的智能体框架，用于超表面逆向设计，通过迭代优化特定求解器策略而不修改模型权重或物理求解器，提高工作流复用性和成功率。


<details>
  <summary>Details</summary>
Motivation: 超表面逆向设计需要复杂的计算电磁学专业知识和特定求解器软件工程技能，现有语言驱动系统缺乏跨任务的可复用工作流知识积累，限制了设计效率。

Method: 开发一个包含编码智能体、进化技能组件和基于物理仿真的确定性评估器的框架，通过上下文级技能进化迭代优化特定求解器策略。

Result: 在训练任务中，成功率从38%提升至74%，标准通过率从0.510提高到0.870，平均尝试次数从4.10降至2.30；在未见任务中，二进制成功率变化不大，但最佳裕度改进、错误组成变化和智能体行为表明工作流知识的部分迁移。

Conclusion: 技能进化的主要价值在于围绕可靠计算引擎积累可复用的特定求解器专业知识，为实现更自主和易访问的超表面逆向设计工作流提供了实用路径。
...
Method: 提出角色化LLM框架，包含三个专业角色：LLM政策分析师（元数据和机制分类）、LLM法律策略专家（识别复杂法律方法）、LLM食品系统专家（分类食品系统阶段）。框架通过将结构化领域知识（法律机制定义和分类标准）融入角色特定提示来模拟专家分析流程。

Result: 在Healthy Food Policy Project数据库的608个健康食品政策上评估，使用Llama-3.3-70B模型，与零样本、少样本和思维链基线比较。所提框架在复杂推理任务中表现出优越性能。

Conclusion: 该角色化LLM框架为健康政策信息提取提供了可靠且透明的方法论，能够有效自动化信息提取并减少LLM的常见错误。
...
TL;DR: PHMForge：首个针对工业PHM（预测与健康管理）任务的LLM智能体基准测试，包含75个专家策划场景、65个专用工具，评估显示当前最佳配置仅完成68%任务，存在工具编排、多资产推理和跨设备泛化等系统性缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试无法满足工业领域对LLM智能体的严格要求，工业领域的错误决策会带来重大的安全和财务后果。需要专门针对PHM任务的基准来评估LLM智能体在工业环境中的实际表现。

Method: 构建PHMForge基准，包含75个专家策划场景，覆盖7种工业资产类别和5个核心任务类别。创建65个专用工具分布在两个MCP服务器上，实现基于执行的评估器，使用任务相适应的指标（MAE/RMSE用于回归，F1分数用于分类，分类匹配用于健康评估）。

Result: 对领先框架（ReAct、Cursor Agent、Claude Code）与前沿LLM（Claude Sonnet 4.0、GPT-4o、Granite-3.0-8B）的评估显示，即使最佳配置也仅完成68%任务，存在工具编排错误（23%错误排序）、多资产推理能力下降（14.9个百分点退化）和跨设备泛化能力差（42.7%在保留数据集上）等系统性缺陷。

Conclusion: PHMForge填补了工业PHM领域LLM智能体评估的关键空白，揭示了当前智能体在工业应用中的严重局限性，开源完整基准以促进工业AI智能体研究。
...
TL;DR: 该论文研究了将表示自编码器（RAE）集成到连续自回归（AR）模型中的挑战，提出了RAE-AR框架，通过分布归一化和高斯噪声注入解决了高维表示自编码器在AR范式中的建模困难和曝光偏差问题。


<details>
  <summary>Details</summary>
Motivation: 传统生成模型的潜在空间主要被VAE编码器主导，而预训练表示编码器（如DINO、SigLIP、MAE）的潜在表示被认为不适合生成建模。虽然RAE方法显示表示自编码器也能达到与VAE编码器相当的性能，但将其集成到连续自回归模型中仍未被充分探索。

Method: 提出了RAE-AR框架，针对AR模型的独特性质，通过两种主要改进：1）通过分布归一化进行标记简化，降低建模难度并改善收敛；2）在训练期间加入高斯噪声注入，增强预测鲁棒性，缓解高维性放大的训练-推理差距（曝光偏差）。
...
TL;DR: 研究发现LLM训练中归一化层与优化器并非独立设计选择，Derf与Muon存在显著负交互作用，导致性能下降0.66 nats，而DyT无此问题


<details>
  <summary>Details</summary>
Motivation: 传统上LLM训练中将归一化层和优化器视为独立设计选择，但本研究通过实验发现这种假设可能不成立，特别是Derf归一化层与Muon优化器之间存在显著的负交互作用

Method: 在10亿参数规模上进行1000步训练的3x2因子实验设计，对比Derf、RMSNorm和DyT三种归一化层与AdamW、Muon两种优化器的组合效果，分析失效模式并提出两种修复方案

Result: Derf与Muon组合出现显著负交互作用，性能差距从AdamW下的+0.31 nats扩大到Muon下的+0.97 nats；失效模式包括饱和（有损压缩）和尺度盲区（丢弃激活幅度）；EMA混合方案恢复约84%差距，降低Derf的alpha参数恢复约80%差距

Conclusion: 归一化层与优化器选择存在重要交互作用，Derf与Muon组合存在隐藏的性能损失风险，需谨慎选择参数设置，建议使用EMA混合或调整alpha参数来缓解问题
...
TL;DR: NED-Tree框架通过逐句提取和递归树结构解决LLMs在非线性OR问题建模中的语义对齐问题，在10个基准测试中达到72.51%平均准确率


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在线性OR问题中表现良好，但在现实世界非线性场景中因数学公式与求解器代码之间的语义错位以及信息提取不稳定而性能严重下降

Method: 提出NED-Tree框架：(a) 采用逐句提取策略确保稳健的参数映射和可追溯性；(b) 使用递归树结构自适应地将复杂非线性项分解为求解器兼容的子元素

Result: 在10个基准测试中平均准确率达到72.51%，建立了新的最先进水平，是首个通过元素分解驱动LLMs解决非线性建模困难的框架

Conclusion: NED-Tree成功弥合了建模语义与代码语义之间的差距，为解决非线性OR问题自动化建模提供了有效方案，并发布了NEXTOR基准测试
...
<details>
  <summary>Details</summary>
Motivation: 当前强化学习与验证（RLVR）方法通常需要正确性信号或批判标注来训练模型进行自我精炼，这限制了方法的普适性和可扩展性。ThinkTwice旨在开发一个无需额外监督信号的联合训练框架，让模型同时学习推理和自我精炼。

Method: ThinkTwice采用两阶段训练框架：第一阶段优化模型解决推理问题，第二阶段优化模型精炼自己的解决方案。两个阶段都使用相同的二元正确性奖励，基于Group Relative Policy Optimization（GRPO）进行优化。模型在训练过程中交替进行推理和精炼优化。

Result: 在五个数学推理基准和两个模型家族（Qwen3-4B和Olmo3-7B）上，ThinkTwice显著优于竞争性的在线策略优化基线。在Qwen3-4B上，ThinkTwice在AIME基准上比GRPO在精炼前提升5个百分点，在一次自我精炼后提升11.5个百分点（pass@4）。训练动态分析显示存在隐式的"纠正-强化"课程：早期精炼主要纠正错误，随着模型改进自然转向保持已正确的解决方案。

Conclusion: ThinkTwice建立了推理和自我精炼联合训练作为RLVR的原则性和有效方法，无需额外监督信号，通过隐式课程学习实现性能提升，为强化学习与验证提供了新方向。
...
TL;DR: LLMs作为教师时，主要通过贝叶斯最优推理（逆规划）而非简单启发式来决定教学内容，其教学策略与人类相似且稳定，但提示干预虽能改变策略却不一定改善教学效果。


<details>
  <summary>Details</summary>
Motivation: 探究LLMs作为教师时的决策机制：是基于对学习者知识的推理（心智化能力），还是使用简单的经验法则？通过受控任务比较LLMs与人类的教学策略。

Method: 使用先前研究人类教学策略的受控任务：教师LLM观察学习者在奖励标注有向图中的轨迹，揭示单一边缘以改善学习者重规划路径。测试多种LLM作为模拟教师，用与人类研究相同的认知模型（贝叶斯最优教师、弱贝叶斯变体、启发式基线、非心智化效用模型）拟合其选择。进行基线实验和支架干预实验。

Result: 在基线实验中，多数LLM表现良好，策略稳定，图间表现与人类相似。模型比较（BIC）显示贝叶斯最优教学最能解释大多数模型的选择。支架干预中，模型遵循推理或奖励导向的提示，但这些干预在启发式不一致的测试图上不一定改善教学，有时甚至降低性能。

Conclusion: 认知模型拟合揭示了LLMs的教学策略本质上是贝叶斯最优的，表明它们具备心智化能力。提示合规性不能保证更好的教学决策，教学效果取决于任务结构与策略的匹配度。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs主要依赖监督微调，缺乏代码执行环境暴露，且自校正能力有限，需要更有效的训练策略来提升图表到代码生成的质量
...
Conclusion: MM-ReCoder通过强化学习和自校正机制有效提升了多模态大语言模型在图表到代码生成任务中的性能，证明了交互式学习和迭代自校正的重要性
...
TL;DR: CRaFT：基于电路引导的拒绝特征选择框架，通过边界附近提示评估特征对拒绝-合规决策的影响，相比传统激活强度方法显著提升攻击成功率
...
<details>
  <summary>Details</summary>
Motivation: 多智能体系统存在协调开销大、上下文碎片化和脆弱的阶段排序问题，而将MAS蒸馏为单智能体技能缺乏原则性指导，导致技能提升效果不一致（从28%提升到2%退化）。需要理论框架来预测何时以及蒸馏什么内容。

Method: 提出Metric Freedom (F)指标，通过Mantel测试量化输出多样性与得分方差之间的耦合程度，衡量度量评分景观的拓扑刚性。基于F设计两阶段自适应蒸馏框架：阶段1针对"自由"度量选择性提取工具和知识，保留探索性；阶段2针对"刚性"度量(F≤0.6)进行计算密集的迭代优化，避免轨迹局部过拟合。

Result: 在4个任务、11个数据集和6个度量上评估，F能强预测技能效用(ρ=-0.62, p<0.05)。相同智能体轨迹在刚性与自由度量下产生截然相反的技能提升效果。自适应智能体在降低8倍成本、15倍延迟的同时，匹配或超越原始MAS性能。

Conclusion: 技能效用本质上是度量层面的属性，而非任务属性。Metric Freedom (F)作为首个先验预测指标，能有效指导自适应蒸馏，在显著降低计算成本的同时保持或提升系统性能。
...
TL;DR: GraphWalk：一种无需训练、基于工具的框架，让现成LLM能够通过顺序图导航进行推理，显著提升在大型知识图谱上的多跳推理性能


<details>
  <summary>Details</summary>
Motivation: 企业级知识图谱规模庞大，无法放入现有最大上下文窗口，传统基于提示或检索增强生成的方法在处理复杂查询时面临严重限制，需要多跳推理能力

Method: GraphWalk为LLM提供一组最小正交图操作工具，使其能够遍历任何图结构，通过顺序工具调用实现多步推理链，每个工具调用代表可验证的执行步骤

Result: 在所有测试模型系列中，基于工具的图遍历方法相比上下文基线均获得显著且一致的性能提升，随着规模增大增益更加明显，而上下文方法在大型图谱上完全失效

Conclusion: GraphWalk框架使现成LLM能够通过工具化图导航解决复杂推理任务，提供透明执行轨迹，在处理企业级知识图谱的复杂查询方面具有显著优势
...
TL;DR: 对四种大型语言模型在收据项目分类任务中的系统化、成本感知评估，发现Claude 3.7 Sonnet在准确性和成本效率之间达到最佳平衡


<details>
  <summary>Details</summary>
Motivation: 在面向生产的分类框架中，系统评估大型语言模型在收据项目分类任务上的性能，重点关注准确性、响应稳定性和令牌级成本，并研究零样本和少样本提示方法在准确性和成本方面的适用性

Method: 使用AWS Bedrock平台上的四种指令调优模型（Claude 3.7 Sonnet、Claude 4 Sonnet、Mixtral 8x7B Instruct、Mistral 7B Instruct），在收据项目分类任务中比较零样本和少样本提示方法，评估准确性、响应稳定性和令牌成本

Result: 实验结果表明，Claude 3.7 Sonnet在分类准确性和成本效率之间达到了最有利的平衡

Conclusion: Claude 3.7 Sonnet是收据项目分类任务中在准确性和成本效率方面表现最佳的模型，为生产环境中的LLM选择提供了实用指导
...
<details>
  <summary>Details</summary>
Motivation: 扩散语言模型暴露去噪轨迹，为推理时控制提供原生信号；理想的幻觉缓解框架应利用这一模型原生信号，而非依赖外部训练的幻觉分类器

Method: 提出承诺不确定性定位：识别交叉链熵超过无监督阈值的token位置；引入OSCAR框架：运行N个并行去噪链，计算交叉链香农熵检测高不确定性位置，然后基于检索证据进行针对性重掩码

Result: 在TriviaQA、HotpotQA、RAGTruth和CommonsenseQA上使用LLaDA-8B和Dream-7B，OSCAR显著减少幻觉内容，提高事实准确性；其基于熵的不确定性信号优于专门的训练检测器

Conclusion: 扩散语言模型具有识别事实不确定性的内在能力，这在其去噪轨迹中体现，而自回归模型的顺序token承诺结构中不存在这种能力；OSCAR展示了利用这一特性进行推理时幻觉缓解的有效性
...
TL;DR: EnviSmart是一个用于环境FAIR数据管理的生产系统，通过三轨知识架构和角色分离的多智能体设计，在LLM驱动的概率工作流中实现可靠性，防止错误传播到不可逆操作。


<details>
  <summary>Details</summary>
Motivation: 将LLM驱动的智能体嵌入环境FAIR数据管理很有吸引力，因为它们可以外部化操作知识并在异构数据和不断发展的规范中进行规模化管理。然而，用概率工作流替换确定性组件改变了故障模式：LLM管道可能生成看似合理但错误的输出，这些输出通过表面检查并传播到不可逆操作（如DOI分配和公开发布）。
...
Result: 在两个生产部署中进行了比较：大学GIS中心生态档案（849个数据集）作为单智能体基线；SF2Bench复合洪水基准（2,452个监测站和8,557个文件，跨越39年）验证了多智能体工作流。多智能体方法提高了效率（由单个操作员在两天内完成，并在多个部署中重复使用工件）和可靠性：审计交接检测并阻止了影响所有2,452个站点的坐标转换错误。代表性事件（ISS-004）展示了基于边界的遏制，检测延迟10分钟，零用户暴露，80分钟解决。

Conclusion: EnviSmart通过架构方法解决了LLM驱动数据管理中的可靠性问题，将可靠性作为系统属性而非事后检查。三轨知识架构和多智能体设计在概率工作流中实现了确定性保证，防止错误传播到不可逆操作。该系统已在PEARC 2026被接受。
...
Result: 在多样化的数学、算法和系统优化任务上，CORAL在10个任务上创造了新的最先进结果，相比固定进化搜索基线，改进率提高3-10倍且评估次数更少。在Anthropic内核工程任务中，四个协同进化智能体将最佳得分从1363周期提升至1103周期。
...
TL;DR: 提出七种面向临床AI系统的本体感知设计模式，用于构建对本体失真具有弹性的AI管道，包括数据验证、信号保留、漂移监控等模式，并提供了参考架构和糖尿病风险预测场景示例。


<details>
  <summary>Details</summary>
Motivation: 临床AI系统训练数据常受到文档工作流程、计费激励和术语碎片化的结构性扭曲。现有研究已描述了这些失真机制，但将这些理论见解转化为可实施的软件架构仍是一个未解决的问题。

Method: 提出七种基于Gang-of-Four模式语言的"本体感知设计模式"：Ontological Checkpoint（数据验证）、Dormancy-Aware Pipeline（低频信号保留）、Drift Sentinel（漂移监控）、Dual-Ontology Layer（并行表示维护）、Reification Circuit Breaker（反馈循环中断）、Terminology Version Gate（术语版本管理）、Regulatory Compliance Adapter（监管适配器）。每种模式都按照问题、驱动力、解决方案、后果、已知应用和相关模式进行规范描述。

Result: 提供了一个初级保健AI系统的参考架构，并通过糖尿病风险预测场景展示了所有七种模式的组合应用。三个模式在现有系统中有部分先例，其余四个模式尚未被正式描述。论文未提供实证验证，而是提供了基于理论分析的设计词汇表。

Conclusion: 这些设计模式为构建对本体失真具有弹性的临床AI系统提供了系统化的架构解决方案，填补了理论洞察与可实施软件架构之间的空白。未来需要在生产系统中进行评估验证，当前限制包括缺乏运行时基准测试和仅限于德国及欧盟监管环境。
...
<details>
  <summary>Details</summary>
Motivation: LLM智能体在长时推理任务中面临上下文长度限制，由于部署因素（内存、延迟、成本）导致上下文预算受限。随着交互历史增长，需要在保留历史信息和遵守上下文限制之间做出权衡，现有方法在预算缩减时性能下降明显。

Method: 提出预算感知上下文管理（BACM），将上下文管理建模为带预算约束的序列决策问题。智能体在纳入新观察前评估可用预算，决定何时以及压缩多少交互历史。进一步开发BACM-RL，采用端到端的基于课程的强化学习方法，学习在不同上下文预算下的压缩策略。

Result: 在组合式多目标QA和长时网页浏览基准测试中，BACM-RL在不同模型规模和任务复杂度下均优于现有方法。在高复杂度设置下相比强基线获得超过1.6倍的性能提升，在预算缩减时仍保持优势，而大多数方法呈现性能下降趋势。

Conclusion: BACM-RL通过将上下文管理形式化为序列决策问题并采用强化学习优化压缩策略，有效解决了长时推理任务中的上下文预算约束问题，为资源受限环境下的LLM智能体部署提供了实用解决方案。
...
Method: 提出分层记忆编排框架，将交互历史组织为三层目录：紧凑主缓存（存储近期和关键记忆）、高优先级次层、全局完整历史档案。用户画像驱动记忆在层次间的重新分配，将与长期模式相关的记录提升到更活跃的层级，降低不相关信息的重要性。

Result: 在多个基准测试中达到最先进性能。在OpenClaw等生态系统中的实际部署表明，HMO显著提升了智能体的流畅性和个性化程度。

Conclusion: 分层记忆编排框架通过用户中心化的上下文相关性驱动记忆管理，在需要时精确呈现历史知识，同时保持精简高效的活动搜索空间，解决了长期记忆带来的性能挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型融合方法主要针对同构模型（来自相同预训练骨干），但在开放模型生态系统中，有用专家模型通常基于不同架构（如Llama、Qwen、Mistral等），直接权重空间融合因架构不匹配、潜在基不对齐和跨源冲突而变得不适定
...
Result: 发现AIGC创作者通过高产量实现与HGC创作者相当的总体参与度，尽管消费者明显更偏好HGC；算法内容分发机制能够调节AIGC相关的竞争利益。
...
TL;DR: 提出LiteInception框架，用于资源受限边缘设备上的通用航空故障诊断，通过两阶段级联架构、多方法融合压缩和知识蒸馏调节，实现高效、准确且可解释的故障检测与分类。


<details>
  <summary>Details</summary>
Motivation: 通用航空故障诊断对飞行安全至关重要，但深度学习模型在资源受限边缘设备部署面临计算能力和可解释性的双重挑战，需要平衡效率、准确性和可解释性。

Method: 采用两阶段级联架构：第一阶段高召回故障检测，第二阶段细粒度故障分类；提出基于互信息、梯度分析和SE注意力权重的多方法融合策略压缩输入传感器通道；设计1+1分支LiteInception架构压缩InceptionTime参数；引入知识蒸馏作为精度-召回调节机制；构建集成四种归因方法的双层可解释性框架。

Result: 在NGAFID数据集上，故障检测准确率81.92%，召回率83.24%；故障识别准确率77.00%；输入传感器通道从23压缩到15，参数压缩70%，CPU推理加速8倍以上，F1损失小于3%。

Conclusion: LiteInception框架在效率、准确性和可解释性之间取得了良好平衡，为资源受限边缘设备上的航空故障诊断提供了轻量级、可解释的解决方案，支持安全关键和辅助诊断等不同场景需求。
...
TL;DR: 开发AnIML本体论，通过OWL 2形式化AnIML语义并与Allotrope数据格式对齐，以解决XML模式表达性导致的语义不一致问题，提升分析化学和生物学数据的互操作性。


<details>
  <summary>Details</summary>
Motivation: 分析化学和生物学中使用的AnIML（分析信息标记语言）虽然作为XML标准被广泛采用，但其XML模式的表达性允许不同利益相关者产生分歧解释，导致数据不一致，破坏了AnIML旨在支持的互操作性。需要形式化语义来确保跨系统和跨实验室的数据交换。

Method: 采用专家参与循环方法，结合LLM辅助需求获取与协作本体工程，开发AnIML本体论。通过多层验证方法：将真实AnIML文件转换为知识图谱的数据驱动转换、通过SPARQL验证能力问题、以及基于对抗性负能力问题的新验证协议，这些问题映射到已建立的本体反模式并通过SHACL约束强制执行。

Result: 成功开发了AnIML本体论，该本体形式化了AnIML语义并与Allotrope数据格式对齐。验证表明本体能够正确捕获AnIML语义，支持跨系统和跨实验室的互操作性，并通过多层验证方法确保了本体的质量和可靠性。

Conclusion: AnIML本体论通过形式化语义解决了AnIML XML模式中的语义不一致问题，为分析化学和生物学数据提供了可靠的语义基础。该方法结合了专家知识、LLM辅助和严格验证，为未来跨系统和跨实验室的互操作性奠定了基础。
...
TL;DR: 提出基于SAT的二维单规格板材排样问题求解框架，通过需求展开、板材分配变量和不可行方向消除规则，比较三种最小化板材数的方法，在基准测试中优于现有求解器。


<details>
  <summary>Details</summary>
Motivation: 二维单规格板材排样问题（2D-CSSP）是制造业中的核心任务，需要从标准板材上切割矩形零件以满足需求并最小化浪费。该问题比装箱问题更复杂，因为每种零件类型需要多个副本，导致组合爆炸。现有求解器在处理这类问题时面临挑战，需要更高效的求解方法。

Method: 1. SAT框架：将零件类型按需求展开，每个副本分配板材变量，仅当副本分配到同一板材时才激活非重叠约束。2. 不可行方向消除规则：当零件只能以一种方向放入板材时，固定其旋转变量。3. 三种最小化板材数方法：非增量SAT结合二分搜索、增量SAT跨迭代重用子句、加权部分MaxSAT。
...
Conclusion: SAT方法在二维单规格板材排样问题上显著优于传统求解器，通过需求展开、板材分配和不可行方向消除等创新技术有效处理组合爆炸问题。方法选择应根据旋转约束情况：无旋转时使用增量SAT，旋转复杂时使用非增量SAT。
...
<details>
  <summary>Details</summary>
Motivation: 通用LLM将生成视为单次文本补全，无法满足安全关键工程工作流程中固有的顺序、多门约束，导致生成可执行仿真工件时出现级联约束违反，成为高超声速热防护系统设计中的瓶颈。
...
Result: 在HyTPS-Bench上评估并在外部基准验证，AeroTherm-GPT实现88.7%端到端成功率（95% CI: 87.5-89.9），比非CDG消融基线提升+12.5个百分点，上游优先机制实现每个动作解决多个下游违反，根因修复效率达4.16（扁平清单修复为1.76），在科学推理和代码生成任务上无灾难性遗忘。

Conclusion: 约束闭环生成框架通过约束依赖图指导的迭代修复，有效解决LLM在高超声速热防护系统设计中生成可执行仿真工件时的级联约束违反问题，显著提升端到端成功率，为安全关键工程领域的LLM应用提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 知识图谱能高效存储大量关系，但难以处理概念在不同领域中意义变化的问题。现有系统通常将领域信息作为元数据、限定符或图级组织，这些机制仅用于过滤和溯源，不改变断言的形式状态。许多知识系统的实际失败源于将领域视为断言的外部因素。

Method: 引入领域情境化概念图（DCG）框架，将领域信息写入关系并解释为模态世界约束。采用形式为(C, R at D, C')的表示，其中at D标识关系成立的世界。通过领域索引的必然性算子解释关系，使真值、推理和冲突检查都限定在相关世界中。开发了Kripke风格语义、紧凑谓词系统、Prolog实现，以及到RDF、OWL和关系数据库的映射。

Result: DCG框架使模糊概念能在表示时消歧，无效断言可根据其领域进行挑战，跨领域关系可通过显式谓词连接。实现了领域在表示中的结构和可计算角色。

Conclusion: 领域应被视为知识表示的一部分而非补充注释。DCG通过将领域纳入表示结构，解决了知识系统中因领域处理不当导致的实践失败问题，提供了对领域本身的表示重新解释。
...
TL;DR: PGPO提出了一种细粒度信用分配框架，通过量化视觉依赖程度并动态调整token级优势值，解决了多模态推理中视觉信号被稀释的问题，在7个基准上平均提升18.7%


<details>
  <summary>Details</summary>
Motivation: 现有基于可验证奖励的强化学习框架存在根本性方法缺陷：通过在所有生成token上分配相同优势值，这些方法本质上稀释了优化多模态推理关键视觉基础步骤所需的学习信号

Method: 提出Token Visual Dependency概念，通过KL散度量化视觉输入带来的因果信息增益；基于此引入Perception-Grounded Policy Optimization (PGPO)，采用阈值门控、质量守恒机制动态重塑token级优势值，放大视觉依赖token的学习信号，抑制语言先验的梯度噪声

Result: 基于Qwen2.5-VL系列在7个具有挑战性的多模态推理基准上进行广泛实验，PGPO使模型平均提升18.7%；理论和实证分析证实PGPO有效降低梯度方差、防止训练崩溃，并作为强大的正则化器实现稳健的感知基础多模态推理

Conclusion: PGPO通过细粒度信用分配框架成功解决了多模态强化学习中视觉信号稀释问题，实现了更稳健、感知基础的多模态推理，代码将在GitHub上开源
...
Result: PFN-based表格上下文学习模型在低数据场景下样本效率高，但在异质性和不平衡增加时，基于朴素距离的检索性能下降。AWARE框架在极端不平衡情况下将AUPRC提升高达12.2%，且随着数据复杂性增加，改进效果更加显著。
...
<details>
  <summary>Details</summary>
Motivation: 传统随机最短路径（SSP）算法使用启发式函数指导搜索，但Bellman备份仍需考虑所有适用动作，即使启发式表明某些动作代价过高，导致计算资源浪费。需要一种能利用启发式信息主动避免昂贵动作的方法。

Method: 将启发式搜索重新表述为线性规划问题，为SSP引入高效的约束生成技术。基于此开发CG-iLAO*算法，在iLAO*基础上集成新方法，通过约束生成机制筛选出有前景的动作，避免评估启发式指示为昂贵的动作。

Result: CG-iLAO*在许多问题上仅考虑iLAO* 40%的动作，某些情况下甚至低至1%。平均减少3.5倍的动作代价计算，比iLAO*快2.8倍，比LRTDP快3.7倍。

Conclusion: 通过将启发式搜索重新表述为线性规划并引入约束生成技术，能够有效避免考虑昂贵动作，显著提升随机最短路径问题的求解效率。CG-iLAO*在减少动作评估数量方面表现优异，实现了实质性加速。
...
Conclusion: 提出的可能性监督学习方法通过构造可接受概率分布集和KL投影，能够有效处理分级可信度信息，提升分类性能
...
Conclusion: Qiana提供了一个强大而灵活的逻辑框架，能够在特定上下文中进行推理，支持量化表达和矛盾处理，同时保持与现有一阶逻辑工具的兼容性，具有广泛的应用潜力。
...
TL;DR: 该研究创建了NeuroPose-AHM数据集，这是一个包含57种神经系统疾病异常头部运动的多条件知识库，通过多LLM框架从1430篇文献中提取2766条患者组记录，并展示了在颈肌张力障碍中的四任务分析应用。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏整合运动学测量、临床严重程度评分和患者人口统计学的多条件资源，阻碍了AI驱动诊断工具的开发。需要构建一个结构化的知识库来支持神经系统异常头部运动的研究。

Method: 采用多LLM提取框架从1430篇同行评审文献中构建NeuroPose-AHM数据集，包含57种神经系统疾病的2766条患者组记录。通过四任务框架分析颈肌张力障碍：1）多标签AHM类型分类；2）构建头颈严重程度指数；3）用真实患者数据验证HNSI；4）运动类型概率与HNSI分数的桥接分析。

Result: LLM提取可靠性高（kappa=0.822）。多标签AHM分类F1=0.856。构建了头颈严重程度指数，与真实患者数据严重带比例（6.7%）一致。运动类型概率与HNSI分数显著相关（p<0.001）。数据集已公开在Zenodo上。

Conclusion: NeuroPose-AHM是一个结构化的知识库资源，为神经系统异常头部运动研究提供了分析工具，特别是在颈肌张力障碍中展示了其应用价值，有助于推动AI驱动的诊断工具开发。
...
TL;DR: SenseMath基准测试评估LLM的结构敏感数值推理能力，发现模型在明确提示下能使用数值捷径，但自发使用率低，且缺乏对捷径适用性的判断能力。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型是否具有类似人类的"数感"——即识别数值结构、在适当时应用捷径、在不适用时避免使用的能力。当前LLM常默认逐步计算，即使存在高效数值捷径。

Method: 引入SenseMath基准测试，包含4,800个项目，涵盖八个捷径类别和四个数字规模，匹配强捷径、弱捷径和控制变体。支持三个评估设置：捷径使用、适用性判断和问题生成。

Result: 评估五个LLM（从GPT-4o-mini到Llama-3.1-8B）显示：明确提示时模型能采用捷径策略，准确率提升达15%；但标准思维链提示下自发使用率低于40%。模型在适用性判断上系统性地过度泛化捷径，且无法从头生成有效的捷径问题。

Conclusion: 当前LLM表现出程序性捷径流畅性，但缺乏支撑人类数感的结构性理解——即理解捷径何时有效及为何有效。这表明模型缺乏真正的数感，只是机械地应用策略。
...
TL;DR: 提出基于Transformer掩码自编码器的无标签框架，用于关节级步态异常检测与运动学矫正，仅使用正常步态数据进行训练，无需疾病标签。


<details>
  <summary>Details</summary>
Motivation: 传统基于监督学习的步态分析方法依赖疾病标签数据，难以泛化到异质性病理表现。需要一种无需标签、能够检测异常并自动矫正运动学数据的框架。

Method: 使用Transformer掩码自编码器，仅用150名成人的正常步态序列训练。推理时采用两阶段处理：1)通过遮挡单个关节并测量与学习到的正常先验的偏差来估计关节不一致性分数；2)将标记的关节从编码器输入中排除，从剩余时空上下文重建完整骨架，在标记位置生成矫正后的运动学轨迹。
...
<details>
  <summary>Details</summary>
Motivation: 强化学习虽然能增强大语言模型的推理能力，但在多轮智能体任务中面临挑战：长时程交互和环境反馈的随机性导致探索困难。研究者发现智能体探索中存在结构性失败模式：次优行动产生误导性观察，进一步削弱后续决策，形成错误累积循环，使标准探索策略失效。

Method: 提出ProCeedRL（Process Critic with Explorative Demonstration RL），将探索从被动选择转向主动干预。该方法使用过程级批评器实时监控交互，结合基于反思的演示来引导智能体停止错误累积。通过从探索性演示和在线策略样本中学习，提升探索效率。

Result: ProCeedRL显著超越了模型饱和的探索性能，展示了实质性的探索优势。在复杂的深度搜索和具身任务中，该方法显著提高了探索效率并实现了优越性能。

Conclusion: ProCeedRL通过主动干预和过程监控有效解决了多轮智能体任务中的探索问题，打破了错误累积循环，为强化学习在复杂智能体任务中的应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有轨迹级基准测试存在交互多样性不足、安全失败观测粗糙和长时域真实性弱的问题，无法有效评估LLM智能体在现实部署中多步交互中涌现的风险。

Method: 基于三维风险分类（风险来源、失败模式、现实危害）构建轨迹，使用异构工具池和长上下文延迟触发协议模拟现实风险涌现过程，包含1000条轨迹（503安全/497不安全），平均9.01轮对话和3.95k tokens，工具池包含2084个可用工具。

Result: ATBench对前沿LLM、开源模型和专用防护系统都具有挑战性，支持分类分层分析、跨基准比较和长时域失败模式诊断，数据质量通过规则和LLM过滤加人工审核保证。

Conclusion: ATBench为LLM智能体安全评估提供了结构化、多样化和现实化的轨迹级基准，能够有效捕捉多阶段交互中涌现的风险，支持更全面的安全性能分析和诊断。
...
TL;DR: 该论文是一篇关于语言模型中潜在空间的综述性文章，系统梳理了潜在空间作为语言模型原生计算基质的演进历程、技术机制、能力支持及未来展望。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型的发展，潜在空间正成为比显式token生成更自然的内部计算基质。现有研究分散且缺乏系统性梳理，需要统一框架来理解潜在空间作为下一代智能计算范式的完整图景。

Method: 采用五维框架组织综述：基础（定义与范围）、演进（历史发展）、机制（技术实现）、能力（功能支持）、展望（未来方向）。从机制角度分析架构、表示、计算、优化四个维度；从能力角度考察推理、规划、建模、感知、记忆、协作、具身等七大功能。

Result: 建立了潜在空间的系统性分类框架，识别了显式空间计算的四大结构限制（语言冗余、离散化瓶颈、序列效率低下、语义损失），梳理了潜在空间支持的多维度能力谱系，并提出了未来研究方向。

Conclusion: 潜在空间不仅是语言模型的技术组件，更是下一代智能系统的通用计算范式。该综述为现有研究提供参考框架，并为理解潜在空间作为智能计算基质奠定基础，推动语言模型向更高效、更自然的内部计算机制发展。
...
<details>
  <summary>Details</summary>
Motivation: 传统保险问卷冗长标准化，难以捕捉个体差异，且依赖用户诚实回答，存在欺诈风险。需要更智能、个性化且可信的承保评估方法。

Method: 提出ARQuest框架，结合大型语言模型和替代数据源（社交媒体图像、地理数据等），采用检索增强生成技术提取用户洞察，生成个性化自适应问卷。

Result: 集成到行业伙伴移动应用的生命保险系统实验显示：传统问卷风险评估准确度略高，但GPT驱动的自适应版本所需问题更少，用户体验更流畅且更受用户青睐。
...
TL;DR: Diff-KD：一种将基于扩散的生成式精炼与师生知识蒸馏相结合的多智能体协同感知框架，用于在传感器和通信损坏情况下恢复清洁语义
...
TL;DR: 该论文研究了在无参考情况下，使用大语言模型作为时间序列解释的生成器和评估器，构建了包含350个案例的合成基准，发现生成任务存在模式依赖性和系统性失败，而评估任务则更为稳定可靠。


<details>
  <summary>Details</summary>
Motivation: 评估基于时间序列数据的大语言模型生成解释的事实正确性是一个开放挑战。现有方法存在局限：基于参考的相似性指标和一致性检查模型需要真实解释，而传统时间序列方法仅处理数值无法评估自由形式的文本推理。目前缺乏无需预定义参考或任务特定规则、能直接验证解释是否忠实于底层时间序列数据的通用方法。

Method: 研究在无参考设置下使用大语言模型作为时间序列解释的生成器和评估器。评估器基于模式识别、数值准确性和答案忠实性为候选解释分配三元正确性标签。构建了包含350个时间序列案例的合成基准，涵盖七种查询类型，每种都配有正确、部分正确和错误的解释。评估模型在四个任务上的表现：解释生成、相对排名、独立评分和多异常检测。

Result: 结果显示出明显的不对称性：生成任务高度依赖模式，在某些查询类型上表现出系统性失败，准确率从季节性下降和波动率变化的0.00-0.12，到结构突变的0.94-0.96不等；而评估任务则更为稳定，即使模型自身输出不正确，也能正确排名和评分解释。这表明基于数据的LLM评估在时间序列解释领域是可行的。

Conclusion: 研究证明了基于数据的大语言模型评估在时间序列解释领域的可行性，并突显了它们作为时间序列领域中数据驱动推理的可靠评估器的潜力。评估任务比生成任务表现更稳定，为无参考设置下的时间序列解释评估提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在检测LLM驱动的社交机器人时存在三个主要问题：过度依赖单模态信号、对AIGC特定生成模式敏感性不足、未能充分建模语言模式与行为动态之间的相互作用。这些局限性导致检测准确率有限。
...
<details>
  <summary>Details</summary>
Motivation: 当前EASA指南要求证明AI/ML组件的操作设计域（ODD）完全覆盖，但现有方法在高维参数空间中缺乏可扩展性和形式化基础，无法满足完整性标准，且没有标准化的工程方法来弥合抽象ODD定义与可验证证据之间的差距。
...
Conclusion: 该方法填补了当前工程方法的空白，为AI/ML系统在安全关键领域的认证提供了可扩展且形式化的ODD覆盖验证框架，有助于满足严格的航空认证要求。
...
TL;DR: 研究评估LLM生成的胸部CT报告日文翻译的教育适用性，发现放射科医师与LLM评估者之间存在显著差异，LLM评估者强烈偏好LLM输出，而放射科医师意见不一，表明自动LLM评估不足以替代专家评审。


<details>
  <summary>Details</summary>
Motivation: 放射学报告的多语言翻译对研究、临床沟通和教育很重要，但LLM评估的有效性尚不明确。本研究旨在评估LLM生成的胸部CT报告日文翻译的教育适用性，并比较放射科医师评估与LLM作为评估者的表现。

Method: 使用CT-RATE-JPN验证集中的150份胸部CT报告。每份英文报告比较人工编辑的日文翻译与DeepSeek-V3.2生成的LLM翻译。由一名认证放射科医师和一名放射科住院医师进行盲法配对评估，评估术语准确性、可读性、整体质量和放射科医师风格真实性。同时使用3个LLM评估者（DeepSeek-V3.2、Mistral Large 3和GPT-5）评估相同配对。使用QWK和百分比一致性评估一致性。

Result: 放射科医师与LLM评估者之间一致性接近零（QWK=-0.04至0.15）。两位放射科医师之间一致性也很差（QWK=0.01至0.06）。放射科医师1认为59%的案例术语等效，在可读性（51%）和整体质量（51%）上偏好LLM翻译。放射科医师2认为75%的案例可读性等效，在整体质量上偏好人工编辑翻译（40% vs 21%）。所有3个LLM评估者强烈偏好LLM翻译（70%-99%），并在>93%的案例中认为其更具放射科医师风格。

Conclusion: LLM生成的翻译常被判断为自然流畅，但两位放射科医师评估存在显著差异。LLM作为评估者表现出对LLM输出的强烈偏好，与放射科医师的一致性可忽略。对于放射学报告翻译的教育用途，仅依赖自动LLM评估不足够；专家放射科医师评审仍然重要。
...
Result: 开发了模型无关的token重要性可视化技术，提供数学基础扎实的token重要性度量，开源实现已发布在GitHub上

Conclusion: 该方法为理解生成式AI系统如何感知和优先处理输入信息提供了轻量级、模型无关的解决方案，无需额外计算成本，有助于可解释性研究的推广
...
TL;DR: 提出Trace Inversion方法，通过分析模型推理轨迹重构其回答的问题，比较原始问题与重构问题的相似度来判断是否应该弃权，显著提升推理模型的弃权能力。
...
Method: 提出查询错位框架，开发Trace Inversion方法：1)生成模型推理轨迹；2)仅基于轨迹重构模型最可能回答的查询；3)比较原始查询与重构查询的相似度，低相似度则标记为弃权。

Result: 在四个前沿LLM和九个弃权QA数据集上的广泛实验表明，Trace Inversion在36个设置中的33个击败了竞争基线，有效提升了弃权性能。

Conclusion: Trace Inversion通过分析模型推理轨迹与原始查询的一致性，提供了一种新的有效弃权方法，显著改善了推理模型的弃权能力。
...
TL;DR: 研究探讨了用户查询中的第一人称情感框架对LLM性能的影响，发现静态情感前缀通常只产生微小变化，但通过自适应情感提示框架EmotionRL可以获得更可靠的性能提升。


<details>
  <summary>Details</summary>
Motivation: 情感语调在人类交流中普遍存在，但其对大型语言模型行为的影响尚不清楚。研究者希望探究用户查询中的第一人称情感框架如何影响LLM在不同基准领域的性能。

Method: 研究在六个基准领域（数学推理、医学问答、阅读理解、常识推理和社会推理）中测试了LLM性能。使用了静态情感前缀、不同强度情感措辞、人类编写前缀和LLM生成前缀进行比较。最后引入了EmotionRL自适应情感提示框架，为每个查询自适应选择情感框架。

Result: 静态情感前缀通常只产生微小的准确性变化，表明情感措辞通常是轻微的扰动而非可靠的通用干预。在社交基础任务中效果更可变，情感语境更可能与人际推理相互作用。更强的情感措辞仅引起适度额外变化。自适应情感选择比固定情感提示产生更可靠的性能提升。
...
TL;DR: De Jure：一个完全自动化的领域无关管道，用于从原始文档中提取结构化监管规则，无需人工标注、领域特定提示或标注黄金数据


<details>
  <summary>Details</summary>
Motivation: 监管文件编码了LLM系统必须遵守的法律义务，但将密集、分层结构的法律文本转换为机器可读规则仍然是一个成本高昂、需要专家参与的过程

Method: 四阶段管道：1) 源文档标准化为结构化Markdown；2) LLM驱动的语义分解为结构化规则单元；3) 多标准LLM-as-a-judge评估（19个维度）；4) 在有限再生预算内迭代修复低分提取

Result: 在金融领域，De Jure在三个法官引导的迭代内达到峰值性能；在医疗和AI治理领域保持高性能；在RAG合规问答评估中，基于De Jure提取规则的响应在73.8%-84.0%情况下优于先前工作
...
Result: 相比基线0.455到0.05的性能下降，该方法将长对话F1提升至0.583以上，提高记忆保持一致性，减少虚假记忆率（从6.8%降低），且不增加上下文使用。

Conclusion: 结构化遗忘在保持推理性能的同时防止了无界记忆增长，为扩展对话场景提供了有效的记忆管理解决方案。
