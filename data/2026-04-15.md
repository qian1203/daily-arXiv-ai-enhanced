...
TL;DR: 该论文将科学发展轨迹视为优化问题，认为科学知识体系在历史任何时刻都处于局部最优而非全局最优，受历史偶然性、认知路径依赖和制度锁定影响，可能错过更优的自然描述。


<details>
  <summary>Details</summary>
Motivation: 科学作为人类探索自然最可靠的方法，其发展轨迹很少被作为优化问题来研究。作者认为当前科学知识体系可能只是局部最优解，而非全局最优，这限制了科学发现更优自然描述的可能性。

Method: 采用类比机器学习梯度下降的方法，分析科学遵循可处理性、经验可及性和制度回报的最陡局部梯度发展。通过数学、物理、化学、生物学、神经科学和统计方法学的详细案例研究，识别认知、形式和制度三种锁定机制。

Result: 识别了三种相互关联的锁定机制：认知锁定（思维框架限制）、形式锁定（数学形式主义固化）、制度锁定（学术机构奖励结构）。这些机制共同导致科学停留在局部最优，可能错过更根本的自然描述。

Conclusion: 认识到这些锁定机制是设计元科学策略以逃离局部最优的前提。论文提出具体干预措施，并讨论了该论点对科学哲学的认识论意义，强调需要系统性反思科学发展轨迹的优化特性。
...
TL;DR: 该论文提出GoodPoint方法，通过作者响应信号训练LLM生成有效且可执行的反馈，显著提升反馈质量


<details>
  <summary>Details</summary>
Motivation: 虽然LLMs有潜力变革科学研究，但应作为增强研究人员能力的工具而非完全自动化研究。作者关注建设性反馈生成任务，旨在产生有针对性的、可执行的反馈来帮助作者改进研究和呈现方式。

Method: 从两个作者中心维度（有效性和作者行动）评估反馈效果。首先构建GoodPoint-ICLR数据集（19K篇ICLR论文，包含作者响应的反馈标注）。然后提出GoodPoint训练方法：在有效且可执行的反馈上进行微调，并在真实和合成的偏好对上进行偏好优化。

Result: 在1.2K篇ICLR论文基准测试中，GoodPoint训练的Qwen3-8B相比基础模型将预测成功率提升83.7%，在类似规模LLMs中在反馈匹配上达到新SOTA，甚至在精确度上超越Gemini-3-flash。专家人类研究进一步验证了GoodPoint能提供更高实用价值。

Conclusion: GoodPoint方法通过利用作者响应中的成功信号，有效提升了LLM生成反馈的质量和实用性，为实现LLM增强而非替代研究人员的愿景提供了可行路径。
...
TL;DR: ArcDeck：基于多智能体的论文转幻灯片生成框架，通过结构化叙事重构任务，显式建模源论文的逻辑流程，提升演示文稿的叙事流畅性和逻辑连贯性。


<details>
  <summary>Details</summary>
Motivation: 现有方法直接将原始文本总结为幻灯片，忽略了源论文的逻辑流程和叙事结构，导致生成的演示文稿缺乏连贯性和逻辑性。

Method: 1. 解析输入构建话语树，建立全局承诺文档以保留高层意图；2. 利用结构化先验指导迭代式多智能体精炼过程，由专门智能体批判和修订演示大纲；3. 最终渲染视觉布局和设计。

Result: 实验结果表明，显式的话语建模结合角色特定的智能体协调，显著提升了生成演示文稿的叙事流畅性和逻辑连贯性。同时引入了ArcBench新基准数据集进行评估。

Conclusion: ArcDeck通过结构化叙事重构方法，在论文转幻灯片生成任务中实现了更好的逻辑连贯性和叙事质量，为文档转换任务提供了新的框架思路。
...
TL;DR: HORIZON：首个跨领域诊断基准，用于系统构建任务和分析LLM智能体在长视野任务中的失败行为，评估了GPT和Claude等SOTA智能体，提出了可扩展的轨迹基础LLM-as-a-Judge故障归因方法。
...
Method: 1. 引入HORIZON跨领域诊断基准，系统构建任务和分析长视野失败行为；2. 评估GPT-5变体和Claude模型的SOTA智能体，收集3100+轨迹；3. 提出轨迹基础的LLM-as-a-Judge管道进行可扩展、可复现的故障归因；4. 通过人工标注验证，达到强一致性（人工标注间κ=0.61；人-判断器κ=0.84）。

Result: 1. 建立了首个跨领域长视野任务诊断基准；2. 收集了3100+轨迹，研究了视野依赖的退化模式；3. 验证了LLM-as-a-Judge管道的有效性；4. 提供了构建更可靠长视野智能体的实用指导。

Conclusion: HORIZON为系统化、跨领域分析长视野智能体失败提供了初步方法论步骤，并为构建更可靠的长视野智能体提供实用指导。项目网站已发布，欢迎社区贡献。
...
TL;DR: 提出Memory Worth（MW）作为智能体记忆质量的运行指标，通过追踪记忆与成功/失败结果的共现频率来指导记忆的信任、抑制和淘汰决策。


<details>
  <summary>Details</summary>
Motivation: 现有智能体记忆系统缺乏原则性的运行指标来管理记忆质量。写时重要性评分是静态的，动态管理系统使用LLM判断或结构启发式而非结果反馈，无法适应任务分布变化。

Method: 提出Memory Worth（MW）：每个记忆单元维护两个计数器，分别追踪该记忆与成功结果和失败结果的共现频率。MW在平稳检索机制和最小探索条件下几乎必然收敛到条件成功概率p+(m)。

Result: 在受控合成环境中，经过10,000轮后，MW与真实效用的Spearman秩相关系数达到ρ=0.89±0.02，而从不更新评估的系统为ρ=0.00。在真实文本和神经嵌入检索的微实验中，陈旧记忆的MW值降至0.17，而专业记忆保持0.77。

Conclusion: MW提供了一种轻量级、理论基础的记忆质量评估方法，仅需每个记忆单元两个标量计数器，可集成到已记录检索和轮次结果的架构中，为记忆管理提供实用的运行信号。
...
TL;DR: 该研究通过实验证明，大型语言模型中智能体身份文档（cognitive_core）会诱导类似吸引子的几何结构，语义相关的表述会收敛到更紧密的激活空间簇中。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索大型语言模型中智能体身份文档是否表现出类似吸引子的行为，类比于语言模型将语义相关提示映射到相似内部表示的现象。

Method: 使用Llama 3.1 8B Instruct进行控制实验，比较原始cognitive_core（条件A）、七个释义（条件B）和七个结构匹配控制（条件C）的隐藏状态。在8、16、24层进行平均池化，使用Cohen's d和Bonferroni校正进行统计分析。在Gemma 2 9B上复现验证跨架构普适性，并进行消融实验分析语义和结构因素。

Result: 释义比控制条件收敛到更紧密的簇（Cohen's d > 1.88，p < 10^{-27}）。在Gemma 2 9B上成功复现，证明跨架构普适性。消融实验表明效应主要是语义而非结构驱动，但结构完整性对于达到吸引子区域是必要的。探索性实验显示，阅读智能体的科学描述会将内部状态移向吸引子，比虚假预印本更接近，区分了"了解身份"和"作为身份操作"。

Conclusion: 智能体身份文档在LLM激活空间中诱导类似吸引子的几何结构，为智能体身份在语言模型中的表示提供了证据，并区分了关于身份的知识和作为身份操作的状态。
...
TL;DR: WiseOWL是一种帮助选择语义网中可重用本体的方法论，通过四个指标评分并提供指导，以解决本体选择缺乏系统标准的问题。


<details>
  <summary>Details</summary>
Motivation: 语义网标准使机器可操作内容，重用本体可加速开发并确保一致性，但作者缺乏系统选择标准，通常依赖难以证明的直觉，限制了本体重用。

Method: 提出WiseOWL方法论，包含四个评分指标：(i)良好描述性-衡量文档覆盖率；(ii)良好定义性-使用最先进嵌入评估标签与定义对齐；(iii)连接性-捕捉结构互连性；(iv)层次广度-反映层次平衡。输出0-10标准化分数并提供可操作反馈，实现为Streamlit应用，可处理OWL格式并转换为RDF Turtle，提供交互式可视化。

Result: 在六个本体上评估：植物本体(PO)、基因本体(GO)、语义科学集成本体(SIO)、食物本体(FoodON)、都柏林核心(DC)和GoodRelations，证明了方法的有效性。

Conclusion: WiseOWL为语义网本体选择提供了系统化、可量化的方法论，通过多维度评分和指导解决了本体重用中的选择难题，有望促进本体重用和语义网发展。
...
TL;DR: 论文提出针对个人LLM知识维基的治理框架，通过五步操作防止用户耦合漂移导致的认知固化，强调记忆系统应作为用户伴侣而非简单存储。


<details>
  <summary>Details</summary>
Motivation: 当前检索增强生成是LLM持久记忆的主流模式，但2026年出现了个人维基式记忆架构（如Karpathy、MemPalace、LLM Wiki v2）。这些单用户知识维基存在用户耦合漂移导致的认知固化风险，需要专门的治理框架来管理这种特定故障模式。

Method: 提出伴侣特定治理框架：包含规范性义务、时间结构化程序规则和可测试一致性不变量。通过五个操作实现：TRIAGE（分类）、DECAY（衰减）、CONTEXTUALIZE（情境化）、CONSOLIDATE（整合）、AUDIT（审计），辅以记忆重力和少数假设保留机制。

Result: 框架的核心预测：积累的矛盾证据应通过多周期缓冲压力积累的结构化路径更新受中心性保护的主导解释，这是现有基准未能捕捉的故障模式。该安全故事在单智能体层面是部分的，明确界定了解决和未解决的问题范围。

Conclusion: 个人LLM记忆应是伴侣系统，在操作维度上镜像用户（工作词汇、负载结构、上下文连续性），在认知故障模式上补偿用户（固化、矛盾证据抑制、库恩式僵化）。需要专门治理框架防止用户耦合漂移导致的认知固化。
...
TL;DR: 研究探讨了基于多智能体教师参与系统的个性化数学问题生成，发现教师和学生更关注真实性和情境适配，而系统检测到的现实性问题在最终版本中较少出现。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型如何通过多智能体教师参与系统实现中学数学问题的个性化生成，并评估该系统在实际教学环境中的效果。

Method: 开发了一个多智能体教师参与系统：教师输入基础问题和主题，LLM生成问题，然后四个AI智能体分别从数学准确性、真实性、可读性和现实性四个专业维度评估问题。8名中学数学教师使用该系统在ASSISTments平台创建了212个问题并分配给学生。

Result: 教师和学生都希望修改问题的真实世界情境中的细粒度个性化元素，表明存在真实性和适配性问题。虽然智能体在问题编写过程中检测到许多现实性问题，但在最终版本中教师和学生指出的现实性问题较少。可读性问题和数学幻觉也相对较少。

Conclusion: 研究为支持教师控制的多智能体个性化系统提供了启示，强调需要关注真实性和情境适配问题，同时表明系统在检测潜在问题方面具有价值。
...
TL;DR: 该论文提出了一种面向社交机器人的上下文选择性多模态记忆架构，通过结合文本和视觉信息来捕捉情感显著和场景新颖的时刻，实现个性化记忆检索，提升人机交互的自然性和社会相关性。


<details>
  <summary>Details</summary>
Motivation: 当前社交机器人和具身智能体主要依赖非选择性的文本记忆，限制了其支持个性化、上下文感知交互的能力。人类记忆具有选择性特征，能够优先记住情感显著和场景新颖的事件，而现有系统缺乏这种认知神经科学启发的记忆机制。

Method: 提出上下文选择性多模态记忆架构：1) 选择性存储机制，基于情感显著性和场景新颖性优先存储重要时刻；2) 多模态记忆捕获，同时记录文本和视觉事件痕迹；3) 个性化关联，将记忆与特定用户关联；4) 融合检索方法，结合文本和视觉信息进行记忆检索。

Result: 1) 选择性存储机制在社交场景数据集上获得Spearman相关系数0.506，超越人类一致性(ρ=0.415)和现有图像记忆性模型；2) 多模态融合检索将Recall@1提升最高13%；3) 系统保持实时性能；4) 定性分析显示框架产生比基线模型更丰富、社会相关性更强的响应。

Conclusion: 该工作通过融合人类启发的选择性和多模态检索，推进了社交机器人的记忆设计，能够增强长期、个性化的人机交互。提出的框架为社交机器人提供了更自然、更接地气的对话能力，使机器人能够像人类一样回忆有意义的过去经历。
...
Method: Aethon采用基于引用的复制原语，将每个智能体实例表示为稳定定义、分层内存和本地上下文覆盖的组合视图。通过从复制转向引用，将创建成本与继承结构解耦，实现分层继承和写时复制语义。

Result: Aethon实现了近乎恒定时间的智能体实例化，显著降低了延迟和内存开销。该系统支持复杂性、可扩展性、多智能体编排和企业治理，为生产规模的智能体软件提供了更合适的系统抽象。

Conclusion: 基于引用的实例化不仅是优化，更是生产规模智能体软件的更合适系统抽象。Aethon指向一类新的AI基础设施，其中智能体成为轻量级、可组合的执行身份，能够大规模生成、专门化和治理。
...
TL;DR: 论文批判了传统表格表示学习（TRL）采用NLP序列化范式的问题，提出柏拉图表示假说（PRH），认为表格的语义稳健潜在空间必须具有内在的排列不变性（PI），并设计了诊断指标和新型结构感知编码器架构。


<details>
  <summary>Details</summary>
Motivation: 传统表格表示学习方法将表格线性化处理，破坏了其固有的几何和关系结构，导致表示对布局排列敏感且脆弱。这种序列化偏差损害了表格推理的结构完整性，使得检索增强生成（RAG）系统中的表格检索容易受到布局相关噪声而非语义内容的影响。

Method: 1. 提出柏拉图表示假说（PRH），强调排列不变性（PI）对表格语义稳健性的必要性；2. 设计基于中心核对齐（CKA）的诊断框架：PI指标衡量完全结构打乱下的嵌入漂移，rho指标跟踪潜在结构向规范形式收敛的过程；3. 开发新型结构感知TRL编码器架构，显式执行单元格标题对齐的认知原则。

Result: 实证分析量化了现代大语言模型（LLMs）的缺陷：即使微小的布局排列也会导致表格嵌入发生显著且不成比例的语义偏移。提出的结构感知编码器展现出优越的几何稳定性，更接近PI理想状态，为表格检索提供了语义稳定、排列不变的理论框架。

Conclusion: 论文对线性化表格编码器进行了基础性批判，为语义稳定、排列不变的表格检索提供了理论支撑，为信息系统中的表格推理指明了新方向。结构感知编码器架构通过显式执行认知对齐原则，实现了更好的几何稳定性，推动了表格表示学习向排列不变性理想迈进。
...
TL;DR: 本文提出了一种观点感知的RAG架构，旨在解决现有RAG系统对客观事实内容的偏见问题，通过提取观点、构建观点图等方式提升对主观内容的检索多样性。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统存在对客观事实内容的偏见，将观点和多样视角视为噪声而非可综合的信息，限制了在社交媒体讨论、产品评论等主观内容场景中的应用。这种偏见还可能导致回音室效应、少数群体声音被系统性低估以及通过偏见信息合成进行观点操纵等风险。

Method: 基于不确定性理论框架，区分事实查询（涉及可通过证据减少的认知不确定性）和观点查询（涉及反映人类视角真实异质性的偶然不确定性）。提出观点感知RAG架构，包含基于LLM的观点提取、实体链接的观点图以及观点增强的文档索引。

Result: 在电子商务卖家论坛数据上的实验表明，与传统基线相比，观点增强知识库在检索多样性方面有显著提升：情感多样性+26.8%，实体匹配率+42.7%，实体匹配文档上的作者人口统计覆盖率+31.6%。
...
TL;DR: 开发自动化AI病历摘要工具用于胸科肿瘤多学科会诊，评估多种AI方法并与医生金标准对比，验证LLM作为评估策略，最终部署自动化工具并监测效果


<details>
  <summary>Details</summary>
Motivation: 肿瘤多学科会诊需要简洁的患者病例摘要以提高讨论效率和准确性，现有手动AI工作流程耗时耗力，需要开发自动化AI病历摘要工具来改进临床工作流程

Method: 开发多种自动化AI病历摘要方法，与医生金标准摘要对比评估，使用基于事实的评分标准，验证LLM作为评估策略，最终部署自动化工具并进行部署后监测

Result: 比较评估了多种AI摘要方法，验证了LLM作为评估策略的有效性，成功部署了最终状态的自动化AI病历摘要工具，并进行了部署后监测

Conclusion: 该研究展示了将AI工作流程整合到常规临床实践中的成功案例，自动化AI病历摘要工具能够提高肿瘤多学科会诊的效率和准确性
...
TL;DR: 论文研究了大型语言模型在高元关系推理方面的局限性，通过关系复杂度框架评估模型在代数、化学和生物学领域的表现，发现随着关系复杂度增加，模型性能持续下降。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型关系推理评估多关注结构化输入（如表格、图表）或合成任务，未能有效隔离高元关系绑定带来的推理难度。需要一种原则性的方法来控制混淆变量，专门研究关系绑定元数对推理能力的影响。

Method: 提出关系复杂度（RC）作为核心框架，定义为应用关系时需要同时绑定的独立实体或操作数的最小数量。基于RC构建REL生成式基准框架，涵盖代数、化学和生物学领域，在每个领域内系统性地变化RC。评估前沿LLMs在不同RC下的表现，并测试增加推理计算和上下文学习的影响。

Result: 前沿LLMs的性能随着RC增加而持续单调下降，即使实体总数保持不变。这种失败模式在增加推理计算和上下文学习后仍然存在，表明问题根源在于所需关系绑定的元数，而非推理步骤不足或缺乏示例。

Conclusion: 研究识别了当前模型在高元关系推理方面的困难区域，表明关系复杂度是评估推理能力的有效维度。结果强调了从关系复杂度视角重新审视现有基准的必要性，为未来模型改进提供了方向。
...
TL;DR: 该论文提出了一种基于认知诊断框架的细粒度能力评估方法，使用多维项目反应理论来估计大语言模型在数学、物理、化学和计算机科学等多个学科中的精细能力维度，超越了传统的单一分数评估。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的评估通常将不同任务的性能聚合为单一分数，这掩盖了模型在细粒度能力上的差异，限制了针对性的模型改进和为特定任务选择合适模型的能力。

Method: 提出了一个认知诊断框架，基于认知理论和领域知识构建细粒度能力分类法（如数学35维），采用多维项目反应理论结合项目-能力关联矩阵来估计模型的精细能力水平，并预测未见项目的性能。

Result: 在41个模型上的评估显示，该方法具有强的准则效度，跨基准的能力估计一致性高，预测未见项目的AUC在基准内为0.80-0.89，跨基准为0.77-0.86，显著超过简单基线。框架可泛化到物理（27维）、化学（58维）和计算机科学（12维）等领域。
...
TL;DR: 提出StsPatient方法，通过提取对比指令-响应对的转向向量来捕捉领域特定特征，并引入随机令牌调制机制，实现对认知障碍患者的细粒度模拟和严重程度控制。


<details>
  <summary>Details</summary>
Motivation: 现有模拟认知障碍患者的方法依赖离散提示工程，无法捕捉不同领域和严重程度缺陷的异质性，需要更精细、可扩展且符合伦理的临床培训解决方案。

Method: 提出StsPatient方法：1) 从对比指令-响应对中提取转向向量来捕捉领域特定特征；2) 引入随机令牌调制机制调节干预概率，实现对损伤严重程度的精确控制并提高稳定性。

Result: 综合实验表明，StsPatient在临床真实性和严重程度可控性方面显著优于基线方法。

Conclusion: StsPatient能够实现对认知障碍患者的细粒度模拟，解决了现有方法在捕捉缺陷异质性和控制损伤严重程度方面的局限性，为临床培训提供了更有效的解决方案。
...
TL;DR: 该研究探讨了GPT-4o在本科物理手写答题评分中的可靠性，发现AI辅助评分与人工评分者间一致性相当，但中等水平答案的评分一致性较低。清晰的技能导向评分量表和结构化提示对可靠性影响最大。


<details>
  <summary>Details</summary>
Motivation: STEM评估中的手写答题包含符号表达式、计算和图表，格式多样且解释复杂，人工评分耗时且存在评分者间不一致问题。尽管大语言模型在AI辅助评分方面有进展，但评分量表设计和LLM配置如何影响不同表现水平的可靠性仍缺乏证据。

Method: 使用GPT-4o对20份真实的本科物理手写考试答题进行评分。四名教师和AI模型使用不同分析粒度的技能导向评分量表进行两轮评分。系统变化提示格式和温度设置，比较人类-AI一致性和人类评分者间可靠性。

Result: 人类-AI在总分上的一致性与人类评分者间可靠性相当，对高水平和低水平答题的一致性最高，但对涉及部分或模糊推理的中等水平答题一致性下降。标准层面分析显示，明确定义的概念技能比对扩展的程序性判断有更强的一致性。更细粒度的检查表式评分量表比整体评分提高了评分一致性。

Conclusion: 可靠的AI辅助评分主要依赖于清晰、结构良好的评分量表，提示格式起次要作用，温度设置影响相对有限。研究为STEM环境中实施可靠的LLM辅助评分提供了可转移的设计建议，包括使用技能导向评分量表和受控的LLM设置。
...
<details>
  <summary>Details</summary>
Motivation: 小型语言模型在复杂数学推理方面存在困难，主要原因是其有限的能力难以维持长链的中间推理步骤，并且难以从早期错误中恢复。需要一种轻量级的方法来提升SLM的数学推理能力。

Method: 提出提示辅助推理框架，将解决方案分解为顺序推理步骤，并提供上下文感知的提示。使用两个SLM协作：一个提示生成SLM（通过从强大LLM蒸馏训练获得）负责生成条件提示，另一个推理SLM负责实际推理。提示基于问题陈述和累积推理历史生成，提供逐步、局部化的指导而不泄露完整解决方案。

Result: 在多样化的数学基准测试和模型上的实验表明，提示辅助方法持续提高了SLM的推理准确性，相比标准提示方法获得了显著提升，同时保持了模型效率。

Conclusion: 通过提示生成和推理的结构化协作，为增强数学推理提供了一种有效且轻量级的机制，展示了SLM之间协作的潜力。
...
Method: 提出多模态分块流水线MultiDocFusion：1) 基于视觉的文档区域检测；2) OCR文本提取；3) 使用LLM进行文档章节层次解析(DSHP-LLM)构建层次树；4) 基于DFS的分组构建层次化分块。

Result: 在工业基准测试中，MultiDocFusion相比基线方法将检索精度提升8-15%，ANLS问答分数提升2-3%，证明了显式利用文档层次结构对多模态文档问答的重要性。

Conclusion: 结构感知的分块方法对于提升基于RAG的问答系统保真度至关重要。MultiDocFusion通过多模态处理和层次结构重建有效解决了工业文档处理中的信息丢失问题。
...
TL;DR: ReflectCAP通过多智能体分析LVLM的幻觉和遗漏模式，生成结构化反思笔记来指导图像描述生成，在事实性和覆盖度之间达到帕累托前沿，同时降低计算成本。
...
Method: 提出ReflectCAP方法：1）多智能体管道分析目标LVLM的持续幻觉模式和系统性遗漏模式；2）将这些模式提炼为可重用的结构化反思笔记；3）在推理时，用这些笔记指导描述模型避免什么和关注什么。

Result: 在8个LVLM（GPT-4.1家族、Qwen系列、InternVL变体）上测试，ReflectCAP达到了事实性和覆盖度之间的帕累托前沿，在CapArena-Auto基准上显著优于强参考模型，计算开销比现有方法低21-36%。

Conclusion: ReflectCAP通过结构化反思笔记指导，实现了高质量详细图像描述的事实性和覆盖度平衡，在现实世界的成本和延迟约束下具有可行性。
...
TL;DR: CWAC通过同时约束权重更新和激活特征来保护LLM安全对齐，在微调时显著降低有害性同时保持任务性能


<details>
  <summary>Details</summary>
Motivation: 现有安全对齐防御方法通常单独约束权重或激活，未考虑二者耦合效应，导致微调时安全行为容易退化

Method: 提出耦合权重和激活约束(CWAC)：1)对权重更新施加预计算的安全子空间约束；2)使用稀疏自编码器识别安全关键特征并应用针对性正则化

Result: 在四个广泛使用的LLM和多样化下游任务上，CWAC始终获得最低有害分数，对微调精度影响最小，即使在高有害数据比例下也显著优于基线方法

Conclusion: 同时约束权重和激活对于保护LLM安全对齐至关重要，CWAC提供了一种有效且鲁棒的防御机制
...
TL;DR: 提出HCoT提示框架解决LLM在复杂问题求解中的两个核心限制：随机生成缺乏确定性规划，以及推理与决策机制静态解耦。通过启发式分类思想控制推理过程，提供可重用抽象解，在复杂归纳推理任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在解决复杂问题时的两个关键限制：1) 推理过程呈现贝叶斯式随机生成，导致决策轨迹随机而非确定性规划；2) 推理与决策机制静态解耦，动态检索的领域知识无法动态调整底层推理策略。这些缺陷导致初始决策缺乏战略锚定，推理链难以收敛到正确解。

Method: 提出Heuristic-Classification-of-Thoughts (HCoT)提示框架，将LLM的推理能力与结构化问题空间协同。通过启发式分类模型控制推理过程，提供可重用抽象解。该方法集成到LLM生成过程中指导推理，兼容多种LLM且具有可重用解决方案特性。

Result: 在两个复杂归纳推理任务中，HCoT优于现有方法（如Tree-of-Thoughts和Chain-of-Thoughts）。在结构良好的24点游戏任务中，相比最先进的Tree-of-Thoughts-Breadth-First-Search，HCoT展现出显著更高的token效率。在准确性和token使用方面，HCoT实现了帕累托前沿平衡，在性能和计算成本之间提供了良好权衡。

Conclusion: HCoT框架有效解决了LLM在复杂问题求解中的随机性和静态解耦问题，通过启发式分类思想实现了更可控、高效的推理过程，在性能与计算效率之间取得了良好平衡，为LLM的复杂推理能力提升提供了新方向。
...
TL;DR: 论文提出了一种轻量级顺序遗忘框架，用于在政治敏感环境中部署的大型语言模型，通过分离保留和抑制目标来实现数据删除要求。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在政治敏感环境中部署时，个人数据或机密内容的记忆会引发GDPR等法规下的监管担忧，特别是"被遗忘权"要求。将法律原则转化为大规模生成系统面临重大技术挑战。
...
TL;DR: 提出一种用于可解释聚类（概念聚类）的模式约简框架，通过消除导致相同k-覆盖的冗余k-松弛频繁模式，减少搜索空间并提高计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于k-松弛频繁模式（k-RFPs）的可解释聚类方法存在冗余问题：多个不同的k-RFPs可能诱导相同的k-覆盖，导致冗余的符号表示，不必要地扩大搜索空间并增加聚类构建的计算复杂度。

Method: 1. 形式化描述不同k-RFPs诱导相同k-覆盖的条件，为冗余检测提供理论基础；2. 提出优化策略，通过为每个不同的k-覆盖保留单一代表性模式来移除冗余模式；3. 分析ILP模型选择模式的鲁棒性，研究其可解释性和代表性。

Result: 在多个真实世界数据集上的广泛实验表明，所提方法显著减少了模式搜索空间，提高了计算效率，并在某些情况下保持甚至提升了所得聚类的质量。

Conclusion: 通过消除冗余k-松弛频繁模式，提出的模式约简框架有效解决了可解释聚类中的冗余问题，在保持聚类质量的同时显著提高了计算效率，为知识驱动的无监督学习提供了更高效的解决方案。
...
TL;DR: 本文提出通信推断攻击(CIA)，一种在限制性黑盒设置下推断多智能体系统通信拓扑的隐私攻击方法，揭示MAS中的重大安全风险。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统的通信拓扑结构安全日益受到关注，本文旨在研究一个关键的隐私风险：在限制性黑盒设置下，MAS的通信拓扑可以被推断出来，这会暴露系统漏洞并构成重大知识产权威胁。

Method: 提出通信推断攻击(CIA)，通过构建新的对抗性查询来诱导中间智能体的推理输出，并通过全局偏置解耦和LLM引导的弱监督来建模它们的语义相关性。

Result: 在具有优化通信拓扑的MAS上进行广泛实验，CIA表现出色，平均AUC达到0.87，峰值AUC高达0.99，证实了MAS中存在的重大隐私风险。

Conclusion: 通信推断攻击(CIA)有效揭示了多智能体系统中通信拓扑推断的隐私风险，表明即使在黑盒设置下，MAS的通信结构也可能被攻击者推断，这对系统安全和知识产权保护构成严重威胁。
...
<details>
  <summary>Details</summary>
Motivation: 视频监控中的车辆计数对交通管理至关重要，但现有方法在保持高计数精度的同时难以实现计算效率的平衡

Method: 提出两阶段框架：1)估计阶段基于检测分数、跟踪分数和车辆密度自动确定最优ROI；2)预测阶段在估计的ROI内高效进行车辆计数

Result: 在UA-DETRAC、GRAM、CDnet 2014和ATON数据集上评估，大多数视频达到100%准确率，计算效率提升4倍，在复杂多道路场景中表现优异

Conclusion: 该框架在保持高精度的同时显著提升计算效率，优于现有技术，特别适合复杂场景，是实时交通监控的有前景解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有相似性方法主要关注命题逻辑，而一阶逻辑论证相似性在论证聚合和省略前提解码等应用中具有重要意义，需要处理结构化内容

Method: 构建包含四个组件的综合框架：1)扩展的公理基础；2)覆盖谓词、文字、子句和公式的四层参数模型；3)两种模型家族（一种基于语言模型的语法敏感方法），均整合上下文权重；4)确保理想属性的形式约束

Result: 提出了首个针对一阶逻辑论证相似性的全面框架，能够处理结构化内容，提供细致且可解释的相似性度量

Conclusion: 该框架为一阶逻辑论证相似性提供了理论基础和实用方法，填补了现有研究的空白，支持论证聚合和省略前提解码等应用
...
TL;DR: 提出两阶段LLM元验证框架，通过解释器LLM生成自然语言解释，验证器LLM评估忠实性、连贯性、完整性和幻觉风险，并利用迭代反馈机制改进解释，提高XAI系统的可信度和可访问性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM用于将XAI技术输出转化为自然语言解释的方法缺乏准确性、忠实性和完整性的保证，现有评估方法主观且局限于事后评分，无法防止有缺陷的解释到达最终用户。

Method: 提出两阶段LLM元验证框架：1)解释器LLM将原始XAI输出转换为自然语言叙述；2)验证器LLM从忠实性、连贯性、完整性和幻觉风险四个方面评估解释；3)迭代反馈机制利用验证器的反馈来细化和改进解释。

Result: 在五种XAI技术和数据集上使用三个开放权重LLM家族的实验表明，验证对于过滤不可靠解释至关重要，同时相比原始XAI输出提高了语言可访问性。细化过程中的熵产生率分析表明，验证器的反馈逐步引导解释器走向更稳定和连贯的推理。
...
<details>
  <summary>Details</summary>
Motivation: LLM在高风险领域部署受限，存在概率校准不足、解释不忠实、无法精确融入专家知识等问题
...
Conclusion: IDEA框架能有效解决LLM决策中的校准和解释性问题，实现定量人机协作，超越单纯提示方法的精度
...
<details>
  <summary>Details</summary>
Motivation: 随着基于LLM的应用日益普及，需要开发有效的测试方法来评估这些系统的可靠性和安全性。本次竞赛旨在推动LLM测试工具的发展，特别是在识别系统未能适当处理警告信息等关键安全缺陷方面。

Method: 竞赛在ICSE 2026的DeepTest研讨会中举办，四个测试工具针对基于LLM的汽车手册信息检索应用进行基准测试。评估标准包括：1）暴露系统失败的有效性；2）发现失败揭示测试用例的多样性。采用实验方法学对竞争者工具进行系统评估。

Result: 报告了四个竞争工具在测试LLM汽车手册应用时的具体表现结果，包括各工具在发现系统未能适当提及警告信息的失败案例方面的效果，以及所发现测试用例的多样性差异。

Conclusion: 首届LLM测试竞赛为评估LLM测试工具提供了重要基准，展示了当前工具在发现安全关键缺陷方面的能力差异，为未来LLM测试方法的发展提供了参考方向。
...
Method: MemJack采用记忆增强的多智能体协作框架：1) 动态映射视觉实体到恶意意图；2) 通过多角度视觉语义伪装生成对抗性提示；3) 使用迭代零空间投影几何过滤绕过潜在空间拒绝；4) 通过持久多模态经验记忆积累和转移成功策略。
...
Conclusion: MemJack成功揭示了视觉语言模型的深层语义漏洞，其多智能体协作和记忆增强机制有效提升了攻击成功率。发布的基准数据集将为未来防御对齐研究提供重要基础，促进开发本质上更鲁棒的视觉语言模型。
...
TL;DR: KnowRL提出了一种知识引导的强化学习框架，通过原子知识点的约束子集搜索来解决提示设计中的冗余和训练开销问题，显著提升了小规模模型在推理任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于提示的RL方法通过添加更多token来扩展指导，但这会引入冗余、不一致性和额外的训练开销。需要一种最小充分指导的方法来缓解奖励稀疏性问题。

Method: KnowRL将指导分解为原子知识点(KPs)，使用约束子集搜索(CSS)构建紧凑、交互感知的训练子集，并优化在依赖结构下的鲁棒子集选择。

Result: 在8个推理基准测试中，KnowRL-Nemotron-1.5B在1.5B规模上持续优于强RL和提示基线。无KP提示时达到70.08平均准确率，比原模型提升+9.63点；有选择KPs时达到74.16，创下该规模的新SOTA。

Conclusion: KnowRL通过将提示设计视为最小充分指导问题，有效解决了奖励稀疏性，为小规模模型的推理能力提升提供了有效框架，模型、训练数据和代码均已开源。
...
TL;DR: 该研究探索了让语言模型预测自身输出质量的方法，使小模型能在自信时自行响应，不确定时求助大模型，以平衡计算效率与输出质量。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型面临计算效率（参数数量）与输出质量之间的权衡，特别是在计算资源有限的设备上。受人类在无法解决问题时寻求帮助的启发，研究者希望让小模型在自信能提供良好响应时自行回答，在不自信时向大模型求助，从而克服这一权衡。
...
Result: 大模型（特别是推理模型）在零样本预测通用LLM评判者时表现良好，而小模型在微调或提供上下文报告卡后能可靠预测这类评判者。报告卡和微调方法分别实现了平均高达55%和52%的预测准确率提升，显著改善了小模型的预测能力。

Conclusion: 模型能够学习预测自身的性能限制，这为开发更高效、更具自我意识的AI系统铺平了道路，使小模型能智能地决定何时自行响应、何时求助大模型，从而优化计算资源使用。
...
TL;DR: 提出了一种广义的条件语法分割概念，扩展了安全条件语法分割的适用性，允许子基共享原子和非平凡条件句，并引入了相应的推理公设。


<details>
  <summary>Details</summary>
Motivation: 传统语法分割要求信念基基于不相交的签名进行分割，这在实践中很少见。虽然安全条件语法分割允许条件句共享原子，但最近研究表明这种重叠仅限于平凡的自满足条件句。因此需要一种更通用的分割概念来扩大分割公设的适用性。

Method: 提出了广义条件语法分割的概念，允许子基共享原子和非平凡条件句。区分了真正分割和简单分割，引入了基于广义分割的调整推理公设，并评估了多种流行归纳推理算子对这些公设的满足情况。

Result: 广义条件语法分割克服了先前分割概念的局限性，能够处理更广泛的信念基分割情况。证明了满足广义条件语法分割的归纳推理算子也满足条件语法分割，但反之不成立。

Conclusion: 广义条件语法分割为条件信念基的非单调推理提供了更灵活和实用的分割框架，扩展了分割公设的适用范围，为实际应用中的推理提供了更好的支持。
...
TL;DR: 提出PF-CD3Q方法，结合粒子滤波与约束双深度Q学习，解决考虑动态疲劳约束的人机协作任务规划分配问题


<details>
  <summary>Details</summary>
Motivation: 工业5.0中人机协作制造强调人体工程学，传统疲劳恢复模型使用静态超参数，但实际中人类疲劳敏感性因工作条件变化和睡眠不足等因素而每日变化，需要在线估计疲劳参数以更好处理不确定性
...
Result: 未在摘要中明确说明具体实验结果，但方法旨在实现实时疲劳预测的人机任务规划分配，确保工人物理疲劳保持在安全限度内

Conclusion: PF-CD3Q方法能够处理疲劳参数的不确定性，通过在线估计和约束动作空间，在保证工人安全的同时优化生产效率，为人机协作制造中的动态任务规划分配问题提供了有效解决方案
...
TL;DR: 提出基于案例的学习框架，将过往任务经验转化为可重用知识资产，使LLM智能体能够将先验案例经验迁移到新任务并进行结构化分析，在复杂任务上表现优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在一般推理任务上表现良好，但在复杂现实场景中难以可靠利用任务结构、关键约束和先验经验。现有方法主要依赖预训练知识或静态提示，缺乏从实际案例中提取和重用相关知识的能力。

Method: 提出基于案例的学习框架，将过往任务经验转化为可重用知识资产，包括任务相关知识、分析提示和操作技能。框架强调从真实案例中提取和重用这些元素，使智能体能够将先验案例经验迁移到新任务并进行结构化分析。

Result: 在六个复杂任务类别的统一基准测试中，该方法在所有任务上均表现稳定强劲，在每种情况下都匹配或优于最佳基线（包括零样本、少样本、检查表提示和规则记忆基线）。在更复杂任务上优势尤为明显，且案例学习优势随任务复杂度增加而增强。一个智能体获取的实践知识可被其他智能体重用。

Conclusion: 案例学习为构建面向现实工作的专业智能体提供了有前景的路径，能够有效提升LLM智能体在复杂任务中利用任务结构、关键约束和先验经验的能力。
...
<details>
  <summary>Details</summary>
Motivation: 虽然已有研究探索AI工具对数学任务质量的分类能力，但对其提升现有任务质量的能力知之甚少。本研究旨在探究AI工具是否能够成功升级低认知需求的数学任务，以了解AI在课程适应中的潜在作用。
...
Result: AI工具平均成功率中等：任务准确升级率仅64%，不同工具表现差异大（33%-88%）。专业工具仅略优于通用工具。失败模式包括"欠升级"（保持低认知需求）和"过升级"（提升到可能被教师拒绝的过高目标类别）。有趣的是，AI工具正确分类任务认知需求的能力与升级任务的能力呈小负相关（r=-0.35）。

Conclusion: AI工具在提升数学任务质量方面能力有限，任务分类与升级是两种不同的能力。研究结果对理解AI在课程适应中的潜在作用具有重要意义，并突显了需要专门方法来支持教师修改教学材料。
...
TL;DR: DocSeeker通过引入"分析-定位-推理"结构化工作流，解决多模态大语言模型在长文档理解中的信号噪声比低和监督稀缺问题，采用两阶段训练框架提升长文档理解性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在长文档理解任务中面临两大挑战：1) 信号噪声比低，关键证据被无关页面淹没；2) 监督稀缺，仅提供最终简短答案的数据集学习信号弱。这些导致文档长度增加时性能显著下降。

Method: 提出结构化"分析-定位-推理"工作流范式。采用两阶段训练框架：首先通过高效知识蒸馏策略生成高质量数据进行监督微调；然后使用证据感知的组相对策略优化，联合优化证据定位和答案准确性。引入证据引导的分辨率分配策略缓解多页文档训练的内存约束。

Result: DocSeeker在领域内和领域外任务上均取得优异性能。实验表明它能从短页训练稳健地泛化到超长文档，并且与视觉检索增强生成系统自然协同，为其实现提供坚实基础。

Conclusion: 通过结构化工作流和两阶段训练框架，DocSeeker有效解决了MLLMs在长文档理解中的核心挑战，实现了从短页训练到超长文档的稳健泛化，为视觉检索增强生成系统提供了可靠基础。
...
<details>
  <summary>Details</summary>
Motivation: LLMs在预训练中不可避免地吸收有害知识、错误信息和个人数据，但现有机器遗忘方法都是提供商中心的，需要重新训练管道、精心设计的保留数据集和模型服务提供商的直接干预，排除了终端用户控制自己数据的能力。

Method: 提出交互式机器遗忘(IMU)范式，开发RePAIR框架：包含检测遗忘意图的watchdog模型、生成修复程序的surgeon模型、自主更新参数的patient模型。核心是STAMP方法，通过伪逆更新将MLP激活重定向到拒绝子空间，实现训练遗忘、单样本遗忘。低秩变体将计算复杂度从O(d^3)降低到O(r^3 + r^2*d)。

Result: 在有害知识抑制、错误信息纠正和个人数据擦除实验中，RePAIR实现接近零的遗忘分数(Acc_f = 0.00, F-RL = 0.00)，同时保持模型效用(Acc_r达84.47, R-RL达0.88)，优于六种最先进的基线方法，在设备上遗忘速度比基于训练的方法快约3倍。

Conclusion: RePAIR是用户驱动模型编辑的有效实用框架，推进了对学习知识的透明和设备上控制，有潜力扩展到多模态基础模型，实现了用户通过自然语言指令控制LLMs遗忘特定知识的目标。
...
TL;DR: 关于混合自主交通仿真中AI方法应用的综述，填补了现有研究在AI方法分类、仿真平台评估和跨学科整合方面的空白


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆在公共道路上运行，需要更有效的测试验证方法。现有仿真工具主要关注图形真实性和基于简单规则，无法准确反映驾驶行为和交互的复杂性。虽然AI有潜力解决这些限制，但缺乏对混合自主交通仿真中AI应用的综合调查，现有调查要么只关注仿真工具不分析AI方法，要么只关注自我中心决策不涉及周围交通建模，且缺乏统一的AI方法分类体系。

Method: 提出结构化分类体系，将AI方法分为三类：智能体级行为模型、环境级仿真方法、认知和物理信息方法。分析现有仿真平台的不足，提供AI方法的时间线概述，回顾评估协议和指标、仿真工具和数据集，整合交通工程和计算机科学视角。

Result: 建立了混合自主交通仿真中AI方法的统一分类框架，识别了现有仿真平台与混合自主研究需求之间的差距，提供了跨学科的综合视角，为未来研究指明了方向。

Conclusion: 本综述填补了混合自主交通仿真中AI方法应用的研究空白，通过结构化分类体系和分析框架，为研究人员提供了系统性的参考，有助于推动更真实、更有效的自动驾驶测试验证方法的发展，并促进交通工程和计算机科学社区的交流合作。
...
TL;DR: 该论文提出人类视觉系统通过分层处理将图像压缩为轮廓草图，再通过自上而下的语义反馈迭代优化，这一神经计算机制可能是古代象形文字起源的基础。


<details>
  <summary>Details</summary>
Motivation: 人类能够从稀疏线条图中识别物体，这种能力具有早期发展和跨文化普遍性，暗示其神经而非纯粹习得的起源。然而，大脑如何将高级语义知识转化为低级视觉符号的计算机制尚不清楚。研究者旨在探索古代象形文字是否源于大脑固有的视觉压缩倾向。

Method: 构建了一个受生物学启发的视觉层次数字孪生模型，该模型将图像编码为低级特征，生成轮廓草图，并通过自上而下的语义反馈进行迭代优化，模拟人类视觉皮层的前馈和循环架构。

Result: 生成的符号在结构上与跨文化的早期象形文字（包括埃及象形文字、中国甲骨文和原始楔形文字）惊人相似，并为未破译的文字提供了候选解释。模型结果支持象形文字的神经计算起源。

Conclusion: 研究支持象形文字源于神经计算起源的观点，并建立了一个框架，使人工智能能够重演人类首次将感知外部化为符号的认知过程。这为理解视觉符号的神经基础提供了新视角。
...
Result: QuarkMedSearch在QuarkMedSearch Benchmark上取得了同规模开源模型中的SOTA性能，同时在通用基准上保持强大竞争力。

Conclusion: 通过系统化的全流程方法（数据构建、训练策略、评估基准），QuarkMedSearch成功提升了智能基础模型在医疗垂直领域的性能上限，为其他垂直领域的应用提供了可借鉴的框架。
...
TL;DR: AISafetyBenchExplorer是一个包含195个AI安全基准测试的结构化目录（2018-2026年），通过多表架构组织元数据，揭示基准测试生态系统存在碎片化而非稀缺性问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型安全评估快速发展产生了大量基准测试，但缺乏相应的统一测量生态系统。当前基准测试激增但测量标准化滞后，导致碎片化问题严重。

Method: 构建结构化目录AISafetyBenchExplorer，采用多表架构记录基准测试级元数据、度量级定义、论文元数据和仓库活动，支持元分析。

Result: 发现基准测试生态系统严重碎片化：中等复杂度基准占主导（94/195），仅7个基准属于流行层级；英语评估占165/195；评估专用资源170/195；GitHub仓库过时137/195；Hugging Face数据集过时96/195；严重依赖arXiv预印本。

Conclusion: AI安全基准测试领域的主要失败模式是碎片化而非稀缺性。AISafetyBenchExplorer通过提供可追溯的基准目录、受控元数据模式和复杂度分类法，支持更严格的基准发现、比较和元评估。
...
TL;DR: BEAM：一种双层记忆自适应算法进化框架，将启发式设计重新表述为双层优化问题，通过遗传算法进化高层算法结构，用蒙特卡洛树搜索实现占位符，显著提升复杂代码生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的超启发式方法主要针对预定义求解器中的单一函数优化，单层进化难以生成完整的求解器。虽然有些变体加入了超参数调优或通过迭代局部修改生成复杂代码，但缺乏高层算法建模，导致探索效率有限。

Method: 将启发式设计重新表述为双层优化问题。外层使用遗传算法进化带有函数占位符的高层算法结构；内层通过蒙特卡洛树搜索实现这些占位符。引入自适应记忆模块促进复杂代码生成，并提出知识增强流水线支持评估。
...
TL;DR: PAL平台将讲座视频转化为交互式学习体验，通过实时分析多模态内容并动态调整问题难度，提供个性化学习支持
...
Result: PAL通过结合多模态内容分析与自适应决策，提供了一个响应式数字学习的新颖框架，展示了AI如何超越静态个性化，实现实时个性化支持

Conclusion: PAL解决了AI赋能教育的核心挑战，将讲座视频转化为交互式学习体验，推动了从静态适应到实时个性化支持的教育技术发展
