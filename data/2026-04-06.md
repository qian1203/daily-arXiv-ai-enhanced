...
TL;DR: XpertBench是一个用于评估大语言模型在真实专业领域能力的高保真基准，包含80个类别1346个任务，采用ShotJudge评估范式，发现顶尖模型在专业任务上存在显著性能天花板（最高成功率约66%）。


<details>
  <summary>Details</summary>
Motivation: 现有评估框架存在领域覆盖狭窄、依赖通用任务或自我评估偏差等问题，无法有效评估LLMs在复杂开放任务中展现的真实专家级认知能力，需要构建更贴近实际专业场景的评估基准。

Method: 构建XpertBench基准：包含1346个任务，覆盖金融、医疗、法律服务、教育及双轨研究（STEM和人文科学）等80个类别；任务来自1000多份领域专家提交；采用详细评分标准（15-40个加权检查点）。引入ShotJudge评估范式：使用经过专家少样本示例校准的LLM评估员，以缓解自我奖励偏差。

Result: 对最先进LLMs的评估显示显著性能天花板：即使领先模型最高成功率仅约66%，平均得分约55%。模型表现出领域特异性差异，在定量推理与语言合成方面显示非重叠优势。

Conclusion: 当前AI系统存在显著的"专家差距"，XpertBench为从通用助手向专业协作伙伴的转型提供了关键评估工具，揭示了模型在真实专业任务中的局限性。
...
TL;DR: 提出一种神经符号架构，通过提取网格中的对象级结构、使用神经先验从固定DSL提出候选变换、并通过跨示例一致性过滤假设，在ARC-AGI-2上显著提升LLM性能
...
Method: 提出神经符号架构：1) 从网格中提取对象级结构；2) 使用神经先验从固定领域特定语言(DSL)提出候选变换；3) 通过跨示例一致性过滤假设；4) 基于单元模式构建组合推理框架，增强LLM的对象表示和变换提议能力
...
TL;DR: 电网资本规划框架：考虑极端天气不确定性，结合数字孪生、蒙特卡洛模拟和多目标优化，发现简单净现值排序法优于复杂模型优化方法
...
Conclusion: 在电网资本规划中，简单的净现值排序方法比复杂的模型感知优化方法表现更好，这对实际应用具有重要启示
...
Method: 提出新的可解释强化学习方法，包含三个主要改进：(a)使用可微软树模型作为行动者函数逼近器，(b)训练过程中的温度退火过程，(c)正则化配合剪枝规则以限制政策复杂度，最终生成确定性倾斜决策树形式的可解释生命周期政策。

Result: 该方法在监督学习和强化学习设置中都展示了其优势与权衡，并在钢梁桥生命周期优化问题中进行了框架演示，能够生成节点数量和深度合理的可直接理解和审计的生命周期政策。

Conclusion: 提出的可解释强化学习方法能够基于元素级状态表示制定最优生命周期政策，以可理解的决策树形式呈现，便于人类直接理解和审计，并易于集成到现有桥梁管理系统中。
...
TL;DR: 提出基于知识图谱的神经符号架构，将能力问题转化为可执行的叙事计划，通过"计划-检索-生成"工作流解决LLM在文化遗产应用中幻觉问题，并在Live Aid KG上评估三种RAG策略。
...
Method: 提出基于知识图谱的神经符号架构，建立透明的"计划-检索-生成"工作流。关键创新是将能力问题从设计时验证工具重新定位为运行时可执行的叙事计划。使用Live Aid KG（多模态数据集，对齐1985年音乐会数据与音乐元本体，链接外部多媒体资产）验证架构，并系统比较三种RAG策略：纯符号KG-RAG、文本增强的Hybrid-RAG和结构感知的Graph-RAG。

Result: 实验揭示了符号检索的事实精度、混合方法的上下文丰富性和基于图遍历的叙事连贯性之间的量化权衡。符号检索提供最高的事实精度，混合方法提供更好的上下文丰富性，而图遍历方法提供最佳的叙事连贯性。

Conclusion: 该方法为设计个性化和可控的叙事系统提供了可操作的见解，通过基于知识图谱的神经符号架构解决了LLM在文化遗产应用中的幻觉问题，实现了证据封闭和完全可审计的生成过程。
...
TL;DR: 论文研究了LLMs在教师教学质量评估中对虚假上下文信息的敏感性，提出了Debiasing-DPO方法减少84%的偏见并提高52%的预测准确性。
...
Method: 使用美国课堂转录本数据集(NCTE)评估7个前沿和开源模型对7类虚假上下文的敏感性。提出Debiasing-DPO方法：通过自监督训练，将仅基于查询的中性推理与基于查询加虚假上下文的偏见推理配对，并结合监督微调以保持预测准确性。

Result: 虚假上下文信息可使模型预测在7分制上偏移高达1.48分，较大模型有时表现出更高敏感性。Debiasing-DPO应用于Llama和Qwen模型后，平均减少84%的偏见并提高52%的预测准确性。

Conclusion: 对虚假上下文的鲁棒性不是模型规模扩展的自然副产品。提出的Debiasing-DPO方法在基于提示的预测任务中能显著提高准确性和鲁棒性，特别是在教育案例研究中表现出色。
...
TL;DR: 对音频-视觉大语言模型（AVLLMs）的首个机制可解释性研究，揭示了模型在处理冲突模态时存在视觉偏向，音频语义在中间层被编码但最终文本生成中被视觉表征抑制。


<details>
  <summary>Details</summary>
Motivation: 作为新兴的多模态感知统一接口，AVLLMs如何整合音频和视觉信息尚不明确。本研究旨在通过机制可解释性方法分析AVLLMs中音频和视觉特征的演化与融合过程，特别是当两种模态存在冲突时的处理机制。

Method: 采用机制可解释性方法，分析AVLLMs不同层中音频和视觉特征的演化与融合过程。通过探测分析验证潜在音频信息的存在性，追踪训练过程以识别模态偏向的根源。

Result: 研究发现：1）AVLLMs在中间层编码了丰富的音频语义；2）当音频与视觉冲突时，这些音频能力在最终文本生成中无法有效体现；3）探测分析显示有用的潜在音频信息存在，但深层融合层不成比例地偏向视觉表征，抑制音频线索；4）训练分析表明AVLLMs的音频行为与其视觉-语言基础模型高度匹配，表明对音频监督的额外对齐有限。

Conclusion: 本研究首次揭示了AVLLMs中存在根本性的模态偏向，为理解多模态大语言模型如何整合音频和视觉提供了新的机制性见解。研究结果表明当前AVLLMs在训练过程中未能充分对齐音频监督，导致视觉表征在冲突情况下主导最终输出。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将模式设计嵌入流水线代码或临时提取关系，导致模式与构建过程紧密耦合，难以重用进行下游本体级任务

Method: 采用本体导向方法，核心是内在-关系路由机制，将每个属性分类为内在属性或关系属性，并路由到相应模式模块，生成声明式模式

Result: 在2026年1月Wikidata数据集上实例化：规则清理得到3460万实体核心集，内在-关系路由将属性分配到94个模块（8个类别），模式覆盖率达93.3%，模块分配率98.0%，导出图谱包含3400万节点和6120万边（38种关系类型）

Conclusion: 通过五个独立应用验证了本体导向方法的有效性：本体结构分析、基准标注审计、实体消歧、领域定制和LLM引导提取，证明模式可独立于构建流水线重用
...
TL;DR: GrandCode是一个多智能体强化学习系统，首次在实时编程竞赛中击败所有人类选手，包括传奇大师级选手，在最近的三个Codeforces比赛中均获得第一名。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统在编程竞赛中仍落后于人类最佳选手，Google的Gemini~3 Deep Think仅获得第8名且未在实时竞赛条件下评估。编程竞赛是AI尚未超越人类的少数领域之一。

Method: 采用多智能体RL系统，协调多种智能体模块（假设提出、求解器、测试生成器、总结等），通过后训练和在线测试时RL联合改进。引入专门设计的Agentic GRPO来处理多阶段智能体展开中的延迟奖励和严重的离策略漂移问题。

Result: 在最近的三个Codeforces实时竞赛（第1087、1088、1089轮）中，GrandCode均获得第一名，击败了所有人类参与者，包括传奇大师级选手。这是首个在实时编程竞赛中一致超越所有人类选手的AI系统。

Conclusion: GrandCode表明AI系统已达到在最具竞争力的编程任务上超越最强人类程序员的水平，标志着AI在编程竞赛领域取得了突破性进展。
...
TL;DR: DeltaLogic是一个基准转换协议，将自然语言推理示例转化为短修订片段，评估模型在最小证据变化下的信念修正能力，揭示固定前提下的逻辑能力并不保证有效的信念更新。


<details>
  <summary>Details</summary>
Motivation: 现有推理基准主要评估模型在固定前提集下得出正确答案的能力，但低估了动态环境中至关重要的能力：在最小证据变化下的信念修正。需要测量逻辑能力与信念修正能力之间的关系。

Method: 提出DeltaLogic基准转换协议，将自然语言推理示例转化为修订片段：首先在前提P下询问初始结论，然后应用最小编辑δ(P)，最后询问先前结论应保持稳定还是需要修订。从FOLIO和ProofWriter实例化DeltaLogic，使用约束标签评分评估小型因果语言模型。

Result: 在30个片段的Qwen评估子集上，更强的初始推理能力并不保证更强的修订行为：Qwen3-1.7B初始准确率0.667但修订准确率仅0.467，在应改变标签的片段中惯性达0.600；Qwen3-0.6B几乎完全弃权；Qwen3-4B保持相同惯性失败模式；Phi-4-mini-instruct表现更强但仍有显著弃权和控制不稳定性。
...
TL;DR: 提出了一种量化角色清晰度的方法，通过构建角色分配矩阵和角色清晰度矩阵来衡量多智能体系统中角色描述与行为轨迹的对齐程度，并将其作为正则化项进行轻量级微调以提升角色一致性和任务性能。


<details>
  <summary>Details</summary>
Motivation: 在大语言模型驱动的多智能体系统中，智能体不遵守角色规范（未能遵守分配角色的职责和约束，可能导致智能体行为像其他角色）是一个主要的失效模式。为了解决这个问题，需要一种量化方法来提高角色一致性。

Method: 首先构建角色分配矩阵S(φ)，其中s_{ij}(φ)表示第i个智能体的行为轨迹与第j个智能体的角色描述之间的语义相似度。然后定义角色清晰度矩阵M(φ)=softmax(S(φ))-I，其中softmax(S(φ))是S(φ)的行向softmax，I是单位矩阵。M(φ)的Frobenius范数量化了智能体角色描述与其行为轨迹之间的对齐程度。最后将角色清晰度矩阵作为正则化项在轻量级微调中使用，以提高角色一致性。
...
Conclusion: 提出的量化角色清晰度方法能有效解决多智能体系统中的角色不遵守问题，通过角色清晰度矩阵作为正则化项进行微调，显著提高了角色一致性和端到端任务性能。
...
### [12] [CharTool: Tool-Integrated Visual Reasoning for Chart Understanding](https://arxiv.org/abs/2604.02794)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在图表推理方面面临三大挑战：缺乏高质量训练数据、需要细粒度视觉定位、需要精确数值计算。这些限制导致MLLMs在图表理解任务上表现不佳。

Method: 1. DuoChart：可扩展的双源数据管道，结合合成图表和真实世界图表构建多样化高质量训练数据；2. CharTool：为MLLMs配备外部工具，包括用于局部视觉感知的图像裁剪和用于精确数值推理的基于代码的计算；3. 在DuoChart上进行智能体强化学习，使CharTool学习基于图表内容的工具集成推理。

Result: 在六个图表基准测试中，该方法持续优于强大的MLLM基线模型。CharTool-7B在CharXiv(Reasoning)上比基础模型提升+8.0%，在ChartQAPro上提升+9.78%，与更大或专有模型相比具有竞争力。此外，CharTool在领域外视觉数学推理基准上表现出正向泛化能力。

Conclusion: DuoChart数据管道和CharTool工具增强方法有效解决了图表推理的关键挑战，通过结合高质量数据和工具集成推理，显著提升了MLLMs的图表理解能力，并展示了良好的泛化性能。
...
TL;DR: EMS框架通过可靠性感知的智能体调度，在达到多数共识时提前终止推理，减少32%的智能体调用
...
Method: 提出高效多数即停(EMS)框架，包含三个核心组件：智能体置信度建模(ACM)基于历史性能和语义相似度估计可靠性；自适应增量投票(AIV)顺序选择智能体并实现早停；个体置信度更新(ICU)动态更新参与智能体的可靠性

Result: 在六个基准测试上的广泛评估表明，EMS能够持续减少平均调用的智能体数量达32%

Conclusion: EMS将多智能体投票重新定义为可靠性感知的智能体调度问题，通过智能排序和早停机制显著提高了推理效率
...
TL;DR: LLMs在规划任务中表现出接近理论最优的性能，超越了传统规划器，即使在去除语义线索的抽象拓扑任务中也能保持高效推理。


<details>
  <summary>Details</summary>
Motivation: 研究前沿大语言模型在规划问题中的推理能力，特别是它们是否进行最优推理还是依赖简单的启发式策略，以及如何解释它们在复杂规划任务中的优异表现。

Method: 使用Blocksworld领域和等价的广义Path-Star（P*）图任务，系统操纵问题深度（塔高）、宽度（塔数）和组合性（目标块数），比较LLMs与传统规划器（如LAMA）的性能。

Result: 增强推理的LLMs在复杂多目标配置中显著优于传统规划器；随着搜索空间扩展，经典搜索算法遇到瓶颈，而LLMs能接近理论最优性限制，即使去除领域特定语义线索也能保持近乎完美的精度。

Conclusion: LLMs通过算法模拟和几何记忆两种机制实现高效规划：算法模拟通过推理标记执行，几何记忆允许模型将P*拓扑表示为可导航的全局几何结构，有效规避指数级组合复杂性。
...
TL;DR: 论文发现大型推理模型存在"第一个即最佳"现象，提出RED框架通过"精炼首个"和"丢弃后续"策略提升推理效率


<details>
  <summary>Details</summary>
Motivation: 尽管大型推理模型在复杂推理任务中表现出色，但研究发现其探索的多个替代解决方案不仅次优，反而可能有害，这挑战了现有的测试时扩展定律

Method: 提出RED自引导高效推理框架：1) Refining First - 抑制首个解决方案中的错误森林增长；2) Discarding Subs - 通过双重一致性剪枝后续错误森林

Result: 在5个基准测试和6个骨干模型上的实验表明，RED优于8个竞争基线，性能提升最高达19.0%，同时减少37.7%~70.4%的token消耗

Conclusion: 论文揭示了大型推理模型中"第一个即最佳"现象，提出的RED框架通过抑制错误森林增长实现了高效推理，为模型推理优化提供了新视角
...
<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的智能体搜索系统过度关注深度多步推理，忽视大规模信息合成挑战，面临上下文饱和、级联错误传播和高延迟等问题

Method: 基于近可分解性原则设计分层框架，包含战略Host、多个Manager和并行Worker三层，通过Manager层的聚合与反思机制实现严格上下文隔离，Worker层并行加速任务执行

Result: 在两个互补基准测试中展示高效性和有效性：速度提升3-5倍，在WideSearch-en上达到8.4%成功率，在BrowseComp-zh上达到52.9%准确率

Conclusion: InfoSeeker框架通过分层设计和并行处理有效解决了数据密集型环境中智能体搜索系统面临的核心挑战，平衡了深度推理与广度信息合成需求
...
TL;DR: AI系统在一周内将500多页研究生级代数组合学教科书自动形式化为Lean代码，包含13万行代码和5900个声明，创下教科书形式化规模和效率新纪录


<details>
  <summary>Details</summary>
Motivation: 展示AI系统在大规模教科书形式化方面的能力，从早期本科拓扑学形式化扩展到完整研究生教材形式化，验证多智能体协作在软件工程中的可行性

Method: 使用30,000个Claude 4.5 Opus智能体通过版本控制并行协作，在共享代码库上工作，实现教科书到Lean代码的自动形式化

Result: 成功形式化完整研究生教材，生成130K行Lean代码和5900个声明，推理成本低于或等于人类专家团队薪资，创下多智能体软件工程记录

Conclusion: AI系统能够高效完成大规模教科书形式化，成本效益优于人类团队，展示了多智能体协作在复杂软件工程任务中的潜力，代码已开源
...
TL;DR: Chart-RL：基于强化学习的视觉语言模型框架，通过反馈驱动的策略优化提升图表问答能力，在减少参数和降低延迟的同时实现性能提升


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在图表问答任务中存在数值提取不精确、隐含视觉关系理解困难、空间关系注意力机制不足等局限性，需要提升图表理解中的推理能力
...
TL;DR: 该论文提出以松鼠生态学作为智能体AI研究的比较案例，探讨在部分可观测性、延迟和策略性观察下，控制、记忆和验证的耦合需求。


<details>
  <summary>Details</summary>
Motivation: 当前AI研究将控制、记忆和验证分开研究，但实际智能体需要同时处理这些耦合需求。松鼠生态学提供了一个理想的比较案例，因为其树栖运动、分散储藏和观众敏感缓存行为同时涉及这三种需求。

Method: 采用三层推理阶梯：经验观察、最小计算推理和AI设计猜想。引入最小分层部分可观测控制模型，包含潜在动态、结构化情景记忆、观察者信念状态、选项级动作和延迟验证信号。

Result: 提出三个假设：H1-快速局部反馈加预测补偿提高隐藏动态变化下的鲁棒性；H2-为未来控制组织的记忆改善延迟检索；H3-动作-记忆循环内的验证器和观察者模型减少静默失败和信息泄漏。下游猜想是角色分化的提议者/执行者/检查者/对抗者系统可减少不对称信息下的相关错误。

Conclusion: 贡献在于提供了一个比较视角和基准议程：一个关于控制、记忆和可验证动作耦合的可证伪主张的严谨研究计划，为智能体AI设计提供生态学启示。
