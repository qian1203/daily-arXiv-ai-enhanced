...
TL;DR: XpertBench是一个用于评估大语言模型在真实专业领域能力的高保真基准测试，包含80个类别1346个任务，通过专家校准的ShotJudge评估范式发现当前领先模型在专业任务上存在显著性能天花板（最高成功率仅约66%）。
...
Method: 构建XpertBench基准测试，包含1346个精心策划的任务，涵盖金融、医疗、法律服务、教育和双轨研究（STEM与人文科学）等80个类别。任务来源于1000多份领域专家提交，每个任务使用包含15-40个加权检查点的详细评分标准。引入ShotJudge评估范式，使用经过专家少样本示例校准的LLM评估器来减轻自奖励偏差。

Result: 对最先进大语言模型的实证评估显示显著性能天花板：即使领先模型最高成功率仅约66%，平均得分约55%。模型还表现出领域特异性差异，在定量推理与语言合成方面显示出非重叠优势。

Conclusion: 研究结果强调了当前AI系统存在的显著"专家差距"，确立了XpertBench作为从通用助手向专业协作伙伴过渡的关键评估工具的重要性。
...
TL;DR: 提出神经符号架构，结合对象级结构提取、神经先验引导的转换提议和符号一致性过滤，在ARC-AGI-2上显著提升LLM性能
...
Method: 提取网格中的对象级结构，使用神经先验从固定DSL提出候选转换，通过跨示例一致性过滤假设，构建基于单元模式的组合推理框架

Result: 在ARC-AGI-2公开评估集上，将基础LLM性能从16%提升至24.4%，结合ARC Lang Solver后达到30.8%

Conclusion: 分离感知、神经引导转换提议和符号一致性过滤可在无需任务特定微调或强化学习的情况下改善泛化，减少对暴力搜索和基于采样的测试时扩展的依赖
...
TL;DR: 电网资本投资优化框架研究：包含极端天气不确定性、数字孪生、蒙特卡洛模拟和多目标优化的四部分框架，发现简单的净现值排序法比复杂的基于模型的元启发式方法更有效。
...
Conclusion: 在电网资本投资优化中，简单的净现值排序方法优于复杂的基于模型的元启发式优化方法，特别是在计算复杂性和有限电网知识的情况下。
...
TL;DR: 提出基于可解释强化学习的方法，使用斜决策树形式生成桥梁全寿命周期优化策略，解决元素级状态表示带来的高维状态空间挑战。


<details>
  <summary>Details</summary>
Motivation: 2022年实施的新国家桥梁清单规范强调使用元素级状态进行基于风险的桥梁管理，这虽然提高了数据粒度，但将状态空间从单一分类整数扩展到四维概率数组，给全寿命周期策略优化带来了挑战。

Method: 提出新的可解释强化学习方法，采用三个主要改进：1) 使用可微分软树模型作为行动者函数逼近器；2) 训练过程中的温度退火过程；3) 正则化配合剪枝规则限制策略复杂度，最终生成确定性斜决策树形式的策略。

Result: 该方法能够生成节点数量和深度合理的斜决策树形式全寿命周期策略，使策略对人类可直接理解和审计，并易于集成到现有桥梁管理系统中。在监督学习和强化学习设置中验证了各技术的效益和权衡。

Conclusion: 提出的可解释强化学习方法能够有效处理元素级状态表示带来的高维状态空间挑战，生成可理解、可审计且易于实施的全寿命周期优化策略，为钢梁桥等基础设施管理提供了实用解决方案。
...
TL;DR: 提出基于知识图谱的神经符号架构，将能力问题转化为可执行的叙事计划，通过"计划-检索-生成"工作流实现可验证的故事生成，在Live Aid知识图谱上评估了三种RAG策略。


<details>
  <summary>Details</summary>
Motivation: 非物质文化遗产保护面临集体记忆消失的挑战，现有大语言模型存在事实不准确的问题，无法满足文化遗产应用中对真实性的核心要求，需要开发可验证的故事生成系统。

Method: 提出基于知识图谱的神经符号架构，将传统设计时验证的能力问题重新用于运行时可执行的叙事计划，建立透明的"计划-检索-生成"工作流，在Live Aid多模态知识图谱上评估了三种RAG策略：纯符号KG-RAG、文本增强Hybrid-RAG和结构感知Graph-RAG。

Result: 实验揭示了符号检索的事实精确性、混合方法的上下文丰富性和基于图遍历的叙事连贯性之间的量化权衡，为设计个性化和可控的故事讲述系统提供了可操作的见解。

Conclusion: 提出的神经符号架构通过将能力问题转化为可执行叙事计划，实现了证据封闭和完全可审计的故事生成，为非物质文化遗产保护提供了可靠的技术解决方案。
...
TL;DR: 论文研究了LLMs在教师教学质量评估中对虚假上下文信息的敏感性，提出了Debiasing-DPO方法，在减少84%偏见的同时提高52%预测准确率。
...
Method: 使用NCTE数据集评估7个前沿和开源模型对7类虚假上下文的敏感性。提出Debiasing-DPO方法：通过自监督训练，将仅基于查询生成的中立推理与基于查询加虚假上下文生成的偏见推理配对，并结合监督微调以防止预测准确率损失。

Result: 虚假上下文信息可使模型预测在7分量表上偏移达1.48分，较大模型有时表现出更高敏感性。Debiasing-DPO在Llama和Qwen模型上将偏见减少84%，预测准确率提高52%。

Conclusion: 对虚假上下文的鲁棒性不是模型规模扩展的自然副产品，Debiasing-DPO方法能在基于提示的预测任务中显著提高准确性和鲁棒性。
...
TL;DR: 本文首次对音频-视觉大语言模型（AVLLMs）进行机制可解释性研究，揭示了模型在处理冲突模态时存在视觉偏见，音频语义在深层融合层被抑制。


<details>
  <summary>Details</summary>
Motivation: 音频-视觉大语言模型作为多模态感知的统一接口正在兴起，但对其内部工作机制缺乏理解。本文旨在通过机制可解释性方法分析AVLLMs如何处理和融合音频与视觉特征，特别是当两种模态存在冲突时。

Method: 采用机制可解释性研究方法，分析音频和视觉特征在AVLLM不同层中的演化与融合过程。通过探测分析检查潜在音频信息的存在性，追踪特征融合层中的模态偏好，并分析训练过程对模态对齐的影响。

Result: 研究发现：1）AVLLMs在中间层编码了丰富的音频语义，但当音频与视觉冲突时，这些能力在最终文本生成中未能体现；2）有用的潜在音频信息确实存在，但深层融合层不成比例地偏向视觉表示，抑制了音频线索；3）这种不平衡源于训练过程，AVLLM的音频行为与其视觉-语言基础模型高度一致，表明对音频监督的额外对齐有限。

Conclusion: 研究揭示了AVLLMs中存在根本性的模态偏见，为理解多模态大语言模型如何整合音频和视觉提供了新的机制性见解。这表明当前AVLLMs在模态融合方面存在系统性不平衡，需要改进训练策略以实现更均衡的多模态整合。
...
TL;DR: 提出一种本体导向的知识图谱构建方法，通过内在-关系路由机制将属性分类到模块化模式中，实现可移植、可重用的模式设计，并在Wikidata上验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱构建方法将模式决策嵌入流水线代码或临时提取关系，导致模式与构建过程紧密耦合，难以重用于下游本体级任务。需要一种从一开始就为本体分析、实体消歧、领域定制和LLM引导提取而设计的模式方法。

Method: 采用本体导向方法，核心是内在-关系路由机制：将每个属性分类为内在属性或关系属性，并路由到相应的模式模块。该方法包括基于规则的清洗阶段和迭代路由过程，结合工具增强的LLM支持和人工审查。

Result: 在2026年1月Wikidata转储上实例化：从完整转储中识别出3460万个实体的核心集，内在-关系路由将属性分配到8个类别下的94个模块。模式达到93.3%的类别覆盖率和98.0%的模块分配率。导出属性图包含3400万个节点和6120万条边，涉及38种关系类型。通过五个独立于构建流水线的应用验证了本体导向主张。

Conclusion: 提出的本体导向方法通过内在-关系路由机制产生了可移植、可重用的声明性模式，支持独立于构建流水线的下游应用，为知识图谱的本体级任务提供了更灵活和可维护的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI系统在编程竞赛中仍落后于人类最佳选手（如Google Gemini~3 Deep Think仅获第8名且非实时竞赛），编程竞赛是AI尚未超越人类的少数领域之一。需要开发能真正超越人类顶尖选手的AI系统。

Method: 采用多智能体RL系统，协调多种智能体模块（假设提出、求解器、测试生成器、总结等），通过后训练和在线测试时RL联合改进。引入专门设计的Agentic GRPO算法，处理多阶段智能体展开中的延迟奖励和严重的离策略漂移问题。

Result: 在最近三场Codeforces实时竞赛（第1087、1088、1089轮）中均获得第一名，击败所有人类参与者，包括传奇大师。这是首个在实时编程竞赛中持续击败所有人类选手的AI系统。
...
TL;DR: DeltaLogic是一个基准测试转换协议，将自然语言推理示例转化为信念修正测试，评估模型在最小证据变化下的信念修正能力，发现初始推理能力强的模型不一定具备良好的信念修正能力。


<details>
  <summary>Details</summary>
Motivation: 现有推理基准主要评估模型在固定前提集下得出正确答案的能力，但低估了动态环境中至关重要的能力：在最小证据变化下的信念修正。需要测量模型在前提发生微小变化时是否能正确调整结论。

Method: 引入DeltaLogic基准转换协议，将自然语言推理示例转化为短修正片段。每个片段首先要求在前提P下得出初始结论，然后应用最小编辑δ(P)，最后询问先前结论应保持稳定还是需要修正。从FOLIO和ProofWriter实例化DeltaLogic，使用约束标签评分评估小型因果语言模型。

Result: 在30个片段的Qwen评估子集上，初始推理能力强的模型不一定具备更强的修正行为：Qwen3-1.7B初始准确率0.667但修正准确率仅0.467，在应改变标签的片段上惯性达0.600；Qwen3-4B保持相同的惯性失败模式；Phi-4-mini-instruct表现更强但仍存在明显的弃权和控制不稳定问题。

Conclusion: 固定前提下的逻辑能力并不意味着在局部证据编辑后能进行有纪律的信念修正。DeltaLogic针对一个独特且实际重要的推理能力，补充了现有的逻辑推理和信念更新基准。
...
TL;DR: 提出了一种量化角色清晰度的方法，通过构建角色分配矩阵和角色清晰度矩阵来评估多智能体系统中角色一致性，并作为正则化项在微调中使用，显著降低了角色越界率并提高了任务成功率。
...
Method: 首先构建角色分配矩阵S(φ)=[s_ij(φ)]，其中s_ij(φ)是第i个智能体行为轨迹与第j个智能体角色描述之间的语义相似度。然后定义角色清晰度矩阵M(φ)=softmax(S(φ))-I，其中softmax(S(φ))是S(φ)的行向softmax，I是单位矩阵。M(φ)的Frobenius范数量化了智能体角色描述与其行为轨迹之间的对齐程度。将该矩阵作为正则化项在轻量级微调中使用，以提高角色一致性。

Result: 在ChatDev多智能体系统上的实验表明，该方法显著提高了角色一致性和任务性能：使用Qwen模型时，角色越界率从46.4%降至8.4%，角色清晰度得分从0.5328提升至0.9097，任务成功率从0.6769提升至0.6909；使用Llama模型时，角色越界率从43.4%降至0.2%，角色清晰度得分从0.5007提升至0.8530，任务成功率从0.6174提升至0.6763。

Conclusion: 提出的量化角色清晰度方法有效解决了多智能体系统中的角色越界问题，通过将角色清晰度矩阵作为正则化项进行微调，显著提高了角色一致性和端到端任务性能，为多智能体系统的角色管理提供了有效的量化工具。
...
TL;DR: EMS是一种高效的多智能体投票方法，通过可靠性感知的智能体调度，在达成多数共识时提前终止推理过程，减少32%的智能体调用。
...
TL;DR: LLMs在Blocksworld规划任务中表现出接近理论最优的性能，超越了传统满足式规划器，即使在去除语义线索的抽象图任务中也能保持高效推理。


<details>
  <summary>Details</summary>
Motivation: 研究前沿大语言模型在经典AI规划问题中的推理能力，特别是它们是否能进行最优规划而非仅依赖启发式策略，以及理解其背后的机制。

Method: 使用Blocksworld领域（带标签的积木塔移动任务）和形式等价的广义Path-Star图任务，系统操纵问题深度（塔高）、宽度（塔数）和组合性（目标积木数），比较LLMs与传统规划器（如LAMA）的性能。

Result: 推理增强的LLMs在复杂多目标配置中显著优于传统满足式规划器。随着搜索空间扩展，经典搜索算法遇到瓶颈，而LLMs能以接近完美的精度跟踪理论最优性极限，即使去除领域特定语义提示后仍保持性能。

Conclusion: LLMs在规划任务中表现出令人惊讶的最优推理能力，可能通过两种机制实现：通过推理令牌执行的算法模拟，以及允许模型将P*拓扑表示为可导航全局几何的几何记忆，有效绕过指数级组合复杂性。
...
TL;DR: 论文发现大推理模型存在"第一个即最优"现象，提出RED框架通过精炼首个解和剪枝后续解来提升推理效率


<details>
  <summary>Details</summary>
Motivation: 尽管大推理模型在复杂推理任务中表现出色，但研究发现其探索多个替代解的策略存在缺陷：第一个解往往是最优的，后续解不仅次优甚至有害，这挑战了测试时扩展定律

Method: 提出RED自引导高效推理框架，包含两个组件：1) Refining First - 抑制首个解中的错误森林增长；2) Discarding Subs - 通过双重一致性剪枝后续错误森林

Result: 在5个基准测试和6个骨干模型上的实验表明，RED优于8个竞争基线，性能提升高达19.0%，同时减少37.7%~70.4%的token消耗

Conclusion: 研究揭示了错误森林导致"第一个即最优"现象，提出的RED框架通过抑制错误森林增长和剪枝策略，实现了更高效准确的推理
...
Result: 在两个互补基准测试中展示了效率和效果：3-5倍速度提升，在WideSearch-en上达到8.4%成功率，在BrowseComp-zh上达到52.9%准确率
...
TL;DR: Agentic-MME：首个面向多模态智能体能力的流程验证基准，包含418个真实世界任务，通过2000+步骤检查点评估视觉扩展与知识扩展的协同能力，支持沙箱代码和API，实现细粒度中间状态审计而非仅最终答案。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM评估存在三大不足：1) 缺乏灵活工具集成；2) 将视觉工具和搜索工具分开测试；3) 仅评估最终答案而无法验证工具是否实际调用、正确应用或高效使用。因此需要开发能真正评估多模态智能体协同能力的流程验证基准。

Method: 构建Agentic-MME基准：包含418个真实世界任务，覆盖6个领域和3个难度级别；设计超过2000个步骤检查点，每个任务平均10+人工标注小时；提供统一评估框架支持沙箱代码和API；包含人工参考轨迹，沿S轴（语义）和V轴（视觉）标注步骤检查点；通过审计细粒度中间状态而非仅最终答案实现流程级验证；引入"过度思考"指标量化效率。

Result: 最佳模型Gemini3-pro在整体任务上达到56.3%准确率，但在Level-3任务上显著下降至23.0%，表明真实世界多模态智能体问题解决的难度；实验验证了基准能有效评估模型在视觉扩展与知识扩展协同方面的能力差距。

Conclusion: Agentic-MME填补了多模态智能体评估的空白，首次实现流程级验证，能准确评估工具调用、应用正确性和使用效率；基准揭示了当前MLLM在复杂真实世界任务中的显著局限性，为未来多模态智能体研究提供了重要评估工具。
...
<details>
  <summary>Details</summary>
Motivation: 展示AI系统在教科书形式化方面的能力，从本科拓扑学和现有库内容的重构，扩展到完整的研究生教材形式化，实现规模化自动形式化

Method: 使用3万个Claude 4.5 Opus智能体通过版本控制并行协作，在共享代码库上工作，实现多智能体软件工程

Result: 成功形式化整本教材，包含130K行Lean代码和5900个声明，推理成本与人类专家团队薪资相当或更低，代码和蓝图网站已开源

Conclusion: 这是教科书形式化规模和熟练度的新里程碑，展示了AI系统在复杂数学形式化方面的潜力，且仍有进一步效率提升空间
...
Method: 提出Chart-RL强化学习框架，结合策略优化技术和自适应奖励函数，通过反馈驱动的视觉感知和逻辑推理策略优化，并集成LoRA参数高效微调技术，仅需单GPU配置

Result: 在ChartQAPro数据集上，RL微调的Qwen3-VL-4B-Instruct模型达到0.634准确率，超越基础模型Qwen3-VL-8B-Instruct的0.580准确率，同时参数减半，推理延迟从31秒降至9秒

Conclusion: Chart-RL框架有效解决了VLMs在图表问答中的关键挑战，通过强化学习优化实现了性能提升与效率改进的平衡，为复杂数据可视化理解提供了高效解决方案
...
TL;DR: 松鼠生态学为研究智能体AI的控制、记忆和验证耦合提供了比较案例，提出分层部分可观测控制模型及相关假设


<details>
  <summary>Details</summary>
Motivation: 现有研究将控制、记忆和验证分开研究，而松鼠生态学将三者耦合在一个生物体中，为AI智能体设计提供了比较视角

Method: 综合狐狸松鼠、东部灰松鼠和红松鼠的证据，建立分层部分可观测控制模型，包含潜在动态、结构化情景记忆、观察者信念状态、选项级动作和延迟验证信号

Result: 提出三个可证伪假设：H1-快速局部反馈加预测补偿提高隐藏动态变化下的鲁棒性；H2-为未来控制组织的记忆提高延迟检索性能；H3-动作-记忆循环中的验证器和观察者模型减少静默失败和信息泄漏

Conclusion: 松鼠生态学为AI智能体设计提供了比较视角和基准议程，角色分化的提议者/执行者/检查者/对抗者系统可能减少不对称信息和验证负担下的相关错误
