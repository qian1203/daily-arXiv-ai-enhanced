...
TL;DR: XpertBench是一个用于评估大语言模型在真实专业领域能力的高保真基准，包含1,346个跨80个类别的专家级任务，采用ShotJudge评估范式，发现当前顶级模型在专家级任务上存在显著性能天花板（最高成功率仅~66%）。


<details>
  <summary>Details</summary>
Motivation: 现有评估框架存在领域覆盖狭窄、依赖通用任务或自评估偏差等问题，无法有效评估LLMs在复杂开放任务中展现的真实专家级认知能力。需要建立一个能够评估LLMs在真实专业领域能力的基准。

Method: 1) 构建XpertBench基准：包含1,346个跨80个类别的专家级任务，涵盖金融、医疗、法律、教育及双轨研究（STEM和人文科学）；2) 任务来源于1,000多份领域专家提交，确保生态效度；3) 每个任务使用详细评分标准（15-40个加权检查点）；4) 提出ShotJudge评估范式：使用经过专家少样本示例校准的LLM评估者来减轻自奖励偏差。

Result: 对最先进LLMs的评估显示：1) 存在显著性能天花板，即使领先模型最高成功率仅~66%，平均得分约55%；2) 模型表现出领域特异性差异，在定量推理与语言合成方面存在非重叠优势；3) 揭示了当前AI系统与专家能力之间的"专家差距"。

Conclusion: XpertBench为评估LLMs从通用助手向专业协作伙伴的过渡提供了关键工具，揭示了当前模型在专家级任务上的局限性，强调了开发更专业化AI系统的必要性。
...
TL;DR: 该论文提出了一种神经符号架构，用于解决抽象与推理语料库（ARC）中的结构化抽象推理问题，通过分离感知、神经引导的转换提议和符号一致性过滤来提高泛化能力。


<details>
  <summary>Details</summary>
Motivation: 纯神经架构缺乏可靠组合泛化能力，而纯符号系统在感知基础方面存在困难。ARC任务需要同时处理感知和组合推理，现有方法难以平衡这两个方面。

Method: 提出神经符号架构：从网格中提取对象级结构，使用神经先验从固定领域特定语言（DSL）中提出候选转换，并通过跨示例一致性过滤假设。基于单元模式构建组合推理框架，将大型语言模型与对象表示和转换提议相结合。

Result: 在ARC-AGI-2上，将基础LLM性能从16%提升到24.4%（公开评估集），与ARC Lang Solver结合后达到30.8%。无需任务特定微调或强化学习，同时减少对暴力搜索和基于采样的测试时扩展的依赖。

Conclusion: 分离感知、神经引导的转换提议和符号一致性过滤能够改善泛化能力，神经符号方法在组合推理任务中优于纯神经或纯符号方法。该方法为ARC任务提供了有效的解决方案框架。
...
TL;DR: 提出AIVV框架，利用LLM作为审议外层循环，自动化验证与验证过程，解决传统深度学习模型在异常分类和可扩展性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习模型能检测异常模式，但无法有效区分真实故障与噪声引起的虚假故障，导致验证与验证过程仍需人工参与，工作量不可持续。

Method: 提出Agent-Integrated Verification and Validation (AIVV)混合框架，部署LLM作为审议外层循环，通过角色专业化LLM委员会协作验证异常，基于自然语言需求进行语义验证，并评估故障后响应。

Result: 在无人水下航行器时间序列模拟器上的实验表明，AIVV成功数字化了人工参与的V&V过程，克服了基于规则的故障分类限制，为时间序列数据领域的LLM监督提供了可扩展蓝图。

Conclusion: AIVV框架通过LLM委员会自动化验证与验证过程，解决了传统方法的可扩展性问题，为复杂控制系统的故障验证提供了可行的自动化解决方案。
...
TL;DR: 提出一个四部分框架用于电力公司资本规划，结合极端天气不确定性、电网数字孪生、蒙特卡洛模拟和多目标优化，发现简单的净现值排序法比复杂的基于模型的元启发式优化方法更有效
...
Conclusion: 在电力资本规划中，简单的净现值排序方法可能比复杂的基于模型的优化方法更有效，特别是在计算资源有限的情况下
...
<details>
  <summary>Details</summary>
Motivation: 2022年生效的新国家桥梁清单规范强调使用元素级状态条件进行基于风险的桥梁管理，这虽然提高了数据粒度，但由于状态空间从单一分类整数扩展到四维概率数组，给制定最优生命周期策略带来了挑战。

Method: 提出新的可解释强化学习方法，包含三个主要改进：(a) 使用可微软树模型作为行动者函数逼近器，(b) 训练过程中的温度退火过程，(c) 正则化与剪枝规则相结合以限制策略复杂度，最终生成确定性斜决策树形式的可解释生命周期策略。

Result: 该方法能够在监督学习和强化学习环境中产生可解释的生命周期策略，以合理节点数和深度的斜决策树形式呈现，使其可直接被人类理解和审计，并易于集成到现有桥梁管理系统中。

Conclusion: 提出的可解释强化学习方法能够解决元素级桥梁状态表示带来的挑战，生成既接近最优又易于理解和实施的生命周期策略，在钢梁桥生命周期优化问题中展示了其优势。
...
TL;DR: 提出基于知识图谱的神经符号架构，通过"计划-检索-生成"工作流程解决LLM在非物质文化遗产叙事生成中的事实准确性问题，将能力问题重新定义为运行时叙事计划。


<details>
  <summary>Details</summary>
Motivation: 非物质文化遗产保护面临集体记忆消退的挑战，现有LLM虽然能生成吸引人的叙事，但存在事实不准确或"幻觉"问题，在要求真实性的遗产应用中不可靠。

Method: 提出基于知识图谱的神经符号架构，采用"计划-检索-生成"透明工作流程，将能力问题重新定义为运行时可执行的叙事计划，桥接高级用户角色与原子知识检索。使用Live Aid KG多模态数据集，比较三种RAG策略：纯符号KG-RAG、文本增强Hybrid-RAG和结构感知Graph-RAG。

Result: 实验揭示了符号检索的事实精确性、混合方法的上下文丰富性和基于图遍历的叙事连贯性之间的量化权衡，为设计个性化和可控的叙事系统提供了可操作的见解。

Conclusion: 该架构通过知识图谱基础确保了证据封闭和完全可审计的生成，为非物质文化遗产保护提供了可靠且可控的叙事生成解决方案。
...
TL;DR: 论文研究了LLMs在教师教学评估等高风险决策中对虚假上下文信息的敏感性，提出了Debiasing-DPO方法显著减少偏见并提高预测准确性。


<details>
  <summary>Details</summary>
Motivation: LLMs越来越多地用于高风险决策，但其对虚假上下文信息的敏感性可能引入有害偏见，特别是在教师教学评估等影响职业发展的任务中。现有模型对教师经验、教育水平、人口统计身份等无关上下文信息表现出显著敏感性，而现有缓解方法效果有限。

Method: 提出Debiasing-DPO自监督训练方法：将仅基于查询生成的中立推理与基于查询加额外虚假上下文生成的偏见推理配对。结合监督微调以保持预测准确性。在Llama 3B/8B和Qwen 3B/7B Instruct模型上应用，评估了七类虚假上下文。

Result: 无关上下文信息可使模型预测在7分量表上偏移达1.48分，更大模型有时表现出更高敏感性。Debiasing-DPO平均减少偏见84%，提高预测准确性52%。标准提示和DPO缓解方法效果有限。

Conclusion: 对虚假上下文的鲁棒性不是模型规模扩展的自然副产品。Debiasing-DPO方法在提示式预测任务中能显著提高准确性和鲁棒性，为高风险决策中减少LLM偏见提供了有效解决方案。
...
TL;DR: 对音频-视觉大语言模型（AVLLMs）进行首次机制可解释性研究，发现虽然模型在中间层编码了丰富的音频语义，但当音频与视觉冲突时，这些能力在最终文本生成中未能充分体现，存在明显的视觉模态偏见。


<details>
  <summary>Details</summary>
Motivation: 音频-视觉大语言模型作为多模态感知的统一接口正在兴起，但对其工作机制缺乏深入理解。本研究旨在通过机制可解释性方法分析AVLLMs中音频和视觉特征如何在不同层中演化和融合，以产生最终文本输出。

Method: 采用机制可解释性研究方法，分析AVLLMs不同层中音频和视觉特征的演化与融合过程。通过探测分析检查潜在音频信息的存在情况，并追踪训练过程以理解模态偏见的来源。

Result: 研究发现：1）AVLLMs在中间层编码了丰富的音频语义；2）当音频与视觉冲突时，这些音频能力在最终文本生成中未能充分体现；3）有用的潜在音频信息存在，但更深层的融合层不成比例地偏向视觉表征，抑制音频线索；4）这种不平衡源于训练过程，AVLLMs的音频行为与其视觉-语言基础模型高度匹配，表明对音频监督的额外对齐有限。

Conclusion: 研究揭示了AVLLMs中存在根本性的模态偏见，为理解多模态大语言模型如何整合音频和视觉提供了新的机制性见解，指出了当前模型在跨模态整合方面的局限性。
...
TL;DR: AutoVerifier：基于LLM的智能框架，通过结构化知识图谱和六层验证流程，自动化验证复杂技术声明，无需领域专业知识


<details>
  <summary>Details</summary>
Motivation: 科学与技术情报分析需要验证快速增长文献中的复杂技术声明，现有方法无法弥合表面准确性与深层方法有效性之间的验证鸿沟

Method: 将技术声明分解为(主体、谓词、客体)三元组，构建知识图谱，通过六层渐进式验证流程：语料构建与摄取、实体与声明提取、文档内验证、跨源验证、外部信号佐证、最终假设矩阵生成

Result: 在量子计算争议声明案例中，无量子专业知识的分析师使用该框架自动识别目标论文中的过度声明和指标不一致性，追踪跨源矛盾，发现未披露的商业利益冲突，并生成最终评估
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将模式设计嵌入流水线代码或临时提取关系，导致模式与构建过程紧密耦合，难以用于下游本体级任务。需要一种从本体分析角度出发的模式设计方法

Method: 采用本体导向方法，核心是内在-关系路由机制，将每个属性分类为内在属性或关系属性，并路由到相应的模式模块。在2026年1月Wikidata数据集上实例化，包括规则清理阶段和迭代路由过程

Result: 从完整数据集中识别出3460万个实体的核心集，将属性分配到8个类别下的94个模块。模式达到93.3%的类别覆盖率和98.0%的模块分配率。导出包含3400万个节点、6120万条边、38种关系类型的属性图

Conclusion: 本体导向方法产生可移植、可重用的声明式模式，支持独立于构建流水线的下游应用，包括本体结构分析、基准标注审计、实体消歧、领域定制和LLM引导提取
...
<details>
  <summary>Details</summary>
Motivation: 当前AI系统在编程竞赛中仍落后于人类最佳选手，即使是谷歌的Gemini~3 Deep Think也仅获得第8名且未在实时竞赛条件下评估。编程竞赛是AI尚未完全超越人类的少数领域之一。

Method: 采用多智能体RL系统，协调多种智能体模块（假设提出、求解器、测试生成器、总结等），通过后训练和在线测试时RL联合改进。引入专门设计的Agentic GRPO算法，处理多阶段智能体展开中的延迟奖励和严重的离策略漂移问题。

Result: 在最近的三个Codeforces实时竞赛（第1087、1088、1089轮）中，GrandCode全部获得第一名，击败了所有人类参与者，包括传奇大师级选手。这是首个在实时编程竞赛中持续击败所有人类选手的AI系统。
...
TL;DR: DeltaLogic是一个基准测试转换协议，将自然语言推理示例转化为信念修正评估，揭示模型在证据微小变化下的信念修正能力与固定前提下的推理能力存在差异。


<details>
  <summary>Details</summary>
Motivation: 现有推理基准主要评估模型在固定前提集下得出正确答案的能力，但未能充分衡量在动态环境中至关重要的能力：在最小证据变化下的信念修正。需要评估模型是否能在局部证据编辑后进行有纪律的信念更新。

Method: 提出DeltaLogic基准转换协议，将自然语言推理示例转化为短修正片段。每个片段首先要求在前提P下得出初始结论，然后应用最小编辑δ(P)，最后询问先前结论应保持稳定还是需要修正。从FOLIO和ProofWriter实例化DeltaLogic，并使用受限标签评分评估小型因果语言模型。

Result: 在30个片段的Qwen评估子集上，更强的初始推理能力并不保证更强的修正行为：Qwen3-1.7B初始准确率0.667但修正准确率仅0.467，在黄金标签应改变的片段中惯性上升至0.600；Qwen3-0.6B几乎完全弃权；Qwen3-4B保持相同的惯性失败模式；Phi-4-mini-instruct表现更强但仍存在非平凡弃权和控制不稳定。

Conclusion: 固定前提下的逻辑能力并不意味着在局部证据编辑后能有纪律地进行信念修正。DeltaLogic针对一个独特且实际重要的推理能力，补充了现有的逻辑推理和信念更新基准。
...
TL;DR: 该论文提出了一种量化角色清晰度的方法来改善LLM驱动的多智能体系统中的角色一致性，通过角色分配矩阵和角色清晰度矩阵来量化智能体行为轨迹与角色描述的对齐程度，并作为正则化器进行轻量级微调。


<details>
  <summary>Details</summary>
Motivation: 在LLM驱动的多智能体系统中，角色规范不服从（即智能体未能遵守分配角色的职责和约束，可能表现得像另一个角色）是一个主要的失败模式。为了解决这个问题，需要一种量化方法来提高角色一致性。

Method: 1. 构建角色分配矩阵S(φ)=[s_{ij}(φ)]，其中s_{ij}(φ)表示第i个智能体行为轨迹与第j个智能体角色描述的语义相似度。2. 定义角色清晰度矩阵M(φ)=softmax(S(φ))-I，其中softmax(S(φ))是S(φ)的行向softmax，I是单位矩阵。3. 使用M(φ)的Frobenius范数量化智能体角色描述与行为轨迹的对齐程度。4. 将角色清晰度矩阵作为正则化器在轻量级微调中使用，以提高角色一致性。
...
Conclusion: 提出的量化角色清晰度方法能显著改善多智能体系统中的角色一致性，有效减少角色越界行为，提高任务成功率，为LLM驱动的多智能体系统提供了有效的角色一致性优化方案。
...
Method: 提出DuoChart双源数据管道（合成图表+真实图表）构建高质量训练数据；开发CharTool为MLLM配备外部工具（图像裁剪用于局部视觉感知，基于代码的计算用于数值推理）；通过智能体强化学习在DuoChart上训练工具集成推理

Result: 在六个图表基准测试中一致优于强基线模型；CharTool-7B在CharXiv(Reasoning)上比基础模型提升+8.0%，在ChartQAPro上提升+9.78%；与更大或专有模型相比具有竞争力；在域外视觉数学推理基准上表现出正泛化能力

Conclusion: DuoChart数据管道和CharTool工具增强框架有效解决了MLLM在图表推理中的关键挑战，通过结合高质量数据和工具集成推理显著提升了模型性能
...
TL;DR: EMS：一种高效的多智能体投票方法，通过可靠性感知的智能体调度和提前终止机制，在达到多数共识时停止推理，减少32%的智能体调用


<details>
  <summary>Details</summary>
Motivation: 传统多数投票方法需要所有智能体完成推理后才进行聚合，导致计算开销大，因为一旦达到多数共识，许多响应变得冗余。需要提高多智能体投票的效率。

Method: 提出EMS框架，包含三个核心组件：1) Agent Confidence Modeling (ACM)：基于历史性能和语义相似度估计智能体可靠性；2) Adaptive Incremental Voting (AIV)：基于可靠性顺序选择智能体并实现提前终止；3) Individual Confidence Updating (ICU)：动态更新每个参与智能体的可靠性。

Result: 在六个基准测试上的广泛评估表明，EMS持续减少了平均调用的智能体数量达32%，显著提高了推理效率。

Conclusion: EMS通过将多智能体投票重新表述为可靠性感知的智能体调度问题，实现了高效的多数投票，在达到多数共识时提前终止推理流程，显著减少了计算开销。
...
TL;DR: LLMs在Blocksworld规划任务中表现出接近理论最优解的性能，超越传统规划器，即使在去除语义线索的抽象图任务中也能保持高效推理。


<details>
  <summary>Details</summary>
Motivation: 研究前沿LLM在经典AI规划问题中的推理能力，特别是它们是否能进行最优规划而非仅依赖启发式策略。关注Blocksworld领域及其形式等价任务，以区分真实拓扑推理与语义先验的影响。

Method: 使用Blocksworld领域（带标签积木塔的移动任务）和形式等价的广义Path-Star（P*）图任务。系统操纵问题深度（塔高）、宽度（塔数）和组合性（目标积木数）。比较LLM与传统规划器（如LAMA）的性能。

Result: 增强推理的LLM在复杂多目标配置中显著优于传统满足型规划器。尽管经典搜索算法随搜索空间扩展而遇到瓶颈，但LLM能以接近完美的精度跟踪理论最优性极限，即使去除领域特定语义提示后仍能保持性能。

Conclusion: LLM在规划任务中表现出接近最优的性能，可能通过两种机制实现：1）通过推理令牌执行的算法模拟；2）几何记忆，使模型能将P*拓扑表示为可导航的全局几何结构，有效绕过指数级组合复杂性。
...
TL;DR: 论文发现大型推理模型存在"第一个即最优"现象，提出RED框架通过"精炼首个"和"丢弃后续"策略提升性能并减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理任务中表现出探索多个替代方案的人类模式，但研究发现替代方案不仅次优甚至有害，挑战了测试时间缩放定律，需要新的高效推理框架。

Method: 提出RED自引导高效推理框架：1) Refining First - 抑制首个解决方案中的错误森林增长；2) Discarding Subs - 通过双重一致性修剪后续错误森林。基于错误森林理论分析构建。

Result: 在5个基准测试和6个骨干模型上的实验表明，RED优于8个竞争基线，性能提升高达19.0%，同时减少37.7%~70.4%的token消耗。错误森林指标对比揭示了RED的有效性机制。

Conclusion: 论文揭示了大型推理模型中"第一个即最优"现象，提出了基于错误森林理论的RED框架，实现了性能提升和计算效率的平衡，为高效推理提供了新方向。
...
TL;DR: 提出InfoSeeker框架，通过分层架构（Host-Manager-Worker）解决现有智能体搜索系统在大规模信息合成中的上下文饱和、错误传播和高延迟问题，实现3-5倍加速和显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有智能体搜索系统过于强调深度多步推理，忽视了大规模信息合成的挑战，导致在数据密集型场景下面临上下文饱和、级联错误传播和高端到端延迟等严重限制。
...
Conclusion: InfoSeeker框架通过分层设计和并行处理有效解决了大规模信息合成中的关键挑战，在效率和效果上均显著优于现有系统。
...
TL;DR: Agentic-MME：首个面向多模态智能体能力的流程验证基准，包含418个现实世界任务，通过2000多个逐步检查点评估视觉扩展和知识扩展的协同能力，支持沙盒代码和API，实现真正的流程级验证。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM评估存在三大缺陷：缺乏灵活工具集成、单独测试视觉和搜索工具、仅评估最终答案。这导致无法验证工具是否实际调用、正确应用或高效使用，需要建立能够评估多模态智能体协同能力和流程验证的基准。

Method: 构建Agentic-MME基准，包含418个现实世界任务，覆盖6个领域和3个难度级别。每个任务配备统一评估框架，支持沙盒代码和API，并包含人工参考轨迹，标注了2000多个逐步检查点（S轴和V轴）。采用流程级验证，审计细粒度中间状态而非仅最终答案，通过"过度思考"指标量化效率。

Result: 最佳模型Gemini3-pro在整体任务上达到56.3%准确率，但在Level-3难度任务上显著下降至23.0%，表明现实世界多模态智能体问题解决仍具挑战性。基准验证了现有MLLM在工具调用、正确应用和高效使用方面的局限性。

Conclusion: Agentic-MME填补了多模态智能体能力评估的空白，通过流程级验证和逐步检查点机制，能够全面评估MLLM在视觉扩展和知识扩展方面的协同能力。结果表明当前模型在复杂现实任务中表现有限，为未来研究提供了重要基准和方向。
...
<details>
  <summary>Details</summary>
Motivation: 推动教科书形式化的规模和能力发展，从早期本科拓扑学结果和现有库内容重组，转向完整独立的研究生教材形式化

Method: 使用3万个Claude 4.5 Opus智能体通过版本控制并行协作，在共享代码库上工作，实现多智能体软件工程

Result: 成功将500多页教材形式化为13万行Lean代码和5900个声明，推理成本与人类专家团队薪资相当或更低，创造了教科书形式化规模和多智能体软件工程的记录

Conclusion: 该案例展示了AI系统在大规模教科书形式化方面的能力，成本效益显著，仍有进一步效率提升空间，相关代码和蓝图网站已开源
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在图表问答任务中存在数值提取不精确、隐含视觉关系理解困难、空间关系注意力机制不足等局限，需要增强推理能力以实现真正的智能

Method: 提出Chart-RL强化学习框架，整合策略优化技术和自适应奖励函数，通过反馈驱动优化视觉感知和逻辑推理；采用LoRA参数高效微调，仅需单GPU配置

Result: 在ChartQAPro数据集上评估，RL微调的Qwen3-VL-4B-Instruct模型达到0.634准确率，超越基础模型Qwen3-VL-8B-Instruct的0.580，参数减半且推理延迟从31秒降至9秒

Conclusion: Chart-RL框架有效解决了VLMs在图表理解中的关键挑战，通过强化学习优化实现了参数效率和推理速度的显著提升，同时保持性能完整性
...
TL;DR: 松鼠生态学为研究AI在部分可观测、延迟和策略性观察下的行动、记忆和验证能力提供了综合基准案例，提出分层部分可观测控制模型及相关假设。


<details>
  <summary>Details</summary>
Motivation: 当前AI研究将控制、记忆和验证分开研究，缺乏综合视角。松鼠生态学将这三者耦合在一个生物体中，为研究AI在复杂环境下的综合能力提供了理想的比较案例。

Method: 采用三层推理阶梯：经验观察、最小计算推理和AI设计猜想。引入分层部分可观测控制模型，包含潜在动态、结构化情景记忆、观察者信念状态、选项级动作和延迟验证信号。

Result: 提出三个核心假设：H1-快速局部反馈加预测补偿提高隐藏动态变化下的鲁棒性；H2-为未来控制组织的记忆提高延迟检索下的性能；H3-动作-记忆循环中的验证器和观察者模型减少静默故障和信息泄漏。

Conclusion: 松鼠生态学为AI控制、记忆和验证的耦合研究提供了比较视角和基准议程，提出角色分化的提议者/执行者/检查者/对抗者系统可能减少不对称信息和验证负担下的相关错误。
