...
TL;DR: XpertBench是一个用于评估大语言模型在真实专业领域能力的高保真基准，包含80个类别1346个任务，通过专家校准的ShotJudge评估范式发现当前顶级模型在专业任务上存在显著性能天花板（最高成功率仅66%）。


<details>
  <summary>Details</summary>
Motivation: 现有评估框架存在领域覆盖狭窄、依赖通用任务或自评估偏差等问题，无法有效评估LLMs在复杂开放式专业任务上的真实能力，需要建立能反映真实专家级认知的评估基准。

Method: 构建了包含1346个任务的XpertBench基准，涵盖金融、医疗、法律、教育及STEM与人文双轨研究等80个专业类别，任务来自1000多名领域专家提交。引入ShotJudge评估范式，使用经过专家few-shot示例校准的LLM评估器来缓解自奖励偏差。

Result: 对最先进LLMs的评估显示显著性能天花板：即使领先模型最高成功率仅约66%，平均得分约55%。模型还表现出领域特异性差异，在定量推理与语言合成方面存在非重叠优势。

Conclusion: 当前AI系统存在显著的"专家差距"，XpertBench为从通用助手向专业协作伙伴转型提供了关键评估工具，揭示了模型在真实专业任务上的局限性。
...
TL;DR: 提出一种神经符号架构，通过提取网格中的对象级结构、使用神经先验从特定领域语言中提出候选变换，并利用跨示例一致性过滤假设，来改进抽象推理任务的表现。
...
Method: 提出神经符号架构：1) 从网格中提取对象级结构；2) 使用神经先验从固定领域特定语言(DSL)中提出候选变换；3) 利用跨示例一致性过滤假设。基于人类视觉抽象启发的单元模式构建组合推理框架，增强大语言模型的对象表示和变换提议能力。

Result: 在ARC-AGI-2上，将基础LLM性能从16%提升到24.4%（公开评估集），与ARC Lang Solver结合后达到30.8%。无需任务特定微调或强化学习即可改善泛化，减少对暴力搜索和基于采样的测试时扩展的依赖。

Conclusion: 分离感知、神经引导的变换提议和符号一致性过滤可改善泛化能力，同时减少对暴力搜索和采样扩展的依赖。该方法为神经符号推理提供了有效框架，代码已开源。
...
TL;DR: 电网投资组合优化框架对比：考虑极端天气不确定性的数字孪生+蒙特卡洛模拟+多目标优化方法，发现简单的净现值排序法比复杂的基于模型的元启发式优化方法更优
...
Result: 研究发现，考虑到基于模型的元启发式优化方法的计算复杂性，仅需有限电网知识的简单净现值排序法能够找到更优的投资组合

Conclusion: 在电网投资组合优化中，简单实用的净现值排序方法优于复杂的模型驱动优化方法，这对电力公司资本规划具有重要实践意义
...
TL;DR: 提出一种基于可解释强化学习的新方法，用于桥梁全寿命周期优化决策，将复杂的四维概率数组状态空间转化为可理解的斜决策树形式


<details>
  <summary>Details</summary>
Motivation: 2022年实施的新国家桥梁清单规范强调使用构件级状态进行基于风险的桥梁管理，虽然提高了数据粒度，但将状态空间从单一分类整数扩展到四维概率数组，给制定最优全寿命周期策略带来了挑战

Method: 提出可解释强化学习方法，采用三个主要改进：(a)使用可微软树模型作为演员函数逼近器，(b)训练过程中的温度退火过程，(c)正则化配合剪枝规则限制策略复杂度，最终生成确定性斜决策树形式的策略

Result: 该方法能够生成节点数量和深度合理的可解释全寿命周期策略，使策略可直接被人理解、审计，并易于集成到现有桥梁管理系统中，在监督学习和强化学习环境中都展示了其优势和权衡

Conclusion: 提出的可解释强化学习方法能够有效处理构件级状态表示带来的复杂状态空间问题，为桥梁全寿命周期优化提供可直接理解和实施的决策策略，在钢梁桥全寿命周期优化问题中得到了验证
...
TL;DR: 提出基于知识图谱的神经符号架构，通过"计划-检索-生成"工作流解决大语言模型在非物质文化遗产保护中的事实准确性问题，将能力问题转化为可执行的叙事计划。


<details>
  <summary>Details</summary>
Motivation: 非物质文化遗产保护面临集体记忆消退的挑战，大语言模型虽然能生成吸引人的叙事，但其"幻觉"问题导致事实不准确，无法满足文化遗产应用中对真实性的核心要求。

Method: 提出基于知识图谱的神经符号架构，采用"计划-检索-生成"透明工作流，将传统设计时验证工具的能力问题重新定义为运行时可执行的叙事计划，弥合高层用户角色与原子知识检索之间的差距，确保生成过程证据封闭且完全可审计。

Result: 使用Live Aid知识图谱（多模态数据集）验证架构，系统比较三种检索增强生成策略：纯符号KG-RAG、文本增强Hybrid-RAG和结构感知Graph-RAG。实验揭示了符号检索的事实精度、混合方法的上下文丰富性和基于图遍历的叙事连贯性之间的量化权衡。

Conclusion: 该研究为设计个性化和可控的叙事系统提供了可操作的见解，通过知识图谱基础架构确保非物质文化遗产叙事的事实准确性，同时保持叙事吸引力。
...
TL;DR: 本文研究了LLMs在教师教学质量评估等高风险决策任务中对虚假上下文信息的敏感性，提出了Debiasing-DPO方法，显著降低了84%的偏见并提高了52%的预测准确性。
...
Method: 使用美国课堂转录本数据集(NCTE)评估7个前沿和开源模型对7类虚假上下文的敏感性。提出了Debiasing-DPO自监督训练方法：将仅基于查询生成的中立推理与基于查询加虚假上下文生成的偏见推理配对，并与监督微调结合以防止预测准确性损失。

Result: 虚假上下文信息可使模型预测在7分量表上偏移高达1.48分，较大模型有时表现出更高敏感性。提示缓解和标准DPO效果有限。Debiasing-DPO在Llama和Qwen模型上平均减少84%偏见并提高52%预测准确性。

Conclusion: 对虚假上下文的鲁棒性不是模型规模扩展的自然副产品。Debiasing-DPO方法在基于提示的预测任务中能显著提高准确性和鲁棒性，为高风险决策应用中的LLM偏见缓解提供了有效解决方案。
...
TL;DR: AVLLMs存在模态偏见：音频语义在中间层编码丰富，但在与视觉冲突时无法体现在最终文本输出中，深层融合层过度偏向视觉表征，训练对齐不足导致音频能力受限。


<details>
  <summary>Details</summary>
Motivation: 对音频-视觉大语言模型进行首次机制可解释性研究，分析音频和视觉特征在不同层中的演化与融合过程，探究多模态LLM如何整合音频和视觉信息。

Method: 采用机制可解释性分析方法，追踪AVLLM各层中音频和视觉特征的演化与融合；通过探测分析验证潜在音频信息的存在；追溯训练过程，比较AVLLM与其视觉-语言基础模型的行为匹配度。

Result: 发现AVLLMs在中间层编码了丰富的音频语义，但当音频与视觉冲突时，这些能力无法体现在最终文本生成中；探测分析显示有用的潜在音频信息存在，但深层融合层不成比例地偏向视觉表征，抑制音频线索；训练分析表明AVLLM的音频行为与其视觉-语言基础模型高度匹配，显示对音频监督的额外对齐有限。

Conclusion: 揭示了AVLLMs中存在根本性的模态偏见，为理解多模态LLM如何整合音频和视觉提供了新的机制性见解，表明当前AVLLMs在音频-视觉融合方面存在不平衡，需要改进训练对齐策略。
...
Method: AutoVerifier将技术声明分解为(主体、谓词、客体)三元组，构建知识图谱，通过六层渐进式丰富层进行结构化推理：语料库构建与摄取、实体与声明提取、文档内验证、跨源验证、外部信号佐证和最终假设矩阵生成。
...
TL;DR: 提出一种本体导向的知识图谱构建方法，通过内在-关系路由机制将属性分类并分配到模块化模式中，实现可移植、可重用的图谱模式设计。


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱构建方法将结构决策嵌入流水线代码或临时提取关系，导致模式与构建过程紧密耦合，难以用于下游本体级任务。需要一种从设计之初就面向本体分析、实体消歧、领域定制和LLM引导提取的模式设计方法。

Method: 采用本体导向方法，核心是内在-关系路由机制：将每个属性分类为内在属性或关系属性，并路由到相应的模式模块。该方法包括基于规则的清洗阶段和迭代路由过程，结合工具增强的LLM支持和人工审查。

Result: 在2026年1月Wikidata数据集上实例化：从完整数据集中识别出3460万个实体的核心集，将属性分配到94个模块（组织为8个类别）。模式达到93.3%的类别覆盖率和98.0%的模块分配率。导出包含3400万个节点、6120万条边、38种关系类型的属性图。

Conclusion: 本体导向方法通过内在-关系路由产生可移植的声明性模式，独立于构建流水线，成功应用于本体结构分析、基准标注审计、实体消歧、领域定制和LLM引导提取等五个下游任务，验证了其可重用性。
...
TL;DR: GrandCode是一个多智能体强化学习系统，首次在Codeforces现场编程竞赛中击败所有人类选手，包括传奇大师，标志着AI在最具竞争力的编程任务上超越最强人类程序员。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统在竞争性编程中仍落后于人类顶尖选手，最好的AI系统（如Google的Gemini~3 Deep Think）即使在非现场竞赛条件下也仅能达到第8名。竞争性编程是AI尚未超越人类的少数领域之一。

Method: 1. 构建多智能体系统，协调多种智能体模块（假设提出、求解器、测试生成器、总结等），通过后训练和在线测试时强化学习联合改进；2. 引入专门设计的Agentic GRPO算法，处理多阶段智能体展开中的延迟奖励和严重的离策略漂移问题。

Result: 在最近三场Codeforces现场竞赛（第1087、1088、1089轮）中，GrandCode均获得第一名，击败所有人类参与者，包括传奇大师。这是首个在竞争性编程现场竞赛中持续击败所有人类选手的AI系统。

Conclusion: GrandCode证明AI系统已经达到在最具竞争力的编程任务上超越最强人类程序员的水平，标志着竞争性编程领域AI能力的重要突破。
...
TL;DR: DeltaLogic是一个基准转换协议，将自然语言推理示例转换为短修订片段，评估模型在最小证据变化下的信念修订能力，发现固定前提下的逻辑能力并不保证有效的信念修订。


<details>
  <summary>Details</summary>
Motivation: 现有推理基准主要评估模型从固定前提集推导正确答案的能力，但低估了动态环境中至关重要的能力：在最小证据变化下的信念修订。需要衡量模型在前提发生微小变化时能否正确调整结论。
...
Result: 在30个片段的Qwen评估子集上，更强的初始推理能力并不保证更强的修订行为：Qwen3-1.7B初始准确率0.667但修订准确率仅0.467，在应改变标签的片段中惯性达0.600；Qwen3-0.6B几乎完全弃权；Qwen3-4B保持相同惯性失败模式；Phi-4-mini-instruct表现显著更强但仍有非平凡弃权和控制不稳定。

Conclusion: 固定前提下的逻辑能力并不意味着在局部证据编辑后能有纪律的信念修订。DeltaLogic针对一个独特且实际重要的推理能力，补充了现有的逻辑推理和信念更新基准。
...
TL;DR: 提出一种量化角色清晰度的方法，通过构建角色分配矩阵和角色清晰度矩阵来评估多智能体系统中角色一致性，并将其作为正则化器进行轻量级微调以提高任务性能。


<details>
  <summary>Details</summary>
Motivation: 在大语言模型驱动的多智能体系统中，角色规范不服从（智能体未能遵守分配角色的职责和约束，可能表现得像另一个角色）是一个主要的失败模式，需要解决这一问题以提高系统可靠性。

Method: 1. 构建角色分配矩阵S(φ)，其中s_ij(φ)表示第i个智能体行为轨迹与第j个智能体角色描述的语义相似度；2. 定义角色清晰度矩阵M(φ)=softmax(S(φ))-I，其中I是单位矩阵；3. 使用M(φ)的Frobenius范数量化角色描述与行为轨迹的对齐程度；4. 将角色清晰度矩阵作为正则化器在轻量级微调中使用，以提高角色一致性。
...
Conclusion: 提出的量化角色清晰度方法有效解决了多智能体系统中的角色规范不服从问题，通过将角色清晰度矩阵作为正则化器进行微调，显著提高了角色一致性和端到端任务性能，为多智能体系统的可靠性提供了重要改进。
...
TL;DR: EMS：一种高效的多智能体投票框架，通过可靠性感知的智能体调度和提前终止机制，在达成多数共识时停止推理，减少32%的智能体调用


<details>
  <summary>Details</summary>
Motivation: 传统多数投票方法需要所有智能体完成推理才开始聚合，导致大量计算开销，因为一旦达成多数共识，许多响应就变得冗余。需要提高多智能体投票的推理效率。
...
Result: 在六个基准测试上的广泛评估表明，EMS持续减少了平均32%的智能体调用数量，显著提高了推理效率。

Conclusion: EMS通过将多智能体投票重新定义为可靠性感知的智能体调度问题，并引入提前终止机制，有效解决了传统多数投票方法的计算冗余问题，显著提升了多智能体系统的推理效率。
...
TL;DR: LLMs在规划任务中表现出接近理论最优的性能，超越传统规划器，特别是在复杂多目标配置中，即使剥离语义线索也能保持性能。


<details>
  <summary>Details</summary>
Motivation: 研究前沿LLM在规划问题中的推理能力，检验其是进行最优推理还是依赖简单启发式策略，特别是在语义线索被剥离的情况下。

Method: 使用Blocksworld领域和形式等价的广义Path-Star(P*)图任务，系统操纵问题深度（塔高）、宽度（塔数）和组合性（目标块数），比较LLM与传统规划器（如LAMA）的性能。

Result: 推理增强的LLM在复杂多目标配置中显著优于传统满足型规划器；经典搜索算法随搜索空间扩展而性能下降，而LLM能跟踪理论最优性限制，即使剥离领域特定语义线索也能保持接近完美的精度。

Conclusion: LLM通过算法模拟和几何记忆两种机制实现高效规划：算法模拟通过推理标记执行，几何记忆允许模型将P*拓扑表示为可导航的全局几何结构，有效绕过指数组合复杂性。
...
TL;DR: 研究发现大型推理模型存在"第一个答案即最佳"现象，后续推理路径不仅次优反而有害，提出了基于错误森林理论的RED框架来提升推理效率。


<details>
  <summary>Details</summary>
Motivation: 尽管大型推理模型在复杂任务中表现出色，但研究发现其多路径推理存在"第一个答案即最佳"的意外现象，后续推理路径不仅次优反而可能有害，这挑战了现有的测试时扩展定律。

Method: 提出RED自引导高效推理框架：1) 精炼首个答案以抑制错误森林增长；2) 通过双重一致性剪枝后续推理路径。基于错误森林理论分析，将推理错误建模为森林结构。

Result: 在5个基准测试和6个骨干模型上，RED优于8个竞争基线，性能提升最高达19.0%，同时减少37.7%~70.4%的token消耗。错误森林指标实验揭示了RED的有效性机制。

Conclusion: 研究揭示了大型推理模型中"第一个答案即最佳"现象，提出了基于错误森林理论的RED框架，显著提升了推理效率和性能，为高效推理系统设计提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有智能体搜索系统过于强调深度多步推理，忽视了大规模信息合成的挑战，导致在数据密集型场景中出现上下文饱和、级联错误传播和高延迟等问题

Method: 基于近可分解性原则的分层框架，包含战略Host、多个Manager和并行Worker三层结构，通过Manager层的聚合和反思机制实现严格上下文隔离，Worker层并行加速任务执行

Result: 在两个互补基准测试中表现出高效性和有效性：实现3-5倍加速，在WideSearch-en上达到8.4%成功率，在BrowseComp-zh上达到52.9%准确率

Conclusion: InfoSeeker框架通过分层架构有效解决了大规模信息合成中的关键挑战，在效率和效果上均优于现有方法，为数据密集型智能体搜索提供了可行解决方案
...
TL;DR: Agentic-MME：首个面向多模态智能体能力的流程验证基准，包含418个现实世界任务，通过2000+逐步检查点评估视觉扩展和知识扩展的协同能力，支持沙箱代码和API，实现真正的流程级验证。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM评估存在三大缺陷：缺乏灵活工具集成、单独测试视觉和搜索工具、仅评估最终答案。这导致无法验证工具是否被实际调用、正确应用或高效使用，需要建立能评估多模态智能体协同能力的流程验证基准。

Method: 构建包含418个现实世界任务的Agentic-MME基准，涵盖6个领域和3个难度级别。每个任务包含统一评估框架（支持沙箱代码和API）和人工参考轨迹，标注了2000+个逐步检查点（S轴和V轴双轴评估）。采用流程级验证方法，审核细粒度中间状态而非仅最终答案，并通过"过度思考"指标量化效率。

Result: 最佳模型Gemini3-pro在整体准确率上达到56.3%，但在Level-3任务上显著下降至23.0%，表明现实世界多模态智能体问题解决具有挑战性。基准成功揭示了现有模型在复杂任务中的能力局限。

Conclusion: Agentic-MME填补了多模态智能体评估的空白，通过流程验证和效率量化提供了更全面的能力评估。结果表明当前MLLM在复杂现实世界任务中仍有很大提升空间，为未来研究提供了重要基准。
...
Method: 使用3万个Claude 4.5 Opus智能体通过版本控制并行协作，在共享代码库上自动形式化教材内容

Result: 成功形式化了500多页研究生级代数组合学教材，包含13万行Lean代码和5900个声明，推理成本与人类专家团队薪资相当或更低

Conclusion: 这是教科书形式化规模和能力的新里程碑，展示了多智能体系统在复杂软件工程任务中的可行性，且仍有大幅提升效率的潜力
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在图表问答任务中存在数值提取不精确、隐含视觉关系理解困难、空间关系注意力机制不足等问题，需要提升模型的推理能力和视觉理解整合能力

Method: 提出Chart-RL强化学习框架，结合策略优化技术和自适应奖励函数，通过反馈驱动的视觉感知和逻辑推理策略优化，并集成LoRA参数高效微调技术

Result: 在ChartQAPro数据集上，RL微调的Qwen3-VL-4B-Instruct模型达到0.634准确率，超越8B参数基础模型的0.580，同时推理延迟从31秒降至9秒

Conclusion: Chart-RL框架有效解决了VLM在图表理解中的关键限制，通过强化学习优化实现了性能提升和效率改进，为复杂数据可视化理解提供了新方法
...
TL;DR: 该论文提出松鼠生态学作为研究智能体AI在部分可观测性、延迟和策略性观察下行动、记忆和验证能力的比较案例，建立了包含控制、记忆和验证的耦合框架。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常将控制、记忆和验证分开研究，缺乏一个整合框架来研究智能体AI在复杂环境中的综合能力。松鼠生态学提供了一个理想的比较案例，因为其树栖运动、分散贮藏和观众敏感缓存行为将这三个需求耦合在一个生物体中。

Method: 采用三层推理阶梯：经验观察、最小计算推理和AI设计猜想。引入了一个最小层次部分可观测控制模型，包含潜在动态、结构化情景记忆、观察者信念状态、选项级动作和延迟验证信号。基于此提出了三个可证伪假设。

Result: 提出了三个核心假设：(H1)快速局部反馈加预测补偿在隐藏动态变化下提高鲁棒性；(H2)为未来控制组织的记忆在提示冲突和负载下改善延迟检索；(H3)动作-记忆循环内的验证器和观察者模型减少静默失败和信息泄漏，但仍易受错误规范影响。提出了角色分化的提议者/执行者/检查者/对抗者系统概念。

Conclusion: 该研究提供了一个比较视角和基准议程：一个关于控制、记忆和可验证行动耦合的可证伪主张的严谨研究计划，为智能体AI设计提供了新的理论框架和测试基准。
