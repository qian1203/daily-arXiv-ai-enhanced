...
TL;DR: 该论文提出了一个用于评估语言模型智能体探索与利用能力的可控环境框架，通过部分可观测的2D网格地图和未知任务DAG设计实验，开发了无需访问内部策略即可量化探索与利用错误的度量标准，发现当前最先进模型在该任务上仍存在困难，但推理模型表现更好且可通过简单工程改进。


<details>
  <summary>Details</summary>
Motivation: 语言模型智能体在复杂开放决策任务中应用日益广泛，需要有效平衡探索与利用。然而，在无法访问智能体内部策略的情况下，系统地区分和量化观察到的行动中的探索与利用行为仍然具有挑战性。

Method: 设计了受实际具身AI场景启发的可控环境：每个环境包含部分可观测的2D网格地图和未知任务有向无环图（DAG）。地图生成可通过编程方式调整以强调探索或利用难度。设计了无需访问策略即可从智能体行动中量化探索与利用错误的度量标准。

Result: 评估了多种前沿语言模型智能体，发现即使最先进的模型在该任务上也表现困难，不同模型展现出不同的失败模式。推理模型能更有效地解决任务，且通过最小化工程改进可以显著提升探索与利用能力。

Conclusion: 提出了一个用于评估语言模型智能体探索与利用能力的框架，揭示了当前模型的局限性，展示了推理模型的优势，并提供了通过工程改进的潜力。代码已开源供进一步研究使用。
...
TL;DR: 提出一个安全、轻量、用户友好的智能体框架，用于自主执行定义明确的科学任务，结合隔离执行环境、三层智能体循环和自评估do-until机制，确保安全可靠运行。


<details>
  <summary>Details</summary>
Motivation: 现有智能体AI系统在现实世界科学研究中实现可靠部署仍面临重大挑战，需要开发能够安全、可靠执行科学任务的自主框架，让研究人员能够卸载常规工作负载，专注于创造性活动。

Method: 框架包含三个核心组件：1) 隔离执行环境确保安全；2) 三层智能体循环（规划-执行-评估）；3) 自评估do-until机制，利用不同能力水平的大型语言模型，专注于具有明确定义上下文和停止条件的结构化任务。

Result: 该框架支持端到端自动化，最小化人工干预，使研究人员能够将常规工作负载委托给智能体，从而有更多精力投入创造性活动和开放式科学探索。

Conclusion: 提出的框架为科学研究中的自主任务执行提供了一种安全、可靠且实用的解决方案，通过结构化方法和安全机制平衡了自主性与可靠性，促进了智能体AI在科学工作流程中的实际应用。
...
TL;DR: 论文分析了LLM中不可预测性的根源，发现浮点数有限精度导致的舍入误差在Transformer计算层中的传播、放大或消散，识别出早期层的"雪崩效应"，并揭示了LLM具有三种普遍、尺度依赖的混沌行为机制。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地集成到智能体工作流中，其由数值不稳定性导致的不可预测性已成为关键的可靠性问题。虽然近期研究已证明这些不稳定性对下游任务有显著影响，但其根本原因和内在机制仍不清楚。

Method: 通过严格分析浮点数有限精度表示如何导致不可预测性，追踪舍入误差在Transformer计算层中的传播、放大或消散过程。特别识别了早期层的混沌"雪崩效应"，并系统研究了LLM中普遍存在的尺度依赖混沌行为。

Result: 发现LLM存在三种不同的混沌行为机制：1)稳定机制——扰动低于输入相关阈值并消失，输出恒定；2)混沌机制——舍入误差主导并驱动输出发散；3)信号主导机制——真实输入变化压倒数值噪声。这些发现在多个数据集和模型架构中得到广泛验证。

Conclusion: LLM的不可预测性根源于浮点数有限精度，通过Transformer层的误差传播形成了复杂的混沌动力学。识别出的三种行为机制为理解和缓解LLM在关键应用中的可靠性问题提供了理论基础。
...
TL;DR: ReSS框架通过结合符号决策树和LLM，生成高质量、忠实于决策逻辑的自然语言推理，用于训练专门的表格推理模型，在医疗和金融基准上提升性能达10%


<details>
  <summary>Details</summary>
Motivation: 在医疗和金融等高风险领域，表格数据的预测模型需要同时提供高准确性和可解释的推理。符号模型虽然可验证但表达能力有限，而通用LLM需要专门微调才能掌握领域特定的表格推理。需要解决数据规模化构建和推理一致性的双重挑战。

Method: 提出ReSS框架，使用决策树模型提取实例级决策路径作为符号支架，这些支架与输入特征和标签一起指导LLM生成严格遵循底层决策逻辑的接地自然语言推理。生成的高质量数据集用于微调预训练LLM成为专门的表格推理模型，并通过支架不变的数据增强策略提升泛化能力和可解释性。

Result: 在医疗和金融基准测试中，ReSS训练的模型相比传统决策树和标准微调方法提升达10%，同时产生忠实且一致的推理。引入了幻觉率、解释必要性和解释充分性等定量指标来严格评估忠实性。

Conclusion: ReSS框架成功桥接了符号和神经推理模型，通过系统化方法生成了高质量、忠实于决策逻辑的自然语言推理，显著提升了表格推理模型的性能和可解释性，为高风险领域的决策支持提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统不确定性量化方法缺乏对推理-答案生成的有样本保证，现有方法忽略了推理轨迹与最终答案的逻辑联系，且难以解释不确定性覆盖的来源

Method: 1) 提出新颖方法量化推理-答案结构的不确定性并提供统计保证；2) 开发基于Shapley值的统一示例到步骤解释框架，识别保持保证所需的训练示例子集和关键推理步骤

Result: 在具有挑战性的推理数据集上进行广泛实验，验证了所提方法的有效性

Conclusion: 该方法为大型推理模型提供了具有统计保证的不确定性量化，同时通过可解释框架揭示了不确定性覆盖的来源，解决了推理质量与答案正确性分离的挑战
...
TL;DR: 提出Weight Patching方法，通过参数空间干预在相同架构的配对模型中进行源导向分析，揭示模型能力层次结构


<details>
  <summary>Details</summary>
Motivation: 现有机制可解释性方法主要关注激活空间定位和因果追踪，但激活空间中重要的模块可能只是聚合或放大上游信号，而非在自身参数中编码目标能力。需要参数空间干预方法来填补这一空白。

Method: 提出Weight Patching方法：在具有相同架构但目标能力表达强度不同的配对模型（基础模型和行为专业化模型）中，在固定输入下将专业化模型的选定模块权重替换到基础模型中。在指令跟随任务上实例化该方法，并引入基于向量锚点的行为接口框架。

Result: 分析揭示了从浅层候选源侧载体到聚合与路由模块，再到下游执行电路的层次结构。恢复的组件分数可以指导机制感知的模型合并，在评估的专家组合中改善选择性融合。

Conclusion: Weight Patching填补了参数空间干预方法的空白，提供了源导向分析框架，揭示了模型能力的层次结构，并为机制感知的模型合并提供了指导。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI系统（云中心AI、设备端推理、边缘云管道）将规划、推理和执行视为单一过程，导致不必要的延迟、能耗和行为连续性碎片化。下一代自主AI系统不仅受模型能力限制，还受跨异构硬件的智能结构约束。

Method: 提出Tri-Spirit架构：三层认知框架（Super Layer规划层、Agent Layer推理层、Reflex Layer执行层），每层映射到不同计算基板，通过异步消息总线协调。包含参数化路由策略、习惯编译机制（将重复推理路径提升为零推理执行策略）、收敛内存模型和显式安全约束。

Result: 在2000个合成任务的可重复模拟中，与云中心和仅边缘基线相比：平均任务延迟降低75.6%，能耗降低71.1%，LLM调用减少30%，77.6%的任务可离线完成。

Conclusion: 认知分解（而非仅模型扩展）是AI硬件系统级效率的主要驱动力。Tri-Spirit架构通过将智能分解为规划、推理和执行层并映射到不同计算基板，显著提升系统效率。
...
TL;DR: AlphaCNOT：基于蒙特卡洛树搜索的强化学习框架，用于CNOT门最小化，在线性可逆合成中相比PMH基准减少高达32%的CNOT门数量


<details>
  <summary>Details</summary>
Motivation: 量子电路优化是量子计算的核心任务，因为当前NISQ设备存在误差传播问题，且CNOT门作为Clifford+T通用集中的唯一双量子比特门，其最小化对减少误差至关重要。现有启发式算法（如PMH）和基于强化学习的方法在拓扑约束合成中仍有改进空间。

Method: 提出AlphaCNOT，一个基于蒙特卡洛树搜索的强化学习框架，将CNOT最小化问题建模为规划问题。该方法采用基于模型的策略，能够利用前瞻搜索评估未来轨迹，从而找到更高效的CNOT序列。

Result: 在线性可逆合成中，相比PMH基准实现了高达32%的CNOT门数量减少；在约束版本中，在多种最多8量子比特的拓扑结构上，相比最先进的基于强化学习的解决方案报告了一致的门数量减少。
...
<details>
  <summary>Details</summary>
Motivation: 科学同行评审面临投稿量激增的压力，难以维持评审质量、一致性和及时性。虽然AI在同行评审中的应用备受关注，但关键问题在于AI能否在真实会议规模下生成技术可靠的评审意见。

Method: 采用前沿模型、工具使用和安全保障的多阶段流程，在不到一天内为AAAI-26所有主轨道的22,977篇完整评审论文生成AI评审。系统明确标识AI评审，并进行大规模作者和程序委员会成员调查。

Result: 参与者不仅认为AI评审有用，而且在技术准确性和研究建议等关键维度上更偏好AI评审而非人类评审。系统在检测科学缺陷方面显著优于简单的LLM生成评审基线。
...
Method: 提出人工三方智能(ATI)架构，采用生物启发的分层设计：脑干(L1)提供反射性安全和信号完整性控制；小脑(L2)执行连续传感器校准；大脑推理子系统(L3/L4)支持常规技能选择与执行、协调和深度推理。这种模块化组织允许传感器控制、自适应感知、边缘-云端执行和基础模型推理在单一闭环架构中协同进化。

Result: 在移动相机原型上的评估显示，相比默认自动曝光设置，ATI(L1/L2自适应感知)将端到端准确率从53.8%提升至88%，同时将远程L4调用减少43.3%。

Conclusion: ATI展示了为具身AI共同设计感知和推理的价值，通过分层架构将时间关键的感知和控制保留在设备端，仅在需要时调用高级推理，从而满足物理AI的实时性、能耗和隐私约束。
...
TL;DR: 本文系统研究了GRPO训练中不同奖励信号对VLM物理推理行为的影响，发现奖励设计会诱导领域特定的推理行为而非统一提升性能。


<details>
  <summary>Details</summary>
Motivation: 尽管SFT和GRPO等后训练算法在语言模型中显示出强大的推理增益，但奖励设计如何影响VLM的物理推理行为仍不清楚。当前最先进的VLMs在物理基准测试中仍远低于人类表现，需要深入理解奖励信号对物理推理的影响。

Method: 对基于GRPO的VLM训练进行系统性奖励消融研究，比较四种语义丰富度递增的奖励信号：格式合规性、答案准确性、复合评分奖励（答案正确性、物理原理识别和单位一致性）、以及基于模型注意力权重的内部奖励。在PhyX基准（3000个问题，涵盖6个物理领域和6种推理类型）上使用IBM Granite Vision 3.3 (2B)进行评估。

Result: 准确性奖励在大多数领域上优于SFT，但增益因奖励类型和领域而异。奖励设计不会统一提升性能，而是诱导领域特定的推理行为：准确性奖励提供最强的整体增益；评分奖励提高结构化推理质量但准确性提升不一致；注意力奖励增强空间推理但在符号领域性能下降。内部注意力权重奖励无需空间标注，将空间关系准确性从0.27提升至0.50。

Conclusion: 奖励设计对VLM物理推理行为具有领域特异性影响，监督模型在生成过程中的注意力分布是视觉基础物理推理的有前景方向，无需额外空间标注即可显著提升空间推理能力。
