...
Method: 采用隔离执行环境确保安全，三层智能体循环结构，以及自评估的do-until机制，有效利用不同能力水平的大型语言模型

Result: 框架支持端到端自动化，最小化人工干预，使研究人员能够卸载常规工作负载，专注于创造性活动和开放式科学探究

Conclusion: 该框架为定义明确的科学任务提供了安全可靠的自主执行解决方案，平衡了自动化与安全性，促进了科学研究效率的提升
...
TL;DR: ReSS框架通过结合符号决策树和神经语言模型，生成忠实于决策逻辑的自然语言推理，用于训练专门的表格推理模型，在医疗和金融基准上提升性能达10%


<details>
  <summary>Details</summary>
Motivation: 解决表格数据预测中需要高准确性和可解释性的双重挑战。符号模型缺乏语义表达能力，而通用LLM需要专门微调才能掌握领域特定的表格推理。需要同时解决可扩展数据整理和推理一致性的问题。

Method: 提出ReSS框架：1) 使用决策树模型提取实例级决策路径作为符号支架；2) 用这些支架、输入特征和标签指导LLM生成严格遵循底层决策逻辑的自然语言推理；3) 用生成的高质量数据集微调预训练LLM为专门的表格推理模型；4) 采用支架不变的数据增强策略提高泛化能力和可解释性。

Result: 在医疗和金融基准测试中，ReSS训练模型相比传统决策树和标准微调方法提升达10%。引入的忠实性量化指标（幻觉率、解释必要性、解释充分性）显示模型能产生忠实且一致的推理。

Conclusion: ReSS框架成功桥接了符号和神经推理模型，通过符号支架引导生成忠实推理，训练出的专门表格推理模型在保持高准确性的同时提供可验证、可理解的自然语言解释。
...
TL;DR: 提出了一种具有统计保证的推理-答案不确定性量化方法，并开发了统一的示例到步骤解释框架，用于识别训练示例和关键推理步骤以保持保证。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理方面取得显著进展，但传统不确定性量化方法缺乏有限样本保证，且现有方法忽略推理轨迹与最终答案的逻辑联系，无法解释不确定性覆盖的来源。

Method: 首先提出具有统计保证的推理-答案结构不确定性量化方法；然后开发基于Shapley值的统一示例到步骤解释框架，识别保持保证所需的训练示例子集及其关键推理步骤。

Result: 在具有挑战性的推理数据集上进行广泛实验，验证了所提方法的有效性。

Conclusion: 该方法能够量化推理-答案结构的不确定性并提供统计保证，同时通过解释框架识别关键训练因素，解决了传统方法在推理模型不确定性量化中的局限性。
...
TL;DR: RiskWebWorld：首个面向电商风险管理的GUI智能体交互式基准测试，包含1513个真实任务，揭示了大模型规模在专业任务中比零样本界面理解更重要


<details>
  <summary>Details</summary>
Motivation: 现有GUI智能体基准主要针对良性、可预测的消费环境，而在高风险调查性领域（如电商风险管理）的有效性尚未充分探索，需要更真实的评估环境

Method: 构建RiskWebWorld基准：包含1513个来自生产风险控制管道的任务，覆盖8个核心领域；建立Gymnasium兼容的基础设施，将策略规划与环境机制解耦；评估多种模型并实施智能体强化学习

Result: 顶级通用模型成功率49.1%，而专业开源GUI模型接近完全失败；通过智能体强化学习可将开源模型性能提升16.2%；大模型规模在长周期专业任务中比零样本界面理解更重要

Conclusion: RiskWebWorld为开发稳健数字工作者提供了实用测试平台，揭示了当前GUI智能体在专业领域的能力差距，并展示了智能体强化学习的改进潜力
...
Method: 提出Weight Patching方法：给定基础模型和行为专业化对应模型，在固定输入下将专业化模型的选定模块权重替换到基础模型中。在指令跟随任务中实例化该方法，并引入以向量锚点行为接口为中心的框架，为开放生成中任务相关控制状态的形成或恢复提供共享内部标准

Result: 分析揭示了从浅层候选源侧载体到聚合和路由模块，再到下游执行电路的层次结构。恢复的组件分数可以指导机制感知的模型合并，在评估的专家组合中改善选择性融合，并提供额外的外部验证

Conclusion: Weight Patching填补了参数空间干预方法的空白，为机制可解释性提供了新的分析工具，能够更准确地定位模型中实现特定能力的内部组件，并支持机制感知的模型合并
...
<details>
  <summary>Details</summary>
Motivation: 当前AI系统（云中心AI、设备端推理、边缘-云管道）将规划、推理和执行视为单一过程，导致不必要的延迟、能耗和行为连续性碎片化。需要一种新的架构来解耦认知功能并映射到异构硬件。

Method: 提出Tri-Spirit三层认知架构：Super Layer（规划）、Agent Layer（推理）、Reflex Layer（执行），各层映射到不同计算基板并通过异步消息总线协调。系统包含参数化路由策略、习惯编译机制（将重复推理路径提升为零推理执行策略）、收敛内存模型和显式安全约束。

Result: 在2000个合成任务的模拟评估中，相比云中心和仅边缘基线，Tri-Spirit将平均任务延迟降低75.6%，能耗降低71.1%，LLM调用减少30%，实现77.6%的离线任务完成率。

Conclusion: 认知分解而非单纯模型扩展是AI硬件系统级效率的主要驱动力。Tri-Spirit架构通过将智能功能解耦并映射到异构硬件，显著提升了自主AI系统的性能和能效。
...
<details>
  <summary>Details</summary>
Motivation: 量子电路优化对当前NISQ设备至关重要，因为错误传播通常随操作数量增加。CNOT门作为Clifford+T通用集中的唯一双量子比特门，其最小化问题尤为关键。现有启发式算法（如PMH）和强化学习方法存在局限，需要更有效的解决方案。

Method: 提出AlphaCNOT框架，基于蒙特卡洛树搜索的强化学习方法，将CNOT最小化问题建模为规划问题。该方法为基于模型的方法，能够利用前瞻搜索评估未来轨迹，从而找到更高效的CNOT序列。

Result: 在线性可逆合成（无拓扑约束）中，相比PMH基线减少高达32%的CNOT门数；在拓扑约束版本中，在最多8个量子比特的各种拓扑结构上，相比最先进的基于强化学习的解决方案实现了一致的门数减少。
...
<details>
  <summary>Details</summary>
Motivation: 随着投稿量激增，科学同行评审面临质量、一致性和及时性的压力，需要探索AI在评审中的实际应用能力

Method: 结合前沿模型、工具使用和安全保障的多阶段流程，在一天内为所有论文生成明确标识的AI评审

Result: 大规模调查显示参与者认为AI评审有用，且在技术准确性和研究建议等关键维度上优于人工评审；新基准测试表明系统显著优于简单LLM基线
...
TL;DR: ATI是一种受生物启发的传感器优先架构，用于物理AI，通过分层模块化设计将传感器控制、自适应感知、边缘-云端执行和基础模型推理集成在闭环架构中，显著提升端到端精度并减少远程推理调用。
...
Method: 提出人工三方智能（ATI）架构，采用生物启发的三层设计：脑干（L1）提供反射性安全和信号完整性控制；小脑（L2）执行连续传感器校准；大脑推理子系统（L3/L4）支持常规技能选择执行、协调和深度推理。这种模块化组织允许传感器控制、自适应感知、边缘-云端执行和基础模型推理在闭环架构中协同进化。

Result: 在移动相机原型测试中，与默认自动曝光设置相比，ATI（L1/L2自适应感知）将端到端精度从53.8%提升至88%，同时减少43.3%的远程L4调用。路由评估（L3-L4分割推理）验证了架构有效性。

Conclusion: ATI展示了将感知与推理协同设计对于具身AI的价值，通过分层架构在保持设备端时间关键感知控制的同时，仅在需要时调用高级推理，有效解决了物理AI的约束问题。
...
<details>
  <summary>Details</summary>
Motivation: 尽管监督微调(SFT)和组相对策略优化(GRPO)等后训练算法在语言模型中显示出强大的推理增益，但奖励设计如何塑造VLM的物理推理行为仍不清楚。当前最先进的视觉语言模型在物理基准测试上仍远低于人类表现。

Method: 在PhyX基准测试（3000个问题，涵盖6个物理领域和6种推理类型）上，使用IBM Granite Vision 3.3 (2B)模型，系统比较了四种语义丰富度递增的奖励信号：格式合规性、答案准确性、复合评分奖励（答案正确性、物理原理识别和单位一致性）、以及基于模型对输入图像区域注意力权重的新型内部奖励。

Result: 基于准确性的奖励在大多数领域上优于SFT，但增益因奖励类型和领域而异。奖励设计不会统一提升性能，而是诱导领域特定的推理行为：准确性奖励提供最强整体增益；评分奖励改善结构化推理质量但未一致提升准确性；注意力奖励增强空间推理但在符号领域降低性能；内部注意力权重奖励无需空间标注，将空间关系准确性从0.27提升至0.50。

Conclusion: 监督模型在生成过程中的注意力位置是视觉基础物理推理的有前景方向。奖励设计对VLM物理推理行为具有领域特异性影响，不同奖励类型会诱导不同的推理策略，而非简单地统一提升性能。
...
TL;DR: 该论文研究了跨领域记忆迁移学习（MTL），通过利用异构领域的统一记忆池来提升编码智能体的性能，发现高层次抽象记忆比低层次具体痕迹具有更好的迁移性。


<details>
  <summary>Details</summary>
Motivation: 现有基于记忆的自进化编码智能体通常将记忆利用限制在单一任务领域，未能充分利用不同真实世界编码问题之间共享的基础设施（如运行时环境和编程语言）。

Method: 提出记忆迁移学习（MTL）框架，利用来自异构领域的统一记忆池。评估了四种记忆表示（从具体痕迹到抽象见解），在6个编码基准上进行实验。

Result: 跨领域记忆将平均性能提升3.7%，主要通过迁移元知识（如验证例程）而非任务特定代码。抽象程度决定迁移性：高层次见解泛化良好，低层次痕迹因过度具体化常导致负迁移。迁移效果随记忆池规模扩大而提升，记忆甚至可在不同模型间迁移。

Conclusion: 该工作为超越单领域限制的记忆利用建立了实证设计原则，表明跨领域记忆迁移是可行的，且抽象程度是决定迁移成功的关键因素。
...
TL;DR: 提出了一种用于电网运行的安全约束分层控制框架，将长期决策与实时可行性执行解耦，通过运行时安全屏蔽确保物理约束，在多种测试场景下优于传统方法
...
Method: 提出安全约束分层控制框架：高层强化学习策略提出抽象控制动作，确定性运行时安全屏蔽通过快速前向模拟过滤不安全动作。安全作为运行时不变式强制执行，独立于策略质量或训练分布。
