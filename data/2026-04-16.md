...
TL;DR: 该论文提出了一个用于量化语言模型智能体探索与利用行为的可控环境框架和评估指标，发现即使最先进的模型在该任务上也存在困难，但推理模型表现更好且通过简单工程改进可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 语言模型智能体在复杂开放式决策任务中需要有效平衡探索与利用，但目前缺乏无需访问智能体内部策略就能从观察到的行动中系统区分和量化探索与利用行为的方法。

Method: 设计了受实际具身AI场景启发的可控环境，包含部分可观测的2D网格地图和未知任务有向无环图；地图生成可编程调整以强调探索或利用难度；设计了从智能体行动量化探索与利用错误的策略无关评估指标。

Result: 评估多种前沿语言模型智能体发现，即使最先进模型在该任务上也表现困难，不同模型展现出不同的失败模式；推理模型能更有效地解决任务；通过最小化工程改进可显著提升探索与利用能力。

Conclusion: 提出的框架能够量化评估语言模型智能体的探索与利用行为，揭示了当前模型的局限性，同时表明通过适当工程改进可显著提升性能，为未来研究提供了有价值的基准工具。
...
<details>
  <summary>Details</summary>
Motivation: 尽管智能体AI取得了进展，但现有系统在现实世界科学研究中的可靠部署仍面临重大挑战，需要更安全、轻量且用户友好的框架来支持科学任务的自主执行。
...
Conclusion: 提出了一个安全可靠的智能体框架，能够自主执行定义明确的科学任务，为科学研究中的自动化工作流程提供了实用解决方案。
...
TL;DR: ReSS框架通过决策树提取符号支架，引导LLM生成忠实于决策逻辑的自然语言推理，用于微调专门的表格推理模型，在医疗和金融基准上提升性能达10%


<details>
  <summary>Details</summary>
Motivation: 在医疗和金融等高风险领域，表格数据预测模型需要同时提供高准确性和可理解、可信的推理过程。符号模型可验证但表达能力有限，通用LLM需要专门微调才能掌握领域特定的表格推理。需要解决数据扩展和推理一致性的双重挑战。

Method: 提出ReSS框架，连接符号和神经推理模型：1) 使用决策树模型提取实例级决策路径作为符号支架；2) 这些支架与输入特征和标签一起引导LLM生成严格遵循底层决策逻辑的自然语言推理；3) 生成的高质量数据集用于微调预训练LLM为专门的表格推理模型；4) 采用支架不变的数据增强策略提升泛化能力和可解释性。

Result: 在医疗和金融基准测试中，ReSS训练模型比传统决策树和标准微调方法提升高达10%的性能，同时产生忠实且一致的推理。引入了幻觉率、解释必要性和解释充分性等定量指标来严格评估忠实性。

Conclusion: ReSS框架成功解决了表格数据推理中的可扩展数据生成和推理一致性问题，通过桥接符号和神经模型，实现了高性能、可解释且忠实的表格推理系统，在高风险领域具有重要应用价值。
...
TL;DR: 提出一种为大型推理模型提供统计保证的推理-答案不确定性量化方法，并开发基于Shapley值的可解释框架，识别训练样本和关键推理步骤以保持保证。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理方面取得显著进步，但传统不确定性量化方法缺乏有限样本保证，且忽视推理轨迹与最终答案的逻辑联系。现有方法无法解释不确定性覆盖的来源，难以分离推理质量与答案正确性，同时缺乏计算高效的解释方法理论保证。

Method: 1) 提出新颖方法量化推理-答案结构的不确定性并提供统计保证；2) 开发基于Shapley值的统一示例到步骤解释框架，识别可证明充分的训练示例子集及其关键推理步骤以保持保证；3) 提供所提方法的理论分析。

Result: 在具有挑战性的推理数据集上进行广泛实验，验证了所提方法的有效性。

Conclusion: 该方法为大型推理模型提供了具有统计保证的不确定性量化，并通过可解释框架识别关键训练因素，解决了传统方法忽视推理-答案逻辑联系和缺乏理论保证的问题。
...
TL;DR: 提出Weight Patching方法，通过参数空间干预分析模型内部机制，结合向量锚行为接口框架，揭示指令跟随任务中的层次化电路结构


<details>
  <summary>Details</summary>
Motivation: 现有激活空间定位和因果追踪方法存在局限：在激活空间中重要的模块可能只是聚合或放大上游信号，而非在其自身参数中编码目标能力。需要参数空间干预方法来识别真正编码能力的组件

Method: 提出Weight Patching方法：在具有相同架构但目标能力表达强度不同的配对模型之间，将专门化模型的选定模块权重替换到基础模型中。引入向量锚行为接口框架，为开放式生成提供共享的内部标准来判断任务相关控制状态是否形成或恢复

Result: 分析揭示了从浅层候选源侧载体到聚合和路由模块，再到下游执行电路的层次结构。恢复的组件分数可指导机制感知的模型合并，在评估的专家组合中改善选择性融合并提供额外外部验证

Conclusion: Weight Patching填补了参数空间干预的空白，结合行为接口框架，能够更精确地定位模型内部机制，并为模型合并提供机制感知指导
...
<details>
  <summary>Details</summary>
Motivation: 当前AI系统（云端AI、设备端推理、边缘-云端流水线）将规划、推理和执行视为单一过程，导致不必要的延迟、能耗和行为连续性碎片化。需要一种新的架构来优化异构硬件上的智能结构。

Method: 提出Tri-Spirit三层认知框架：Super Layer（规划层）、Agent Layer（推理层）、Reflex Layer（执行层），每层映射到不同计算基板，通过异步消息总线协调。系统包含参数化路由策略、习惯编译机制（将重复推理路径提升为零推理执行策略）、收敛内存模型和显式安全约束。

Result: 在2000个合成任务的仿真评估中，相比云端中心和仅边缘基线：平均任务延迟降低75.6%，能耗降低71.1%，LLM调用减少30%，77.6%的任务可离线完成。

Conclusion: 认知分解（而非仅模型扩展）是AI硬件系统级效率的主要驱动力。Tri-Spirit架构通过将智能分解为规划、推理和执行层，显著提升了系统性能。
...
<details>
  <summary>Details</summary>
Motivation: 量子电路优化对当前NISQ设备至关重要，因为错误传播通常随操作数量增加。CNOT门作为Clifford+T通用集中的唯一双量子比特门，其最小化问题尤为重要。现有启发式算法（如PMH）和基于强化学习的方法存在局限性，需要更有效的解决方案。

Method: 提出AlphaCNOT框架，基于蒙特卡洛树搜索的强化学习方法，将CNOT最小化问题建模为规划问题。与现有基于强化学习的解决方案不同，该方法为基于模型的方法，能够利用前瞻搜索评估未来轨迹，从而找到更高效的CNOT序列。

Result: 在线性可逆合成中，相比PMH基线实现高达32%的CNOT门数减少。在约束版本中，在多种最多8量子比特的拓扑结构上，相比最先进的基于强化学习的解决方案实现了一致的门数减少。

Conclusion: 强化学习与基于搜索策略的结合可应用于不同的电路优化任务（如Clifford最小化），从而促进向"量子效用"时代的过渡。AlphaCNOT展示了模型化强化学习方法在量子电路优化中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 科学同行评审面临提交量激增的压力，难以维持评审质量、一致性和及时性。虽然AI在同行评审中的应用受到关注，但关键问题是AI能否在真实会议规模上生成技术可靠的评审

Method: 结合前沿模型、工具使用和安全保障的多阶段流程，在不到一天内为所有22,977篇完整评审论文生成AI评审。系统在AAAI-26主轨道所有提交论文中部署，每个论文获得一个明确标识的AI评审
...
TL;DR: ATI是一种受生物启发的传感器优先架构，用于物理AI系统，通过分层模块化设计将传感器控制、自适应感知、边缘计算和基础模型推理集成在闭环架构中，显著提升动态环境下的端到端精度并减少远程推理调用。


<details>
  <summary>Details</summary>
Motivation: 随着AI从数据中心扩展到机器人和可穿戴设备，单纯扩大模型规模已不足够。物理AI面临严格的延迟、能耗、隐私和可靠性约束，其性能不仅取决于模型容量，还取决于如何在动态环境中通过可控传感器获取信号。需要一种新的架构方法来协同设计感知和推理。

Method: 提出人工三方智能（ATI），一种受生物启发的传感器优先架构契约。ATI在系统层面分为三部分：脑干（L1）提供反射性安全和信号完整性控制；小脑（L2）执行连续传感器校准；大脑推理子系统（L3/L4）支持常规技能选择与执行、协调和深度推理。这种模块化组织允许传感器控制、自适应感知、边缘-云执行和基础模型推理在一个闭环架构中协同演化。

Result: 在移动相机原型上的路由评估（L3-L4分割推理）中，相比默认自动曝光设置，ATI（L1/L2自适应感知）将端到端精度从53.8%提升至88%，同时减少43.3%的远程L4调用。这证明了为具身AI协同设计感知和推理的价值。

Conclusion: ATI架构展示了通过分层模块化方法将传感器控制、自适应感知和推理集成的重要性，为物理AI系统在动态环境中实现高效、可靠和节能的操作提供了有效解决方案。协同设计感知和推理是提升具身AI性能的关键。
...
TL;DR: 该论文系统研究了不同奖励设计对基于GRPO的视觉语言模型在物理推理任务上的影响，发现奖励设计会诱导领域特定的推理行为，而非统一提升性能。
...
Result: 基于准确性的奖励在大多数领域上优于SFT，但增益因奖励类型和领域而异。奖励设计不会统一提升性能，而是诱导领域特定的推理行为：准确性奖励提供最强的整体增益；评分奖励提高结构化推理质量但无一致的准确性改进；基于注意力的奖励增强空间推理但降低符号领域性能。内部注意力权重奖励无需空间标注，将空间关系准确性从0.27提升至0.50。

Conclusion: 奖励设计对VLM物理推理行为有显著影响，诱导领域特定的推理模式而非统一改进。监督模型在生成过程中的注意力分布是视觉基础物理推理的有前景方向，基于注意力的内部奖励在无需空间标注的情况下显著提升空间推理能力。
...
### [11] [Memory Transfer Learning: How Memories are Transferred Across Domains in Coding Agents](https://arxiv.org/abs/2604.14004)
*Kangsan Kim,Minki Kang,Taeil Kim,Yanlai Yang,Mengye Ren,Sung Ju Hwang*

Main category: cs.AI

TL;DR: 该论文研究跨领域记忆迁移学习，通过统一记忆池提升代码智能体性能，发现高层次抽象知识比低层次具体痕迹具有更好的迁移性。


<details>
  <summary>Details</summary>
Motivation: 现有基于记忆的代码智能体通常局限于同质任务领域，未能充分利用不同编程问题之间共享的基础设施（如运行时环境和编程语言）。为了解决这一限制，研究者探索跨领域记忆迁移学习。

Method: 研究者利用来自异质领域的统一记忆池，在6个编码基准上评估四种记忆表示（从具体痕迹到抽象见解），分析跨领域记忆迁移的效果。

Result: 跨领域记忆使平均性能提升3.7%，主要通过转移元知识（如验证例程）而非任务特定代码。抽象程度决定迁移性：高层次见解泛化良好，低层次痕迹常因过度具体导致负迁移。迁移效果随记忆池规模扩大而提升，记忆甚至可在不同模型间迁移。

Conclusion: 该工作为扩展记忆利用超越单领域孤岛建立了经验设计原则，表明跨领域记忆迁移是可行的，且抽象层次是关键影响因素。
...
TL;DR: 提出了一种安全约束的分层控制框架，将强化学习的高层决策与实时安全屏蔽分离，确保电网操作的安全性和泛化能力


<details>
  <summary>Details</summary>
Motivation: 强化学习在电网操作中应用受限，主要因为严格的安全要求、罕见扰动下的脆弱性以及对未见电网拓扑的泛化能力差。在安全关键基础设施中，灾难性故障不可接受，学习型控制器必须在硬物理约束内运行。

Method: 提出安全约束的分层控制框架：高层强化学习策略提出抽象控制动作，确定性运行时安全屏蔽通过快速前向仿真过滤不安全动作。安全作为运行时不变量强制执行，独立于策略质量或训练分布。

Result: 在Grid2Op基准测试中，平坦强化学习策略在压力下脆弱，纯安全方法过于保守。而提出的分层安全感知方法实现了更长的生存时间、更低的峰值线路负载，并在ICAPS 2021大规模输电网上实现了零样本泛化。
...
TL;DR: TREX是一个多智能体系统，能够自动化整个LLM训练生命周期，通过研究者和执行者两个核心模块的协作，完成需求分析、文献数据研究、训练策略制定、数据准备和模型训练评估等任务。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型已经赋能AI研究智能体执行独立的科学任务，但自动化复杂的真实世界工作流程（如LLM训练）仍然是一个重大挑战。现有方法难以处理从需求分析到模型评估的完整训练生命周期。

Method: TREX采用多智能体系统架构，包含研究者（Researcher）和执行者（Executor）两个核心模块。系统将多轮实验过程建模为搜索树，能够高效规划探索路径、重用历史结果，并从迭代试验中提炼高层见解。研究者负责需求分析、开放域文献和数据研究、制定训练策略，执行者负责准备数据配方、模型训练和评估。

Result: 为了评估自动化LLM训练能力，作者构建了FT-Bench基准，包含10个源自真实场景的任务，涵盖优化基础模型能力到提升特定领域任务性能。实验结果表明，TREX智能体能够持续优化目标任务的模型性能。

Conclusion: TREX系统成功实现了LLM训练生命周期的自动化，通过多智能体协作和搜索树建模，能够有效处理复杂的真实世界训练工作流程，在FT-Bench基准上展示了优化模型性能的能力。
