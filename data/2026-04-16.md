...
TL;DR: 该论文提出了一种在语言模型智能体中进行探索与利用行为量化评估的方法，通过可控的2D网格环境和未知任务DAG设计，实现了无需访问内部策略的探索-利用错误度量。


<details>
  <summary>Details</summary>
Motivation: 语言模型智能体在复杂开放决策任务中的应用日益增多，但缺乏在不访问内部策略的情况下系统区分和量化探索与利用行为的方法，这阻碍了对智能体决策能力的深入评估。

Method: 设计了受实际具身AI场景启发的可控环境：部分可观测的2D网格地图和未知任务有向无环图(DAG)，地图生成可编程调整以强调探索或利用难度；设计了从智能体动作量化探索和利用错误的度量标准，实现策略无关的评估。

Result: 评估了多种前沿语言模型智能体，发现即使是当前最先进的模型在该任务上也表现不佳，不同模型展现出不同的失败模式；推理模型能更有效地解决任务，且通过最小化工程改进可以显著提升探索和利用能力。

Conclusion: 该研究提供了一种量化语言模型智能体探索与利用行为的方法框架，揭示了当前模型在平衡探索-利用方面的局限性，并展示了通过工程改进提升性能的潜力，为未来智能体决策能力评估提供了新工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有智能体AI系统在现实世界科学研究中的可靠部署仍面临重大挑战，需要更安全、轻量且用户友好的框架来支持定义明确的科学任务的自主执行。

Method: 框架包含三个核心组件：1) 隔离执行环境确保安全性；2) 三层智能体循环结构；3) 自评估do-until机制。通过聚焦于具有明确定义上下文和停止条件的结构化任务，实现端到端自动化。

Result: 该框架能够在最小化人工干预的情况下，安全可靠地执行科学任务，有效利用不同能力水平的大型语言模型，支持研究人员将常规工作负载卸载，专注于创造性活动和开放式科学探究。

Conclusion: 提出的智能体框架通过安全隔离、结构化循环和自评估机制，为定义明确的科学任务提供了可靠的自主执行解决方案，有助于提高科研效率并释放研究人员的创造力。
...
TL;DR: 论文分析了LLM不可预测性的根源在于浮点数表示的有限精度，揭示了Transformer计算层中舍入误差传播、放大或消散的机制，识别了早期层的"雪崩效应"，并发现了LLM普遍存在的三种尺度依赖混沌行为模式。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地集成到智能体工作流中，其数值不稳定性导致的不可预测性已成为关键的可靠性问题。虽然已有研究表明这些不稳定性会产生显著的下游影响，但其根本原因和内在机制仍未被充分理解。

Method: 通过严格分析浮点数表示有限精度如何导致不可预测性，追踪舍入误差在Transformer计算层中的传播、放大或消散过程。特别识别了早期层的混沌"雪崩效应"，并系统研究了LLM在不同尺度下的三种行为模式。

Result: 发现LLM存在三种尺度依赖的混沌行为模式：1)稳定模式：扰动低于输入相关阈值时消失，输出恒定；2)混沌模式：舍入误差主导并驱动输出发散；3)信号主导模式：真实输入变化覆盖数值噪声。这些发现在多个数据集和模型架构中得到广泛验证。

Conclusion: LLM的不可预测性根源于浮点数表示的有限精度，Transformer计算层中的舍入误差传播机制导致了早期层的"雪崩效应"。模型表现出三种普遍存在的尺度依赖混沌行为模式，这对LLM在关键应用中的可靠性具有重要影响。
...
TL;DR: ReSS框架通过决策树提取符号支架，指导LLM生成基于决策逻辑的自然语言推理，然后微调LLM成为专门的表格推理模型，在保持高准确性的同时提供忠实可解释的推理。


<details>
  <summary>Details</summary>
Motivation: 在医疗和金融等高风险领域，表格数据预测模型需要同时具备高准确性和忠实、可理解的推理能力。符号模型提供可验证逻辑但缺乏语义表达能力，而通用LLM需要专门微调才能掌握领域特定的表格推理。需要解决数据扩展和推理一致性的双重挑战。

Method: 提出ReSS框架，结合符号和神经推理模型：1) 使用决策树模型提取实例级决策路径作为符号支架；2) 用这些支架、输入特征和标签指导LLM生成严格遵循底层决策逻辑的自然语言推理；3) 用生成的高质量数据集微调预训练LLM为专门的表格推理模型；4) 采用支架不变的数据增强策略提升泛化能力和可解释性。

Result: 在医疗和金融基准测试中，ReSS训练的模型相比传统决策树和标准微调方法提升高达10%，同时产生忠实一致的推理。引入了幻觉率、解释必要性和解释充分性等量化指标来严格评估忠实性。

Conclusion: ReSS框架成功解决了表格数据推理中的可扩展数据构建和推理一致性问题，通过结合符号和神经模型的优势，在保持高准确性的同时提供了忠实、可解释的自然语言推理，适用于高风险领域的表格数据分析。
...
TL;DR: 本文提出了一种为大型推理模型（LRMs）提供统计保证的推理-答案不确定性量化方法，以及基于Shapley值的训练示例-推理步骤解释框架。


<details>
  <summary>Details</summary>
Motivation: 传统不确定性量化方法无法为推理-答案生成提供有限样本保证，且忽略推理轨迹与最终答案之间的逻辑联系。现有方法也难以解释LRMs不确定性覆盖的来源，无法分离推理质量与答案正确性，同时缺乏计算高效的解释方法理论保证。

Method: 首先提出了一种量化推理-答案结构不确定性的新方法，具有统计保证。随后开发了基于Shapley值的统一示例-步骤解释框架，识别能够保持保证的充分训练子集及其关键推理步骤。提供了所提方法的理论分析。

Result: 在具有挑战性的推理数据集上进行了广泛实验，验证了所提方法的有效性。

Conclusion: 本文解决了LRMs不确定性量化的关键挑战，提供了具有统计保证的推理-答案不确定性量化方法，以及可解释的训练示例-推理步骤分析框架，为大型推理模型的可靠应用提供了理论和方法基础。
...
TL;DR: LAMO框架通过角色导向数据合成和两阶段训练，赋予轻量级MLLM GUI特定知识和任务可扩展性，支持单体执行和多智能体系统编排，实现资源受限设备上的GUI自动化。


<details>
  <summary>Details</summary>
Motivation: 当前轻量级GUI智能体面临资源受限设备部署成本高、容量有限、任务可扩展性差的问题，难以适应复杂真实场景和多智能体系统，而训练多个技能专家成本高昂。

Method: 提出LAMO框架：1) 角色导向数据合成；2) 两阶段训练：监督微调使用困惑度加权交叉熵优化进行知识蒸馏和视觉感知增强，强化学习进行角色导向协同探索；3) 开发LAMO-3B智能体，支持单体执行和MAS编排。

Result: LAMO-3B作为即插即用的策略执行器，可与高级规划器配对，持续受益于规划器进步，实现更高性能上限。广泛的静态和在线评估验证了设计的有效性。

Conclusion: LAMO框架在成本-可扩展性困境中实现了有效权衡，使轻量级MLLM能够参与真实GUI工作流，通过多角色编排扩展能力边界，为资源受限设备上的GUI自动化提供了可行方案。
...
### [7] [RiskWebWorld: A Realistic Interactive Benchmark for GUI Agents in E-commerce Risk Management](https://arxiv.org/abs/2604.13531)
*Renqi Chen,Zeyin Tao,Jianming Guo,Jing Wang,Zezhou Xu,Jingzhe Zhu,Qingqing Sun,Tianyi Zhang,Shuai Chen*

Main category: cs.AI

TL;DR: RiskWebWorld：首个针对电商风险管理的GUI智能体交互基准测试，包含1513个真实任务，揭示当前模型在专业领域存在显著能力差距


<details>
  <summary>Details</summary>
Motivation: 现有GUI智能体基准测试主要针对良性、可预测的消费环境，而在高风险、调查性领域如电商风险管理的有效性尚未充分探索。需要填补这一空白，评估智能体在真实风险控制场景中的表现。

Method: 构建RiskWebWorld基准测试，包含1513个来自生产风险控制管道的任务，涵盖8个核心领域。创建Gymnasium兼容的基础设施，将策略规划与环境机制解耦，支持可扩展评估和智能体强化学习。

Result: 评估显示显著能力差距：顶级通用模型成功率49.1%，而专门的开源GUI模型接近完全失败。智能体强化学习将开源模型性能提升16.2%。这表明在长视野专业任务中，基础模型规模比零样本界面接地更重要。

Conclusion: RiskWebWorld为开发稳健数字工作者提供了实用测试平台，揭示了当前GUI智能体在专业领域的能力局限，同时展示了通过强化学习改进的可行性。
...
### [8] [Weight Patching: Toward Source-Level Mechanistic Localization in LLMs](https://arxiv.org/abs/2604.13694)
*Chenghao Sun,Chengsheng Zhang,Guanzheng Qin,Rui Dai,Xinmei Tian*

Main category: cs.AI

TL;DR: 提出Weight Patching方法，通过参数空间干预在具有相同架构但目标能力表达强度不同的配对模型中进行源导向分析，揭示从浅层候选源侧载体到聚合路由模块再到下游执行电路的层次结构。


<details>
  <summary>Details</summary>
Motivation: 现有激活空间定位和因果追踪方法存在局限：在激活空间中显得重要的模块可能只是聚合或放大上游信号，而非在其自身参数中编码目标能力。需要填补这一空白，实现更精确的机制解释性。

Method: 提出Weight Patching方法：在具有相同架构的基础模型和行为专业化模型之间，在固定输入下将专业化模型的选定模块权重替换到基础模型中。引入基于向量锚点的行为接口框架，为开放式生成中任务相关控制状态的形成或恢复提供共享内部标准。

Result: 分析揭示了从浅层候选源侧载体到聚合和路由模块，再到下游执行电路的层次结构。恢复的组件分数可以指导机制感知的模型合并，在评估的专家组合中改进选择性融合，并提供额外的外部验证。

Conclusion: Weight Patching填补了参数空间干预方法的空白，为机制解释性提供了源导向分析的新工具，能够更精确地定位模型行为到内部组件，并支持机制感知的模型合并。
...
TL;DR: Tri-Spirit Architecture将AI智能分解为规划、推理和执行三层，分别映射到不同计算硬件，通过异步消息总线协调，显著降低延迟和能耗。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统（云端AI、设备端推理、边云管道）将规划、推理和执行视为单一过程，导致不必要的延迟、能耗和行为连续性碎片化。需要一种能充分利用异构硬件优势的智能分解架构。

Method: 提出Tri-Spirit三层认知框架：Super Layer（规划）、Agent Layer（推理）、Reflex Layer（执行），每层映射到不同计算基板。采用异步消息总线协调，包含参数化路由策略、习惯编译机制（将重复推理路径提升为零推理执行策略）、收敛内存模型和显式安全约束。

Result: 在2000个合成任务的模拟评估中，相比云端中心和仅边缘基线，Tri-Spirit将平均任务延迟降低75.6%，能耗降低71.1%，LLM调用减少30%，实现77.6%的离线任务完成率。

Conclusion: 认知分解而非单纯模型缩放是AI硬件系统级效率的主要驱动力。Tri-Spirit架构通过智能分层和异构硬件映射，显著提升自主AI系统的性能和能效。
...
### [10] [AlphaCNOT: Learning CNOT Minimization with Model-Based Planning](https://arxiv.org/abs/2604.13812)
...
<details>
  <summary>Details</summary>
Motivation: 量子电路优化对当前NISQ设备至关重要，因为错误传播通常随操作数量增加。CNOT门作为Clifford+T通用集中的唯一双量子比特门，其最小化问题在无拓扑约束的线性可逆合成中已有启发式算法（如PMH），在拓扑感知合成中则有基于强化学习的方法。本文旨在开发更有效的CNOT最小化方法。

Method: 提出AlphaCNOT框架，基于蒙特卡洛树搜索的强化学习方法，将CNOT最小化问题建模为规划问题。与现有基于强化学习的方法不同，该方法为基于模型的方法，能够利用前瞻搜索评估未来轨迹，从而找到更高效的CNOT序列。

Result: 在线性可逆合成中，相比PMH基线实现了高达32%的CNOT门数减少；在约束版本（拓扑感知合成）中，在多种最多8量子比特的拓扑结构上，相比最先进的基于强化学习的解决方案，实现了持续的门数减少。
...
### [11] [AI-Assisted Peer Review at Scale: The AAAI-26 AI Review Pilot](https://arxiv.org/abs/2604.13940)
...
<details>
  <summary>Details</summary>
Motivation: 科学同行评审面临投稿量激增的压力，难以维持评审质量、一致性和及时性。虽然AI在同行评审中的应用备受关注，但关键问题是AI能否在现实会议规模上生成技术可靠的评审。

Method: 采用前沿模型、工具使用和安全保障的多阶段流程，在不到一天内为所有22,977篇完整评审论文生成AI评审。系统在AAAI-26主赛道全面部署，所有AI评审都明确标识。

Result: 大规模调查显示，AAAI-26作者和程序委员会成员不仅认为AI评审有用，而且在技术准确性和研究建议等关键维度上更偏好AI评审而非人类评审。新基准测试表明，该系统在检测各种科学缺陷方面显著优于简单的LLM生成评审基线。
...
### [12] [[Emerging Ideas] Artificial Tripartite Intelligence: A Bio-Inspired, Sensor-First Architecture for Physical AI](https://arxiv.org/abs/2604.13959)
*You Rim Choi,Subeom Park,Hyung-Sin Kim*

Main category: cs.AI

TL;DR: ATI是一种受生物启发的三部分物理AI架构，通过分层设计将传感器控制、自适应感知和推理相结合，在动态环境中显著提升端到端精度并减少云端调用。
...
Method: 提出人工三部分智能（ATI）架构：脑干（L1）提供反射性安全和信号完整性控制；小脑（L2）执行连续传感器校准；大脑推理子系统（L3/L4）支持常规技能选择与执行、协调和深度推理。该模块化组织允许传感器控制、自适应感知、边缘-云端执行和基础模型推理在闭环架构中协同演化。

Result: 在移动相机原型测试中，相比默认自动曝光设置，ATI（L1/L2自适应感知）将端到端精度从53.8%提升至88%，同时减少远程L4调用43.3%。

Conclusion: ATI展示了将感知与推理协同设计对于具身AI的价值，通过分层架构在保持设备端时间关键控制的同时，按需调用高级推理，显著提升系统性能。
...
### [13] [Reward Design for Physical Reasoning in Vision-Language Models](https://arxiv.org/abs/2604.13993)
*Derek Lilienthal,Manisha Mukherjee,Sameera Horawalavithana*

Main category: cs.AI

TL;DR: 该论文系统研究了奖励设计对基于GRPO的VLM物理推理训练的影响，比较了四种不同语义丰富度的奖励信号，发现奖励设计会诱导领域特定的推理行为而非均匀提升性能。


<details>
  <summary>Details</summary>
Motivation: 尽管SFT和GRPO等后训练算法在语言模型中显示出强大的推理增益，但奖励设计如何塑造VLM物理推理行为仍然知之甚少。当前最先进的视觉语言模型在物理基准测试上远低于人类表现，需要深入理解奖励机制对视觉物理推理的影响。

Method: 在PhyX基准测试（3000个问题，涵盖六个物理领域和六种推理类型）上，使用IBM Granite Vision 3.3 (2B)模型，系统比较了四种奖励信号：格式合规性、答案准确性、综合评分奖励（答案正确性、物理原理识别和单位一致性），以及基于模型注意力权重的内部奖励。采用GRPO训练方法进行奖励消融研究。

Result: 基于准确性的奖励在大多数领域上优于SFT，但增益因奖励类型和领域而异。奖励设计不会均匀提升性能，而是诱导领域特定的推理行为：准确性奖励提供最强整体增益；评分奖励改善结构化推理质量但无一致性准确性提升；注意力奖励增强空间推理但降低符号领域性能；内部注意力权重奖励无需空间标注，将空间关系准确性从0.27提升至0.50。

Conclusion: 奖励设计对VLM物理推理行为具有重要影响，不同奖励信号会诱导特定的推理模式。监督模型在生成过程中的注意力分布是视觉物理推理的一个有前景方向，特别是基于注意力权重的内部奖励能够显著提升空间推理能力而无需额外标注。
...
### [14] [Memory Transfer Learning: How Memories are Transferred Across Domains in Coding Agents](https://arxiv.org/abs/2604.14004)
*Kangsan Kim,Minki Kang,Taeil Kim,Yanlai Yang,Mengye Ren,Sung Ju Hwang*

Main category: cs.AI

TL;DR: 该论文提出跨领域记忆迁移学习(MTL)方法，通过利用异构领域的统一记忆池来提升编码智能体的性能，实验表明跨领域记忆能平均提升3.7%的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于记忆的编码智能体方法通常将记忆使用限制在单一任务领域，未能充分利用不同编码问题之间共享的基础设施（如运行时环境和编程语言）。需要探索如何跨异构领域进行记忆迁移学习。

Method: 提出记忆迁移学习(MTL)框架，利用来自异构领域的统一记忆池。评估了四种记忆表示方法，从具体执行轨迹到抽象见解。在6个编码基准上进行实验，分析跨领域记忆迁移的效果。

Result: 跨领域记忆平均提升性能3.7%，主要通过转移元知识（如验证例程）而非任务特定代码。抽象程度决定可迁移性：高层次见解泛化良好，低层次轨迹因过度具体化常导致负迁移。记忆池规模越大迁移效果越好，记忆甚至可在不同模型间迁移。

Conclusion: 该研究建立了扩展记忆利用超越单一领域孤岛的经验设计原则，为编码智能体的记忆系统设计提供了实证指导，展示了跨领域记忆迁移的可行性和有效性。
...
### [15] [Hierarchical Reinforcement Learning with Runtime Safety Shielding for Power Grid Operation](https://arxiv.org/abs/2604.14032)
*Gitesh Malik*

Main category: cs.AI

TL;DR: 提出了一种安全约束的分层控制框架，将电网操作中的长期决策与实时可行性执行解耦，通过运行时安全屏蔽确保安全约束，实现鲁棒的零样本泛化。


<details>
  <summary>Details</summary>
Motivation: 强化学习在电网操作中显示出潜力，但在实际部署中面临严格安全要求、罕见扰动下的脆弱性以及对未见电网拓扑的泛化能力差等限制。安全关键基础设施不能容忍灾难性故障，学习型控制器必须在硬物理约束内运行。

Method: 提出安全约束的分层控制框架：高层强化学习策略提出抽象控制动作，确定性运行时安全屏蔽通过快速前向仿真过滤不安全动作。安全作为运行时不变量强制执行，独立于策略质量或训练分布。

Result: 在Grid2Op基准套件上评估，包括正常条件、强制线路中断压力测试以及在ICAPS 2021大规模输电网上零样本部署。结果显示：平坦强化学习策略在压力下脆弱，纯安全方法过于保守；而提出的分层安全感知方法实现了更长的生存时间、更低的峰值线路负载和鲁棒的零样本泛化。

Conclusion: 电网控制中的安全性和泛化能力最好通过架构设计而非复杂的奖励工程实现，为实际能源系统中可部署的学习型控制器提供了实用路径。
...
### [16] [TREX: Automating LLM Fine-tuning via Agent-Driven Tree-based Exploration](https://arxiv.org/abs/2604.14116)
*Zerun Ma,Guoqiang Wang,Xinchen Xie,Yicheng Chen,He Du,Bowen Li,Yanan Sun,Wenran Liu,Kai Chen,Yining Li*

Main category: cs.AI

TL;DR: TREX是一个自动化LLM训练全生命周期的多智能体系统，通过研究者和执行者模块协作，实现从需求分析到模型训练评估的完整流程自动化。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM赋能了AI研究智能体执行孤立科学任务，但自动化复杂真实世界工作流（如LLM训练）仍然是一个重大挑战。现有方法难以处理从需求分析到模型评估的完整训练生命周期。

Method: TREX采用多智能体系统架构，包含研究者（Researcher）和执行者（Executor）两个核心模块。系统将多轮实验过程建模为搜索树，能够高效规划探索路径、重用历史结果，并从迭代试验中提炼高层见解。系统执行需求分析、开放领域文献与数据研究、训练策略制定、数据配方准备、模型训练与评估等完整流程。

Result: 为评估自动化LLM训练能力，构建了FT-Bench基准，包含10个来自真实场景的任务，涵盖从优化基础模型能力到提升特定领域任务性能。实验结果表明，TREX智能体能够持续优化目标任务的模型性能。

Conclusion: TREX成功实现了LLM训练生命周期的自动化，通过多智能体协作和搜索树建模，能够有效处理复杂训练工作流，在真实场景任务中展现出持续的性能优化能力。
