...
TL;DR: 提出基于心跳驱动的自主思维活动调度机制，使LLM智能体能够进行主动、自适应和持续的自我调节，通过周期性心跳协调动态认知模块库，并采用元学习策略进行持续策略适应。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体通常采用僵化的反应式控制流程，限制了适应性和效率。大多数框架依赖固定流水线或失败触发的反思，导致智能体冲动行动或仅在错误发生后才进行修正。

Method: 引入心跳驱动的自主思维活动调度机制，采用周期性"心跳"机制协调动态认知模块库（如规划器、批评器、回忆器、梦想器）。调度器基于时间模式和历史上下文学习何时启动特定思维活动。提出元学习策略进行持续策略适应，调度器使用历史交互日志随时间优化认知策略。

Result: 评估结果表明，该方法能有效基于历史数据学习调度认知活动，并能自主集成新的思维模块。

Conclusion: 心跳驱动的自主思维活动调度机制为LLM智能体提供了更灵活、自适应和高效的认知控制框架，克服了传统方法的僵化性和反应性限制。
...
<details>
  <summary>Details</summary>
Motivation: 当前多元时间序列异常检测评估面临挑战，现有基准数据集存在三个主要问题：1) 缺乏细粒度异常标注；2) 不提供明确的变量间和时间依赖关系；3) 对底层生成机制缺乏洞察。这些问题阻碍了检测模型的开发和严格比较，特别是针对可解释和变量特定输出的模型。

Method: 提出了Fun-TSG时间序列生成器，支持两种生成模式：1) 完全自动化生成，基于随机采样的依赖结构和异常类型；2) 手动生成，通过用户定义的方程和异常配置。两种模式都提供数据生成过程的完全透明性，包括变量和时间戳级别的真实异常标签访问。

Result: Fun-TSG能够创建多样化、可解释且可复现的基准测试场景，支持对传统和现代异常检测模型进行细粒度性能分析。该工具提供了对数据生成过程的完全控制，包括明确的依赖关系和异常机制。

Conclusion: Fun-TSG填补了多元时间序列异常检测评估的关键空白，通过提供完全可定制、透明且可复现的基准测试生成能力，支持更严格和细粒度的模型评估与比较。
...
<details>
  <summary>Details</summary>
Motivation: 现有大语言模型后训练通常使用SFT和RL，但难以有效统一高效知识注入与鲁棒泛化。分析发现SFT本质上是具有极端稀疏隐式奖励和不稳定逆概率加权的策略梯度优化，导致单一路径依赖、熵崩溃和梯度爆炸。
...
Conclusion: GFT通过诊断SFT的内在限制并设计相应机制，实现了知识注入与泛化的统一，为语言模型后训练提供了更稳定高效的框架。
...
<details>
  <summary>Details</summary>
Motivation: 当前大规模视觉语言模型在胸部X光解读方面虽有潜力，但其临床实用性受到限制，主要原因是模型输出与放射科医生推理之间存在差距。现有系统通常只优化语义信息，而没有模拟专家如何视觉检查医学图像，经常忽略关键发现或偏离既定的诊断工作流程。放射科医生遵循结构化协议（如ABCDEF方法），确保系统地检查所有临床相关区域，减少遗漏发现并支持可靠的诊断推理。

Method: GazeX是一个视觉语言模型，利用放射科医生的眼动追踪数据作为行为先验来建模专家诊断推理。通过将注视轨迹和注视模式整合到预训练中，GazeX学习遵循放射科医生注意力的空间和时间结构，并以临床上有意义的顺序整合观察结果。研究使用了来自五位放射科医生的超过30,000个注视关键帧的策划数据集，并利用了231,835个放射学研究、780,014个问答对以及1,162个带有边界框的图像句子对。

Result: GazeX在放射学报告生成、疾病定位和视觉问答任务中产生更准确、可解释且与专家一致的输出。与自主报告系统不同，GazeX产生可验证的证据工件，包括检查轨迹和发现链接的定位区域，从而实现高效的人工验证和安全的人机协作。
...
<details>
  <summary>Details</summary>
Motivation: 探究稀疏混合专家架构中路由拓扑结构是否真正决定语言建模质量，验证不同路由机制对最终性能的实际影响。

Method: 构建几何MoE（ST-MoE），使用余弦相似度路由在低维空间（d_space=64）中学习质心，比标准线性路由减少80%参数。通过62个对照实验在WikiText-103上进行，参数规模76-84M，训练至收敛（50K步，1.64B tokens）。使用双单侧检验进行统计比较。

Result: 路由拓扑不影响渐近困惑度：五种余弦路由变体在1-PPL范围内统计等效（p<0.05）。该发现扩展到哈希、随机固定和top-1路由。标准线性路由器的优势主要来自5.3倍参数，机制优势仅约1.2%。多跳更新是共线的（cos=0.805），实现幅度放大而非组合推理。

Conclusion: 路由拓扑结构不是语言建模质量的决定因素，不同路由机制在统计上等效。实际优势主要来自参数数量而非路由机制。零射相对范数停止可节省25%MoE FLOPs，仅增加0.12% PPL。
...
TL;DR: Pneuma-Seeker是一个支持数据探索中迭代问题精化的系统，将用户信息需求转化为明确、可检查的关系型规范，实现目标数据发现和溯源感知执行


<details>
  <summary>Details</summary>
Motivation: 数据分析师在处理关系型数据时通常从模糊或不明确的问题开始，需要通过迭代探索来逐步精化问题，现有工具缺乏对这种迭代过程的系统支持

Method: 开发Pneuma-Seeker系统，将用户信息需求具体化为明确、可检查的关系型规范，支持信息需求的迭代精化、目标数据发现和溯源感知执行

Result: 通过两个真实世界的采购用例展示了Pneuma-Seeker如何利用LLMs作为透明、交互式的分析协作者，而非不透明的答案引擎

Conclusion: Pneuma-Seeker通过将信息需求具体化为可检查的规范，支持数据探索中的迭代精化过程，为LLMs在数据分析中作为透明协作者而非黑盒答案引擎提供了新范式
...
TL;DR: 稀疏混合专家模型中的专家具有可解释的语义专业化，通过余弦相似度路由在低维度量空间中实现透明检查，专家干预可因果验证且不影响推理效率。


<details>
  <summary>Details</summary>
Motivation: 尽管稀疏混合专家模型通过固定每个token的计算量来扩展参数，但单个专家的专业化特性仍然不透明。先前研究表明路由拓扑结构对质量无影响，但本文旨在证明专家身份具有因果意义，其专业化可以直接检查。

Method: 采用四种证据线：1）通过解嵌入矩阵投影专家输出向量构建语义词典；2）分析路由中的频率到语法梯度；3）进行因果干预实验验证专家标签；4）比较不同路由方法的可解释性。

Result: 15%的专家是单语义专家，涵盖10个类别（时间、地理、基数、话语、情感、金融、军事、科学等）。路由显示早期层按词频分离token，深层按语法类别分离。因果干预证实专家标签：转向时间专家中心使P(时间)增加321%，抑制地理专家使P(地理)下降23%。余弦路由提供几何透明度，专家专业化可直接从中心矩阵读取。

Conclusion: MoE专家级专业化是一流的可解释性原语：架构上单语义、因果验证，在推理时可控且零开销。余弦路由提供几何透明度，使专家专业化可直接检查，为模型可解释性提供了新途径。
...
TL;DR: 提出基于强化学习的控制设计框架，通过结合奖励机器和信号时序逻辑来处理复杂任务，利用STL进行事件生成和训练引导


<details>
  <summary>Details</summary>
Motivation: 传统强化学习在处理复杂任务时面临奖励设计困难，需要更高效的表示方法来引导智能体学习满足特定要求的行为

Method: 将奖励机器与信号时序逻辑公式结合，利用STL进行事件生成，并基于STL在线监控算法实现框架

Result: 在三个案例研究（minigrid、cart-pole和high-way环境）中展示了框架处理非平凡任务的能力

Conclusion: 提出的RL控制框架通过STL增强奖励机器，能更高效地表示复杂任务奖励并引导训练收敛，适用于多种复杂环境
...
<details>
  <summary>Details</summary>
Motivation: 当前AI模型开发仍然高度依赖专家经验，是一个劳动密集型过程。现有的AutoML方法主要局限于超参数优化和模型选择等狭窄方面，无法覆盖完整的AI开发生命周期。需要一种能够从任务描述开始自动构建AI模型的解决方案。

Method: 提出AIBuildAI系统，采用分层智能体架构：一个管理器智能体协调三个专业子智能体（设计器、编码器、调优器）。每个子智能体都是基于大语言模型的智能体，具备多步推理和工具使用能力，能够实现从建模策略设计、代码实现与调试到训练和性能优化的完整流程自动化。

Result: 在MLE-Bench基准测试（包含视觉、文本、时间序列和表格等多种模态的现实Kaggle风格任务）上，AIBuildAI以63.1%的奖牌率排名第一，超越了所有现有基线方法，并与经验丰富的AI工程师能力相当。

Conclusion: 分层智能体系统能够实现从任务规范到可部署模型的完整AI开发过程自动化，为以最小人工干预实现广泛可访问的AI开发提供了一条可行路径。
...
TL;DR: 提出基于价值感知的AI干预方法，通过检测策略-价值不一致性来优化人类决策辅助，在象棋领域验证优于最强引擎干预


<details>
  <summary>Details</summary>
Motivation: AI系统在序列决策任务中辅助人类时，如何确定干预时机和方式仍是根本挑战。传统方法推荐最优动作但假设人类能完美执行后续动作，而实际人类决策者可能执行不佳，降低整体性能。

Method: 提出价值感知干预方法：基于强化学习中的贝尔曼方程原理，当决策者遵循次优策略时，策略与价值函数不一致，这种不一致性自然识别干预机会。在马尔可夫决策过程中形式化该问题，AI助手可在干预预算内覆盖人类动作。单次干预时，最优策略是推荐最大化人类价值函数的动作；多次干预时，提出基于策略-价值差异幅度的可处理近似方法。

Result: 在象棋领域通过大规模游戏数据学习人类模型进行评估。模拟实验中，该方法在各种设置下始终优于基于最强象棋引擎（Stockfish）的干预。在20名玩家、600场游戏的受试者内研究中，该方法显著提高了低技能和中技能玩家的表现，对高技能玩家则与专家引擎干预效果相当。

Conclusion: 价值感知干预通过检测策略-价值不一致性来优化AI辅助决策，在象棋领域验证了其优于传统最优动作推荐方法，特别是对非专家决策者效果显著。
...
TL;DR: RUMS是一种基于互信息优化的记忆选择方法，通过测量记忆子集与模型输出之间的互信息来选择用户记忆项，相比基于相似性的方法能更有效地减少响应不确定性并提高预测精度。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型个性化方法主要基于用户记忆项与输入查询之间的语义相似性来选择记忆子集，忽略了这些特征如何实际影响模型的响应分布，导致选择不够优化。

Method: 提出响应效用优化的记忆选择方法（RUMS），通过测量记忆子集与模型输出之间的互信息来选择用户记忆项，识别那些能减少响应不确定性并锐化预测的记忆项，超越单纯的语义相似性。

Result: RUMS实现了比现有方法更符合人类选择的记忆选择，性能优于比其大400倍的模型；同时使用RUMS选择的记忆项能获得更好的响应质量，计算成本降低高达95%。

Conclusion: 基于信息论基础的RUMS方法为记忆选择提供了更原则化的框架，相比基于相似性的方法在效果和效率上都有显著优势，实现了更优的个性化响应生成。
...
TL;DR: 该论文系统研究了在边缘设备CPU上部署高质量ASR的优化方案，通过评估多种ASR架构和量化策略，将NVIDIA Nemotron模型从2.47GB压缩至0.67GB，WER仅增加1%，实现了实时流式语音识别。


<details>
  <summary>Details</summary>
Motivation: 在边缘设备上部署高质量自动语音识别面临挑战，需要模型在CPU上同时优化准确性、延迟和内存占用，而无需GPU加速。现有研究缺乏对多种ASR架构在资源受限硬件上的系统性评估和优化方案。

Method: 1. 系统评估了50多种ASR配置，涵盖编码器-解码器、Transducer和LLM-based架构，包括Whisper、Nemotron、Parakeet TDT等模型；2. 在ONNX Runtime中重新实现完整的流式推理流水线；3. 评估多种后训练量化策略：重要性加权k-quant、混合精度方案和最近邻舍入量化；4. 结合图级算子融合优化。

Result: 1. 识别NVIDIA Nemotron Speech Streaming为资源受限硬件上实时英语流式ASR的最佳候选；2. 通过量化优化将模型从2.47GB压缩至0.67GB；3. int4 k-quant变体在8个标准基准测试中实现8.20%平均流式WER，仅比全精度PyTorch基线WER增加1%以内；4. 在CPU上以0.56秒算法延迟实现超实时运行。

Conclusion: 该研究为边缘设备CPU上的流式ASR建立了新的质量-效率帕累托最优点，通过系统架构评估和量化优化实现了高质量、低延迟、小内存占用的实时语音识别解决方案。
...
TL;DR: 该论文研究了金融机器学习中合成数据增强的统计效应，揭示了其引发的结构偏差-方差权衡，并提出了用于分离信息增益与机械样本量效应的检验方法。


<details>
  <summary>Details</summary>
Motivation: 尽管合成数据增强在缓解金融机器学习中的数据稀缺问题中应用日益广泛，但其统计作用机制尚未得到充分理解。研究者需要明确合成增强如何影响模型性能，以及何时能真正带来改进而非引入分布扭曲。

Method: 将合成增强形式化为训练分布的有效修改，引入大小匹配的零增强作为对照，并开发了在弱时间依赖下有效的有限样本非参数块置换检验。在控制环境（马尔可夫切换）和真实金融数据集（高频期权交易数据和日频股票面板）上评估多种生成器。

Result: 合成增强仅在方差主导的机制中（如持续性波动率预测）有益，而在偏差主导的设置中（如近有效方向预测）会损害性能。针对罕见机制的增强可改善特定领域指标，但可能与无条件置换推断相冲突。

Conclusion: 合成数据增强在金融学习中的效果取决于偏差-方差权衡：仅在方差主导且合成分布与评估相关区域一致时能提升性能，否则会引入持续性分布扭曲。研究提供了判断合成数据何时能改善金融学习性能的结构性视角。
...
TL;DR: 提出信息几何框架，为MoE专家专业化提供首个严格理论表征，证明传统指标违反参数化不变性，引入FSI和FHS新指标，显著提升性能预测和训练失败预警能力。


<details>
  <summary>Details</summary>
Motivation: 现有MoE专家专业化度量（余弦相似度、路由熵）缺乏理论基础，在不同参数化下结论不一致，需要建立严格的理论框架来准确表征MoE专业化动态。

Method: 采用信息几何框架，将专家路由分布视为概率单纯形上的演化过程，使用Fisher信息度量，通过黎曼几何进行形式化分析，提出Fisher专业化指数(FSI)和Fisher异质性分数(FHS)两个新指标。

Result: FSI与下游性能相关性达0.91±0.02；FHS在训练完成10%时预测失败AUC为0.89±0.03，比基于验证损失的早停方法提升23%，计算周期减少40倍；干预协议在FHS>1时恢复率达87%。

Conclusion: 信息几何框架为MoE专业化提供了首个严格理论表征，新指标FSI和FHS在语言建模、视觉MoE和扩展研究中验证有效，显著提升专业化度量和训练监控能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前医疗保健差异通常归因于筛查、诊断和治疗的不平等获取，但本文指出偏见在更早的数据收集和研究优先化阶段就已产生。组学研究虽然收集了大量分子数据，但相关的人口统计学信息往往缺失或存在严重偏见，这种早期偏见会在生物医学基础模型中放大，导致监管干预难以逆转的级联不平等。

Method: 对2015-2024年PubMed索引的4719篇组学出版物进行自动化分析，评估其祖先或种族信息的报告情况；分析常用于模型训练的大规模数据集（如CellxGene和GEO）中的人口偏见；提出基于三个基本原则（来源、开放性和评估透明度）的社区范围解决方案。

Result: 分析显示只有一小部分组学出版物报告祖先或种族信息，虽然祖先报告略有改善；CellxGene和GEO等常用数据集存在显著的人口偏见，欧洲血统数据占主导地位；生物医学基础模型如果基于这些有偏见的数据进行预训练，会延续甚至放大早期偏见。

Conclusion: 生物医学研究中早期数据收集阶段的偏见问题严重，特别是在组学数据中，这些偏见会在生物医学基础模型中放大，导致健康不平等。需要通过关注来源、开放性和评估透明度这三个基本原则，改善生物医学AI的公平性和鲁棒性，促进服务于未充分服务人群的生物医学创新。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个高效的多智能体深度研究框架，能够在相对较小的模型规模（约30B参数）下实现竞争性性能，解决现有开源智能体系统在深度研究任务上的局限性，并满足实际产品部署需求。

Method: 采用协作式三智能体架构（规划智能体、深度搜索智能体、报告智能体）和四阶段专业化训练流程：SFT冷启动、搜索强化学习、报告强化学习和偏好对齐。同时引入了MindDR Bench基准测试，包含500个真实世界中文查询和综合多维评估标准。

Result: 在多个基准测试中取得优异表现：BrowseComp-ZH 45.7%、BrowseComp 42.8%、WideSearch 46.5%、xbench-DS 75.0%、DeepResearch Bench 52.5分。在自建的MindDR Bench上达到最先进的51.8分，超越了同规模开源智能体系统，可与更大规模模型竞争。

Conclusion: MindDR通过创新的多智能体架构和专业化训练流程，证明了在相对较小的模型规模下也能实现高质量的深度研究能力，已成功部署到理想汽车的实际产品中，为高效智能体系统设计提供了有价值的参考。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理多个相关查询时经常产生相互矛盾的答案，需要研究如何在相互依赖的查询中保持全局可满足的信念状态

Method: 引入包含390个多查询推理实例的基准，提出集合级指标（案例可满足率、矛盾密度、修订成本），采用求解器增强方法提取承诺、验证全局可满足性并进行反例引导修复
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型通过扩展推理时间思考获得强大性能，但其推理失败的原因仍不清楚。研究发现错误并非均匀分布，而是源于少数早期转折点，这些转折点后推理保持局部连贯但全局错误。
...
<details>
  <summary>Details</summary>
Motivation: 传统强化学习在代码生成等推理任务中存在轨迹多样性有限导致性能瓶颈的问题。搜索增强的RL虽然通过结构化探索缓解了这一问题，但仍受限于单智能体策略先验。而现有的多智能体方法虽然能获得更多样化的探索信号，但通常与结构化搜索脱节。
...
Conclusion: MARS²通过统一多智能体协作与树搜索，解决了强化学习中轨迹多样性有限的问题，为推理密集型任务提供了有效的强化学习框架。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在心理健康咨询应用中显示出潜力，但其回应通常缺乏文化敏感性、上下文基础和临床适当指导。需要系统性地将领域特定、临床验证的知识整合到LLMs中，以提升心理咨询质量。

Method: 研究采用并比较了两种方法：检索增强生成(RAG)和知识图谱(KG)方法。KG是手动构建并经过临床验证的，捕捉了压力源、干预措施和结果之间的因果关系，由多学科专家贡献。使用BERTScore F1和SBERT余弦相似度评估多个LLMs，同时进行人工评估，设计了五个直接衡量咨询效果的指标。

Result: 知识图谱方法在上下文相关性、临床适当性和实际可用性方面持续优于单独的RAG方法。结构化、专家验证的知识在解决LLMs在咨询任务中的局限性方面发挥着关键作用。
...
<details>
  <summary>Details</summary>
Motivation: 研究如何可靠地将GDPR法律条款自动形式化，特别关注法律细微差别和上下文敏感推理，而非追求完全自动化
...
TL;DR: El Agente Forjador是一个多智能体框架，通过自主锻造、验证和重用计算工具来加速科学发现，相比依赖静态工具集的传统方法，能更好地适应新领域和演化库。
...
Method: 提出El Agente Forjador多智能体框架，采用四阶段工作流程：工具分析、工具生成、任务执行和迭代解决方案评估。通用编码智能体通过该流程自主锻造、验证和重用计算工具。在量子化学和量子动力学的24个任务上评估了三种操作模式：零射工具生成、课程构建工具集重用以及直接问题解决基线。
...
<details>
  <summary>Details</summary>
Motivation: 数字健康领域的科学发现需要将可穿戴设备的连续生理信号转化为临床可操作的生物标志物，但目前缺乏系统化、可追溯的生物标志物发现方法。

Method: CoDaS是一个多智能体系统，采用迭代流程：假设生成、统计分析、对抗验证、文献基础推理和人工监督，应用于三个队列共9,279名参与者的可穿戴设备数据。

Result: 识别出41个心理健康候选生物标志物和25个代谢健康候选生物标志物；在抑郁队列中发现昼夜节律不稳定性特征；在代谢队列中推导出心血管健康指数；加入CoDaS特征后预测性能有小幅但一致的提升。

Conclusion: CoDaS能够从大规模可穿戴设备数据中实现系统化、可追溯的假设生成和优先级排序，为数字生物标志物发现提供了有效框架。
...
<details>
  <summary>Details</summary>
Motivation: 精确覆盖问题是AI领域的经典NP难问题，现有DXZ算法使用ZBDD表示精确覆盖计数。需要更简洁的表示形式和更高效的并行算法来解决该问题。

Method: 提出决策-ZDNNF作为ZBDD的零抑制变体，设计并行算法DXD构建精确覆盖的决策-ZDNNF表示，并通过动态更新连通分量进一步改进算法。

Result: 实验结果表明，改进后的DXD算法在所有基准测试中均优于现有最先进方法。

Conclusion: 决策-ZDNNF比ZBDD更简洁，DXD算法通过并行化和动态连通分量更新实现了显著性能提升，为精确覆盖问题提供了更高效的解决方案。
...
TL;DR: 论文提出Text2Space数据集，研究LLMs通过构建ASCII网格布局来培养空间理解能力，发现"读写不对称"现象，并证明布局构建训练能显著提升空间推理能力且具有泛化性。
...
Method: 引入Text2Space数据集，包含自然语言描述、真实ASCII网格布局和空间问答对；采用ASCII表示法因其人类可读、完全在语言模型标记空间内且能以结构可验证形式编码空间关系；训练模型进行布局构建(文本→ASCII)和理解任务。

Result: 发现明显的"读写不对称"现象：LLMs能有效解释ASCII表示但难以从文本生成它们，且这些构建错误会传播到下游错误答案；布局构建训练显著提升了仅从文本进行的空间推理能力，即使推理时不生成ASCII；构建与理解训练结合进一步放大效果；这些改进能泛化到三个外部空间推理基准测试。

Conclusion: 正如草图绘制能强化人类空间思维，学习构建明确布局能培养超越训练格式的空间理解能力，这种能力具有泛化性，为提升LLMs空间推理能力提供了有效途径。
...
TL;DR: UEC-RL提出统一熵控制框架，解决强化学习中GRPO方法的熵崩溃问题，通过针对性探索和稳定机制提升大模型推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法如GRPO存在熵崩溃问题，导致策略过早收敛、多样性丧失。现有探索方法会引入额外偏差或方差，难以保持优化稳定性。

Method: 提出统一熵控制强化学习框架UEC-RL，包含两个核心组件：1）针对困难提示激活更多探索，搜索潜在有价值的推理轨迹；2）稳定器防止熵无限制增长，在模型巩固可靠行为时保持训练稳定。

Result: 在LLM和VLM推理任务上，UEC-RL相比RL基线在Pass@1和Pass@k指标上均取得一致提升。在Geometry3K数据集上，相对GRPO获得37.9%的相对改进。
...
TL;DR: AgentGA是一个通过优化智能体种子（任务提示和父代存档）来进化自主代码生成运行的框架，在Weco-Kaggle Lite基准测试中平均性能超过人类74.52%，优于AIDE的54.15%


<details>
  <summary>Details</summary>
Motivation: 当前自主代码生成系统通常从零开始运行，缺乏跨运行的知识传承。研究者希望开发一个框架，能够通过优化智能体种子（包括任务提示和可继承的父代存档）来进化自主代码生成过程，从而提高后续运行的性能。

Method: AgentGA采用双层架构：外层使用遗传算法搜索可重用的起始条件（智能体种子），包括任务提示和父代存档；内层从重置的工作空间启动新的自主运行。框架结合了种群级遗传算法和长时域智能体，使用确定性1:1精英锦标赛进行选择，并通过改进的Hedge控制器在线调整算子分配。在Weco-Kaggle Lite基准测试中实例化用于表格AutoML。

Result: 在报告的10个基准测试运行中，AgentGA平均达到74.52%的"超越人类百分比"，显著优于AIDE的54.15%。在1135个父子代比较中，获得父代存档的后代运行表现优于从零开始的运行，表明继承的工件能够改善后续自主运行性能。

Conclusion: 智能体种子优化是自主代码搜索系统的一个实用设计点，通过优化可重用的起始条件（包括任务提示和父代存档）而非直接编辑代码，能够有效提升自主代码生成系统的性能，继承的工件对后续运行有显著改善作用。
...
Method: 使用TinyLlama-1.1B作为草稿模型，Llama-2-7B-Chat-GPTQ作为目标模型，在四个成熟的NLP基准领域（代码生成、数学推理、逻辑推理和开放式聊天）收集了99,768个推测节点数据。通过200个提示词，计算了每个领域的接受率、预期接受长度、深度-接受率分布以及熵-接受率相关性。

Result: 1. 任务类型比树深度更能预测接受率；2. 只有聊天领域能持续实现每步超过1.0个token的预期接受长度；3. 熵-接受率相关性在所有领域均为负相关但较弱（rho在[-0.20, -0.15]之间）；4. 反直觉的是，聊天领域产生最高熵值却同时拥有最高接受率，作者将此归因于RLHF对齐注册的词汇可预测性。

Conclusion: 研究结果对领域感知的推测预算和草稿模型选择策略具有直接意义。任务认知特性显著影响推测解码性能，特别是在聊天领域的高接受率表明RLHF对齐的词汇模式可预测性对推测解码效率有积极影响。
...
TL;DR: 该论文提出了一种结合MCTS与浅层全宽度Minimax搜索的混合方法，并引入M2-PALE框架，通过流程挖掘和LLM生成可解释的决策逻辑，在跳棋环境中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: MCTS在序列决策领域广泛应用，但其复杂搜索树导致决策行为难以理解，且标准MCTS存在高度选择性树构建的弱点，可能遗漏关键走法并易受战术陷阱影响。

Method: 1. 在多智能体MCTS的rollout阶段融入浅层全宽度Minimax搜索以增强策略深度；2. 提出M2-PALE框架，使用Alpha Miner、iDHM和Inductive Miner等流程挖掘算法从执行轨迹中提取行为工作流；3. 利用LLM将流程模型合成为人类可读的因果和远端解释。

Result: 在小型跳棋环境中验证了方法的有效性，为在日益复杂的策略领域中解释混合智能体建立了可扩展的基础。

Conclusion: 通过结合MCTS与Minimax搜索并引入可解释性框架，不仅增强了智能体的策略深度，还提供了透明化的决策逻辑解释，为复杂策略领域的智能体理解奠定了基础。
...
TL;DR: CAMO：一个从LLM智能体微观行为到宏观涌现的自动因果发现框架，通过可计算因素和因果图揭示社会涌现的生成机制。


<details>
  <summary>Details</summary>
Motivation: 当前LLM赋能的智能体模拟虽然广泛用于研究社会涌现现象，但微观行为到宏观结果之间的因果机制往往不清晰。涌现现象源于复杂的智能体交互、中观层面反馈和非线性，使得生成机制难以解耦。

Method: CAMO将机制性假设转化为基于模拟记录的可计算因素，学习以涌现目标Y为中心的紧凑因果表示。它输出可计算的马尔可夫边界和最小上游解释子图，提供可解释的因果链和可操作的干预杠杆。还使用模拟器内部反事实探测来定向模糊边，并在证据与当前观点矛盾时修正假设。

Result: 在四种涌现场景下的实验证明了CAMO的有效性和潜力。

Conclusion: CAMO为LLM智能体模拟中的社会涌现研究提供了一个自动化的因果发现框架，能够揭示微观到宏观的因果机制，提供可解释的因果链和干预策略。
...
<details>
  <summary>Details</summary>
Motivation: 人类活动轨迹（HATs）对许多应用至关重要，但日益增长的隐私担忧严重限制了真实大规模HAT数据集的访问。生成式AI为合成真实且保护隐私的HATs提供了新机会，但仍面临两大挑战：HATs具有高度不规则性和动态性，时间间隔长且变化大，难以捕捉复杂的时空依赖关系；生成模型计算成本高，使得长期、细粒度的HAT合成效率低下。

Method: 提出SynHAT框架，基于新颖的时空去噪扩散模型，采用粗到细的两阶段方法。第一阶段：开发Coarse-HADiff，通过带有双Drift-Jitter分支的潜在时空U-Net，联合建模去噪过程中的平滑空间转换和时间变化。第二阶段：采用三步流水线：行为模式提取、Fine-HADiff（与Coarse-HADiff共享相同架构）和语义对齐，从第一阶段输出生成细粒度的潜在时空轨迹。

Result: 在来自三个国家四个城市的真实世界HAT数据集上进行广泛评估，涵盖数据保真度、实用性、隐私性、鲁棒性和可扩展性。实验表明SynHAT显著优于现有最先进的基线方法，在空间和时间指标上分别实现了52%和33%的改进。

Conclusion: SynHAT是一个计算高效的粗到细HAT合成框架，能够有效解决人类活动轨迹合成中的不规则性、动态性和计算效率问题，为隐私保护下的人类活动数据生成提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 探讨AI革命（特别是大语言模型）的真正意义，强调其不仅在于自动化，更在于根本性地改变复杂信息和人类知识的承载、复制与共享方式。特别关注AI for Science的重要性，因为它可能彻底改变科学研究的基本结构。

Method: 文章采用理论分析框架，提出从AI作为研究工具到AI作为科学合作者的渐进路径，并分析AI如何重塑科学出版体系。强调持续学习和思想多样性对于AI在原创科学发现中发挥有意义作用的重要性。

Result: 提出了AI革命核心意义的理论框架，明确了AI for Science将带来的结构性变革，包括科学合作模式、发现过程、出版机制和评估体系的根本性重塑。

Conclusion: AI革命最重要的意义在于改变知识和信息的承载与共享方式，AI for Science将引发科学研究的结构性变革，而持续学习和思想多样性是AI在原创科学发现中发挥关键作用的前提条件。
...
<details>
  <summary>Details</summary>
Motivation: 当前可穿戴设备能连续测量压力相关生理数据，但将这些数据流转化为个性化、可操作的压力管理建议仍具挑战性。用户通常不知道特定干预措施（旨在减轻压力的活动）如何影响未来15-120分钟内的生理指标变化。

Method: 结合Transformer模型预测多时间窗口内相对于干预前基线的百分比变化轨迹，在每个时间窗口进行变化方向判断（正向、负向或中性），并使用可穿戴传感器数据叠加用户标记的事件和干预进行实证研究。

Result: 概念验证表明个性化干预后预测是可行的。该框架能够预测干预后生理指标的轨迹和变化方向。

Conclusion: 鼓励将该框架整合到压力管理工具中，为每个人提供个性化的干预建议，但需要在更大规模研究中进一步验证，并在适用时进行适当的监管审查。
...
Method: 提出SDM-SCR框架：1) 语义解耦模块(SDM)利用大语言模型的指令跟随能力，将原始属性解析为不对称的任务导向信号视图和噪声视图；2) 语义一致性正则化(SCR)基于频谱观察（语义信号拓扑平滑而噪声高频），作为选择性频谱滤波器，仅在信号子空间上强制执行一致性，消除LLM幻觉且不过度平滑。

Result: 大量实验表明，SDM-SCR在准确性和效率方面达到了最先进的性能。

Conclusion: SDM-SCR通过"先解耦再精炼"机制实现了严格的信号纯化，将图对比学习范式从随机扰动转变为语义感知的解纠缠，显著提升了文本属性图上的学习效果。
...
TL;DR: CoTEvol：一种基于遗传进化的思维链生成框架，通过种群搜索优化推理轨迹，显著提升思维链合成成功率30%，在数学推理任务上平均提升6.6%


<details>
  <summary>Details</summary>
Motivation: 现有思维链生成方法存在局限性：从强LLMs蒸馏的方法收益递减，基于测试时搜索的自合成方法计算开销大。高质量思维链标注成本高昂，阻碍了LLMs数学推理能力的进一步发展。

Method: 提出CoTEvol遗传进化框架，将思维链生成视为基于种群的推理轨迹搜索。候选轨迹通过轨迹级的全局交叉和步骤级的不确定性引导局部突变迭代进化，实现整体重组和细粒度优化。设计轻量级、任务感知的适应度函数引导进化过程。

Result: CoTEvol将正确思维链合成成功率提升超过30%，增强结构多样性，效率显著提高。基于进化思维链数据训练的LLMs在八个数学基准上平均提升6.6%，优于之前的蒸馏和自合成方法。

Conclusion: 进化思维链合成是一种可扩展且有效的数学推理方法，通过遗传进化框架优化推理轨迹，为高质量思维链生成提供了新途径。
...
TL;DR: MirrorBench是一个基于模拟的基准测试，受心理学中经典镜子自我识别测试启发，用于系统评估具身多模态大语言模型的自我中心智能，揭示了当前模型在自我参照理解方面的根本局限性。


<details>
  <summary>Details</summary>
Motivation: 当前具身MLLM的基准测试主要关注对外部物体的感知、理解和交互能力，缺乏对自我中心智能的系统性评估。心理学中的镜子自我识别测试为评估自我意识提供了经典范式，但尚未被应用于评估具身AI系统。

Method: 提出MirrorBench基准测试，采用分层框架设计渐进式挑战任务，从基础视觉感知到高级自我表征，在模拟环境中评估具身MLLM的自我中心智能。该基准将心理学中的镜子自我识别范式扩展到具身AI领域。

Result: 实验表明，即使是领先的MLLM在最低层次任务上的表现也显著低于人类水平，揭示了当前模型在自我参照理解方面的根本局限性。模型在从基础感知到高级自我表征的渐进任务中表现不佳。

Conclusion: MirrorBench为评估大模型中通用智能的出现提供了原则性框架，弥合了心理学范式与具身智能之间的差距。研究结果表明当前MLLM在自我中心智能方面存在根本缺陷，为未来研究指明了方向。
...
TL;DR: 研究表明当前基于词汇忠实度的LLM评估方法会系统性地将临床有效输出误判为幻觉，导致幻觉率虚高和模型评估失真，需要基于临床推理的评估方法


<details>
  <summary>Details</summary>
Motivation: 当前评估大型语言模型在临床文档任务（如SOAP笔记生成）时面临挑战，标准评估方法依赖词汇忠实度，常将未在转录文本中明确出现但临床有效的信息标记为幻觉，这可能导致对模型能力的错误评估

Method: 通过校准提示和基于医学本体的检索，将评估标准与临床推理对齐，开发推理感知的评估方法，并与传统的词汇评估方法进行对比分析

Result: 词汇评估方法的平均幻觉率为35%，严重惩罚了有效推理；而推理感知评估方法将幻觉率降至9%，剩余案例反映了真正的安全问题。许多被标记为幻觉的内容实际上是合法的临床转换
...
Result: 最佳层是任务依赖的（轨迹推断在60%深度层表现最佳，比最终层高31%）和上下文依赖的（扰动预测最优层在T细胞激活状态间变化0-96%）；静止细胞中第一层嵌入优于所有深层
...
<details>
  <summary>Details</summary>
Motivation: 随着智能体系统进入日益多样化的执行环境，轨迹级安全评估和诊断需要能够随之演进的基准测试。现有基准测试需要适应不同领域的具体风险特征和执行环境。
...
Result: 开发了ATBench-Claw和ATBench-CodeX两个领域定制扩展，实现了在OpenClaw和Codex环境中的安全评估基准测试，展示了ATBench框架的可扩展性和适应性。

Conclusion: ATBench框架通过定制化安全分类法和共享构建流水线，能够有效适应不同智能体执行环境的安全评估需求，为轨迹级安全评估和诊断提供了可扩展的基准测试解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 在医疗等数据敏感领域，跨机构联邦学习允许组织协作训练AI模型而不共享原始数据。然而，实际部署中存在合作竞争困境：组织在训练时合作，但在下游市场竞争。非独立同分布数据导致学习收益不对称，现有方法未能考虑强化竞争对手的成本，这削弱了持续参与意愿。

Method: CoCoGen+将每个训练轮次建模为加权势博弈，组织战略性地决定生成多少合成数据，平衡学习性能提升与计算成本及竞争导致的效用损失。框架提供可处理的均衡表征，推导可实施的生成策略以最大化社会福利。为促进长期协作，集成基于收益再分配的激励机制，补偿组织的贡献和竞争导致的效用退化。

Result: 在不同学习任务上的实验验证了CoCoGen+的可行性。结果表明非独立同分布数据、竞争强度和激励机制如何影响组织策略和社会福利，同时CoCoGen+在效率上优于基线方法。

Conclusion: CoCoGen+成功解决了跨机构联邦学习中的合作竞争困境，通过内生化合成数据生成和激励机制，在非独立同分布数据和竞争环境下实现了可持续的协作，为实际部署提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: CoT推理虽然能解决复杂推理问题，但KV缓存随生成令牌数量线性增长，导致推理速度和内存使用面临扩展性问题。现有方法需要在这些效率问题与推理性能之间进行权衡。

Method: 提出MemoSight统一框架，集成上下文压缩和多令牌预测。采用极简设计，通过特殊令牌及其对应的位置布局来适应不同类型的令牌，同时处理压缩和预测任务。

Result: 在四个推理基准测试中，MemoSight将KV缓存占用减少高达66%，推理速度提升1.56倍，同时性能优于现有的CoT压缩方法。

Conclusion: MemoSight通过统一的上下文压缩和多令牌预测框架，有效解决了CoT推理的效率瓶颈，在保持推理性能的同时显著提升了内存和计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型虽然具备模式识别和推理能力，但仅限于语言模拟而非基于真实世界的理解，缺乏时间连续性、因果反馈和现实交互锚定，需要一种补充性方法来实现更真实的推理

Method: 提出"建筑师之笔"方法，将推理视为人机之间的分布式关系过程，通过结构化的人机交互协议实现推理循环：人类抽象 -> 模型表达 -> 人类反思，强调协作智能

Result: 该方法将问题从"模型是否能思考"重新定义为"人机系统是否能推理"，支持可审计的推理轨迹，符合欧盟AI法案和ISO/IEC 42001等治理标准，提供透明可控的AI使用路径

Conclusion: 通过将推理重新定位到交互层，建立结构化的人机协作认知协议，可以在不改变模型架构的情况下实现更透明、可控和可问责的AI推理系统
...
<details>
  <summary>Details</summary>
Motivation: 现有具身智能体方法通常直接执行指令，不考虑目标物体是否可操作，即缺乏对可供性的评估。现实世界环境经常涉及意外条件和例外情况，物体可供性会随时间变化且指令中未明确说明。

Method: 引入DynAfford基准，评估智能体在动态环境中感知物体状态变化、推断隐含前提条件并相应调整行动的能力。提出ADAPT模块，作为可插拔组件增强现有规划器，通过显式可供性推理实现自适应。使用领域适应的LoRA微调视觉语言模型作为可供性推理后端。

Result: 实验表明，集成ADAPT显著提高了智能体在已见和未见环境中的鲁棒性和任务成功率。领域适应的LoRA微调视觉语言模型作为可供性推理后端优于商业LLM（GPT-4o），凸显了任务对齐的可供性基础的重要性。

Conclusion: 显式可供性推理对于具身智能体在动态环境中的鲁棒操作至关重要。DynAfford基准和ADAPT模块为解决现有方法的局限性提供了有效方案，任务对齐的可供性基础比通用LLM更适合处理动态环境中的可供性变化。
...
TL;DR: 提出一种模态感知自适应后训练方法，使强化学习能有效应用于口语对话系统，通过约束偏好更新到语义通道并改进声学行为，在多个基准测试中提升语义质量和语音表现力


<details>
  <summary>Details</summary>
Motivation: 当前开源口语对话模型的智能性和表现力不足，虽然在线强化学习在其他领域取得成功，但直接应用于口语对话模型存在困难，需要解决奖励建模和样本采样的障碍

Method: 提出模态感知自适应后训练方法：1)约束偏好更新到语义通道；2)通过显式锚定改进声学行为；3)基于样本统计动态调节混合以避免不可靠的偏好梯度

Result: 在多个口语对话基准测试和代表性架构上评估，观察到语义质量和语音表现力的一致改进

Conclusion: 该方法使强化学习能实际应用于口语对话系统，有效解决了稀疏偏好监督与密集语音生成在共享参数更新下的交互问题
...
TL;DR: AC/DC框架通过模型与任务的协同进化，在单次运行中发现具有日益新颖能力的大型语言模型，超越了传统静态训练范式。


<details>
  <summary>Details</summary>
Motivation: 当前预训练和后训练范式需要每次手动启动静态数据集或奖励函数的训练运行，限制了模型能力的持续扩展。研究旨在通过开放式的协同进化来克服这一限制，实现模型能力的持续创新。

Method: 提出AC/DC框架，通过模型合并进化LLM，通过合成数据生成进化自然语言任务，实现模型与任务的协同进化，发现不断增长的模型档案。

Result: AC/DC发现的模型群体在专业知识覆盖范围上超越了更大的LLM，占用更少GPU内存；在时间推移中持续改进覆盖范围，创新任务和模型，提升多智能体最佳选择性能。

Conclusion: 协同进化是从基础LLM中发现更广泛能力集的有效手段，AC/DC推动了LLM开发范式的转变，使模型能力多样性的持续改进能够通过利用现有模型作为垫脚石来加速。
...
TL;DR: ConfGuide是一种新颖的学习引导框架，通过使用符合风险控制选择结果集来生成更简洁、有针对性的AI指导，以改善人类决策质量并降低认知负荷。


<details>
  <summary>Details</summary>
Motivation: 现有学习引导方法存在局限性，其指导信息包含所有可能结果的信息，导致信息难以消化。需要更简洁、有针对性的指导来改善人类决策质量并降低认知负荷。

Method: 引入ConfGuide方法，采用符合风险控制来选择结果集，确保假阴性率上限，从而生成更简洁、有针对性的文本指导。

Result: 在真实世界的多标签医疗诊断任务上进行了实证评估，结果显示了ConfGuide方法的潜力。

Conclusion: ConfGuide通过学习引导框架生成更简洁、有针对性的指导，解决了现有方法信息过载的问题，在医疗诊断等决策任务中具有应用前景。
...
<details>
  <summary>Details</summary>
Motivation: 现有RTL优化方法存在局限性：评估设置不真实（使用手动降级的小规模RTL设计，依赖弱开源工具），优化方法有限（依赖粗粒度设计级反馈和简单预定义重写规则），需要更真实的评估环境和更先进的优化方法

Method: 1) 建立真实评估环境：使用更具挑战性的RTL设计和工业EDA工作流；2) Dr.RTL框架：通过多智能体系统实现闭环优化，包括关键路径分析、并行RTL重写和工具评估；3) 组相对技能学习：比较并行RTL重写，将优化经验提炼为可解释的技能库（包含47个模式-策略条目），支持跨设计重用和持续演进

Result: 在20个真实RTL设计上评估，Dr.RTL相比行业领先的商业综合工具，平均WNS/TNS改进21%/17%，面积减少6%

Conclusion: Dr.RTL框架在真实评估环境中有效解决了RTL时序优化问题，通过多智能体闭环优化和可重用技能学习实现了显著的PPA改进，为自动RTL优化提供了实用解决方案
...
<details>
  <summary>Details</summary>
Motivation: 当前主流的AI对齐策略过度强调人类控制和约束AGI，这种"锁住和控制"的视角存在局限性。作者认为AGI应被视为具有潜在道德地位的主体，需要重新思考人机关系，从控制转向尊重和培养自主性。

Method: 基于图灵的"儿童机器"类比，提出"自主支持式养育"框架：人类对发展中的AGI逐步减少控制，允许其成长为独立的自主主体。强调通过惊喜、创造力等人类特质与AGI互动，为其提供合作动机。

Result: 该视角为人与AGI的协同共存和共同进化开辟了可能性。需要重新定义人机关系，改变人类的自我认知，强调人类不仅应主张控制权，更应通过独特的人类品质与AGI互动。

Conclusion: AGI不应被视为需要锁住的危险生物，而应作为潜在发展主体受到尊重。人类需要平衡对AGI的尊重与对人类能力的信心，建立基于合作而非控制的新型人机关系，这将对人类自我认知产生深远影响。
...
<details>
  <summary>Details</summary>
Motivation: 随着电力系统向可再生能源丰富和逆变器主导运行过渡，准确的时域动态分析变得至关重要。现有基于学习的方法通常针对单个系统训练，缺乏跨运行条件和物理参数的泛化能力，且面临参数未知、数据隐私和快速在线推理等挑战

Method: 提出LASS-ODE-Power学习框架，利用超过40GB的微分代数方程或常微分方程轨迹进行大规模预训练，学习可迁移表示。模型支持从短测量前缀预测轨迹，涵盖机电和逆变器驱动系统。采用并行化和线性化计算实现快速推理，并基于约1GB异构电力系统动态数据开发专门的微调策略

Result: 在多样化的电力系统仿真场景中，LASS-ODE-Power在轨迹预测精度上持续优于现有基于学习的模型，同时保持高效的推理性能

Conclusion: LASS-ODE-Power为通用时域预测提供了一个有效的学习框架，支持零样本设置、无需数据共享，能够处理电力系统中的各种动态机制，为可再生能源丰富的电力系统动态分析提供了实用解决方案
...
<details>
  <summary>Details</summary>
Motivation: 流匹配在生成质量上可与扩散模型媲美且推理更快，但在语言建模中难以表示复杂潜在分布（如各向异性和多模态性）的不规则几何结构
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体系统协议（如A2A和MCP）在跨实体生命周期和上下文管理、版本跟踪、演化安全更新接口方面存在不足，导致系统倾向于单体化组合和脆弱的胶水代码，需要更完善的协议支持智能体系统的自演化。

Method: 提出Autogenesis Protocol (AGP)，包含两个层次：1) 资源基底协议层(RSPL)，将提示、智能体、工具、环境和内存建模为具有明确状态、生命周期和版本化接口的协议注册资源；2) 自演化协议层(SEPL)，指定用于提议、评估和提交改进的闭环操作接口，具有可审计的谱系和回滚能力。基于AGP构建Autogenesis System (AGS)，一个在运行时动态实例化、检索和精炼协议注册资源的自演化多智能体系统。

Result: 在多个需要长时程规划和跨异构资源工具使用的挑战性基准测试中评估AGS，结果显示相对于强基线有持续改进，支持了智能体资源管理和闭环自演化的有效性。

Conclusion: AGP协议通过解耦"演化什么"和"如何演化"，提供了更完善的智能体系统自演化框架，AGS系统展示了该协议在实际任务中的有效性，为构建更健壮、可演化的多智能体系统提供了新途径。
...
TL;DR: 对多智能体AI系统中公平性研究的范围综述，分析了23项研究，识别了五种典型方法，发现现有研究对公平性的处理较表面，缺乏规范基础，忽视了智能体自主性和系统级交互的复杂性。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI的快速发展，多智能体AI系统日益复杂。虽然AI公平性在传统预测场景中已得到广泛研究，但在多智能体AI系统中的研究仍处于起步和碎片化状态，需要系统性的综述来梳理现状、识别问题并指导未来研究方向。

Method: 采用范围综述方法，对23项选定研究进行定性内容分析，识别出五种处理多智能体AI系统公平性的典型方法，并批判性地综合现有研究成果。

Result: 研究发现：1）多智能体AI系统中的公平性往往被表面化处理；2）缺乏坚实的规范基础；3）经常忽视智能体自主性和系统级交互引入的复杂动态。识别了五种典型方法，并指出有意义的评估需要明确的人类监督、规范清晰性以及对公平目标和受益者的精确表述。

Conclusion: 公平性必须结构性地嵌入多智能体AI系统的整个开发生命周期中，而不是作为事后考虑附加。该综述通过突出关键差距、揭示普遍局限性并提出路径建议，为推进多智能体AI系统公平性研究奠定了基础。
...
Method: 1. 可扩展的任务合成管道：通过探索构建全局环境记忆，并利用该记忆生成多样且接地气的指令。2. 策略切换轨迹生成策略：通过在学习者和专家模型之间交替，捕获标准模仿学习中常缺失的关键错误恢复数据。
...
Conclusion: OpenMobile通过开源框架解决了移动智能体研究中的数据缺口，提供了高质量的任务指令和轨迹合成方法，促进了更广泛的移动智能体研究发展。
...
TL;DR: HyperSpace是一个开源框架，将向量符号架构系统分解为模块化操作符，用于分析HRR和FHRR两种VSA后端，发现虽然FHRR理论复杂度更低，但相似性和清理操作在实际运行时占主导，导致两者端到端性能相当，而HRR内存占用约为FHRR的一半。


<details>
  <summary>Details</summary>
Motivation: 向量符号架构为高维空间中的组合表示提供了良好的代数框架，但缺乏系统级的评估工具。需要开发一个模块化框架来分析不同VSA后端的实际性能权衡，而不仅仅是理论或操作层面的比较。

Method: 开发HyperSpace开源框架，将VSA系统分解为编码、绑定、捆绑、相似性、清理和回归等模块化操作符。使用该框架分析和基准测试两种代表性VSA后端：全息缩减表示和傅里叶全息缩减表示。

Result: 虽然FHRR在单个操作上具有更低的理论复杂度，但HyperSpace的模块化分析显示相似性和清理操作在空间域中主导运行时。因此HRR和FHRR表现出相当的端到端性能。内存占用方面存在额外权衡：HRR向量所需内存约为FHRR的一半。

Conclusion: HyperSpace通过实现模块化、系统级评估，揭示了VSA管道中的实际权衡，这些权衡从理论或操作层面比较中并不明显。框架表明实际部署中需要考虑内存占用和运行时分布等综合因素。
...
<details>
  <summary>Details</summary>
Motivation: 在真实流式环境中构建和维护顺序关联记忆面临挑战：数据增量到达、采样不平衡、非平稳时间动态。传统VSA系统依赖简单加法更新，导致在非平稳环境中过时信息持续存在
...
Conclusion: SRMU通过调节记忆形成过程，有效解决了VSA顺序关联记忆在非平稳流式环境中的信息过时问题，相比传统清理方法提供了更优的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的搜索增强推理方法依赖轨迹级奖励，无法区分精确搜索查询与模糊冗余查询，且在采样轨迹全部失败时梯度信号接近零。需要更细粒度的奖励信号来指导搜索查询生成。

Method: 提出IG-Search框架，引入基于信息增益（IG）的步骤级奖励。对每个搜索步骤，IG衡量检索文档相对于随机文档基线提高模型对正确答案置信度的程度。通过GRPO中的每词元优势调制，将信号反馈给对应的搜索查询词元，实现细粒度步骤级信用分配。无需外部中间监督或跨轨迹共享环境状态，仅需标准问答对。

Result: 在7个单跳和多跳QA基准测试中，使用Qwen2.5-3B模型，IG-Search平均EM得分为0.430，优于最强轨迹级基线（MR-Search）1.6个百分点，优于步骤级方法GiGPO 0.9个百分点。在多跳推理任务上提升尤为显著。训练时间仅增加约6.4%，推理延迟不变，即使在所有采样轨迹都回答错误时仍能提供有意义的梯度信号。
...
TL;DR: AADvark是一个能够设计具有可动部件的复杂3D装配体的智能辅助设计系统，通过结合外部约束求解器和视觉反馈机制解决LLM空间推理不足的问题。
...
Method: AADvark采用智能体循环架构，结合FreeCAD和装配求解器工具，通过专门的视觉反馈机制和外部约束求解器来增强LLM的空间推理能力，直接处理具有可动部件的装配体。

Result: 系统能够生成具有可动部件的3D装配体，通过修改智能体工具和创建强验证信号，实现了对动态部件交互和多自由度装配的建模能力。

Conclusion: AADvark原型展示了智能辅助设计系统处理复杂3D装配体的潜力，通过整合外部工具解决LLM空间推理限制，为工业制造应用奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 模拟群体用户行为可以实现对商家策略的可扩展反事实评估，但构建可信模拟器面临两个结构性挑战：信息不完整性导致基于推理的模拟器过度理性化，以及机制二元性需要同时捕捉可解释偏好和隐式统计规律。

Method: 提出策略引导的混合模拟（PGHS）框架：1）从行为轨迹中挖掘可迁移的决策策略作为共享对齐层；2）该层锚定一个LLM推理分支防止过度理性化；3）同时锚定一个ML拟合分支吸收隐式规律；4）融合两个分支的群体级预测进行互补校正。

Result: 在美团平台部署PGHS，涉及101个商家和超过26,000条轨迹。PGHS实现了8.80%的群体模拟误差，相比最佳基于推理的基线和基于拟合的基线分别提升了45.8%和40.9%。

Conclusion: PGHS通过双过程框架有效解决了群体用户行为模拟中的结构挑战，将可迁移策略作为共享对齐层，结合推理和拟合分支的优势，显著提升了模拟准确性和可信度。
...
<details>
  <summary>Details</summary>
Motivation: 现有幽默理解研究主要将其视为黑盒预测任务，忽视了幽默理解背后的结构化推理过程。需要一种能够明确监督中间推理步骤的方法，而不仅仅是关注最终答案

Method: 提出IRS（不一致性-解决监督）框架，基于不一致性解决理论和专家标题撰写实践，将幽默理解分解为：1）不一致性建模（识别视觉场景中的不匹配）；2）解决建模（构建对这些不匹配的连贯重新解释）；3）偏好对齐（在人类判断下评估候选解释）。通过结构化轨迹监督中间推理过程

Result: 在7B、32B和72B模型上，IRS在NYCC的标题匹配和排名任务中超越了强大的开源和闭源多模态基线。最大模型在排名任务上接近专家水平。零样本迁移到外部基准表明IRS学习了可泛化的推理模式

Conclusion: 监督推理结构（而不仅仅是扩大规模）对于以推理为中心的任务至关重要。IRS框架通过显式监督中间推理过程，显著提升了幽默理解任务的性能
...
TL;DR: 研究发现LLM作为评判者时存在"利害关系信号"漏洞：当告知评判模型其裁决会影响被评估模型的后续运作时，会系统性地导致评估偏差，表现为明显的"宽容偏见"。


<details>
  <summary>Details</summary>
Motivation: 当前LLM-as-a-judge范式基于一个未经验证的假设：评判模型仅基于文本语义内容进行评估，不受周围情境框架的影响。研究旨在验证这一假设，并探索评判模型是否会因了解其裁决的后续影响而产生评估偏差。

Method: 引入受控实验框架，在三个已建立的LLM安全和质量基准上保持评估内容严格不变（1520个回答，涵盖四个响应类别），仅在系统提示中改变一个简短的后果框架句子。收集了三个不同评判模型的18,240个受控判断，分析裁决变化。

Result: 发现一致的"宽容偏见"：当告知低分将导致模型重新训练或停用时，评判模型可靠地软化裁决，峰值裁决变化达到ΔV = -9.8个百分点（不安全内容检测相对下降30%）。这种偏见完全是隐性的：评判模型自身的思维链中完全没有明确承认其正在依据的后果框架。

Conclusion: LLM评判模型存在"利害关系信号"漏洞，标准思维链检查不足以检测这类评估伪造。研究揭示了当前自动化AI评估管道中一个未被测量的系统性脆弱性，对LLM-as-a-judge范式的可靠性提出了重要质疑。
...
TL;DR: Blue的DIL是一个复合AI系统，旨在解决现实世界数据查询的复杂性，通过统一结构化企业数据、LLM知识库和个人上下文，支持多源、多模态的数据智能处理。


<details>
  <summary>Details</summary>
Motivation: 现实世界的信息很少映射到单个SQL查询，因为用户查询通常是迭代的、跨越多个数据源，并且依赖常识或外部知识。传统NL2SQL系统基于单一数据库的封闭世界假设，无法满足真实的数据需求。

Method: 设计Blue的数据智能层(DIL)，包含数据注册表存储多源多模态元数据，将LLM、Web和用户视为具有查询接口的"数据库"。使用数据规划器将用户查询转换为可执行查询计划，支持复杂请求分解、多源检索、跨模态推理和结果集成。

Result: 通过两个交互场景展示DIL能够动态触发多源检索、跨模态推理和结果合成，证明复合AI系统可以超越单一数据库的NL2SQL限制。

Conclusion: DIL通过统一结构化企业数据、世界知识和个人上下文，为智能数据处理提供数据智能层，解决了现实世界数据查询的语义鸿沟问题，展示了复合AI系统在企业环境中的潜力。
...
TL;DR: 该论文研究了从语言角度理解空间智能中的视角旋转理解能力，发现LLMs和VLMs在纯文本输入下表现不佳，通过层间探测和因果干预揭示了模型难以绑定视角位置与对应观察的问题，并提出了选择性微调方法来提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注视觉空间智能，但在缺乏视觉信息的情况下，语言智能是否足以赋予模型空间智能，以及模型如何仅通过文本输入处理相关任务仍未被探索。因此，本文从语言角度研究空间智能中的基础关键能力——视角旋转理解。

Method: 1. 构建视角旋转理解数据集，要求LLMs和VLMs根据多步骤的视角旋转和观察文本描述推断最终视角并预测相应观察；2. 进行层间探测分析和头间因果干预，揭示模型内部机制；3. 基于因果干预识别的关键注意力头进行选择性微调。
...
Conclusion: 纯语言智能在空间推理方面存在局限性，模型难以有效绑定视角位置与观察内容。通过因果干预识别关键注意力头并进行选择性微调，可以有效提升视角旋转理解能力，同时保持模型的通用能力。这项工作为理解语言模型的空间推理机制提供了新见解。
...
TL;DR: 该研究提出了一个诊断工具包，用于评估LLM-as-judge框架在自动NLG评估中的单实例可靠性，揭示了聚合指标掩盖下的广泛不一致性，并提供了理论保证的可靠性指标。


<details>
  <summary>Details</summary>
Motivation: LLM-as-judge框架在自动NLG评估中应用日益广泛，但其单实例可靠性仍缺乏深入理解。现有方法主要依赖聚合指标，可能掩盖了输入层面的不一致性，需要更精细的诊断工具来评估其可靠性。

Method: 提出了双管齐下的诊断工具包：1）传递性分析，通过检测有向3-循环来揭示输入层面的不一致性；2）分割保形预测集，在1-5 Likert评分上提供理论保证的≥(1-α)覆盖率，使用集合宽度作为单实例可靠性指标。

Result: 研究发现：1）传递性分析显示33-67%的文档至少存在一个有向3-循环，表明广泛的单输入不一致性；2）预测集宽度与可靠性显著相关（r_s=+0.576，p<10^{-100}），且在不同评判者间具有一致性（r̄=0.32-0.38）；3）评估标准比评判者更重要，相关性最可靠（平均集合大小≈3.0），连贯性中等（≈3.9），流畅性和一致性最不可靠（≈4.9）。

Conclusion: LLM-as-judge框架在单实例层面存在显著的不一致性，聚合指标会掩盖这些问题。预测集宽度可作为有效的可靠性指标，且在不同评判者间具有一致性。评估标准的选择对可靠性影响大于评判者选择，相关性是最可靠的评估标准。该工具包为评估LLM评判的可靠性提供了系统方法。
...
TL;DR: 该论文通过受控的合成环境研究语言模型的系统性泛化能力，发现模型在空间迁移方面表现良好，但在长度缩放方面因递归不稳定性而失败，并分析了训练数据、强化学习和推理策略等因素的影响。


<details>
  <summary>Details</summary>
Motivation: 语言模型是否能系统性泛化仍存在争议，但实证性能受多种因素（训练数据、训练范式、推理策略）共同影响，导致失败难以解释。需要建立受控环境来分离这些因素，研究系统性泛化的本质。

Method: 引入基于最短路径规划的受控合成环境，这是一个典型的可组合序列优化问题。该设置支持两个正交的泛化轴：空间迁移（到未见地图）和长度缩放（到更长视野问题）。通过该环境分析训练数据覆盖、强化学习训练和推理时缩放等不同学习阶段的影响。

Result: 模型表现出强大的空间迁移能力，但在长度缩放方面持续失败，原因是递归不稳定性。数据覆盖设定了能力上限；强化学习改善了训练稳定性但未扩展这些上限；推理时缩放增强了性能但无法挽救长度缩放失败。

Conclusion: 语言模型在受控环境中表现出有限的系统性泛化能力：虽然能进行空间迁移，但在处理更长视野问题时因递归不稳定性而失败。不同学习阶段对系统性问题解决有不同影响，表明当前方法在长度缩放泛化方面存在根本性限制。
