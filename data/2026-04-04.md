...
TL;DR: 结构化LLM路由本质上是系统级负载分配问题，而非单纯的提示工程问题。研究发现没有通用的最佳路由模式，性能主要由后端特定的交互效应决定。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型成为智能AI系统的核心控制组件，可靠的结构化路由必须在实际部署约束下平衡正确性、延迟和实现成本。当前研究过于关注提示工程，而忽略了更根本的系统级负载分配问题。

Method: 通过全面的全因子基准测试，评估48种部署配置和15,552个请求，涵盖OpenAI、Gemini和Llama后端。研究比较了三种结构分配方式：模型直接输出、传输中压缩、生成后本地重建。

Result: 没有通用的最佳路由模式，性能主要由后端特定的交互效应决定。在Gemini和OpenAI上可靠的路由模式在Llama上可能出现显著的正确性下降，压缩实现的效率增益也强烈依赖于后端。
...
<details>
  <summary>Details</summary>
Motivation: 传统因果推断方法依赖于不可观测的反事实假设（如可忽略性、平行趋势等），这些假设无法直接验证。本文旨在建立一个框架，将因果推断从纯粹的假设依赖转向部分可验证的模拟方法。

Method: 提出数字孪生反事实框架(DTCF)：1) 将数字孪生模拟器形式化为潜在结果框架内的随机映射；2) 建立孪生保真度假设层次结构（边际保真度、联合保真度、结构保真度）；3) 设计五层验证架构将不可证伪的模拟器主张转化为可检验的数据测试；4) 形式化分解将因果量区分为可边际验证和copula依赖两类。

Result: 建立了系统的因果推断验证框架：1) 边际因果主张（ATE、CATE、QTE）可通过可观测臂比较进行检验；2) 联合因果主张（ITE分布、获益/损害概率、处理效应方差）明确依赖于不可观测的个体内依赖结构；3) 提供了边界、敏感性和不确定性量化工具来显式处理copula依赖。

Conclusion: DTCF不解决因果推断的根本问题，但提供了一个框架：使边际因果主张变得可检验，使联合因果主张明确依赖于假设索引，并形式化表征了两者之间的差距。该框架将因果推断从纯粹的假设依赖转向部分可验证的模拟方法。
...
TL;DR: IDEA2是一个基于大语言模型的半自动化工作流，通过专家在环的协作过程，解决本体工程中能力问题(CQ)提取的资源密集型瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 能力问题提取是本體工程中的关键但资源密集型瓶颈阶段，存在领域专家（拥有知识）和本体工程师（形式化知识）之间的沟通鸿沟，需要更高效、协作的解决方案。

Method: IDEA2采用半自动化工作流，整合大语言模型和专家在环协作过程：1）从需求文档中LLM初步提取CQs；2）领域专家在协作平台上进行共同创建式评审和反馈；3）LLM迭代重新制定被拒绝的CQs直至达成共识；4）使用溯源模型跟踪每个CQ的完整生命周期。

Result: 在两个真实场景（科学数据、文化遗产）中验证，IDEA2能够加速需求工程过程，提高最终CQs的接受度和相关性，在领域专家中表现出高可用性和有效性。

Conclusion: IDEA2通过整合LLM和专家协作，有效解决了CQ提取的瓶颈问题，提供了透明、可复现的工作流，显著提升了本体工程需求阶段的效率和质量。
...
<details>
  <summary>Details</summary>
Motivation: 传统智能体中心架构在企业和制度系统等结构化领域中存在语义一致性、可解释性和长期稳定性方面的不足，需要一种共享的显式世界表示来支持多智能体系统的协调运作。

Method: 提出世界中心多智能体系统架构，通过维度分类（本体显式性、规范性等）来刻画世界表示；使用语义模型作为表示世界的数学形式化方法；开发Ontobox平台作为WMAS的具体实现。

Result: 建立了世界中心多智能体系统的理论框架，提出了语义模型作为形式化表示方法，并实现了Ontobox平台作为该架构的具体实例。

Conclusion: 世界中心多智能体系统为结构化领域中的多智能体协调提供了更优的架构选择，通过共享世界模型确保全局一致性、可验证性和长期稳定性，具有重要的理论和实践意义。
...
TL;DR: 研究发现AI自动化呈现"涨潮"模式而非"浪潮"模式，AI能力在广泛任务上持续提升，预计到2029年LLM能以80%-95%成功率完成大多数文本任务


<details>
  <summary>Details</summary>
Motivation: 研究旨在理解AI自动化的本质模式，区分"浪潮"式（特定任务能力突增）和"涨潮"式（广泛任务能力持续提升）两种自动化模式，为AI对经济和劳动力市场的影响提供实证基础

Method: 基于美国劳工部O*NET分类体系，构建了3000多个文本型任务的评估框架，收集了来自相关工作岗位的17000多个评估结果，通过系统化测试分析AI在不同任务上的表现和进步趋势

Result: 发现AI自动化主要表现为"涨潮"模式而非"浪潮"模式；2024年第二季度AI能以约50%成功率完成人类需3-4小时的任务，预计到2025年第三季度提升至65%；若趋势持续，到2029年LLM在大多数文本任务上的平均成功率可达80%-95%

Conclusion: AI能力提升呈现广泛而持续的特征，将对经济和劳动力市场产生深远影响，但组织层面的AI采用可能需要更长时间；研究为理解AI自动化进程提供了重要实证依据
...
TL;DR: 该研究探讨了LLM中的认知偏差，建立了LLM CogBias基准，发现偏差在激活空间中呈线性可分，并通过激活导向技术实现了偏差减少，同时保持下游能力。


<details>
  <summary>Details</summary>
Motivation: LLM越来越多地应用于高风险决策场景，先前研究表明LLM在行为上表现出认知偏差，但这些偏差是否对应可识别的内部表征以及能否通过针对性干预来缓解，仍是一个开放性问题。

Method: 1) 定义LLM认知偏差为系统性的、可复现的偏离正确答案的行为；2) 引入LLM CogBias基准，围绕四个认知偏差家族组织：判断偏差、信息处理偏差、社会偏差和反应偏差；3) 评估三个LLM；4) 使用对比设计下的线性探针分析偏差在激活空间中的表征；5) 应用激活导向技术来调节偏差行为。

Result: 1) 认知偏差在所有四个家族中系统性出现，其幅度和去偏差响应强烈依赖于家族：提示级去偏差显著减少反应偏差，但对判断偏差产生反效果；2) 偏差在模型激活空间中编码为线性可分方向；3) 激活导向使偏差分数（偏差响应的比例）减少26-32%，同时在25个基准测试中保持下游能力（Llama：性能下降可忽略；Qwen：判断偏差最多下降19.0个百分点）；4) 尽管不同模型间的偏差表征近乎正交（平均余弦相似度0.01），但导向在不同架构中以相似速率减少偏差，表明存在共享的功能组织。

Conclusion: LLM认知偏差在激活空间中具有可识别的线性表征，可以通过激活导向技术有效减少，同时保持模型能力。不同模型间偏差表征的差异性与去偏差效果的相似性，揭示了LLM中可能存在的共享功能组织，为理解和缓解LLM中的系统性偏差提供了新途径。
...
TL;DR: 本文提出了RIFT（RubrIc Failure mode Taxonomy），一个用于系统化描述评分标准（rubric）设计失败模式的分类法，包含8种失败模式，分为可靠性、内容效度和结果效度三大类。
...
Method: 采用扎根理论方法，通过迭代标注来自五个不同基准测试（通用指令遵循、代码生成、创意写作、专家级深度研究）的评分标准，直到不再发现新的失败模式，从而开发出RIFT分类法。评估了分类法的一致性，并提出了自动评分标准质量指标。

Result: 开发了包含8种失败模式的RIFT分类法，分为三大类。人类标注者间一致性达到87%的成对一致性和0.64的平均Cohen's kappa。自动指标与人工失败模式标注对齐，F1分数最高达到0.86。

Conclusion: RIFT为系统化诊断评分标准质量问题提供了框架，自动指标支持可扩展的诊断，有助于改进LLM评估和训练中的评分标准设计。
...
<details>
  <summary>Details</summary>
Motivation: POMDP在有限规划时间内获得高性能策略仍面临维度灾难和历史诅咒的挑战，而信息价值在信念空间中的变化性为优化计算分配提供了机会

Method: 提出动态规划框架，基于信念空间中的信息价值有条件地处理观测；在此基础上提出VOIMCP算法，在蒙特卡洛树搜索中根据VOI选择性忽略观测信息，避免不必要的观测分支

Result: 提供了VOI推理框架的近似最优性理论保证，推导了VOIMCP的非渐进收敛界；仿真评估显示VOIMCP在多个POMDP基准测试中优于基线方法

Conclusion: 通过利用信息价值在信念空间中的变化性，提出的VOIMCP算法能够更高效地分配计算资源，在有限规划时间内获得更好的POMDP策略性能
...
TL;DR: 该论文研究了AI辅助药物系统的可靠性，重点关注系统失败类型及其临床后果，强调需要超越传统性能指标的风险感知评估方法。


<details>
  <summary>Details</summary>
Motivation: AI系统在医疗和药房工作流程中日益普及，用于药物推荐、剂量确定和药物相互作用检测等任务。尽管这些系统在标准评估指标下通常表现良好，但其在真实世界决策中的可靠性仍未充分理解。在药物管理等高风险领域，即使是单个错误推荐也可能导致严重的患者伤害。

Method: 通过一系列涉及药物相互作用和剂量决策的受控模拟场景，分析不同类型的系统失败，包括遗漏相互作用、错误的风险标记和不适当的剂量推荐。研究重点从聚合性能指标转向错误发生的方式以及AI系统产生错误输出时的后果。

Result: 研究发现AI在药物相关环境中的错误可能导致不良反应、无效治疗或延迟护理，特别是在没有足够人工监督的情况下使用系统时。研究还揭示了过度依赖AI推荐的风险以及决策过程透明度有限带来的挑战。

Conclusion: 该研究为医疗AI评估提供了一个以可靠性为中心的视角，强调理解失败行为和真实世界影响的重要性。在药房实践等安全关键领域，需要将传统性能指标与风险感知评估方法相结合。
...
TL;DR: Elhuyar是一个多智能体、人在回路的AI系统，用于从科学文献中提取、分析和迭代提炼见解，通过将任务分配给专门的智能体并结合人类监督来确保可靠性。


<details>
  <summary>Details</summary>
Motivation: 科学发现受到碎片化文献的限制，需要大量人工努力来收集、分析和理解。现有的AI工具缺乏结构化、多步骤的方法来从科学文献中提取深度见解，而大型语言模型虽然提供了新的可能性，但由于幻觉和不完整提取而不可靠。

Method: Elhuyar是一个多智能体、人在回路的系统，整合了大型语言模型、结构化AI和人类科学家。该系统将任务分配给专门的智能体，包括：过滤论文、提取数据、拟合模型和总结发现，并通过人类监督确保可靠性。系统生成结构化报告，包含提取的数据、可视化、模型方程和文本摘要，支持通过迭代提炼进行深入探究。

Result: 在材料科学领域部署该系统，分析了钨在氦离子辐照下的文献，揭示了氦气泡生长与辐照剂量和温度呈指数相关的实验相关性，为聚变反应堆中的等离子体面向材料提供了见解。这证明了AI辅助文献综述可以发现科学模式并加速发现。

Conclusion: Elhuyar系统展示了如何通过整合大型语言模型、结构化AI和人类监督，构建一个可靠的多智能体系统来加速科学发现。这种方法能够从科学文献中提取深度见解，并通过迭代提炼支持更深入的研究。
...
TL;DR: 提出一个不可行性感知的LLM框架，用于组合优化问题，结合可验证数据集构建、监督微调和LLM辅助下游搜索，在minor-embedding问题上实现不可行性检测和解决方案生成。


<details>
  <summary>Details</summary>
Motivation: 现有LLM方法主要关注可行实例的解决方案生成，缺乏对不可行性检测的显式处理，需要开发能够同时处理可行性和不可行性判定的框架。

Method: 提出不可行性感知框架：1）通过数学规划公式和零相位不可行性筛选构建可验证数据集；2）使用精确优化流程生成训练数据；3）对8B参数LLM进行监督微调；4）利用LLM输出作为下游局部搜索的暖启动。

Result: 微调模型相比GPT-5.2整体准确率提升30%；LLM引导的暖启动使下游局部搜索速度提升2倍；在minor-embedding问题上实现解决方案生成和不可行性检测的联合性能。

Conclusion: 该框架成功将LLM应用于组合优化问题，不仅生成解决方案，还能检测不可行性，并通过LLM输出加速下游优化过程，为NP难问题提供了实用的LLM增强方法。
...
TL;DR: 提出P-COD方法，利用文档间关系检测异常，减少LLM在科学文献摘要中的幻觉，通过同行验证提高数据提取准确性


<details>
  <summary>Details</summary>
Motivation: 现有方法（如提示工程、思维链提示）仅关注单个文档，忽略了文档间的关联关系，导致大型语言模型在从大型文本语料库中提取数据时产生幻觉，影响准确性

Method: 提出Peer Context Outlier Detection (P-COD)方法，利用文档间关系改进提取准确性。通过比较提取数据与语料库中已验证的同行信息，调整置信度分数，标记低置信度结果供专家审查。高置信度结果经同行验证后视为可靠

Result: 在6个科学领域的实验中，异常检测精度高达98%。该方法减少了幻觉，增强了对自动化系统的信任，使研究人员能够专注于模糊案例，简化数据提取工作流程

Conclusion: P-COD通过利用文档间关系显著提高了数据提取的准确性，减少了LLM的幻觉问题，为科学文献摘要等应用提供了可靠的自动化解决方案
...
TL;DR: 提出一个基于技能进化的智能体框架，用于超表面逆向设计，通过积累可重用的求解器特定专业知识，提高设计工作流程的自主性和可访问性。


<details>
  <summary>Details</summary>
Motivation: 超表面逆向设计需要计算电磁学和求解器特定软件工程的专业知识，现有语言驱动系统缺乏跨任务的可重用工作流程知识，限制了设计效率。

Method: 开发了一个智能体框架，包含编码智能体、进化技能构件和基于物理仿真的确定性评估器，通过迭代精化求解器特定策略而不修改模型权重或底层物理求解器。

Result: 进化技能将分布内任务成功率从38%提升到74%，标准通过率从0.510提高到0.870，平均尝试次数从4.10减少到2.30；在分布外任务中，二进制成功率变化不大，但最佳边界改进和错误组成变化表明工作流程知识的部分迁移。

Conclusion: 技能进化的主要价值在于围绕可靠计算引擎积累可重用的求解器特定专业知识，为实现更自主和可访问的超表面逆向设计工作流程提供了实用路径。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM在健康食品政策领域的信息提取方法受到多种因素阻碍，包括因政策文档结构多样性和不一致性导致的幻觉、误分类和遗漏。需要一种更可靠、透明的方法来自动化政策信息提取。
...
Conclusion: 该框架为健康政策信息提取自动化提供了一种可靠、透明的方法论，在复杂推理任务中优于现有基线方法。
...
TL;DR: 该研究探索了在连续自回归模型中集成表示自编码器（RAE-AR），解决了高维表示自编码器在AR范式中的两大挑战：复杂的分词分布建模和训练-推理差距，通过分布归一化和高斯噪声注入显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 传统VAE编码器长期主导生成模型的潜在空间，而预训练表示编码器（如DINO、SigLIP、MAE）的潜在表示被认为不适合生成建模。虽然RAE方法显示了表示自编码器在生成建模中的潜力，但其在连续自回归模型中的集成仍未被充分探索。

Method: 提出了RAE-AR框架，针对AR模型的独特性质解决两大挑战：1）通过分布归一化进行分词简化，降低建模难度并改善收敛；2）在训练中加入高斯噪声注入，增强预测鲁棒性以缓解暴露偏差（训练-推理差距）。

Result: 实验结果表明，这些改进显著缩小了性能差距，使表示自编码器在AR模型上能够达到与传统VAE相当的结果，为视觉理解和生成建模的统一架构铺平了道路。

Conclusion: 该工作成功地将表示自编码器集成到连续自回归模型中，通过解决高维表示在AR范式中的特有挑战，实现了与传统VAE编码器相当的性能，推动了视觉理解与生成建模架构的统一。
...
TL;DR: 研究发现归一化层与优化器之间存在显著交互作用：Dynamic Erf (Derf) 与 Muon 优化器组合时性能大幅下降，而 Dynamic Tanh (DyT) 无此问题。通过EMA混合或调整alpha参数可部分恢复性能。


<details>
  <summary>Details</summary>
Motivation: 传统上在LLM训练中，归一化层和优化器被视为独立的设计选择。本研究旨在验证这一假设是否成立，特别是探究不同归一化方法（如Derf和DyT）与不同优化器（如AdamW和Muon）组合时的交互效应。

Method: 采用3x2因子设计实验，在10亿参数规模和1000训练步数下，测试不同归一化层（Derf、DyT、RMSNorm）与优化器（AdamW、Muon）的组合性能。通过分析激活函数的饱和度和尺度感知特性，识别Derf与Muon组合的失效模式。提出两种解决方案：1）EMA混合方法重新引入运行尺度估计；2）调整Derf的alpha参数（从默认0.5降至0.3）。

Result: Derf与Muon组合存在显著的负交互作用：与RMSNorm相比，Derf在AdamW下的优势为+0.31 nats，但在Muon下优势扩大至+0.97 nats（约3倍）。DyT作为有界归一化控制组未显示类似惩罚。失效原因包括：1）饱和导致的损失性压缩；2）尺度盲区导致的激活幅度信息丢失。EMA混合方法可恢复约84%的性能差距，调整alpha参数至0.3可恢复约80%差距。使用Derf默认alpha与Muon组合会产生0.66-nat的交互惩罚，且不会产生NaN或发散，容易在短期试验中被忽略。

Conclusion: 归一化层与优化器并非独立设计选择，存在显著的交互效应。Derf与Muon组合的失效源于Muon更快的谱范增长导致erf函数饱和和尺度信息丢失。通过调整参数或引入尺度估计可缓解此问题。研究强调了在LLM训练中需要系统评估归一化-优化器组合的重要性，避免因忽略交互效应而导致性能下降。
...
TL;DR: NED-Tree是一个解决运筹学问题从自然语言到可执行模型自动翻译的系统框架，通过句子级提取和递归树结构分解非线性项，在NEXTOR基准测试中达到72.51%的平均准确率。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型在线性任务中表现良好，但在现实世界非线性场景中性能严重下降，主要原因是数学公式与求解器代码之间的语义不对齐以及信息提取不稳定。

Method: NED-Tree采用两种核心策略：(1)逐句提取确保参数映射的鲁棒性和可追溯性；(2)递归树结构自适应地将复杂非线性项分解为求解器兼容的子元素。

Result: 在10个基准测试中，NED-Tree达到72.51%的平均准确率，建立了新的最先进水平。这是首个通过元素分解驱动LLM解决非线性建模困难的框架。

Conclusion: NED-Tree通过语义对齐解决了运筹学问题自动翻译中的非线性挑战，提出的NEXTOR基准为复杂非线性、多约束运筹问题提供了专门的评估标准。
...
<details>
  <summary>Details</summary>
Motivation: 当前强化学习与验证推理（RLVR）方法通常需要额外的正确性信号或批评标注来训练模型进行自我精炼。本文旨在开发一种无需这些额外监督信号的联合训练方法，让模型同时学习解决推理问题和精炼自己的答案。

Method: 提出ThinkTwice两阶段框架：1）第一阶段优化模型解决推理问题；2）第二阶段优化模型精炼同一问题的自身解决方案。两个阶段都使用相同的二元正确性奖励，基于Group Relative Policy Optimization（GRPO）进行训练，无需正确性信号或批评标注。

Result: 在五个数学推理基准和两个模型家族（Qwen3-4B和Olmo3-7B）上，ThinkTwice显著优于竞争性在线策略优化基线。在Qwen3-4B上，ThinkTwice在AIME基准上比GRPO在精炼前提升5个百分点，一次自我精炼后提升11.5个百分点（pass@4）。训练动态分析显示隐含的"纠正-强化"课程：早期精炼主要纠正错误，随着模型改进自然转向保持已正确的解决方案。

Conclusion: ThinkTwice建立了推理和自我精炼的联合训练作为一种原则性和有效的RLVR方法，无需额外监督信号，通过两阶段优化和隐含课程学习实现了性能显著提升。
...
Method: 使用先前研究人类教学策略的受控任务：LLMs作为模拟教师，观察学习者在奖励标注有向图中的轨迹，揭示单一边缘以改善学习者路径规划。将LLMs的逐次选择与认知模型（贝叶斯最优教师、弱贝叶斯变体、启发式基线、非心智化效用模型）进行拟合比较

Result: 在基线实验中，大多数LLMs表现良好，策略变化小，图间表现与人类相似。模型比较显示贝叶斯最优教学最能解释大多数模型的选择。脚手架干预使模型遵循推理或奖励导向的提示，但这些干预不能可靠改善启发式不一致测试图上的教学，有时甚至降低性能

Conclusion: 认知模型拟合揭示了LLMs的教学策略，表明LLMs主要采用贝叶斯推理进行教学决策，但提示合规性不能保证更好的教学决策，需要更深入理解LLMs的教学机制
...
Result: 在三个图表到代码基准测试中实现了最先进的性能，模型能够通过环境交互和迭代自校正生成更准确、可执行的代码。

Conclusion: MM-ReCoder通过强化学习和自校正能力有效解决了现有MLLMs在图表到代码生成中的局限性，为多模态编码任务提供了新的训练范式。
...
Method: 提出CRaFT框架，使用拒绝边界附近的提示，根据特征对模型拒绝-合规决策的影响度来排序特征，而非基于激活强度。在Gemma-3-1B-it模型上实现。

Result: 在Gemma-3-1B-it上，CRaFT将攻击成功率从6.7%提升至48.2%，在多个越狱基准测试中优于基线方法。

Conclusion: 电路影响度比激活强度更可靠，可作为识别因果中介拒绝行为特征的标准，为理解LLM拒绝机制提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 多智能体系统(MAS)在处理复杂任务时存在协调开销大、上下文碎片化和脆弱的阶段排序等问题。将MAS蒸馏为单智能体技能可以绕过这些成本，但缺乏关于何时以及蒸馏什么的原则性指导，导致实证结果不一致。

Method: 引入Metric Freedom (F)作为技能效用的先验预测指标，通过Mantel测试量化输出多样性与得分方差之间的耦合程度，测量度量评分景观的拓扑刚性。基于F提出两阶段自适应蒸馏框架：第一阶段作为选择性提取机制，在"自由"度量上提取工具和知识同时丢弃限制性结构；第二阶段专门针对"刚性"度量(F≤0.6)进行计算密集的迭代优化。

Result: 在4个任务、11个数据集和6个度量上的评估显示，F能强预测技能效用(ρ=-0.62, p<0.05)。相同的智能体轨迹在刚性与自由度量下产生截然相反的性能提升。自适应智能体在匹配或超越原始MAS的同时，将成本降低高达8倍，延迟降低高达15倍。

Conclusion: 技能效用本质上是一个度量层面的属性，而非任务属性。Metric Freedom (F)作为先验预测指标能有效指导蒸馏过程，提出的两阶段自适应蒸馏框架能在显著降低协调开销和延迟的同时保持或提升系统性能。
...
TL;DR: GraphWalk：一种免训练、基于工具的框架，让现成LLM通过顺序图导航进行推理，显著提升大规模知识图谱上的复杂查询性能
...
Method: GraphWalk框架为LLM提供最小正交图操作集（遍历任何图结构的基本工具），通过顺序工具调用实现多步推理链，每个工具调用代表可验证步骤并创建透明执行轨迹

Result: 基于工具的遍历方法在所有测试模型系列上均比上下文基线获得显著且一致的性能提升，随着规模增大收益更明显（恰好是上下文方法完全失效的地方）；在迷宫遍历和合成企业知识图谱上均验证有效性

Conclusion: GraphWalk证明了免训练、工具化方法使现成LLM能够通过顺序图导航解决复杂推理任务，为大规模知识图谱上的问答应用提供了可扩展解决方案
...
<details>
  <summary>Details</summary>
Motivation: 在面向生产的分类框架中，系统评估大型语言模型在收据项目分类任务中的性能，重点关注准确性、响应稳定性和token级成本，并研究零样本和少样本提示方法的适用性

Method: 使用AWS Bedrock平台上的四种指令调优模型（Claude 3.7 Sonnet、Claude 4 Sonnet、Mixtral 8x7B Instruct、Mistral 7B Instruct），在收据项目分类任务中比较零样本和少样本提示方法，评估准确性、响应稳定性和token成本
...
<details>
  <summary>Details</summary>
Motivation: 扩散语言模型提供去噪轨迹作为模型原生信号，应利用这一特性在推理时进行干预，而非依赖外部训练的幻觉分类器。现有方法缺乏对轨迹级不确定性的系统评估和利用。

Method: 提出承诺不确定性定位：通过N条并行去噪链（随机揭示顺序）计算跨链香农熵，检测高不确定性位置；引入轨迹级评估指标（如CDH）；OSCAR框架执行针对性重掩码并整合检索证据

Result: 在TriviaQA、HotpotQA、RAGTruth、CommonsenseQA数据集上，使用LLaDA-8B和Dream-7B模型，OSCAR显著减少幻觉内容、提高事实准确性；定位和校正贡献互补增益；熵基不确定性信号优于专用训练检测器

Conclusion: 扩散语言模型具有识别事实不确定性的内在能力，其轨迹级熵信号优于自回归模型；OSCAR为DLM中的定位和不确定性感知生成提供了有效框架
...
<details>
  <summary>Details</summary>
Motivation: 将LLM驱动的智能体嵌入环境FAIR数据管理很有吸引力，因为它们可以外部化操作知识并在异构数据和不断演变的规范中扩展数据管理。但用概率工作流替换确定性组件改变了故障模式：LLM管道可能生成看似合理但错误的输出，这些输出通过表面检查并传播到不可逆操作（如DOI分配和公开发布）。
...
Conclusion: EnviSmart通过架构机制在LLM驱动的概率工作流中确保可靠性，防止错误传播到不可逆操作，为环境FAIR数据管理提供了可扩展且可靠的解决方案。该论文已被PEARC 2026接受。
...
TL;DR: CORAL是一个基于大语言模型的多智能体自主进化框架，用于开放式发现问题，通过持久记忆、异步执行和心跳干预实现智能体自主探索、反思和协作，在多个任务上取得显著性能提升。
...
Method: CORAL框架采用长期运行的智能体，通过共享持久记忆、异步多智能体执行和基于心跳的干预实现自主探索、反思和协作。提供实用保障机制，包括隔离工作空间、评估器分离、资源管理以及智能体会话和健康管理。

Result: 在多样化数学、算法和系统优化任务评估中，CORAL在10个任务上创造了新的最先进结果，相比固定进化搜索基线，改进率提高3-10倍且评估次数更少。在Anthropic内核工程任务中，四个协同进化智能体将最佳已知分数从1363周期提升到1103周期。

Conclusion: 机制分析表明这些收益源于知识重用和多智能体探索与通信。结果表明更大的智能体自主性和多智能体进化能够显著改善开放式发现。代码已开源。
...
Result: 建立了七种设计模式的完整规范，包括问题、驱动力、解决方案、后果、已知应用和相关模式，其中三种模式在现有系统中有部分先例，四种模式首次正式描述。

Conclusion: 该论文提供了基于理论分析的设计词汇，为构建抗本体扭曲的临床AI系统提供了架构模式，但缺乏实证验证和运行时基准测试，主要适用于德国和欧盟监管环境。
...
TL;DR: HMO（分层记忆编排）是一个三层次记忆管理框架，通过用户画像驱动的上下文相关性组织交互历史，解决长期记忆存储扩展导致的性能瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 长期记忆对智能体保持历史一致性至关重要，但大量交互数据的积累会导致检索噪声增加、计算延迟上升，特别是在资源受限的个人设备上会压垮模型的推理能力。

Method: 提出分层记忆编排（HMO）框架，将交互历史组织为三层结构：紧凑的主缓存（存储近期和关键记忆+动态用户画像）、高优先级次层，以及全局完整历史档案。用户画像驱动记忆在层次间的重新分配，将与长期模式相关的记录提升到更活跃的层级，降低不相关信息的重要性。

Result: 在多个基准测试中达到最先进性能。在OpenClaw等生态系统中的实际部署表明，HMO显著提升了智能体的流畅性和个性化程度。

Conclusion: HMO通过用户中心的分层记忆管理，实现了精确的历史知识检索，同时保持了精简高效的活跃搜索空间，解决了长期记忆扩展的性能瓶颈问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型融合方法主要针对同构模型（共享相同预训练主干），但在开放模型生态系统中，有用专家模型通常基于不同架构（如Llama、Qwen、Mistral），直接权重空间融合因架构不匹配、潜在基不对齐和跨源冲突而变得不适定

Method: HeteroFusion包含两个关键组件：1）基于拓扑的对齐，通过匹配功能模块结构而非原始张量坐标在不同异构主干间转移知识；2）冲突感知去噪，在融合过程中抑制不兼容或噪声转移信号。该方法还通过分析证明，在预测结构化更新时保持目标适配器基能实现稳定且良条件的转移过程
...
TL;DR: 研究基于中国视频平台数据，发现AIGC通过规模化生产获得与HGC相当的总体参与度，但用户仍偏好HGC，算法分发机制能调节这种矛盾。
...
Method: 利用中国领先视频分享平台的纵向数据集，包含数千万用户，分析AIGC与HGC的创作和消费行为特征，识别规模优先于偏好的动态。
...
TL;DR: 提出LiteInception框架，用于通用航空故障诊断的边缘部署，通过两阶段级联架构、多方法融合压缩、知识蒸馏和双层可解释性框架，在计算效率和可解释性间取得平衡。
...
Result: 在NGAFID数据集上：故障检测准确率81.92%，召回率83.24%；故障识别准确率77.00%。模型压缩效果：传感器通道从23减至15，InceptionTime参数压缩70%，CPU推理加速8倍以上，F1损失小于3%。

Conclusion: LiteInception框架在效率、准确性和可解释性之间取得了有利平衡，适用于资源受限的边缘设备部署，并能适应不同诊断场景需求。
...
TL;DR: 本文提出了AnIML本体，一个OWL 2本体，用于形式化分析化学和生物学中使用的Analytical Information Markup Language (AnIML)的语义，并与Allotrope Data Format对齐，以支持跨系统和跨实验室的互操作性。


<details>
  <summary>Details</summary>
Motivation: 异构实验数据系统之间的语义互操作性仍然是数据驱动科学发现的主要障碍。虽然AnIML作为分析化学和生物学的XML标准在工业研发实验室中广泛使用，但其XML模式的表达能力导致不同利益相关者产生分歧解释，引入了破坏互操作性的不一致性。

Method: 采用专家参与的方法，结合LLM辅助的需求获取与协作本体工程开发AnIML本体。通过多层验证：将真实AnIML文件转换为知识图谱的数据驱动转换、通过SPARQL进行能力问题验证，以及基于对抗性负能力问题的新验证协议，这些问题映射到已建立的本体反模式并通过SHACL约束强制执行。

Result: 成功开发了AnIML本体，该本体形式化了AnIML的语义并与Allotrope Data Format对齐。通过多层验证方法证明了本体的有效性和实用性，能够支持跨系统和跨实验室的互操作性。

Conclusion: AnIML本体通过形式化AnIML的语义并与其他标准对齐，为解决实验数据系统中的语义互操作性问题提供了有效解决方案。提出的多层验证方法为类似本体工程提供了可复制的验证框架。
...
TL;DR: 提出基于SAT的框架解决二维单规格板材切割问题，通过需求扩展、板材分配变量和不可行方向消除规则，比较了三种最小化板材数量的SAT方法，在基准测试中优于现有求解器。


<details>
  <summary>Details</summary>
Motivation: 二维单规格板材切割问题（2D-CSSP）是制造业中的核心任务，需要从标准板材上切割矩形零件以满足需求并最小化浪费。该问题比装箱问题更复杂，因为每种零件类型需要多个副本，导致组合爆炸。现有求解器在处理该问题时面临挑战，需要更有效的解决方案。

Method: 1. SAT框架：将零件类型按需求扩展，每个副本分配板材变量，仅当副本分配到同一板材时才激活非重叠约束；2. 不可行方向消除规则：当零件只有一种方向能放入板材时固定旋转变量；3. 三种最小化板材数方法：非增量SAT+二分搜索、增量SAT+跨迭代子句重用、加权部分MaxSAT。
...
Conclusion: SAT方法在解决2D-CSSP问题上显著优于传统求解器，通过创新的约束建模和优化技术有效处理组合复杂性。不同SAT方法的性能取决于问题特征，特别是旋转约束的影响，为实际应用提供了灵活的选择。
...
Result: 在HyTPS-Bench上评估，AeroTherm-GPT实现88.7%端到端成功率（95%置信区间：87.5-89.9），比匹配的非CDG消融基线提升12.5个百分点，根因修复效率达4.16（扁平清单修复为1.76），在科学推理和代码生成任务上无灾难性遗忘

Conclusion: AeroTherm-GPT通过约束闭环生成框架和约束依赖图，有效解决了LLM在高超声速热防护系统设计中生成可执行仿真工件时的级联约束违反问题，显著提升了生成成功率和修复效率
...
<details>
  <summary>Details</summary>
Motivation: 知识图谱能高效存储大量关系，但难以处理概念在不同领域中的语义漂移问题。现有系统通常将领域信息作为元数据、限定符或图级组织处理，这虽然有助于过滤和溯源，但不改变断言的形式状态。许多知识系统的实际失败源于将领域视为断言的外部因素。

Method: 提出领域情境化概念图(DCG)框架，将领域写入关系中并解释为模态世界约束。采用形式为(C, R at D, C')的三元组，其中at D标识关系成立的世界。通过Kripke风格语义学、紧凑谓词系统、Prolog实现，以及到RDF、OWL和关系数据库的映射来发展这一主张。

Result: DCG框架使领域在表示中具有结构和可计算的角色，能够：1)在表示点消歧模糊概念；2)根据其领域挑战无效断言；3)通过显式谓词连接跨领域关系。实现了对领域本身的表示重新解释。

Conclusion: 领域应被视为知识表示的一部分而非补充注释。DCG通过将领域作为结构性和可计算的角色纳入表示内部，解决了知识系统中因领域处理不当导致的许多实际失败。这是对领域本身的表示重新解释。
...
TL;DR: PGPO提出了一种细粒度的信用分配框架，通过基于视觉依赖性的阈值门控机制动态重塑token级优势，解决了传统RLVR方法中学习信号稀释的问题，显著提升了多模态推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于可验证奖励的强化学习框架存在根本性方法缺陷：通过在所有生成的token上分配相同的优势值，这些方法本质上稀释了优化多模态推理中关键视觉基础步骤所需的学习信号。

Method: 提出Token Visual Dependency概念，通过视觉条件与纯文本预测分布之间的KL散度量化视觉输入的因果信息增益；基于此引入Perception-Grounded Policy Optimization框架，采用阈值门控、质量守恒机制动态重塑token级优势，放大视觉依赖token的学习信号，抑制语言先验的梯度噪声。

Result: 在基于Qwen2.5-VL系列的七个挑战性多模态推理基准测试中，PGPO平均提升模型性能18.7%；理论和实证分析证实PGPO有效降低梯度方差，防止训练崩溃，并作为强大的正则化器实现稳健的感知基础多模态推理。

Conclusion: PGPO通过细粒度的token级信用分配，解决了传统RLVR方法的学习信号稀释问题，显著提升了多模态推理的感知基础能力，为强化学习在多模态领域的应用提供了更有效的优化框架。
...
Method: 研究构建了多队列EHR基准，比较经典模型、深度表格模型和TICL模型。针对PFN-based TICL模型在异质性和不平衡下性能下降的问题，提出了AWARE框架，采用监督嵌入学习和轻量级适配器实现任务对齐的检索。

Result: PFN-based TICL模型在低数据场景下样本效率高，但在朴素距离检索下随着异质性和不平衡增加而性能下降。AWARE框架在极端不平衡下将AUPRC提升高达12.2%，且随着数据复杂性增加，性能增益更加明显。

Conclusion: 检索质量和检索-推理对齐是部署表格上下文学习于临床预测的关键瓶颈。AWARE框架通过任务对齐的检索方法有效解决了这些问题，为临床预测中的表格上下文学习提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有SSP算法虽然使用启发式函数筛选有希望的状态，但在每个Bellman备份中仍考虑所有适用动作，即使启发式表明某些动作代价过高，导致计算资源浪费。

Method: 将启发式搜索重新表述为线性规划问题，引入针对SSP的高效约束生成实现，开发CG-iLAO*算法，该算法基于iLAO*但能主动避免考虑昂贵的动作。

Result: CG-iLAO*在许多问题上仅考虑iLAO* 40%的动作，某些情况下低至1%；平均减少3.5倍的动作代价计算，比iLAO*和LRTDP分别快2.8倍和3.7倍。

Conclusion: 通过约束生成技术选择性忽略昂贵动作，能显著提升SSP求解效率，CG-iLAO*在减少计算开销方面表现出色，为启发式搜索提供了新的优化方向。
...
<details>
  <summary>Details</summary>
Motivation: 在多类分类任务中，传统监督通常提供确定性标签，但实际应用中可能存在不确定性或模糊性。可能性分布能更细致地表达类别间的相对可信度，需要开发能有效利用这种监督信息的学习方法。

Method: 1) 从可能性分布构建可接受概率分布集：结合概率兼容性要求和线性形状约束；2) 计算模型输出的概率向量到可接受集的KL投影；3) 使用Dykstra算法和负熵相关的Bregman投影计算投影；4) 最小化预测与其投影之间的散度来训练模型。

Result: 在合成数据和真实世界自然语言推理任务（基于ChaosNLI数据集）上的实验表明：提出的投影算法具有足够高的计算效率，可用于实际应用；基于投影的学习目标能提升预测性能。

Conclusion: 该方法成功地将可能性监督信息融入多类分类学习框架中，通过KL投影机制确保模型输出满足可能性分布所蕴含的定性结构约束，在保持计算可行性的同时提升了模型性能。
...
Method: 基于一阶逻辑构建Qiana框架，使其可有限公理化，支持对公式和上下文的量化，允许上下文内使用准一致逻辑，并讨论不同的设计替代方案。

Result: 展示了Qiana能够表示时间性、事件演算和模态逻辑，证明了该框架与现有一阶逻辑定理证明器的兼容性，并探讨了不同的设计选择。

Conclusion: Qiana提供了一个强大而灵活的逻辑框架，能够在特定上下文中进行推理，支持复杂的量化表达，允许矛盾存在，并与传统一阶逻辑工具保持兼容。
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏整合运动学测量、临床严重程度评分和患者人口统计学的多条件资源，这阻碍了AI驱动的神经疾病诊断工具的发展。

Method: 采用多LLM提取框架从1,430篇同行评审文献中构建NeuroPose-AHM数据集，包含57种神经疾病的2,756条患者组记录。通过四任务框架验证数据集效用，包括多标签AHM类型分类、构建头颈严重程度指数(HNSI)、临床验证和相关性分析。

Result: 数据集提取可靠性高（kappa=0.822），多标签分类F1分数达0.856。构建的HNSI指数与真实世界数据验证一致（重度带比例6.7%），运动类型概率与HNSI分数显著相关（p<0.001）。

Conclusion: NeuroPose-AHM是一个结构化、基于知识的资源，为神经性异常头部运动研究提供了有价值的分析工具，数据集已在Zenodo公开可用。
...
TL;DR: SenseMath基准测试评估LLMs的数字结构敏感性推理能力，发现模型能使用计算捷径但缺乏人类数字感知的结构性理解


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常默认逐步计算，即使存在高效数值捷径。这引发了一个基本问题：它们是否表现出类似人类的数字感知能力，即识别数字结构、在适当时应用捷径、在不适用时避免使用捷径的能力。

Method: 引入SenseMath基准测试，包含4,800个项目，涵盖八个捷径类别和四个数字规模，具有匹配的强捷径、弱捷径和控制变体。支持三个评估设置：捷径使用（模型能否在适用捷径的问题上应用捷径）、适用性判断（能否识别捷径何时适用或误导）、问题生成（能否生成正确允许给定类型捷径的新问题）。评估了从GPT-4o-mini到Llama-3.1-8B的五个LLMs。
...
Conclusion: 当前LLMs表现出程序性捷径流畅性，但缺乏支撑人类数字感知的结构性理解，即理解捷径何时以及为何有效。这表明模型缺乏真正的数字感知能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前步态分析中的深度学习方法大多依赖有监督分类器，需要疾病标签数据，限制了在异质性病理表现中的泛化能力。需要一种无需疾病标签的方法来检测和校正步态异常。
...
Result: 在10名保留的正常参与者（模拟七种步态异常）上验证，结果显示：准确识别生物力学不一致关节；所有分析关节的角度偏差显著减少，效应量大；保持正常运动学特征。
...
<details>
  <summary>Details</summary>
Motivation: 强化学习能增强大语言模型的推理能力，但在多轮智能体任务中存在挑战：长时程交互和环境反馈的随机性导致次优行动产生误导性上下文，形成错误累积的负反馈循环，使标准探索策略失效

Method: 提出ProCeedRL方法，将探索从被动选择转向主动干预。使用过程级批评器实时监控交互，结合基于反思的演示来指导智能体停止错误累积。通过探索性演示和在线策略样本学习

Result: ProCeedRL显著超越了模型饱和的探索性能，展示了实质性的探索优势。在复杂深度搜索和具身任务中，显著提高了探索效率并实现了优越性能

Conclusion: 通过主动干预错误累积过程，ProCeedRL有效解决了多轮智能体任务中的探索挑战，为强化学习在复杂智能体系统中的应用提供了新思路
...
<details>
  <summary>Details</summary>
Motivation: 现有轨迹级基准测试存在交互多样性不足、安全失败观测粗糙和长时域现实性弱的问题，而现实部署中的风险往往在多步交互中显现，因此需要更全面的智能体安全评估框架。

Method: 基于三维风险分类（风险来源、失败模式、现实危害）构建轨迹，使用异构工具池和长上下文延迟触发协议模拟多阶段风险涌现，包含1000条轨迹（503安全/497不安全），平均9.01轮对话和3.95k tokens，涉及1954次工具调用和2084个可用工具。

Result: 实验表明ATBench对前沿LLM、开源模型和专用防护系统都具有挑战性，支持基于分类的分层分析、跨基准比较以及长时域失败模式诊断，数据质量通过规则和LLM过滤加人工审核保证。

Conclusion: ATBench为智能体安全评估提供了结构化、多样化和现实化的轨迹级基准，能够有效捕捉多步交互中涌现的风险，支持更全面的安全性分析和诊断。
...
TL;DR: 该论文是一篇关于语言模型中潜在空间的系统性综述，从五个视角（基础、演化、机制、能力、展望）全面梳理了该领域的研究现状，旨在将潜在空间确立为下一代智能系统的通用计算范式。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型的发展，研究者们发现许多关键内部过程在连续潜在空间中比在人类可读的显式标记空间中更自然。显式空间计算存在语言冗余、离散化瓶颈、序列效率低下和语义损失等结构限制，因此需要转向潜在空间作为更有效的计算基底。

Method: 采用系统性综述方法，从五个连续视角组织调查：1) 基础：界定潜在空间范围，区分显式/语言空间和视觉生成模型的潜在空间；2) 演化：追溯从早期探索到当前大规模扩展的发展历程；3) 机制：从架构、表示、计算和优化四个主要发展路线分析技术景观；4) 能力：展示潜在空间如何支持推理、规划、建模、感知、记忆、协作和具身等广泛能力谱系；5) 展望：讨论开放挑战和未来研究方向。

Result: 该综述提供了语言模型中潜在空间的统一且最新的研究图景，系统性地整理了现有工作，揭示了潜在空间作为计算基底的重要价值。通过机制和能力的双重视角，展示了潜在空间如何支持广泛的高级认知功能，为理解下一代智能系统提供了理论基础。

Conclusion: 潜在空间正在迅速成为语言模型的原生计算基底，具有超越显式标记空间的优势。该综述不仅为现有研究提供了参考框架，更重要的是为理解潜在空间作为下一代智能的通用计算和系统范式奠定了基础。未来研究需要解决该领域的关键开放挑战，推动潜在空间计算范式的发展。
...
<details>
  <summary>Details</summary>
Motivation: 传统保险问卷冗长标准化，难以捕捉个体差异，且依赖用户诚实回答易导致欺诈。需要更智能、个性化且可信的评估方法。

Method: 使用大型语言模型(LLM)和替代数据源(社交媒体图像分析、地理数据分类、检索增强生成RAG)提取用户洞察，生成个性化自适应问卷。

Result: 与传统问卷相比，GPT驱动的自适应版本问题数量更少，用户体验更流畅有趣；传统问卷在风险评估准确性上略高，但自适应问卷更受用户青睐。

Conclusion: ARQuest在提升用户满意度和简化保险流程方面潜力巨大，进一步开发后可能在风险准确性上超越传统方法，推动保险行业创新。
...
Conclusion: Diff-KD通过整合扩散生成细化到知识蒸馏中，有效解决了多智能体协同感知中的传感器和通信损坏问题，实现了对损坏输入的主动语义恢复。
...
TL;DR: 该论文研究了在无参考设置下，使用大语言模型作为时间序列解释的生成器和评估器，构建了包含350个时间序列案例的合成基准，发现生成任务存在模式依赖性且在某些查询类型上失败，而评估任务则更稳定可靠。


<details>
  <summary>Details</summary>
Motivation: 评估基于时间序列数据的大语言模型生成的自然语言解释的事实正确性是一个开放挑战。现有评估方法存在局限：基于参考的相似性指标和一致性检查模型需要真实解释，而传统时间序列方法仅处理数值且无法评估自由形式的文本推理。因此，缺乏无需预定义参考或任务特定规则的通用方法来直接验证解释是否忠实于底层时间序列数据。

Method: 研究在无参考设置下使用大语言模型作为时间序列解释的生成器和评估器。构建包含350个时间序列案例的合成基准，涵盖七种查询类型，每种都配有正确、部分正确和错误的解释。评估模型在四个任务上的表现：解释生成、相对排名、独立评分和多异常检测。评估器基于模式识别、数值准确性和答案忠实性分配三元正确性标签。

Result: 结果显示明显的不对称性：生成任务高度依赖模式，在某些查询类型上出现系统性失败，准确率从季节性下降和波动率变化的0.00-0.12，到结构突变的0.94-0.96不等；而评估任务更稳定，即使模型自身输出不正确，也能正确排名和评分解释。这表明基于数据的大语言模型评估在时间序列解释领域是可行的。

Conclusion: 研究证明了基于数据的大语言模型评估时间序列解释的可行性，并突显了它们作为时间序列领域中数据驱动推理的可靠评估器的潜力。评估任务比生成任务更稳定可靠，为无参考设置下的时间序列解释评估提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有社交机器人检测方法存在三个主要问题：1) 过度依赖单模态信号导致检测精度有限；2) 对AIGC特定生成模式不够敏感；3) 未能充分建模语言模式与行为动态之间的相互作用。LLM驱动的社交机器人能够生成类人内容，逃避传统检测，对在线话语构成日益增长的威胁。
...
Result: 在两个公开的LLM驱动社交机器人数据集上实现了SOTA性能，准确率分别达到98.46%和97.50%。结果表明对高级机器人策略具有强鲁棒性，验证了联合利用隐式语义表示和AIGC增强行为模式的有效性。
...
TL;DR: 提出一种结构化多步骤ODD覆盖验证方法，通过参数离散化、约束过滤和关键性降维，解决AI系统在高维参数空间中满足EASA认证标准所需的完备性证明问题。


<details>
  <summary>Details</summary>
Motivation: AI在航空等安全关键领域部署需满足严格认证标准，EASA要求证明AI/ML组件的操作设计域（ODD）完全覆盖，但现有方法在高维参数空间中缺乏可扩展性和形式化基础，无法满足完备性要求，缺乏标准化工程方法连接抽象ODD定义与可验证证据。

Method: 提出结构化多步骤ODD覆盖验证流程，整合参数离散化、基于约束的过滤和基于关键性的维度约减，基于先前AI空中防撞研究的仿真数据，系统化定义和实现满足EASA完备性要求的覆盖度量。

Result: 该方法能够在更高维度验证ODD覆盖，推进"安全设计"方法，同时符合EASA标准，为AI系统在安全关键领域认证提供了可扩展的工程化验证途径。

Conclusion: 提出的方法填补了抽象ODD定义与可验证证据之间的空白，通过结构化工程方法解决了高维参数空间中ODD覆盖验证的可扩展性和形式化问题，满足EASA对AI系统认证的完备性要求。
...
TL;DR: 该研究评估了LLM生成的胸部CT报告日语翻译的教育适用性，发现放射科医师与LLM评估者之间存在显著分歧，LLM评估者强烈偏好LLM生成的翻译，而放射科医师意见也不一致，表明自动化LLM评估不足以替代专家评审。


<details>
  <summary>Details</summary>
Motivation: 放射学报告的准确翻译对于多语言研究、临床沟通和放射学教育至关重要，但基于LLM的翻译评估有效性尚不明确。需要评估LLM生成的日语翻译在教育场景中的适用性，并比较放射科医师评估与LLM作为评估者（LLM-as-a-judge）的结果。

Method: 研究分析了CT-RATE-JPN验证集中的150份胸部CT报告。每份英文报告的人工编辑日语翻译与DeepSeek-V3.2生成的LLM翻译进行对比。一位认证放射科医师和一位放射科住院医师独立进行盲法配对评估，涵盖4个标准：术语准确性、可读性、整体质量和放射科医师风格真实性。同时，3个LLM评估者（DeepSeek-V3.2、Mistral Large 3和GPT-5）评估相同配对。使用QWK和百分比一致性评估一致性。

Result: 放射科医师与LLM评估者之间的一致性接近零（QWK=-0.04至0.15）。两位放射科医师之间的一致性也很差（QWK=0.01至0.06）。放射科医师1认为59%的案例术语等效，并在可读性（51%）和整体质量（51%）上偏好LLM翻译。放射科医师2认为75%的案例可读性等效，并在整体质量上偏好人工编辑翻译（40% vs 21%）。所有3个LLM评估者强烈偏好LLM翻译（70%-99%），并在>93%的案例中认为其更具放射科医师风格。

Conclusion: LLM生成的翻译常被判断为自然流畅，但两位放射科医师评估存在显著差异。LLM作为评估者表现出对LLM输出的强烈偏好，与放射科医师的一致性可忽略不计。对于翻译放射学报告的教育用途，仅依赖自动化LLM评估是不够的；专家放射科医师评审仍然至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 现有注意力可视化技术通常针对特定模型架构（特别是Transformer），需要反向传播，导致GPU内存使用几乎翻倍和计算成本增加，缺乏轻量级、模型无关的方法

Method: 采用基于扰动的策略结合三矩阵分析框架：角度偏差矩阵（捕捉语义方向变化）、幅度偏差矩阵（测量语义强度变化）、维度重要性矩阵（评估各向量维度贡献）。通过系统移除每个token并测量这三个互补维度的影响，得出综合重要性分数

Result: 开发了模型无关的token重要性可视化技术，能够生成显示token级贡献的相关性图，提供细致且数学基础的重要性度量，所有可解释性技术均已开源

Conclusion: 提出了一种轻量级、模型无关的注意力可视化方法，无需额外计算成本，有助于更好地理解生成式AI系统如何处理输入文本信息，促进可解释AI的发展
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型需要有效知道何时弃权，而推理模型在弃权能力方面表现较差。模型幻觉导致的弃权失败可被重新解释为模型回答了错误的问题而非错误地回答问题

Method: 提出查询错位框架和Trace Inversion方法：首先生成模型的推理轨迹，仅基于该轨迹重构模型最可能回答的查询，然后比较原始查询与重构查询的相似度，低相似度表明模型可能错误回答问题并标记为弃权

Result: 在四个前沿LLM和九个弃权QA数据集上的广泛实验表明，Trace Inversion在36个设置中的33个中击败了竞争基线，有效提升了弃权性能

Conclusion: Trace Inversion方法通过查询错位框架显著提升了推理模型的弃权能力，为解决模型幻觉导致的弃权失败问题提供了有效解决方案
...
TL;DR: 情感提示对LLM性能影响有限，但可通过自适应情感提示框架获得更可靠的性能提升


<details>
  <summary>Details</summary>
Motivation: 研究情感语气在用户查询中对大语言模型行为的影响，探索情感提示是否能够作为提升模型性能的有效干预手段

Method: 1) 在六个基准领域测试静态情感前缀的影响；2) 分析情感强度变化的影响；3) 比较人类编写和LLM生成的情感前缀；4) 提出EmotionRL自适应情感提示框架，为每个查询自适应选择情感框架

Result: 静态情感前缀通常只产生小的准确率变化；情感影响在社交基础任务中更易变；强情感措辞仅引起适度额外变化；人类编写和LLM生成前缀呈现相同定性模式；自适应选择比固定情感提示产生更可靠的性能提升
...
TL;DR: De Jure：全自动、领域无关的监管规则提取流水线，无需人工标注即可从原始文档中提取结构化监管规则


<details>
  <summary>Details</summary>
Motivation: 监管文档包含LLM系统必须遵守的法律义务，但将密集、分层结构的法律文本转换为机器可读规则仍然是成本高昂、专家密集的过程

Method: 四阶段流水线：1) 源文档标准化为结构化Markdown；2) LLM驱动的语义分解为结构化规则单元；3) 19个维度的多标准LLM-as-a-judge评估；4) 有界再生预算内的迭代修复

Result: 在金融、医疗和AI治理三个监管语料库上评估，De Jure在金融领域实现提取质量的一致性和单调改进，在三次迭代内达到峰值性能。在RAG合规问答评估中，基于De Jure提取规则的响应在73.8%-84.0%情况下优于先前工作
...
<details>
  <summary>Details</summary>
Motivation: 长对话智能体需要持久记忆进行连贯推理，但无控制的记忆累积会导致时间衰减和虚假记忆传播。现有基准显示性能显著下降（如LOCOMO从0.455降至0.05），MultiWOZ在持久保留下准确率78.2%但虚假记忆率6.8%。

Method: 引入自适应预算遗忘框架，通过相关性引导评分（结合时效性、频率和语义对齐）和有界优化来调节记忆，在受限上下文中保持稳定性。

Result: 比较分析显示：长对话F1分数超过0.583基线水平，保留一致性更高，虚假记忆行为减少，且未增加上下文使用量。
