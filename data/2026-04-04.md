...
TL;DR: 该论文将结构化LLM路由重新定义为系统级负载分配问题，而非单纯的提示工程问题，并通过多后端实验证明不存在通用的最佳路由模式，性能受后端特定交互效应主导。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型成为智能体AI系统的核心控制组件，可靠的结构化路由需要在真实部署约束下平衡正确性、延迟和实现成本。当前研究主要将结构化路由视为提示工程问题，但作者认为这本质上是一个系统级负载分配问题。

Method: 提出了一个可部署的框架，用于在异构后端条件下进行结构化路由推理。通过全面的全因子基准测试，评估了48种部署配置和15,552个请求，覆盖OpenAI、Gemini和Llama后端。重点分析结构工作如何在生成栈中分配：模型直接输出结构、传输期间压缩、或生成后本地重建。

Result: 核心发现：不存在通用的最佳路由模式。后端特定的交互效应主导性能表现。在Gemini和OpenAI上保持高可靠性的模式，在Llama上可能出现显著的正确性下降；压缩实现的效率增益也强烈依赖于后端。不同后端之间的性能差异显著。

Conclusion: 该工作贡献了一个可部署的框架，用于在生产级智能体专家系统中导航正确性-成本-延迟边界。提供了跨后端评估方法和实际部署指导，强调需要根据具体后端特性进行路由策略选择，而非寻找通用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统因果推断方法依赖不可观测的假设(如可忽略性、平行趋势等)来填补反事实数据的缺失，但都无法直接产生反事实结果本身。需要一种新框架将因果主张转化为可检验的形式

Method: 提出数字孪生反事实框架：1) 使用数字孪生模拟器在潜在结果框架内进行随机映射；2) 建立孪生保真度假设层次结构(边际保真度→联合保真度→结构保真度)；3) 设计五层验证架构将不可证伪的因果主张转化为可检验的假设；4) 形式化分解将因果量区分为边际可验证和copula依赖两类

Result: 1) 建立了将不可证伪的反事实模拟器主张转化为可检验测试的验证架构；2) 形式化区分了边际可验证的因果量(ATE、CATE、QTE)和copula依赖的因果量(ITE分布、获益/损害概率、处理效应方差)；3) 提供了边界、敏感性和不确定性量化工具来显式处理copula依赖性

Conclusion: DTCF不解决因果推断的根本问题，但提供了一个框架：使边际因果主张变得可检验，使联合因果主张变得显式假设索引化，并形式化表征了两者之间的差距。该框架将因果推断从纯粹假设驱动转向部分可验证的模拟驱动范式
...
TL;DR: IDEA2是一个结合大语言模型和专家在环的半自动化工作流，用于从需求文档中提取能力问题，通过迭代反馈循环提高CQ的质量和接受度。


<details>
  <summary>Details</summary>
Motivation: 能力问题（CQ）提取是本体工程中的关键但资源密集型瓶颈，存在领域专家和本体工程师之间的沟通鸿沟，需要更高效的协作方法。

Method: IDEA2采用半自动化工作流，核心是迭代循环：LLM从需求文档提取CQ→领域专家在协作平台上评审反馈→LLM基于反馈重新表述被拒绝的CQ→直至达成共识，并使用溯源模型跟踪CQ全生命周期。

Result: 在两个真实场景（科学数据、文化遗产）中验证，IDEA2能加速需求工程过程，提高CQ的接受度和相关性，在领域专家中表现出高可用性和有效性。

Conclusion: IDEA2通过LLM与专家协作的迭代工作流，有效解决了CQ提取的瓶颈问题，提高了本体工程效率和质量，代码和实验已开源。
...
<details>
  <summary>Details</summary>
Motivation: 传统智能体中心架构在企业和制度系统等结构化领域中存在语义不一致、可解释性差和长期稳定性不足的问题，需要一种能够确保全局一致性和可验证系统行为的新架构。

Method: 引入世界为中心的多智能体系统，通过共享的显式世界表示来协调智能体行为；提出世界分类维度（本体显式性、规范性等）；使用语义模型作为表示世界的数学形式化方法；开发Ontobox平台作为WMAS的实现。

Result: 提出了WMAS理论框架，包括世界分类维度、语义模型形式化方法，以及Ontobox平台作为具体实现，为结构化领域提供了确保语义一致性、可解释性和长期稳定性的新架构方案。

Conclusion: 世界为中心的多智能体系统通过共享的显式世界表示，能够有效解决结构化领域中的语义一致性、可解释性和长期稳定性问题，为企业和制度系统提供了更可靠的多智能体架构方案。
...
TL;DR: 该研究提出AI自动化是一个连续谱：一端是"崩溃浪潮"（AI能力在少数任务上突然爆发），另一端是"涨潮"（AI能力持续广泛提升）。基于对3000多个文本任务的评估，研究发现AI自动化主要表现为"涨潮"模式，AI能力正在广泛持续提升，预计到2029年LLM能在80%-95%成功率下完成大多数文本任务。


<details>
  <summary>Details</summary>
Motivation: 研究旨在理解AI自动化的本质模式：是集中在少数任务上的突发性突破（崩溃浪潮），还是广泛持续的渐进提升（涨潮）。这一区分对于预测AI对经济和劳动力市场的影响至关重要，因为不同的自动化模式会导致不同的采用时间线和影响范围。

Method: 基于美国劳工部O*NET分类，构建了3000多个文本型任务数据集，这些任务都是LLM可处理的。通过来自相关职业的工人进行了超过17000次评估，测量AI模型在不同任务上的表现。使用时间序列分析追踪AI能力的改进轨迹，并基于当前趋势预测未来表现。

Result: 研究发现几乎没有证据支持"崩溃浪潮"模式（与METR的近期研究相反），但发现了"涨潮"模式的显著证据。AI在广泛任务上表现良好且改进迅速。2024年第二季度，AI模型能以约50%成功率完成人类需要3-4小时的任务，到2025年第三季度预计提升至65%。如果当前趋势持续，到2029年LLM平均能在80%-95%成功率下完成大多数文本任务。

Conclusion: AI自动化主要表现为"涨潮"模式，即能力在广泛任务上持续渐进提升。这种模式意味着AI对经济和劳动力市场的影响将是渐进而广泛的，而非集中在少数领域的突发性颠覆。虽然AI能力提升迅速，但组织采用AI的过程可能需要更长时间，这为劳动力市场调整提供了缓冲期。
...
TL;DR: 该研究探讨了LLM中的认知偏见，发现偏见在模型激活空间中呈现线性可分方向，并通过激活导向技术成功减少了26-32%的偏见，同时保持了模型在其他任务上的能力。
...
Method: 定义了LLM认知偏见为系统性的、可重复的偏离正确答案的偏差，并引入LLM CogBias基准，围绕四个认知偏见家族进行评估。使用线性探针和对比设计分析偏见在激活空间中的编码，并应用激活导向技术来调节偏见行为。

Result: 研究发现认知偏见在所有四个家族中都系统性出现，其程度和去偏见效果强烈依赖于偏见家族。偏见在模型激活空间中编码为线性可分方向，激活导向技术实现了26-32%的偏见分数减少，同时在25个基准测试中保持了模型的下游能力。尽管不同模型的偏见表征近乎正交，但导向技术在不同架构中都能以相似速率减少偏见。

Conclusion: LLM中的认知偏见在激活空间中具有可识别的线性表征，可以通过激活导向技术有效缓解。不同模型虽然偏见表征不同，但共享相似的功能组织，使得跨架构的偏见干预成为可能。这为开发更可靠、无偏见的AI系统提供了理论基础和技术路径。
...
TL;DR: 提出RIFT分类法，用于系统化诊断评估量规的质量问题，包含8种失效模式，分为3大类，并通过人工标注和自动化指标验证其有效性。
...
Method: 提出RIFT分类法，包含8种失效模式，分为可靠性失效、内容效度失效和后果效度失效三大类。采用扎根理论方法，从5个多样化基准中迭代标注量规直至饱和。评估分类法一致性，并提出自动化量规质量指标。

Result: 分类法在人工标注中显示出良好一致性（87%配对一致性和0.64平均Cohen's kappa）。自动化指标与人工失效模式标注对齐，最高达到0.86 F1分数。

Conclusion: RIFT为量规质量诊断提供了系统化框架，支持可扩展的诊断，自动化指标与人工标注对齐良好，有助于改进LLM评估中的量规设计。
...
Method: 1) 提出动态规划框架，基于每个信念点的信息价值有条件地处理观测；2) 基于该框架提出VOIMCP算法，这是一种蒙特卡洛树搜索算法，当VOI较低时选择性忽略观测信息，避免不必要的观测分支；3) 提供理论保证和收敛界。

Result: 理论分析提供了VOI推理框架的近似最优性保证和VOIMCP的非渐进收敛界。仿真评估表明，VOIMCP在多个POMDP基准测试中优于基线方法。
...
TL;DR: 该论文研究了AI辅助用药系统的可靠性，重点关注系统失败类型及其临床后果，而非仅依赖传统性能指标评估


<details>
  <summary>Details</summary>
Motivation: AI系统在医疗和药房工作流程中日益普及，支持用药推荐、剂量确定和药物相互作用检测等任务。虽然这些系统在标准评估指标下通常表现良好，但其在真实世界决策中的可靠性仍未充分理解。在用药管理等高风险领域，即使单个错误推荐也可能导致严重的患者伤害

Method: 通过一系列涉及药物相互作用和剂量决策的受控模拟场景，分析不同类型的系统失败，包括遗漏相互作用、错误风险标记和不适当的剂量推荐。研究重点从聚合指标评估转向错误如何发生以及AI系统产生错误输出时的后果分析

Result: 研究发现，在用药相关情境中，AI错误可能导致药物不良反应、无效治疗或延迟护理，特别是在没有足够人工监督的情况下使用系统时。研究还揭示了过度依赖AI推荐的风险以及决策过程透明度有限带来的挑战

Conclusion: 该研究为医疗AI评估提供了以可靠性为中心的视角，强调理解失败行为和真实世界影响的重要性。在药房实践等安全关键领域，需要将传统性能指标与风险感知评估方法相结合，特别是在缺乏足够人工监督的情况下
...
TL;DR: Elhuyar是一个多智能体、人机协同系统，整合LLM、结构化AI和人类科学家，用于从科学文献中提取、分析和迭代提炼见解，旨在解决文献碎片化问题并加速科学发现。


<details>
  <summary>Details</summary>
Motivation: 科学发现因文献碎片化而减缓，现有AI工具缺乏结构化、多步骤的方法来从科学文献中提取深度见解。LLM虽提供新可能性，但存在幻觉和不完整提取的问题，需要更可靠的解决方案。

Method: 开发多智能体、人机协同系统，将任务分配给专门智能体进行论文筛选、数据提取、模型拟合和结果总结，通过人类监督确保可靠性。系统生成结构化报告，包含提取数据、可视化、模型方程和文本摘要，支持迭代精炼。

Result: 在材料科学领域部署，分析钨在氦离子辐照下的文献，发现氦气泡生长与辐照剂量和温度呈指数相关，为聚变反应堆中面向等离子体材料提供见解，展示了AI辅助文献综述发现科学模式的能力。

Conclusion: Elhuyar系统通过整合LLM、结构化AI和人类科学家，能够从科学文献中提取深度见解，加速科学发现，证明了AI辅助文献综述在揭示科学模式和促进发现方面的潜力。
...
Method: 1) 提出不可行性感知框架；2) 针对minor-embedding问题引入新的数学规划公式和可证明的零阶段不可行性筛选；3) 构建可验证的训练数据集；4) 对8B参数LLM进行监督微调；5) 利用LLM输出作为下游局部搜索的暖启动

Result: 微调模型比GPT-5.2整体准确率提升30%；LLM引导的暖启动在下游局部搜索中相比从零开始提供2倍加速

Conclusion: 该框架成功将LLM应用于组合优化问题，不仅提高了解决方案生成质量，还增强了不可行性检测能力，并通过LLM辅助的暖启动机制显著加速了下游优化过程
...
TL;DR: P-COD是一种利用文档间关系检测异常值的新方法，通过比较提取数据与语料库中已验证的同行信息来减少LLM幻觉，在科学文献摘要领域实现高达98%的异常检测精度。


<details>
  <summary>Details</summary>
Motivation: 当前减少LLM幻觉的方法（如提示工程和思维链提示）主要关注单个文档，未能考虑语料库中文档间的关系。在科学文献摘要领域，具有相似实验设置的论文应得出相似结论，这种关系可用于提高数据提取的准确性。

Method: 提出Peer Context Outlier Detection (P-COD)方法，利用文档间的关系来改进提取准确性。通过比较从文档中提取的数据与语料库中已验证的同行信息，调整置信度分数并标记低置信度结果供专家审查。得到同行验证的高置信度结果被视为可靠。

Result: 在6个科学领域的实验中，P-COD实现了高达98%的异常检测精度。该方法有效减少了幻觉，增强了对自动化系统的信任，使研究人员能够专注于模糊案例，从而简化数据提取工作流程。

Conclusion: P-COD通过利用文档间关系进行异常检测，显著提高了LLM数据提取的准确性，减少了幻觉，为科学文献摘要等领域的自动化数据提取提供了更可靠的解决方案。
...
TL;DR: 提出一个基于技能演化的智能体框架，用于超表面逆向设计，通过积累可重用的求解器特定专业知识来减少工作流构建负担。


<details>
  <summary>Details</summary>
Motivation: 超表面逆向设计需要复杂的计算电磁学专业知识和求解器特定的软件工程技能，现有的大语言模型驱动系统缺乏跨任务的可重用工作流知识积累。

Method: 提出一个智能体框架，包含编码智能体、演化技能工件和基于物理仿真的确定性评估器，通过上下文级技能演化迭代精化求解器特定策略，无需修改模型权重或底层物理求解器。

Result: 在训练对齐任务中，任务成功率从38%提升至74%，标准通过率从0.510提升至0.870，平均尝试次数从4.10降至2.30；在保留任务中，成功率变化不大，但最佳边界改进、错误组成变化和智能体行为表明工作流知识的部分迁移。
...
TL;DR: 提出基于角色的LLM框架用于健康食品政策信息抽取，通过分配专家角色解决政策文档结构多样性和不一致性导致的幻觉、误分类和遗漏问题


<details>
  <summary>Details</summary>
Motivation: 当前LLM在健康食品政策领域的信息抽取面临挑战，包括幻觉、误分类和遗漏等问题，这些问题源于政策文档的结构多样性和不一致性，需要更可靠的方法来自动化信息抽取
...
Conclusion: 该基于角色的LLM框架为健康政策信息抽取提供了可靠且透明的方法论，能够有效处理政策文档的结构复杂性，在复杂推理任务中表现优异
...
TL;DR: PHMForge是首个针对工业PHM（预测与健康管理）任务的LLM智能体基准测试，包含75个专家设计的场景、7类工业资产、5个核心任务类别，通过MCP服务器进行真实交互评估，发现当前最佳配置仅完成68%任务，存在工具编排、多资产推理和跨设备泛化等系统性问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试无法满足工业领域对LLM智能体的严格要求，工业领域的错误决策会带来重大的安全和财务后果。需要专门针对PHM任务的基准测试来评估LLM智能体在工业环境中的实际表现。

Method: 构建PHMForge基准测试，包含75个专家设计的场景，涵盖7类工业资产（涡轮风扇发动机、轴承、电机、变速箱、航空发动机）和5个核心任务类别（剩余使用寿命预测、故障分类、发动机健康分析、成本效益分析、安全/策略评估）。创建65个专用工具分布在两个MCP服务器上，实现基于执行的评估器，使用任务相适应的指标：回归任务用MAE/RMSE，分类任务用F1分数，健康评估用分类匹配。

Result: 评估领先框架（ReAct、Cursor Agent、Claude Code）与前沿LLM（Claude Sonnet 4.0、GPT-4o、Granite-3.0-8B）的组合，发现即使最佳配置也仅完成68%的任务。存在系统性问题：工具编排错误（23%序列错误）、多资产推理能力下降（14.9个百分点退化）、跨设备泛化能力差（在未见数据集上仅42.7%）。

Conclusion: PHMForge填补了工业PHM领域LLM智能体评估的关键空白，揭示了当前LLM智能体在工业应用中存在的严重局限性，特别是在工具编排、多资产推理和泛化能力方面。开源完整的基准测试套件以促进工业AI智能体研究。
...
TL;DR: 本文研究了在连续自回归模型中集成表示自编码器（RAE-AR）的挑战，提出了分布归一化和高斯噪声注入两种方法来解决高维表示自编码器在AR范式中的建模难题和训练-推理差距问题。


<details>
  <summary>Details</summary>
Motivation: 生成建模的潜在空间长期由VAE编码器主导，而预训练表示编码器（如DINO、SigLIP、MAE）的潜在表示被认为不适合生成建模。虽然RAE方法展示了表示自编码器也能达到与VAE编码器相当的性能，但将表示自编码器集成到连续自回归模型中仍未被充分探索。

Method: 针对AR模型中高维表示自编码器的两个主要挑战：复杂的令牌级分布建模和高维性放大的训练-推理差距（暴露偏差），提出了两种解决方案：1）通过分布归一化进行令牌简化以降低建模难度；2）在训练期间加入高斯噪声注入以增强预测鲁棒性并缓解暴露偏差。
...
TL;DR: 研究发现LLM训练中归一化层和优化器之间存在显著交互作用：Dynamic Erf与Muon优化器组合时性能下降明显，而Dynamic Tanh则无此问题，揭示了erf在Muon快速谱范增长下的两种失效模式。


<details>
  <summary>Details</summary>
Motivation: 传统上LLM训练中将归一化层和优化器视为独立设计选择，但本研究通过实验发现这种假设可能不成立，需要探究不同归一化方法（特别是Dynamic Erf）与新型优化器（如Muon）之间的交互作用及其失效机制。

Method: 采用3×2因子设计实验，在10亿参数规模、1000训练步数下，测试Dynamic Erf、Dynamic Tanh和RMSNorm三种归一化方法分别与AdamW和Muon两种优化器的组合效果。通过分析erf在Muon快速谱范增长下的行为，识别饱和和尺度盲区两种失效模式，并提出EMA混合和降低alpha参数两种缓解策略。

Result: Dynamic Erf与Muon组合时出现显著负交互作用：与RMSNorm的差距从AdamW下的+0.31 nats扩大到Muon下的+0.97 nats（约3倍）。Dynamic Tanh作为有界归一化对照组未显示类似惩罚。研究发现erf在Muon下存在饱和（有损压缩）和尺度盲区（丢弃激活幅度）两种失效模式。通过EMA混合恢复约84%性能差距，降低alpha参数从0.5到0.3恢复约80%差距。

Conclusion: 归一化层和优化器在LLM训练中并非独立设计选择，存在显著交互作用。Dynamic Erf与Muon组合时因erf函数的饱和特性和尺度盲区导致性能下降，但可通过调整参数或引入EMA机制缓解。研究强调在评估新归一化方法时需考虑与优化器的兼容性，避免因隐蔽的交互作用导致次优性能。
...
TL;DR: NED-Tree框架通过句子级提取和递归树结构解决LLM在非线性OR问题翻译中的语义对齐问题，在10个基准测试中达到72.51%的平均准确率


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在将自然语言描述的运筹学问题转换为可执行模型时，在线性任务中表现良好，但在现实世界非线性场景中性能严重下降。这主要是由于数学公式与求解器代码之间的语义不对齐，以及信息提取不稳定造成的

Method: NED-Tree框架采用：(a) 句子级提取策略确保参数映射的鲁棒性和可追溯性；(b) 递归树结构将复杂非线性项自适应分解为求解器兼容的子元素。同时提出了NEXTOR基准测试用于复杂非线性、多约束OR问题

Result: 在10个基准测试中，NED-Tree实现了72.51%的平均准确率，建立了新的最先进水平。这是首个通过元素分解驱动LLM解决非线性建模困难的框架，实现了建模语义与代码语义的对齐

Conclusion: NED-Tree框架通过系统化的方法解决了LLM在非线性OR问题翻译中的语义对齐挑战，为自动化OR问题建模提供了有效的解决方案。该框架和基准测试已开源
...
<details>
  <summary>Details</summary>
Motivation: 当前强化学习与验证推理（RLVR）方法通常需要正确性信号或批评标注来指导模型精炼答案，这限制了方法的可扩展性和实用性。研究者希望开发一种无需额外监督信号就能联合优化推理和自精炼能力的方法。

Method: 提出ThinkTwice两阶段框架：1) 第一阶段优化模型解决推理问题；2) 第二阶段优化模型精炼自己的解决方案。两个阶段使用相同的二元正确性奖励，基于Group Relative Policy Optimization (GRPO)实现，无需正确性信号或批评标注。

Result: 在五个数学推理基准和两个模型家族（Qwen3-4B和Olmo3-7B）上，ThinkTwice显著优于竞争性在线策略优化基线。在Qwen3-4B上，ThinkTwice在AIME基准上比GRPO在精炼前提升5个百分点，一次自精炼后提升11.5个百分点（pass@4）。训练动态分析显示隐含的"纠正-强化"课程：早期精炼主要纠正错误，随着模型改进自然转向保护已正确解决方案。

Conclusion: ThinkTwice建立了联合训练推理和自精炼作为RLVR的原则性和有效方法学，通过简单的两阶段框架和相同的奖励信号实现了显著的性能提升，揭示了模型在训练过程中自然发展的精炼策略演变。
...
TL;DR: LLMs作为教师时主要使用贝叶斯最优推理而非启发式策略，通过逆规划推断学习者知识状态，其教学决策与人类相似但提示干预效果有限


<details>
  <summary>Details</summary>
Motivation: 探究LLMs在教学决策中是基于对学习者知识状态的推理，还是使用简单的启发式规则，以理解其教学策略的本质

Method: 使用受控任务测试LLMs教学策略：教师LLM观察学习者在奖励标注有向图中的轨迹，揭示单一边缘以改善学习者路径规划；采用与人类研究相同的认知模型（贝叶斯最优教师、弱贝叶斯变体、启发式基线、非心理化效用模型）拟合LLMs选择；进行基线实验和支架干预实验

Result: 大多数LLMs表现良好，策略稳定，图间表现与人类相似；模型比较显示贝叶斯最优教学最能解释多数模型选择；支架干预虽能引导模型遵循推理或奖励导向提示，但未能可靠改善启发式不一致测试图上的教学表现，有时甚至降低性能

Conclusion: LLMs教学策略主要基于对学习者知识状态的贝叶斯推理（逆规划），而非简单启发式；认知模型拟合揭示了LLMs辅导策略，表明提示合规性不能保证更好的教学决策
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在图表到代码生成任务中主要依赖监督微调，缺乏代码执行环境的暴露，且即使最先进的模型也难以有效进行自校正。需要一种能够通过环境交互和迭代自校正来提升代码准确性和可执行性的方法。

Method: 提出MM-ReCoder模型，采用基于Group Relative Policy Optimization的两阶段多轮自校正强化学习策略：第一阶段通过共享首轮增强模型自校正能力，第二阶段通过全轨迹优化提升编码能力。模型通过与环境的交互和迭代自校正来学习生成更准确、可执行的代码。

Result: 在三个图表到代码基准测试中实现了最先进的性能，证明了通过强化学习和自校正策略能够显著提升代码生成质量。

Conclusion: MM-ReCoder通过强化学习和自校正能力有效解决了现有图表到代码生成方法的局限性，为多模态编码任务提供了一种更强大的解决方案。
...
Method: 提出CRaFT框架，使用拒绝边界附近的提示来评估特征对模型拒绝-合规决策的影响，基于电路影响力而非激活强度来排名特征。在Gemma-3-1B-it模型上进行验证。

Result: 在Gemma-3-1B-it上，CRaFT将攻击成功率从6.7%提升至48.2%，在多个越狱基准测试中优于基线方法。

Conclusion: 电路影响力比激活强度更可靠，是识别因果中介拒绝行为特征的更好标准，为理解LLM拒绝机制提供了新视角。
...
TL;DR: 提出了Metric Freedom (F)作为技能效用的先验预测器，通过度量评分景观的拓扑刚性来预测多智能体系统蒸馏为单智能体技能的效果，并基于此设计了自适应蒸馏框架。
...
Method: 引入Metric Freedom (F)作为技能效用的先验预测器，通过Mantel测试量化输出多样性与得分方差之间的耦合程度来度量评分景观的拓扑刚性。基于F设计了两阶段自适应蒸馏框架：阶段1作为选择性提取机制，在"自由"度量上提取工具和知识同时丢弃限制性结构以保持探索；阶段2专门针对"刚性"度量（F≲0.6）进行计算密集的迭代优化以消除轨迹局部过拟合。

Result: 在4个任务、11个数据集和6个度量上的评估显示，F强烈预测技能效用（ρ=-0.62，p<0.05）。相同的智能体轨迹在刚性度量和自由度量下产生截然相反（相反）的技能提升。自适应智能体匹配或超过原始MAS，同时将成本降低高达8倍，延迟降低高达15倍。

Conclusion: 技能效用本质上是一个度量层面的属性，而非任务属性。Metric Freedom (F)作为先验预测器能够有效指导多智能体系统向单智能体技能的蒸馏过程，通过自适应蒸馏框架可以在保持或提升性能的同时显著降低计算成本和延迟。
...
Result: 基于工具的图遍历方法在所有测试模型家族上均比上下文基线有显著且一致的性能提升，随着规模增加优势更加明显，而上下文方法在大型图谱上完全失效

Conclusion: GraphWalk框架通过工具化图导航使LLM能够执行可验证的多步推理，解决了大规模知识图谱上复杂查询的上下文限制问题，为实际应用提供了透明且可扩展的解决方案
...
TL;DR: 对四种AWS Bedrock大语言模型在收据项目分类任务上进行系统性成本感知评估，发现Claude 3.7 Sonnet在准确性和成本效率间达到最佳平衡


<details>
  <summary>Details</summary>
Motivation: 在生产导向的分类框架下，系统评估大语言模型在收据项目分类任务中的性能，特别关注准确性、响应稳定性和令牌级成本，并研究零样本与少样本提示方法在准确性和成本方面的适用性

Method: 比较四种通过AWS Bedrock提供的指令调优模型：Claude 3.7 Sonnet、Claude 4 Sonnet、Mixtral 8x7B Instruct和Mistral 7B Instruct，评估其准确性、响应稳定性和令牌级成本，并分析零样本与少样本提示方法的适用性

Result: 实验结果表明，Claude 3.7 Sonnet在分类准确性和成本效率之间实现了最有利的平衡

Conclusion: Claude 3.7 Sonnet是收据项目分类任务中在准确性和成本效率方面表现最佳的模型，为生产环境中的大语言模型选择提供了实证依据
...
<details>
  <summary>Details</summary>
Motivation: 扩散语言模型暴露去噪轨迹，为推理时控制提供天然机制；理想的幻觉缓解框架应利用模型原生信号而非依赖外部训练的幻觉分类器进行干预

Method: 提出承诺不确定性定位：给定去噪轨迹，识别交叉链熵超过无监督阈值的token位置；引入OSCAR框架：运行N条并行去噪链（随机揭示顺序），计算交叉链香农熵检测高不确定性位置，然后基于检索证据进行针对性重掩码

Result: 在TriviaQA、HotpotQA、RAGTruth和CommonsenseQA上使用LLaDA-8B和Dream-7B，OSCAR显著减少幻觉内容并提高事实准确性；消融实验确认定位和校正贡献互补增益，在N∈{4,8,16}时保持稳健

Conclusion: 扩散语言模型基于熵的不确定性信号优于专门的训练检测器，突显了扩散语言模型识别事实不确定性的固有能力，这是自回归模型的顺序token承诺结构所不具备的
...
TL;DR: EnviSmart是一个用于环境FAIR数据管理的生产系统，通过三轨知识架构和角色分离的多智能体设计，在LLM驱动的智能体工作流中实现可靠性，防止错误传播到不可逆操作。


<details>
  <summary>Details</summary>
Motivation: 将LLM驱动的智能体嵌入环境FAIR数据管理具有吸引力，因为它们可以外部化操作知识并在异构数据和不断发展的规范中进行规模化管理。然而，用概率性工作流替换确定性组件改变了故障模式：LLM管道可能生成看似合理但错误的输出，这些输出通过表面检查并传播到不可逆操作（如DOI注册和公开发布）。
...
Result: 在两个生产部署中进行了比较：大学GIS中心生态档案（849个数据集）作为单智能体基线；SF2Bench复合洪水基准（2,452个监测站和8,557个文件，跨越39年）验证了多智能体工作流。多智能体方法提高了效率（单个操作员在两天内完成，工件在部署间重复使用）和可靠性：审计交接检测并阻止了影响所有2,452个站点的坐标转换错误。代表性事件（ISS-004）展示了基于边界的遏制，检测延迟10分钟，零用户暴露，80分钟解决。

Conclusion: EnviSmart通过架构方法在LLM驱动的环境数据管理中实现了可靠性，将概率性工作流与确定性验证相结合，防止错误传播到不可逆操作。该系统已部署在校园级存储基础设施上，并被PEARC 2026会议接受。
...
TL;DR: CORAL：首个用于开放问题自主多智能体进化的框架，通过持久记忆、异步执行和心跳干预取代固定启发式，在数学、算法和系统优化任务上取得SOTA结果


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的进化方法严重依赖固定启发式和硬编码探索规则，限制了智能体的自主性，需要更自主的框架来支持开放问题的持续搜索和知识积累

Method: CORAL采用长期运行的智能体通过共享持久记忆、异步多智能体执行和心跳干预进行探索、反思和协作，提供隔离工作空间、评估器分离、资源管理和智能体健康管理等安全保障
...
Conclusion: 机制分析表明这些收益源于知识重用和多智能体探索与通信，结果表明更大的智能体自主性和多智能体进化能显著改善开放问题的发现过程
...
TL;DR: 提出七种面向临床AI系统的本体感知设计模式，用于构建对本体扭曲具有弹性的AI流水线，包括数据验证、信号保留、漂移监控、并行表示、反馈中断、术语版本管理和合规适配等模式。


<details>
  <summary>Details</summary>
Motivation: 临床AI系统通常在受文档工作流、计费激励和术语碎片化结构扭曲的健康数据上进行训练。现有研究已描述了这种扭曲的机制，但将这些见解转化为可实施的软件架构仍是一个开放问题。

Method: 提出七种基于Gang-of-Four模式语言的"本体感知设计模式"：1) 本体检查点（数据摄取验证）；2) 休眠感知流水线（低频信号保留）；3) 漂移哨兵（连续漂移监控）；4) 双本体层（并行表示维护）；5) 物化断路器（反馈循环中断）；6) 术语版本门（术语演化管理）；7) 监管合规适配器（可插拔合规性）。每个模式都按问题、力量、解决方案、后果、已知用途和相关模式进行规范。

Result: 论文提供了七种设计模式的详细规范，并通过初级保健AI系统的参考架构和糖尿病风险预测场景的演练来说明它们的组合。三个模式在现有系统中有部分先例，其余四个尚未被正式描述。未报告实证验证，而是提供基于理论分析的设计词汇。

Conclusion: 该论文提出了一套设计模式词汇，用于构建对本体扭曲具有弹性的临床AI系统，填补了理论洞察与可实施架构之间的空白。这些模式为未来的生产系统评估提供了基础，但存在缺乏运行时基准和局限于德国及欧盟监管背景的局限性。
...
TL;DR: BACM-RL：一种基于强化学习的预算感知上下文管理方法，通过序列决策优化上下文压缩策略，在有限上下文预算下提升LLM智能体的长程推理能力


<details>
  <summary>Details</summary>
Motivation: LLM智能体在长程推理中面临上下文大小限制（内存、延迟、成本等因素），随着交互历史增长，需要在保留历史信息和遵守上下文预算之间进行权衡

Method: 提出预算感知上下文管理（BACM），将上下文管理建模为带预算约束的序列决策问题；开发BACM-RL，一种端到端课程式强化学习方法，学习不同上下文预算下的压缩策略

Result: 在组合多目标QA和长程网页浏览基准测试中，BACM-RL在不同模型规模和任务复杂度下均优于现有方法，在高复杂度设置中获得超过1.6倍的性能提升，在预算缩减时仍保持优势

Conclusion: BACM-RL通过智能的上下文压缩策略有效解决了LLM智能体在有限上下文预算下的长程推理挑战，为实际部署中的上下文管理提供了有效解决方案
...
TL;DR: HMO框架通过三层分级内存管理解决智能体长期记忆存储膨胀问题，基于用户画像动态调整记忆层级，在保持个性化推理的同时降低检索噪声和计算延迟。


<details>
  <summary>Details</summary>
Motivation: 智能体需要长期记忆来维持历史感知一致性，但交互数据积累导致存储膨胀、检索噪声增加和计算延迟，特别是在资源受限的个人设备上会压倒模型的推理能力。

Method: 提出分层内存编排(HMO)框架，将交互历史组织成三层目录：紧凑主缓存（存储近期和关键记忆+动态用户画像）、高优先级次层、全局完整历史档案。用户画像驱动记忆在层级间重新分配，将长期模式相关的记录提升到更活跃层级，降低不相关信息优先级。

Result: 在多个基准测试中达到最先进性能。在OpenClaw等生态系统中的实际部署显示，HMO显著提升了智能体的流畅性和个性化程度。

Conclusion: HMO通过用户中心化的分层内存管理，实现了精确的历史知识检索，同时保持了精简高效的活跃搜索空间，解决了长期记忆存储的扩展性问题。
...
<details>
  <summary>Details</summary>
Motivation: 当前模型融合方法主要针对同构模型（来自相同预训练骨干），但在开放模型生态系统中，有用的专家模型通常基于不同架构（如Llama、Qwen、Mistral），异构模型间的直接权重空间融合由于架构不匹配、潜在基不对齐和跨源冲突而变得不适定
...
Conclusion: HeteroFusion为解决异构语言模型融合问题提供了有效解决方案，能够在不同架构模型间实现知识转移和融合，克服了传统方法在异构设置中的局限性
...
TL;DR: 该研究利用中国视频平台数据，分析AI生成内容与人类生成内容在创作和消费行为上的差异，发现AIGC通过高产量获得与HGC相当的总体参与度，但消费者更偏好HGC，算法分发机制在其中起到调节作用。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成内容的快速扩散，其正在重塑在线内容生态，需要对其行为特征和分布影响进行严谨研究，以理解AIGC与人类生成内容的差异及其对平台生态的影响。

Method: 利用中国领先视频分享平台包含数千万用户的纵向数据集，对比分析AIGC和HGC的创作与消费行为特征，识别两者在生产和消费模式上的差异。

Result: 研究发现存在"规模优于偏好"的动态关系：AIGC创作者通过高产量获得与HGC创作者相当的总体参与度，尽管消费者明显更偏好HGC。算法内容分发机制能够调节AIGC相关的竞争利益。

Conclusion: 研究建议实施对AIGC敏感的分发算法和精准治理框架，以确保在线内容平台的长期健康发展，平衡AIGC与HGC的竞争关系。
...
TL;DR: 提出LiteInception框架：用于通用航空故障诊断的轻量级可解释边缘部署方案，通过两阶段级联架构、多方法融合压缩和知识蒸馏调节，在NGAFID数据集上验证了效率、精度和可解释性的良好平衡。


<details>
  <summary>Details</summary>
Motivation: 通用航空故障诊断和高效维护对飞行安全至关重要，但深度学习模型在资源受限的边缘设备上部署面临计算能力和可解释性的双重挑战。

Method: 1. 采用两阶段级联架构：第一阶段高召回率故障检测，第二阶段异常样本细粒度故障分类；2. 基于互信息、梯度分析和SE注意力权重的多方法融合策略压缩输入传感器通道（23→15）；3. 提出1+1分支LiteInception架构，压缩InceptionTime参数70%；4. 引入知识蒸馏作为精度-召回调节机制；5. 构建集成四种归因方法的双层可解释性框架。

Result: 在NGAFID数据集上：故障检测准确率81.92%，召回率83.24%；故障识别准确率77.00%；CPU推理加速超过8倍，F1损失小于3%。

Conclusion: LiteInception框架在效率、准确性和可解释性之间取得了良好平衡，通过轻量化设计、自适应调节和可追溯证据链，为资源受限的边缘设备提供了可行的故障诊断解决方案。
...
Method: 采用专家在环方法，结合LLM辅助的需求获取与协作本体工程，开发了AnIML本体。通过多层验证方法：将真实AnIML文件转化为知识图谱的数据驱动转换、通过SPARQL进行能力问题验证，以及基于对抗性负面能力问题的新验证协议，这些问题映射到已建立的本体反模式并通过SHACL约束强制执行。

Result: 成功开发了AnIML本体，这是一个OWL 2本体，形式化了AnIML的语义并与Allotrope数据格式对齐。通过多层验证方法证实了本体的有效性和可靠性，支持跨系统和跨实验室的互操作性。

Conclusion: AnIML本体通过形式化AnIML的语义并与Allotrope数据格式对齐，为解决实验数据系统间的语义互操作性问题提供了有效解决方案。该方法结合了专家知识和自动化技术，并通过创新的验证协议确保本体质量，为未来跨系统和跨实验室的数据交换奠定了基础。
...
Method: 提出基于SAT的框架：1）将零件类型按需求展开，每个副本分配板材变量；2）仅对分配到同一板材的副本激活非重叠约束；3）引入不可行方向消除规则，当只有一种方向能放入板材时固定旋转变量。比较三种最小化板材数量的方法：非增量SAT（二分搜索）、增量SAT（跨迭代重用子句）和加权部分MaxSAT。

Result: 在Cui-Zhao基准测试套件中，最佳SAT配置证明为最优的实例数量是OR-Tools、CPLEX和Gurobi的2-3倍，且获得更低的优化间隙。SAT方法的相对排名取决于旋转设置：无旋转时增量SAT最强，当旋转增加公式大小时非增量SAT更有效。

Conclusion: SAT方法在解决二维单规格板材切割问题上表现出色，优于主流商业求解器。不可行方向消除规则有效简化问题，而不同SAT策略的适用性取决于旋转约束的复杂性。该方法为制造业切割优化提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 通用大语言模型将生成视为单次文本补全，无法满足安全关键工程工作流中固有的顺序多门约束，导致生成可执行仿真工件时出现级联约束违反，成为高超声速热防护系统设计中的瓶颈。

Method: 提出约束闭环生成框架，将TPS工件生成组织为迭代工作流：生成、验证、CDG引导修复、执行和审计。约束依赖图编码约束类别间的经验共解析结构，基于生命周期排序先验和经验共解析概率指导修复向上游故障候选。

Result: AeroTherm-GPT在HyTPS-Bench上达到88.7%端到端成功率（95%置信区间：87.5-89.9），比非CDG消融基线提升12.5个百分点，根因修复效率达4.16（vs. 平面检查表修复的1.76），且在科学推理和代码生成任务上无灾难性遗忘。

Conclusion: 约束闭环生成框架通过约束依赖图指导的迭代修复，显著提升LLM在高超声速热防护系统设计中的约束满足能力，为安全关键工程领域的LLM应用提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前知识图谱系统在表示概念时面临一个核心问题：概念的含义会随着使用领域的不同而发生变化。现有系统通常将领域信息作为元数据、限定符或图级组织来处理，这些机制主要用于过滤和溯源，但不会改变断言的形式状态。当领域被当作外部信息处理时，会导致知识系统的实际应用失败。

Method: 提出领域情境化概念图（DCG）框架，将领域直接写入关系表示中，形式化为(C, R at D, C')，其中"at D"标识关系成立的世界。采用克里普克式语义学，将关系解释为领域索引的必然性算子，使真值、推理和冲突检查都在相关世界范围内进行。开发了紧凑的谓词系统、Prolog实现，并提供了到RDF、OWL和关系数据库的映射。

Result: DCG框架实现了三个关键结果：1）模糊概念可以在表示点进行消歧；2）无效断言可以根据其领域进行挑战；3）跨领域关系可以通过显式谓词连接。这为知识表示提供了结构化和可计算的角色，解决了领域信息处理不当导致的实践失败。

Conclusion: 论文的核心贡献是对领域本身进行表示性重新解释，主张领域应作为知识表示的内在组成部分而非外部注解。DCG通过赋予领域在表示内部的结构化和可计算角色，解决了当前知识系统中因领域处理不当而导致的许多实际问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于可验证奖励的强化学习框架存在根本性方法缺陷：通过在所有生成token上分配相同的优势值，这些方法本质上稀释了优化多模态推理中关键视觉基础步骤所需的学习信号。

Method: 提出Token Visual Dependency概念，通过KL散度量化视觉输入的信息增益；引入Perception-Grounded Policy Optimization框架，采用阈值门控、质量守恒机制动态重塑token级优势，放大视觉依赖token的学习信号，抑制语言先验的梯度噪声。
...
TL;DR: 该论文提出了AWARE框架，通过监督嵌入学习和轻量级适配器改进临床电子健康记录预测中的表格上下文学习，在极端不平衡情况下显著提升性能。
...
Method: 提出了AWARE框架，包含监督嵌入学习和轻量级适配器，用于任务对齐的检索。通过多队列电子健康记录基准，比较了经典方法、深度表格方法和表格上下文学习方法在不同数据规模、特征维度、结果稀有性和跨队列泛化能力下的表现。

Result: PFN-based表格上下文学习模型在低数据量情况下样本效率高，但在异质性和不平衡增加时，基于朴素距离的检索性能下降。AWARE框架在极端不平衡情况下将AUPRC提升高达12.2%，且随着数据复杂性增加，改进效果更明显。
...
TL;DR: 提出CG-iLAO*算法，通过线性规划和约束生成技术避免考虑昂贵的动作，显著减少Bellman备份中需要评估的动作数量，提高随机最短路径问题的求解效率。


<details>
  <summary>Details</summary>
Motivation: 现有随机最短路径问题求解算法虽然使用启发式函数来指导搜索，但在执行Bellman备份时仍然需要考虑所有适用动作，即使启发式信息表明某些动作代价过高。这种对所有动作的全面评估浪费了计算时间，降低了算法效率。

Method: 将启发式搜索重新表述为线性规划问题，引入针对随机最短路径问题的约束生成高效实现。基于此技术开发了CG-iLAO*算法，该算法在iLAO*基础上改进，利用启发式信息避免考虑昂贵的动作，只评估有前景的动作。

Result: CG-iLAO*在许多问题上只考虑iLAO*所需动作的40%，在某些问题上甚至只考虑1%。平均而言，CG-iLAO*比iLAO*和LRTDP少计算3.5倍的动作代价，求解速度分别快2.8倍和3.7倍。

Conclusion: 通过将启发式搜索重新表述为线性规划并采用约束生成技术，能够有效避免考虑昂贵的动作，显著提高随机最短路径问题的求解效率。CG-iLAO*算法在减少动作评估数量和加速求解方面表现出显著优势。
...
<details>
  <summary>Details</summary>
Motivation: 解决多类分类中可能性监督的学习问题，其中监督信息是归一化的可能性分布，需要将其转化为概率约束并融入模型训练

Method: 1) 从可能性分布构建非空闭凸可容许概率分布集，结合概率兼容性和线性形状约束；2) 使用Dykstra算法计算模型输出到可容许集的KL投影；3) 通过最小化预测与投影之间的散度来训练模型

Result: 在合成数据和真实世界自然语言推理任务（基于ChaosNLI数据集）上的实验表明，投影算法足够高效实用，基于投影的学习目标能提升预测性能

Conclusion: 提出的投影算法有效解决了可能性监督下的多类分类问题，通过KL投影将可能性约束融入模型训练，在保持可能性分布定性结构的同时提升了学习效果
...
Method: 基于一阶逻辑构建逻辑框架，支持对公式和上下文进行量化，允许在上下文中使用超协调逻辑，设计为可有限公理化，确保与现有定理证明器兼容。

Result: Qiana能够表示时间性、事件演算和模态逻辑，展示了框架的表达能力，同时讨论了不同的设计替代方案。

Conclusion: Qiana提供了一个强大而灵活的逻辑框架，能够在特定上下文中进行推理，支持量化表达和矛盾容忍，并与现有一阶逻辑工具兼容，具有广泛的应用潜力。
...
TL;DR: 该研究创建了NeuroPose-AHM数据集，这是一个包含57种神经系统疾病中异常头部运动的多条件知识库，并通过四个任务框架验证了其分析效用，特别是在颈肌张力障碍中的应用。


<details>
  <summary>Details</summary>
Motivation: 神经系统疾病中异常头部运动的研究缺乏整合运动学测量、临床严重程度评分和患者人口统计学的多条件资源，这阻碍了AI驱动诊断工具的开发。

Method: 使用多LLM提取框架从1,430篇同行评审文献中构建NeuroPose-AHM数据集，包含2,756条患者组记录。通过四任务框架验证数据集效用：1) 多标签AHM类型分类；2) 构建头颈严重程度指数(HNSI)；3) 用真实患者数据验证HNSI；4) 运动类型概率与HNSI分数的桥接分析。

Result: 数据集包含57种神经系统疾病的2,756条记录。LLM提取可靠性高(kappa=0.822)。AHM类型分类F1=0.856。HNSI成功统一了异质临床评分量表。真实患者验证显示严重带比例一致(6.7%)。运动类型与HNSI显著相关(p<0.001)。

Conclusion: NeuroPose-AHM是一个结构化、基于知识的资源，对神经系统异常头部运动研究具有分析效用，数据集已公开可用。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型常默认逐步计算，即使存在高效数值捷径。研究旨在评估LLMs是否具备类似人类的数字直觉能力，即识别数值结构、适当应用捷径并在不适用时避免使用的能力。

Method: 引入SenseMath基准测试，包含4,800个项目，涵盖八个捷径类别和四个数字规模，匹配强捷径、弱捷径和控制变体。支持三个评估设置：捷径使用、适用性判断和问题生成。评估五个LLM模型，从GPT-4o-mini到Llama-3.1-8B。

Result: 当明确提示时，模型能采用捷径策略并在捷径适用项目上获得显著准确率提升（高达15%），但在标准思维链提示下，自发使用此类策略的情况少于40%。模型能力仅限于使用层面，系统性地过度泛化捷径到不适用的问题，且无法从头生成有效的捷径承载问题。

Conclusion: 当前LLMs表现出程序性捷径流畅性，但缺乏支撑人类数字直觉的结构性理解，即理解捷径何时及为何有效的深层认知能力。
...
TL;DR: 基于Transformer掩码自编码器的无标签步态异常检测与运动学校正框架，仅使用正常步态数据训练，通过两阶段推理实现关节级异常定位与轨迹校正。


<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的步态分析方法大多依赖有监督分类器，需要疾病标签数据，难以泛化到异质性病理表现。需要一种无需疾病标签、能够解释性定位步态异常的方法。

Method: 提出基于Transformer掩码自编码器的无标签框架：1) 仅使用150名成年人的正常步态序列训练；2) 推理时采用两阶段流程：首先通过遮挡单个关节并测量与学习到的正常先验的偏差来估计关节不一致性分数，然后从编码器输入中排除标记的关节，从剩余时空上下文重建完整骨架，在标记位置生成校正后的运动学轨迹。

Result: 在10名保留的正常参与者（模拟七种步态异常）上的验证显示：准确识别生物力学不一致关节，所有分析关节的角度偏差显著减少（效应量大），并保持正常运动学特征。

Conclusion: 该方法实现了无需疾病标签的可解释、个体特异性步态障碍定位，为异质性病理表现的步态分析提供了有效工具。
...
TL;DR: ProCeedRL：一种针对多轮智能体任务的强化学习方法，通过过程级批评家和探索性演示来解决错误累积问题，显著提升探索效率和任务性能
...
Method: 提出ProCeedRL（Process Critic with Explorative Demonstration RL），将探索从被动选择转向主动干预。采用过程级批评家实时监控交互，结合基于反思的演示来指导智能体停止错误累积。通过从探索性演示和在线策略样本中学习，提升探索效率。

Result: 该方法显著超越了模型的饱和探索性能，展现出实质性的探索优势。在复杂的深度搜索和具身任务中，ProCeedRL显著提高了探索效率并实现了优越的性能表现。

Conclusion: ProCeedRL通过过程级监控和探索性演示的主动干预机制，有效解决了多轮智能体任务中的错误累积问题，为强化学习在复杂智能体任务中的应用提供了新的有效方法。
...
TL;DR: ATBench是一个轨迹级基准测试，用于评估基于LLM的智能体在多步交互中的安全性，通过三维风险分类、异构工具池和长上下文延迟触发协议实现结构化、多样化和真实的安全评估。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹级基准测试存在交互多样性不足、安全失败观测粒度粗、长时域真实性弱等局限，而实际部署中的风险往往在多步交互中显现，而非孤立提示或最终响应，因此需要更全面的智能体安全评估框架。

Method: 提出三维风险分类法（风险来源、失败模式、现实危害），构建包含异构工具池的轨迹，采用长上下文延迟触发协议捕捉多阶段风险涌现。包含1000条轨迹（503安全/497不安全），平均9.01轮对话和3.95k tokens，涉及1954次工具调用，工具池包含2084个可用工具。通过规则和LLM过滤加人工审核确保数据质量。

Result: 实验表明ATBench对前沿LLM、开源模型和专用防护系统都具有挑战性，同时支持分类分层分析、跨基准比较和长时域失败模式诊断。基准测试能够有效评估智能体在多步交互中的安全表现。

Conclusion: ATBench提供了一个结构化、多样化和真实的轨迹级基准测试，能够全面评估LLM智能体在多步交互中的安全性，解决了现有基准在交互多样性、观测粒度和长时域真实性方面的不足，为智能体安全评估提供了有效工具。
...
TL;DR: 该论文是一篇关于语言模型中潜在空间的系统性综述，从基础、演化、机制、能力和展望五个维度全面梳理了该领域的研究现状。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型的快速发展，潜在空间正成为语言模型计算的原生基质。传统基于显式token生成的理解方式存在语言冗余、离散化瓶颈、序列效率低下和语义损失等结构限制，而许多关键内部过程在连续潜在空间中更为自然。本文旨在为基于语言模型的潜在空间研究提供一个统一且最新的全景图。

Method: 采用系统性综述方法，从五个顺序视角组织内容：基础（界定潜在空间范围）、演化（追溯领域发展历程）、机制（从架构、表示、计算和优化四个维度分析技术发展）、能力（展示潜在空间支持的各种能力）和展望（讨论开放挑战和未来方向）。

Result: 论文系统性地梳理了语言模型中潜在空间的研究现状，建立了完整的分类框架，识别了四个主要技术发展方向（架构、表示、计算、优化）和七个能力维度（推理、规划、建模、感知、记忆、协作、具身化），为理解潜在空间作为下一代智能的通用计算范式奠定了基础。

Conclusion: 潜在空间正成为语言模型的关键计算范式，克服了显式空间的结构限制。本文不仅为现有研究提供了参考框架，更重要的是为理解潜在空间作为下一代智能的通用计算和系统范式奠定了基础。未来研究需要解决该领域的关键开放挑战，推动潜在空间在智能系统中的进一步发展。
...
Conclusion: ARQuest能显著提升用户满意度并简化保险流程，进一步发展后可能在风险准确性上超越传统方法，推动保险行业创新。
...
TL;DR: Diff-KD：基于扩散生成精炼和知识蒸馏的鲁棒多智能体协同感知框架，通过渐进知识蒸馏恢复被破坏的语义，自适应门控融合动态加权邻居信息，在多种破坏类型下实现最优性能。
...
Method: 提出Diff-KD框架，包含两个核心组件：1) 渐进知识蒸馏(PKD)：将局部特征恢复建模为条件扩散过程，从破坏观测中恢复全局语义；2) 自适应门控融合(AGF)：基于自车可靠性在融合过程中动态加权邻居信息。

Result: 在OPV2V和DAIR-V2X数据集上，针对七种破坏类型进行评估，Diff-KD在检测精度和校准鲁棒性方面均达到最先进性能。

Conclusion: Diff-KD通过整合扩散生成精炼与知识蒸馏，有效解决了多智能体协同感知中的破坏问题，实现了主动语义恢复和鲁棒融合，为现实世界部署提供了可靠解决方案。
...
TL;DR: 本文研究了LLM作为时间序列解释的生成器和评估器，在无参考设置下评估解释的事实正确性，发现生成性能高度依赖模式且存在系统性失败，而评估性能更稳定可靠。


<details>
  <summary>Details</summary>
Motivation: 评估LLM生成的时间序列解释的事实正确性是一个开放挑战。现有方法有限：基于参考的相似性指标和一致性检查模型需要真实解释，传统时间序列方法只能处理数值无法评估自由文本推理。缺乏无需预定义参考或任务特定规则的通用方法来直接验证解释是否忠实于底层时间序列数据。

Method: 研究LLM作为时间序列解释的生成器和评估器，在无参考设置下，给定时间序列、问题和候选解释，评估器基于模式识别、数值准确性和答案忠实性分配三元正确性标签。构建包含350个时间序列案例的合成基准，涵盖七种查询类型，每种配对正确、部分正确和错误解释。评估模型在四个任务上的表现：解释生成、相对排序、独立评分和多异常检测。

Result: 结果显示明显不对称性：生成性能高度依赖模式，在某些查询类型上存在系统性失败，准确率从季节性下降和波动率变化的0.00-0.12到结构突变的0.94-0.96不等；而评估性能更稳定，即使模型自身输出错误，也能正确排序和评分解释。

Conclusion: 这些发现证明了基于LLM的数据驱动评估在时间序列解释中的可行性，并突显了它们作为时间序列领域数据驱动推理可靠评估器的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在检测LLM驱动的社交机器人时存在三个主要问题：过度依赖单模态信号导致检测准确率有限；对AIGC特定生成模式不够敏感；未能充分建模语言模式与行为动态之间的相互作用。这些局限性促使研究者开发更有效的检测框架。

Method: 提出TRACE-Bot统一双通道框架，从异构数据源（个人信息、交互行为、推文数据）构建细粒度表示。采用双通道架构：一个通道通过预训练语言模型捕获语言表示，另一个通道通过多维活动特征（增强了SOTA AIGC检测器信号）捕获行为异常。最后通过轻量级预测头对融合表示进行分类。

Result: 在两个公开的LLM驱动社交机器人数据集上实现了SOTA性能，准确率分别达到98.46%和97.50%。结果还表明对高级机器人策略具有很强的鲁棒性，验证了联合利用隐式语义表示和AIGC增强行为模式的有效性。

Conclusion: TRACE-Bot通过联合建模隐式语义表示和AIGC增强的行为模式，有效解决了现有LLM驱动社交机器人检测方法的局限性，为检测新兴的LLM驱动社交机器人提供了有效的解决方案，展现了强大的检测性能和鲁棒性。
...
TL;DR: 提出了一种用于验证AI/ML系统在安全关键领域（如航空）中操作设计域（ODD）完整覆盖的方法，以满足EASA认证要求


<details>
  <summary>Details</summary>
Motivation: 当前EASA指南要求证明AI/ML组件的操作设计域（ODD）完全覆盖，但在高维参数空间中，现有方法缺乏可扩展性和形式化基础，无法满足完整性标准，且缺乏标准化工程方法来连接抽象ODD定义与可验证证据

Method: 提出了一种结构化多步骤ODD覆盖验证方法，整合了参数离散化、基于约束的过滤和基于关键性的维度约简，基于先前AI空中防撞研究的仿真数据进行验证

Result: 该方法能够验证高维空间中的ODD覆盖，实现了系统化工程方法来定义和实现满足EASA完整性要求的覆盖指标

Conclusion: 该方法支持在更高维度验证ODD覆盖，推进了"安全设计"方法，同时符合EASA标准，填补了抽象ODD定义与可验证证据之间的工程方法空白
...
TL;DR: 该研究评估了LLM生成的胸部CT报告日文翻译的教育适用性，发现放射科医师与LLM评估者之间几乎没有一致性，LLM评估者强烈偏好LLM输出，表明仅靠自动化LLM评估不足以用于放射学教育翻译的质量控制。


<details>
  <summary>Details</summary>
Motivation: 放射学报告的准确翻译对于多语言研究、临床沟通和放射学教育至关重要，但基于LLM的翻译评估有效性尚不明确。研究旨在评估LLM生成的胸部CT报告日文翻译的教育适用性，并比较放射科医师评估与LLM作为评估者的结果。

Method: 使用CT-RATE-JPN验证集中的150份胸部CT报告。每份英文报告的人工编辑日文翻译与DeepSeek-V3.2生成的LLM翻译进行比较。一名认证放射科医师和一名放射学住院医师独立进行盲法配对评估，评估术语准确性、可读性、整体质量和放射科医师风格真实性。同时，三个LLM评估者（DeepSeek-V3.2、Mistral Large 3和GPT-5）评估相同配对。使用QWK和百分比一致性评估一致性。

Result: 放射科医师与LLM评估者之间的一致性接近零（QWK=-0.04至0.15）。两位放射科医师之间的一致性也很差（QWK=0.01至0.06）。放射科医师1认为59%的病例术语等效，并在可读性（51%）和整体质量（51%）方面偏好LLM翻译。放射科医师2认为75%的病例可读性等效，并在整体质量方面偏好人工编辑翻译（40% vs 21%）。所有三个LLM评估者在所有标准上都强烈偏好LLM翻译（70%-99%），并在>93%的病例中认为其更具放射科医师风格。

Conclusion: LLM生成的翻译常被认为自然流畅，但两位放射科医师评估差异显著。LLM作为评估者表现出对LLM输出的强烈偏好，与放射科医师的一致性可忽略不计。对于放射学报告翻译的教育用途，仅靠自动化LLM评估是不够的；专家放射科医师的审查仍然至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 现有注意力可视化技术通常针对特定Transformer架构，需要反向传播导致GPU内存翻倍和计算成本增加，缺乏轻量级、模型无关的方法来理解生成式AI系统如何感知和优先处理输入文本信息

Method: 采用基于扰动的策略结合三矩阵分析框架：角度偏差矩阵捕捉语义方向变化，幅度偏差矩阵测量语义强度变化，维度重要性矩阵评估各向量维度的贡献。通过系统移除每个token并测量这三个互补维度的影响，得到复合重要性分数

Result: 开发了一种模型无关的token重要性可视化技术，能够生成显示token级贡献的相关性图，提供数学上严谨的token重要性度量，且无需额外计算成本

Conclusion: 提出的方法为理解生成式AI系统如何处理输入文本提供了轻量级、模型无关的可视化工具，开源实现促进了可重复性和广泛采用
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型需要可靠的弃权能力，但推理模型在弃权方面表现较差。模型幻觉导致的弃权失败可重新解释为模型回答了错误的问题而非错误地回答问题

Method: 提出查询错位框架和Trace Inversion方法：首先生成模型的推理轨迹，仅基于轨迹重构模型最可能响应的查询，然后比较初始查询与重构查询的相似度，低相似度表明模型可能回答错误并标记为弃权

Result: 在四个前沿LLM和九个弃权QA数据集上的广泛实验表明，Trace Inversion有效提升了弃权性能，在36个设置中33个击败了竞争基线方法

Conclusion: Trace Inversion通过查询错位框架重新解释幻觉问题，提供了一种新的最先进的弃权方法，显著提升了推理模型的可靠部署能力
...
<details>
  <summary>Details</summary>
Motivation: 探究用户查询中的第一人称情感框架如何影响大语言模型在多个基准领域的性能表现

Method: 在六个基准领域测试静态情感前缀的影响，引入EmotionRL自适应情感提示框架，动态选择情感框架

Result: 静态情感前缀通常只产生小的准确率变化；情感影响在社交任务中更显著；自适应选择比固定情感提示产生更可靠的性能提升

Conclusion: 情感语调不是LLM性能的主要驱动因素，也不是无关噪声，而是一种弱且输入依赖的信号，可通过自适应控制加以利用
...
<details>
  <summary>Details</summary>
Motivation: 将密集、层次结构化的法律文本转换为机器可读规则的过程成本高昂且需要专家参与，需要自动化解决方案来支持LLM系统的监管合规

Method: 四阶段管道：1) 源文档标准化为结构化Markdown；2) LLM驱动的语义分解为结构化规则单元；3) 多标准LLM-as-a-judge评估（19个维度）；4) 在有限再生预算内迭代修复低分提取

Result: 在金融、医疗和AI治理三个监管语料库上评估，金融领域提取质量持续单调提升，3次迭代内达到峰值性能；在医疗和AI治理领域保持高性能；下游合规问答评估中，De Jure提取规则在73.8%-84.0%情况下优于先前工作
