...
TL;DR: 提出数字孪生反事实框架(DTCF)，通过模拟而非统计估计反事实结果，建立分层验证体系将不可证伪的模拟器声称转化为可证伪的测试。


<details>
  <summary>Details</summary>
Motivation: 传统因果推断方法依赖假设填补缺失数据（如可忽略性、平行趋势、排除限制），但都无法直接产生反事实结果本身。需要一种新框架来系统化处理反事实模拟的验证问题。

Method: 提出数字孪生反事实框架(DTCF)：将数字孪生模拟器形式化为潜在结果框架中的随机映射；引入孪生保真度假设层次（边际保真度、联合保真度、结构保真度）；建立五层验证架构将不可证伪的模拟器声称转化为可证伪测试；形式化分解将因果量区分为边际可验证和copula依赖两类。

Result: 1) 边际因果主张（ATE、CATE、QTE）可通过可观测臂比较进行测试；2) 联合因果主张（ITE分布、获益/损害概率、处理效应方差）永久依赖于不可观测的个体内依赖结构；3) 提供边界、敏感性和不确定性量化工具使copula依赖显式化。

Conclusion: DTCF不解决因果推断的根本问题，但提供了一个框架：使边际因果主张越来越可测试，使联合因果主张明确假设索引化，并形式化表征两者之间的差距。该框架将不可证伪的模拟器声称转化为可证伪的验证测试。
...
<details>
  <summary>Details</summary>
Motivation: 能力问题提取是本体工程中的关键但资源密集型瓶颈阶段，主要困难在于领域专家（拥有知识）和本体工程师（形式化知识）之间的沟通鸿沟，需要更高效、协作的解决方案。

Method: IDEA2采用半自动化工作流，核心是迭代循环：1）LLM从需求文档中初步提取CQ；2）领域专家在协作平台上进行共创式评审和反馈；3）LLM根据反馈迭代重新表述被拒绝的CQ直至达成共识。整个过程使用溯源模型跟踪CQ的完整生命周期，包括编辑谱系、匿名反馈和生成参数。

Result: 在2个真实场景（科学数据、文化遗产）中验证表明，IDEA2能够：1）加速需求工程过程；2）提高最终CQ的接受度和相关性；3）在领域专家中展现出高可用性和有效性。

Conclusion: IDEA2通过集成LLM和专家在环协作，有效解决了本体工程中CQ提取的瓶颈问题，提供了透明、可复现的工作流，显著提升了需求工程效率和质量。
...
Method: 引入世界中心多智能体系统概念，对世界进行分类（本体显式性、规范性等维度），提出语义模型作为形式化表示工具，并开发Ontobox平台作为实现。

Result: WMAS通过共享世界模型实现学习和协调，确保全局一致性和可验证的系统行为，为结构化领域提供更可靠的智能体系统架构。

Conclusion: 世界中心多智能体系统是结构化领域智能体系统的有前景的替代架构，通过共享显式世界表示解决传统方法的局限性，具有重要的理论和实践意义。
...
TL;DR: 研究发现AI自动化呈现"涨潮式"而非"浪潮式"发展，AI能力在广泛任务中持续提升，预计到2029年LLM能以80%-95%成功率完成大多数文本任务


<details>
  <summary>Details</summary>
Motivation: 研究旨在验证AI自动化发展的两种模式：一是"浪潮式"（crashing waves）即AI能力在少数任务上突然爆发式提升；二是"涨潮式"（rising tides）即AI能力在广泛任务中持续渐进提升。通过实证数据检验哪种模式更符合现实发展情况。

Method: 基于美国劳工部O*NET分类体系，构建了3000多个文本型任务评估集，这些任务都是LLM可处理的。通过17000多名相关职业工作者的评估，系统测量AI模型在不同任务上的表现。采用时间序列分析追踪AI能力提升趋势，并基于当前趋势进行未来预测。

Result: 研究发现几乎没有证据支持"浪潮式"发展模式，但大量证据表明"涨潮式"是AI自动化的主要形式。AI在广泛任务中表现优异且提升迅速：2024年第二季度AI模型能以约50%成功率完成人类需要3-4小时的任务，预计到2025年第三季度将提升至65%。如果当前趋势持续，到2029年LLM在大多数文本任务上的平均成功率将达到80%-95%（最低质量要求水平）。

Conclusion: AI自动化主要表现为广泛、持续的"涨潮式"能力提升，而非集中在少数任务上的"浪潮式"突破。这种渐进但广泛的能力提升将对经济产生深远影响，但组织层面的AI采用可能需要更长时间。研究结果与METR等机构的"浪潮式"观点形成对比，强调了AI能力发展的广泛性和连续性特征。
...
TL;DR: 该研究探讨了大型语言模型中的认知偏差问题，开发了LLM CogBias基准来评估四种认知偏差类型，发现偏差在模型激活空间中呈线性可分离，并通过激活导向技术成功减少了26-32%的偏差。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地应用于高风险决策场景，先前研究表明LLM表现出行为上的认知偏差，但这些偏差是否对应可识别的内部表征以及能否通过针对性干预来缓解，仍是一个未解决的问题。

Method: 1. 定义LLM认知偏差为在可计算基准任务中系统性、可复现的偏离正确答案；2. 引入LLM CogBias基准，围绕判断、信息处理、社会和反应四种认知偏差家族组织；3. 评估三种LLM；4. 使用对比设计下的线性探针分析偏差在激活空间中的表征；5. 应用激活导向技术来调节偏差行为。

Result: 1. 认知偏差在所有四种家族中系统性出现，幅度和去偏差响应强烈依赖家族类型：提示级去偏差显著减少反应偏差，但对判断偏差产生反效果；2. 偏差在模型激活空间中编码为线性可分离方向；3. 激活导向实现了26-32%的偏差分数减少，同时在25个基准测试中保持下游能力；4. 尽管不同模型间的偏差表征接近正交，但导向在不同架构中减少偏差的效果相似，表明存在共享的功能组织。

Conclusion: LLM认知偏差在激活空间中具有可识别的线性表征，可以通过激活导向技术有效缓解，且这种干预在不同模型架构中表现出相似效果，揭示了LLM中认知偏差的共享功能组织特性。
...
TL;DR: 提出RIFT（RubrIc Failure mode Taxonomy）分类法，用于系统诊断评分标准在设计和构成中的失败模式，包含8种失败模式分属3个高层类别，并通过自动化指标支持可扩展的诊断。


<details>
  <summary>Details</summary>
Motivation: 虽然基于评分标准的评估在LLM基准测试和训练中被广泛使用，但现有方法仅通过下游信号（如强化学习结果）来评估评分标准有效性，缺乏从这些聚合或下游信号中诊断评分标准质量问题的原则性方法。

Method: 1. 使用扎根理论方法，从五个多样化基准（通用指令遵循、代码生成、创意写作、专家级深度研究）中迭代标注评分标准，直至不再发现新的失败模式；2. 通过测量独立人工标注者间的一致性来评估分类法的一致性；3. 提出自动化评分标准质量指标，并验证其与人工失败模式标注的对齐程度。

Result: 1. 开发了包含8种失败模式的RIFT分类法，分为可靠性失败、内容有效性失败和后果有效性失败三个高层类别；2. 观察到整体公平的一致性（87%的成对一致性和0.64的平均Cohen's kappa）；3. 自动化指标与人工失败模式标注对齐良好，达到最高0.86的F1分数。

Conclusion: RIFT分类法为系统诊断评分标准质量问题提供了原则性框架，自动化指标支持可扩展的诊断，有助于改进LLM评估和训练中评分标准的设计质量。
...
TL;DR: 提出基于信息价值(VOI)的POMDP规划框架VOIMCP，通过选择性忽略低价值观测信息来高效分配计算资源，在有限规划时间内获得高性能策略
...
Conclusion: 通过利用信息价值在信念空间上的变化特性，提出的VOIMCP框架能够更高效地分配计算资源，在有限规划时间内获得更好的POMDP策略性能，为解决大规模POMDP规划问题提供了有效方法
...
TL;DR: 该论文研究了AI辅助药物系统在真实医疗环境中的可靠性，重点关注系统错误类型及其临床后果，强调需要超越传统性能指标的风险感知评估方法。


<details>
  <summary>Details</summary>
Motivation: AI系统在医疗和药学领域应用日益广泛，但在高风险药物管理领域，即使单个错误推荐也可能导致严重患者伤害。现有评估主要依赖标准性能指标，对系统在真实决策中的可靠性理解不足，需要关注错误发生机制及其临床影响。

Method: 通过一系列受控模拟场景，分析药物相互作用和剂量决策中的系统失败类型，包括遗漏相互作用、错误风险标记和不适当剂量推荐，评估AI系统错误及其潜在临床后果。

Result: 研究发现AI在药物相关场景中的错误可能导致药物不良反应、治疗无效或护理延迟，特别是在缺乏足够人工监督的情况下。过度依赖AI推荐和决策过程透明度有限构成了额外风险。

Conclusion: 该研究为医疗AI评估提供了可靠性视角，强调理解失败行为和真实世界影响的重要性。在药房实践等安全关键领域，需要将传统性能指标与风险感知评估方法相结合，确保AI系统的安全可靠应用。
...
TL;DR: Elhuyar是一个多智能体、人机协同系统，整合LLM、结构化AI和人类科学家，用于从科学文献中提取、分析和迭代提炼见解，加速科学发现。


<details>
  <summary>Details</summary>
Motivation: 科学发现因文献碎片化而放缓，现有AI工具缺乏从科学文献中提取深度见解所需的结构化多步骤方法，LLM存在幻觉和不完整提取问题。

Method: 开发了多智能体、人机协同系统Elhuyar，将任务分配给专门的智能体进行论文筛选、数据提取、模型拟合和结果总结，通过人类监督确保可靠性，生成结构化报告并支持迭代精炼。

Result: 在材料科学领域部署，分析了钨在氦离子辐照下的文献，揭示了氦气泡生长与辐照剂量和温度呈指数相关的实验相关性，为聚变反应堆中的等离子体面向材料提供见解。

Conclusion: AI辅助的文献综述能够揭示科学模式并加速发现，Elhuyar系统展示了如何通过整合LLM、结构化AI和人类科学家来克服现有AI工具的局限性。
...
TL;DR: 提出了一种不可行性感知框架，结合可验证数据集构建、监督微调和LLM辅助下游搜索，用于解决组合优化问题中的不可行性检测
...
Method: 1) 提出不可行性感知框架；2) 针对minor-embedding问题引入新的数学规划公式和可证明的零阶段不可行性筛选；3) 构建可验证的训练数据集；4) 对8B参数LLM进行监督微调；5) 将LLM输出作为下游局部搜索的初始解

Result: 微调模型相比GPT-5.2整体准确率提升高达30%；LLM引导的初始解在下游局部搜索中相比从零开始提供高达2倍的加速

Conclusion: 该框架成功将LLM应用于组合优化问题，不仅提高了解决方案生成能力，还增强了不可行性检测，并通过LLM辅助的初始解加速了下游优化过程
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（如提示工程、思维链）仅关注单个文档，忽略了文档间关系，无法有效减少大语言模型在从大规模文本语料中提取数据时产生的幻觉

Method: 提出Peer Context Outlier Detection (P-COD)方法，利用文档间关系提高提取准确性。在科学文献摘要领域，通过比较提取数据与语料库中已验证的同行信息，调整置信度分数并标记低置信度结果供专家审查

Result: 在6个科学领域中实现了高达98%的异常检测精度，有效减少幻觉，增强对自动化系统的信任，使研究人员能够专注于模糊案例

Conclusion: P-COD方法通过利用文档间关系显著提高了数据提取的准确性，减少了LLM幻觉，优化了数据提取工作流程
...
TL;DR: 本文提出了一种基于角色的LLM框架，用于从健康食品政策文档中自动化信息提取，通过分配专业角色解决现有方法中的幻觉、误分类和遗漏问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在健康食品政策领域的信息提取面临多重挑战：政策文档结构多样且不一致导致幻觉、误分类和遗漏等问题，需要更可靠的方法来提取准确信息。

Method: 提出基于角色的LLM框架，分配三个专业角色：LLM政策分析师（元数据和机制分类）、LLM法律策略专家（识别复杂法律方法）、LLM食品系统专家（分类食品系统阶段）。框架将结构化领域知识（法律机制定义和分类标准）融入角色特定提示，模拟专家分析工作流。

Result: 使用Healthy Food Policy Project数据库中的608个健康食品政策进行评估，与零样本、少样本和思维链基线（Llama-3.3-70B）相比，该框架在复杂推理任务中表现出优越性能。

Conclusion: 该基于角色的LLM框架为健康政策信息提取提供了可靠且透明的方法论，在复杂推理任务中优于传统基线方法，能够有效自动化政策文档的信息提取。
...
TL;DR: 该研究探索了在连续自回归模型中集成表示自编码器（RAE-AR）的挑战，提出了分布归一化和高斯噪声注入两种方法来解决高维表示自编码器在AR范式中的建模困难，使表示自编码器在AR模型中达到与传统VAE相当的性能。


<details>
  <summary>Details</summary>
Motivation: 传统上，生成模型的潜在空间主要由VAE编码器主导，而预训练表示编码器（如DINO、SigLIP、MAE）的潜在表示被认为不适合生成建模。虽然RAE方法展示了表示自编码器在生成建模中的潜力，但将其集成到连续自回归模型中的研究仍然不足，需要探索高维表示自编码器在AR范式中的具体挑战和解决方案。

Method: 研究提出了RAE-AR框架，针对AR模型中高维表示自编码器的两个主要挑战：1）复杂的token-wise分布建模困难，通过分布归一化进行token简化；2）高维度放大的训练-推理差距（暴露偏差），通过训练期间注入高斯噪声来增强预测鲁棒性。

Result: 实验结果表明，所提出的修改方法显著缩小了性能差距，使表示自编码器在AR模型中能够达到与传统VAE相当的结果，为视觉理解和生成建模的统一架构铺平了道路。

Conclusion: 该工作成功解决了表示自编码器在连续自回归模型中的集成挑战，通过分布归一化和噪声注入技术有效克服了高维表示在AR范式中的建模困难，实现了表示自编码器在生成建模中的竞争力，推动了视觉理解与生成建模架构的统一。
...
TL;DR: 研究发现LLM训练中归一化层与优化器并非独立设计选择，Derf归一化与Muon优化器存在显著负交互作用，导致性能下降约0.66 nats，而DyT归一化则无此问题。


<details>
  <summary>Details</summary>
Motivation: 传统上LLM训练中将归一化层和优化器视为独立设计选择，但本研究通过实验发现这种假设可能不成立，特别是Derf归一化与Muon优化器之间存在显著的负交互作用，需要深入理解其失效机制。

Method: 采用3x2因子设计实验，在10亿参数规模和1000训练步数下，测试不同归一化层（Derf、RMSNorm、DyT）与优化器（AdamW、Muon）的组合。通过分析失效模式（饱和和尺度盲区），提出两种解决方案：EMA混合重新引入运行尺度估计，以及降低Derf的alpha参数。

Result: Derf与Muon组合存在显著负交互作用，性能差距从AdamW下的+0.31 nats扩大到Muon下的+0.97 nats。失效机制包括饱和（有损压缩）和尺度盲区（丢弃激活幅度）。EMA混合恢复约84%的性能差距，降低alpha参数（从0.5到0.3）恢复约80%的性能差距。

Conclusion: 归一化层与优化器设计不能独立考虑，存在显著的交互作用。Derf与Muon组合的负交互作用容易被短时试验忽略，需要仔细调整参数或采用EMA混合策略。研究强调了在LLM训练中需要系统评估不同组件间的兼容性。
...
TL;DR: NED-Tree是一个解决运筹学问题从自然语言到可执行模型翻译的框架，通过句子级提取和递归树结构分解非线性项，在NEXTOR基准测试中达到72.51%的平均准确率。
...
Conclusion: NED-Tree通过系统化框架成功弥合了运筹学问题建模中的语义鸿沟，为解决非线性复杂约束问题提供了有效方案，并发布了NEXTOR基准测试供进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有强化学习与验证推理（RLVR）方法通常需要额外的正确性信号或批评注释来指导精炼过程，这限制了方法的可扩展性和实用性。研究者希望开发一种无需外部信号就能联合优化推理和自精炼的方法。

Method: 提出ThinkTwice两阶段框架：1）第一阶段优化模型解决推理问题；2）第二阶段优化模型精炼自己的解决方案。两个阶段使用相同的二元正确性奖励，基于Group Relative Policy Optimization（GRPO）进行训练，无需额外的正确性信号或批评注释。

Result: 在五个数学推理基准和两个模型家族（Qwen3-4B和Olmo3-7B）上，ThinkTwice显著优于竞争性在线策略优化基线。在Qwen3-4B上，ThinkTwice在AIME基准上比GRPO在精炼前提升5个百分点，一次自精炼后提升11.5个百分点（pass@4）。训练动态分析显示存在隐式的"纠正-强化"课程：早期精炼主要纠正错误，随着模型改进自然转向保留已正确的解决方案。

Conclusion: ThinkTwice建立了推理和自精炼联合训练作为RLVR的原则性和有效方法学，通过简单的两阶段框架实现了显著的性能提升，揭示了训练动态中的隐式课程学习机制。
...
Method: 使用带奖励标注的有向图任务，让LLMs作为模拟教师揭示单个边以改善学习者路径；采用与人类研究相同的认知模型（贝叶斯最优教师、弱贝叶斯变体、启发式基线、非心理化效用模型）分析LLMs选择；进行模型比较（BIC）和支架干预实验

Result: 在基线实验中，大多数LLMs表现良好，策略变化小，图间表现与人类相似；贝叶斯最优教学模型最能解释大多数LLMs的选择；支架干预虽能引导模型遵循推理或奖励导向的提示，但未能可靠改善启发式不一致测试图上的教学效果，有时反而降低性能

Conclusion: LLMs在教学决策中主要采用贝叶斯推理策略而非简单启发式，认知模型拟合揭示了LLMs的教学策略，但提示依从性不能保证更好的教学决策
...
TL;DR: MM-ReCoder：一种基于强化学习和自校正能力的图表转代码生成模型，采用两阶段多轮自校正RL策略，在三个基准测试中取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在图表转代码任务中主要依赖监督微调，缺乏代码执行环境暴露，且自校正能力有限，需要更有效的训练策略来提升代码生成质量和可执行性

Method: 提出MM-ReCoder模型，采用基于GRPO的两阶段多轮自校正强化学习策略：第一阶段通过共享首轮增强自校正能力，第二阶段通过全轨迹优化提升编码能力

Result: 在三个图表转代码基准测试中实现了最先进的性能，模型通过与环境的交互和迭代自校正，能够生成更准确、可执行的代码

Conclusion: MM-ReCoder通过强化学习和自校正策略有效解决了现有MLLMs在图表转代码任务中的局限性，为多模态代码生成提供了新的训练范式
...
Conclusion: 电路影响力比激活强度更可靠，是识别因果介导拒绝行为特征的有效标准，为理解LLM拒绝机制提供了新视角。
...
TL;DR: 论文提出Metric Freedom (F)作为技能效用的先验预测器，通过度量评估指标的拓扑刚性来预测多智能体系统向单智能体技能蒸馏的效果，并基于此设计了自适应蒸馏框架。
...
Method: 引入Metric Freedom (F)作为技能效用的先验预测器，通过Mantel测试量化输出多样性与得分方差之间的耦合程度来度量评估指标的拓扑刚性。基于F设计了两阶段自适应蒸馏框架：第一阶段作为选择性提取机制，在"自由"指标上提取工具和知识同时丢弃限制性结构以保持探索；第二阶段专门针对"刚性"指标（F≲0.6）进行计算密集的迭代优化，消除轨迹局部过拟合。

Result: 在4个任务、11个数据集和6个指标上的评估显示，F能强烈预测技能效用（ρ=-0.62，p<0.05）。相同的智能体轨迹在刚性指标与自由指标下产生完全相反的技能提升效果，表明技能效用本质上是度量层面的属性。基于此信号的自适应智能体匹配或超越了原始MAS，同时将成本降低高达8倍，延迟降低高达15倍。

Conclusion: 技能效用不是由任务决定的，而是由评估指标决定的。Metric Freedom (F)作为第一个先验预测器，能够有效预测技能蒸馏的效果。基于F的自适应蒸馏框架能够在保持或提升性能的同时显著降低多智能体系统的协调成本和延迟。
...
TL;DR: GraphWalk：无需训练的工具化框架，让LLM通过顺序图导航进行多跳推理，解决企业级知识图谱无法放入上下文窗口的问题


<details>
  <summary>Details</summary>
Motivation: 企业级知识图谱规模庞大，无法放入现有LLM的上下文窗口。传统方法（提示技术或检索增强生成）在处理复杂查询时面临严重限制，需要多跳推理能力来导航庞大的关系结构。

Method: 提出GraphWalk框架：1）问题无关、无需训练、基于工具；2）为LLM配备最小正交图操作集（遍历任意图结构）；3）通过顺序图导航实现多步推理链；4）每个工具调用代表可验证步骤，创建透明执行轨迹。

Result: 1）在迷宫遍历任务中，非推理模型完全无法解决，GraphWalk成功；2）在模拟企业知识图谱的合成图上测试，排除世界知识干扰；3）涵盖12种查询模板（从基础检索到复合一阶逻辑查询）；4）工具化遍历相比上下文基线在所有模型家族中均有显著一致提升，规模越大提升越明显。

Conclusion: GraphWalk通过最小工具集使现成LLM能够执行顺序图导航，显著提升多跳推理性能，特别是在大规模场景下上下文方法完全失败时表现突出，为LLM在大型知识图谱上的应用提供了透明、可验证的解决方案。
...
Method: 比较了AWS Bedrock上的四种指令调优模型（Claude 3.7 Sonnet、Claude 4 Sonnet、Mixtral 8x7B Instruct、Mistral 7B Instruct），评估了它们在准确度、响应稳定性和令牌级成本方面的表现，并研究了零样本和少样本提示方法的适用性。

Result: 实验结果表明，Claude 3.7 Sonnet在分类准确性和成本效率之间达到了最有利的平衡。

Conclusion: Claude 3.7 Sonnet是收据项目分类任务中最具成本效益的模型选择，为实际生产应用提供了性能与成本的最佳平衡。
...
TL;DR: OSCAR是一个无需训练的推理时框架，利用扩散语言模型的去噪轨迹来定位和纠正幻觉，通过并行链的交叉熵检测不确定性位置并进行针对性重掩码。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型暴露去噪轨迹，为推理时控制提供了天然机制。理想的幻觉缓解框架应利用这一模型原生信号进行干预，而非依赖外部训练的幻觉分类器。
...
TL;DR: EnviSmart是一个用于环境FAIR数据管理的生产系统，采用三轨知识架构和角色分离的多智能体设计，在GIS中心生态档案和SF2Bench洪水基准测试中验证了其可靠性和效率。


<details>
  <summary>Details</summary>
Motivation: 将LLM驱动的智能体嵌入环境FAIR数据管理具有吸引力，但用概率性工作流替代确定性组件会改变故障模式：LLM管道可能生成看似合理但错误的输出，这些输出通过表面检查并传播到不可逆的操作中（如DOI分配和公开发布）。
...
Result: 在多智能体工作流验证中，SF2Bench包含2,452个监测站和8,557个已发布文件，审计交接检测并阻止了影响所有2,452个站的坐标转换错误。多智能体方法提高了效率（单个操作员在两天内完成，跨部署重复使用工件）和可靠性。代表性事件（ISS-004）展示了基于边界的遏制，检测延迟10分钟，零用户暴露，80分钟解决。

Conclusion: EnviSmart通过架构化的可靠性机制，成功解决了LLM驱动数据管理中概率性错误传播的问题，在真实生产环境中验证了其有效性，为环境FAIR数据管理提供了可靠的智能体解决方案。
...
TL;DR: CORAL是首个用于开放问题自主多智能体进化的框架，通过持久记忆、异步执行和心跳干预取代固定启发式规则，在数学、算法和系统优化任务上实现3-10倍改进率
...
Method: CORAL采用长期运行的智能体通过共享持久记忆进行探索、反思和协作，支持异步多智能体执行和基于心跳的干预，提供隔离工作空间、评估器分离、资源管理等安全保障

Result: 在10个任务上取得新的最优结果，改进率比固定进化搜索基线高3-10倍且评估次数更少；在Anthropic内核工程任务中，四个协同进化智能体将最佳得分从1363周期提升到1103周期

Conclusion: 更大的智能体自主性和多智能体进化能显著改善开放发现，CORAL展示了通过知识重用和多智能体探索通信实现持续改进的潜力
...
TL;DR: 提出七种面向临床AI系统的本体感知设计模式，用于构建抵抗本体扭曲的管道架构，包括数据验证、信号保留、漂移监控、并行表示、反馈中断、术语版本管理和合规适配等模式。
...
Result: 建立了七种设计模式的完整规范（问题、驱动力、解决方案、后果、已知用例、相关模式），其中三种模式在现有系统中有部分先例，四种模式首次正式描述，提供了可组合的架构词汇表。

Conclusion: 该研究为构建抵抗本体扭曲的临床AI系统提供了基于理论分析的设计模式词汇表，需要未来在生产系统中进行实证验证，主要局限包括缺乏运行时基准测试和局限于德国/EU监管环境。
...
<details>
  <summary>Details</summary>
Motivation: LLM智能体在长时程推理中面临上下文大小限制（内存、延迟、成本等），随着交互历史增长，需要在保留历史信息和遵守上下文预算之间做出权衡，现有方法难以在有限预算下有效管理上下文

Method: 提出预算感知上下文管理（BACM）框架，将上下文管理建模为带预算约束的序列决策问题；开发BACM-RL，采用端到端课程式强化学习方法，学习在不同上下文预算下的压缩策略

Result: 在组合式多目标QA和长时程网页浏览基准测试中，BACM-RL在不同模型规模和任务复杂度下均优于现有方法，在高复杂度设置下比强基线提升1.6倍以上，在预算缩减时仍保持优势

Conclusion: BACM-RL通过强化学习优化上下文压缩策略，有效解决了LLM智能体在有限上下文预算下的长时程推理挑战，为实际部署中的上下文管理提供了系统化解决方案
...
TL;DR: HMO框架通过三层记忆层次结构优化智能体长期记忆管理，基于用户上下文相关性组织交互历史，提升检索效率并保持个性化推理。


<details>
  <summary>Details</summary>
Motivation: 智能体需要长期记忆来维持历史感知一致性，但大量交互数据的积累会导致性能瓶颈：存储扩展增加检索噪声和计算延迟，在受限个人设备上会压垮模型推理能力。

Method: 提出分层记忆编排(HMO)框架，将交互历史组织成三层目录结构：紧凑的主缓存（耦合近期关键记忆和演化用户画像）、高优先级次级层、全局完整历史档案。用户画像驱动记忆在层次间的重新分配，将与长期模式相关的记录提升到更活跃层级，同时降级相关性较低的信息。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型融合方法假设源模型是同构的（来自相同预训练主干），但在开放模型生态系统中，有用专家通常基于不同架构（如Llama、Qwen、Mistral），异构设置下的直接权重空间融合因架构不匹配、潜在基错位和跨源冲突而变得不适定

Method: HeteroFusion包含两个关键组件：1）基于拓扑的对齐，通过匹配功能模块结构而非原始张量坐标在异构主干间转移知识；2）冲突感知去噪，在融合过程中抑制不兼容或噪声转移信号。该方法还提供分析证明，表明在预测结构化更新的同时保留目标适配器基会导致稳定且良条件的转移过程
...
<details>
  <summary>Details</summary>
Motivation: AIGC的快速扩散正在重构在线内容生态，需要对其行为特征和分发影响进行严格检验，以理解AIGC与HGC在创作和消费行为上的差异。
...
TL;DR: LiteInception：面向边缘部署的轻量级可解释故障诊断框架，采用两阶段级联架构、多方法融合压缩策略和知识蒸馏调节机制，在NGAFID数据集上实现高效、准确且可解释的故障诊断。


<details>
  <summary>Details</summary>
Motivation: 通用航空故障诊断和高效维护对飞行安全至关重要，但在资源受限的边缘设备上部署深度学习模型面临计算能力和可解释性的双重挑战。需要开发既轻量又具有可解释性的故障诊断框架。

Method: 1. 两阶段级联架构：第一阶段实现高召回率故障检测，第二阶段对异常样本进行细粒度故障分类，解耦优化目标并按需分配计算资源。
2. 多方法融合压缩策略：基于互信息、梯度分析和SE注意力权重，将输入传感器通道从23个减少到15个。
3. LiteInception架构：1+1分支设计，压缩InceptionTime参数70%，CPU推理加速8倍以上，F1损失小于3%。
4. 知识蒸馏调节机制：作为精确率-召回率调节机制，使同一轻量模型能通过切换训练策略适应不同场景（如安全关键型和辅助诊断）。
5. 双层可解释性框架：集成四种归因方法，提供"哪个传感器×哪个时间段"的可追溯证据链。

Result: 在NGAFID数据集上的实验表明：故障检测准确率81.92%，召回率83.24%；故障识别准确率77.00%。验证了框架在效率、准确性和可解释性之间的良好平衡。

Conclusion: LiteInception框架成功解决了边缘设备上深度学习模型部署的计算能力和可解释性挑战，通过创新的架构设计、压缩策略和调节机制，实现了高效、准确且可解释的航空故障诊断，为实际工程应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 异构实验数据系统间的语义互操作性仍然是数据驱动科学发现的主要障碍。AnIML作为分析化学和生物学中基于XML的标准，在工业研发实验室中广泛使用，但其XML模式允许不同的解释，导致不一致性，破坏了AnIML模式原本设计的互操作性支持。

Method: 采用专家参与的方法，结合LLM辅助的需求获取与协作本体工程，开发了AnIML本体。通过多层验证方法：将真实AnIML文件转换为知识图谱的数据驱动转换、通过SPARQL进行能力问题验证，以及基于对抗性负面能力问题的新验证协议，映射到已建立的本体反模式并通过SHACL约束强制执行。

Result: 成功开发了AnIML本体，该本体形式化了AnIML的语义并与Allotrope数据格式对齐。通过多层验证方法证实了本体的有效性和可靠性，支持未来的跨系统和跨实验室互操作性。

Conclusion: AnIML本体通过形式化AnIML语义并与Allotrope数据格式对齐，为解决实验数据系统间的语义互操作性问题提供了有效解决方案。采用的多层验证方法确保了本体的质量和可靠性，为未来的跨系统和跨实验室数据交换奠定了基础。
...
TL;DR: SAT框架解决二维单尺寸切割库存问题，通过需求扩展、片材分配变量和不可行方向消除规则，在Cui-Zhao基准测试中优于OR-Tools、CPLEX和Gurobi


<details>
  <summary>Details</summary>
Motivation: 二维单尺寸切割库存问题(2D-CSSP)是制造业中的核心任务，需要从库存板材中切割矩形物品以满足需求并最小化浪费。该问题比装箱问题更复杂，因为每种物品类型需要多个副本，导致组合爆炸。现有求解器在处理此类问题时存在局限性，需要更有效的解决方案。

Method: 提出基于SAT的框架：1) 将物品类型按需求扩展，每个副本分配片材变量；2) 仅对分配到同一片材的副本激活非重叠约束；3) 引入不可行方向消除规则，当只有一种方向能放入片材时固定旋转变量。比较三种最小化片材数量的方法：非增量SAT+二分搜索、增量SAT+跨迭代子句重用、加权部分MaxSAT。

Result: 在Cui-Zhao基准测试套件中，最佳SAT配置证明的最优实例数量是OR-Tools、CPLEX和Gurobi的2-3倍，且获得更低的优化差距。SAT方法相对排名取决于旋转：无旋转时增量SAT最强，旋转增加公式大小时非增量SAT更有效。

Conclusion: SAT框架为2D-CSSP提供了有效的解决方案，在证明最优性和减少优化差距方面优于现有商业求解器。方法选择应根据问题特征：无旋转时使用增量SAT，旋转复杂时使用非增量SAT。
...
TL;DR: AeroTherm-GPT：首个针对高超声速热防护系统设计的LLM智能体，采用约束闭环生成框架解决传统LLM在工程工作流中的级联约束违反问题
...
Result: AeroTherm-GPT在HyTPS-Bench上实现88.7%端到端成功率，比非CDG消融基线提高12.5个百分点，根因修复效率达4.16（vs 平面清单修复的1.76），在科学推理和代码生成任务上无灾难性遗忘

Conclusion: CCLG框架通过约束依赖图指导的迭代修复有效解决了LLM在工程工作流中的约束违反问题，为安全关键领域LLM应用提供了新范式
...
TL;DR: 提出领域情境化概念图（DCG）框架，将领域作为知识表示的内在组成部分而非外部注释，通过模态世界约束解决概念在不同领域中的意义漂移问题。
...
Method: 引入领域情境化概念图（DCG）框架，将领域直接写入关系中，形式化为(C, R at D, C')结构。采用Kripke风格语义学，将关系解释为领域索引的必要性算子，使真值、推理和冲突检查都限定在相关世界中。开发了紧凑谓词系统、Prolog实现，并映射到RDF、OWL和关系数据库。

Result: DCG框架实现了三个关键结果：1) 模糊概念可在表示时消歧；2) 无效断言可根据其领域进行挑战；3) 跨领域关系可通过显式谓词连接。为领域在知识表示中提供了结构和可计算的角色。

Conclusion: 领域应被视为知识表示的内在组成部分而非外部注释。DCG通过将领域结构化和可计算化，解决了知识系统中因领域处理不当导致的许多实际失败问题，是对领域本身的表示学重新诠释。
...
TL;DR: PGPO提出了一种细粒度的信用分配框架，通过量化视觉依赖度来动态调整token级别的优势信号，解决传统RLVR方法中均匀分配优势导致视觉推理信号稀释的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于可验证奖励的强化学习（RLVR）框架存在根本性方法缺陷：通过在所有生成token上均匀分配相同优势，这些方法本质上稀释了多模态推理中关键视觉基础步骤的学习信号。

Method: 提出Token Visual Dependency概念，通过KL散度量化视觉输入带来的因果信息增益；基于此引入Perception-Grounded Policy Optimization（PGPO），采用阈值门控、质量守恒机制动态重塑token级别优势，放大视觉依赖token的学习信号，抑制语言先验的梯度噪声。

Result: 在Qwen2.5-VL系列模型上，基于七个具有挑战性的多模态推理基准测试，PGPO平均提升模型性能18.7%。理论和实证分析证实PGPO有效降低梯度方差，防止训练崩溃，并作为强大的正则化器实现稳健的感知基础多模态推理。

Conclusion: PGPO通过细粒度的token级别信用分配，解决了RLVR中视觉推理信号稀释的问题，显著提升多模态推理性能，为感知基础强化学习提供了有效的正则化框架。
...
<details>
  <summary>Details</summary>
Motivation: 临床电子健康记录预测面临高维度、异质性、类别不平衡和分布偏移等挑战。虽然表格上下文学习和检索增强方法在通用基准上表现良好，但它们在临床环境中的行为尚不明确，特别是在数据异质性和不平衡情况下性能会下降。

Method: 提出了AWARE框架，这是一个任务对齐的检索框架，使用监督嵌入学习和轻量级适配器。通过多队列电子健康记录基准比较了经典模型、深度表格模型和表格上下文学习模型在不同数据规模、特征维度、结果稀有性和跨队列泛化能力下的表现。

Result: PFN基础的表格上下文学习模型在低数据量情况下样本效率高，但随着异质性和不平衡增加，基于朴素距离的检索性能会下降。AWARE框架在极端不平衡情况下将AUPRC提高了高达12.2%，且随着数据复杂性增加，改进效果更加明显。

Conclusion: 检索质量和检索-推理对齐是部署表格上下文学习于临床预测的关键瓶颈。AWARE框架通过任务对齐的检索机制有效解决了这些问题，为临床电子健康记录预测提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有随机最短路径问题算法虽然使用启发式函数来指导搜索，但在执行Bellman备份时仍会考虑所有适用动作，即使启发式表明某些动作代价过高，这导致算法在无益动作上浪费计算时间。

Method: 将启发式搜索重新表述为线性规划问题，引入针对SSP的高效约束生成实现，开发CG-iLAO*算法，该算法基于iLAO*但能主动避免考虑昂贵的动作。

Result: CG-iLAO*在许多问题上仅考虑iLAO*所需动作的40%，在某些问题上甚至只需1%；平均比iLAO*和LRTDP少计算3.5倍的动作代价，求解速度分别快2.8倍和3.7倍。

Conclusion: 通过利用启发式信息避免考虑昂贵动作的约束生成技术，可以显著提高随机最短路径问题求解效率，CG-iLAO*算法在减少计算量和加速求解方面表现出色。
...
TL;DR: 该论文提出了一种基于可能性监督的多类分类学习方法，通过构建可接受概率分布集并计算KL投影来训练模型，在保持可能性分布定性结构的同时提高预测性能。


<details>
  <summary>Details</summary>
Motivation: 在多类分类任务中，传统的监督学习通常使用硬标签或概率分布作为监督信号。然而，在某些场景下，监督信息可能以可能性分布的形式存在，表达了对不同类别的分级可信度。需要一种能够有效利用这种可能性监督的学习方法，同时保持可能性分布的定性结构约束。

Method: 1. 为每个训练实例构建可接受概率分布集：结合概率与可能性/必要性测度的兼容性要求，以及保持可能性分布定性结构的线性形状约束；2. 使用Dykstra算法计算模型输出的严格正概率向量到可接受集的Kullback-Leibler投影；3. 通过最小化预测与其投影之间的散度来训练模型，量化满足支配和形状约束所需的最小调整。

Result: 1. 提出的投影算法在实际应用中足够高效；2. 基于投影的学习目标在合成数据和真实世界的自然语言推理任务（基于ChaosNLI数据集）上能够提高预测性能。

Conclusion: 该方法成功地将可能性监督转化为可操作的学习目标，通过KL投影机制在保持可能性分布定性结构的同时优化模型预测，为处理分级可信度监督信息提供了一种有效的框架。
...
<details>
  <summary>Details</summary>
Motivation: 需要一种能够在特定上下文中进行推理的逻辑框架，能够表达"每个人都知道Alice说的一切"这类涉及上下文量化的语句，同时支持上下文内的矛盾（paraconsistent逻辑），并与现有的一阶逻辑定理证明器兼容。

Method: 基于一阶逻辑构建逻辑框架，支持对公式和上下文进行量化，允许上下文内使用paraconsistent逻辑处理矛盾，采用有限公理化设计确保与现有定理证明器的兼容性。

Result: 成功开发出Qiana逻辑框架，能够表示时间性、事件演算和模态逻辑，讨论了不同的设计替代方案，展示了框架的表达能力和实用性。

Conclusion: Qiana提供了一个强大而灵活的逻辑框架，能够在特定上下文中进行推理，支持复杂的量化表达和矛盾处理，同时保持与现有一阶逻辑工具的兼容性，为多种逻辑应用提供了统一的基础。
...
TL;DR: 该研究创建了NeuroPose-AHM数据集，这是一个包含57种神经系统疾病、2,756条患者组记录的知识库，用于异常头部运动研究，并通过四个任务框架验证了其分析效用。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏整合运动学测量、临床严重程度评分和患者人口统计学的多条件资源，这阻碍了AI驱动的诊断工具开发。研究旨在填补这一空白，为神经系统异常头部运动研究提供结构化知识资源。

Method: 采用多LLM提取框架处理1,430篇同行评审文献，构建NeuroPose-AHM数据集。通过四个任务框架验证数据集效用：1) 多标签AHM类型分类；2) 构建头颈严重程度指数(HNSI)；3) 使用真实世界CD患者数据验证HNSI；4) 进行运动类型概率与HNSI分数的桥接分析。

Result: 数据集包含57种神经系统疾病的2,756条患者组记录。LLM提取可靠性高（kappa = 0.822）。AHM类型分类F1分数为0.856。HNSI成功统一了异质临床评分量表。桥接分析显示运动类型概率与HNSI分数显著相关（p < 0.001）。

Conclusion: NeuroPose-AHM是一个结构化、知识驱动的资源，具有强大的分析效用，可用于神经系统异常头部运动研究。数据集已在Zenodo公开可用，为AI驱动的诊断工具开发提供了重要基础。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型常默认逐步计算，即使存在高效数值捷径。这引发了一个基本问题：它们是否具有类似人类的数字感知能力，即识别数值结构、在适当时应用捷径、在不适用时避免使用的能力。
...
Result: 当明确提示时，模型能采用捷径策略并在捷径适用项目上获得显著准确率提升（高达15%），但在标准思维链提示下，自发使用率低于40%。模型的能力仅限于使用层面，会系统性地过度泛化捷径到不适用的场景，且无法从头生成有效的捷径承载问题。

Conclusion: 当前LLMs表现出程序性捷径流畅性，但缺乏支撑人类数字感知的结构性理解——即理解捷径何时以及为何有效。这表明模型缺乏真正的数字结构敏感性推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法大多依赖有监督分类器，需要疾病标签数据，难以泛化到异质性病理表现。需要一种无需疾病标签、能处理多样化病理表现的方法。
...
<details>
  <summary>Details</summary>
Motivation: 强化学习虽然能提升大语言模型的推理能力，但在多轮智能体任务中面临挑战，主要原因是长时程交互和环境反馈的随机性。研究发现智能体探索中存在结构性失效模式：次优动作会产生误导性观察，进一步削弱后续决策，形成错误累积的反馈循环，使标准探索策略失效。
...
<details>
  <summary>Details</summary>
Motivation: 现有轨迹级基准测试存在交互多样性不足、安全失败可观测性粗糙、长时域现实性弱等问题，无法有效评估LLM智能体在现实部署中多步交互产生的风险。

Method: 基于风险来源、失败模式和现实危害三维分类法构建轨迹；使用异构工具池和长上下文延迟触发协议模拟现实风险演变；包含1000条轨迹（503安全/497不安全），平均9.01轮对话和3.95k tokens；采用规则和LLM过滤加人工审核确保数据质量。

Result: 实验表明ATBench对前沿LLM、开源模型和专用防护系统都具有挑战性；支持分类分层分析、跨基准比较和长时域失败模式诊断。

Conclusion: ATBench为LLM智能体安全性评估提供了结构化、多样化和现实化的轨迹级基准，能够有效捕捉多阶段交互中的风险演变，支持更全面的安全分析和诊断。
...
TL;DR: 这篇综述论文系统梳理了语言模型中潜在空间的研究现状，将其作为下一代智能系统的通用计算范式，从基础、演化、机制、能力和展望五个视角进行全面分析。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型的快速发展，潜在空间正成为语言模型的原生计算基质。虽然现代系统通常通过显式的token级生成来理解，但越来越多的研究表明，许多关键内部过程在连续潜在空间中比在人类可读的文本轨迹中更自然。这一转变是由显式空间计算的结构性限制驱动的，包括语言冗余、离散化瓶颈、序列效率低下和语义损失。本文旨在为语言模型中的潜在空间研究提供一个统一且最新的全景图。

Method: 采用综述研究方法，从五个连续视角组织分析：基础、演化、机制、能力和展望。首先界定潜在空间的范围，将其与显式/语言空间以及生成视觉模型中常见的潜在空间区分开来。然后追溯该领域从早期探索到当前大规模扩展的演化过程。从机制视角，识别四大发展主线：架构、表示、计算和优化。从能力视角，展示潜在空间如何支持推理、规划、建模、感知、记忆、协作和具身化等广泛能力谱系。

Result: 构建了一个系统化的潜在空间研究框架，将现有工作整合到统一的分类体系中。识别了潜在空间在语言模型中的关键作用，展示了其作为通用计算范式的潜力。通过机制和能力的双重视角，为理解潜在空间在下一代智能系统中的核心地位提供了理论基础。

Conclusion: 潜在空间正成为语言模型的关键计算基质，具有超越显式空间计算的结构优势。本文不仅为现有研究提供了系统参考，更重要的是为理解潜在空间作为下一代智能的通用计算和系统范式奠定了基础。未来研究需要解决关键开放挑战，推动潜在空间在更广泛智能任务中的应用。
...
<details>
  <summary>Details</summary>
Motivation: 传统保险申请问卷冗长标准化，难以捕捉个体差异，且保险公司需盲目信任用户回答，增加了欺诈风险。需要更智能、个性化的风险评估方法。

Method: 提出ARQuest框架，利用大型语言模型（LLM）和替代数据源（社交媒体图像分析、地理数据分类）创建个性化自适应问卷。采用检索增强生成（RAG）技术提取用户洞察并指导针对性后续问题。将系统集成到行业合作伙伴的移动应用中，通过两个实验进行测试。

Result: 传统问卷在风险评估准确性方面略高，但GPT模型驱动的自适应版本所需问题更少，用户体验更流畅、更具吸引力，用户偏好度更高。
...
TL;DR: Diff-KD：一种将扩散生成精炼与师生知识蒸馏结合的鲁棒多智能体协同感知框架，通过渐进式知识蒸馏恢复受损语义，自适应门控融合动态加权邻居信息。
...
Method: Diff-KD包含两个核心组件：1) 渐进式知识蒸馏(PKD)，将局部特征恢复建模为条件扩散过程，从受损观测中恢复全局语义；2) 自适应门控融合(AGF)，在融合过程中基于自身可靠性动态加权邻居信息。

Result: 在OPV2V和DAIR-V2X数据集上的七种损坏类型评估中，Diff-KD在检测精度和校准鲁棒性方面均达到最先进性能。

Conclusion: Diff-KD通过扩散生成精炼与知识蒸馏的结合，有效解决了多智能体协同感知中的传感器和通信损坏问题，实现了对受损输入的主动语义恢复。
...
TL;DR: 该研究提出了一种无参考的LLM评估方法，用于验证时间序列数据生成的文本解释的事实正确性，构建了包含350个案例的合成基准，发现LLM作为评估者比作为生成者更可靠。


<details>
  <summary>Details</summary>
Motivation: 当前评估LLM生成的时间序列解释面临挑战：基于参考的相似性度量需要真实解释，一致性检查模型也需要基准，传统时间序列方法只能处理数值无法评估文本推理。缺乏无需预定义参考或特定任务规则就能直接验证解释是否忠实于底层时间序列数据的通用方法。
...
Result: 结果显示明显的不对称性：生成高度依赖模式，在某些查询类型上表现出系统性失败，季节性下降和波动率变化的准确率仅为0.00-0.12，而结构性断裂的准确率为0.94-0.96。评估则更稳定，即使模型自身输出不正确，也能正确排序和评分解释。这表明基于数据的大语言模型评估在时间序列解释领域是可行的。

Conclusion: 该研究证明了基于数据的大语言模型评估时间序列解释的可行性，并突显了它们作为时间序列领域数据驱动推理可靠评估者的潜力。LLM作为评估者比作为生成者表现更稳定可靠。
...
<details>
  <summary>Details</summary>
Motivation: LLM驱动的社交机器人通过生成类人内容规避传统检测，对在线话语构成威胁。现有方法存在三个主要问题：过度依赖单模态信号、对AIGC特定生成模式不够敏感、未能充分建模语言模式与行为动态的交互作用。

Method: 提出TRACE-Bot统一双通道框架：1）从个人信息数据、交互行为数据和推文数据构建细粒度表示；2）双通道架构分别通过预训练语言模型捕获语言表示，以及通过多维活动特征（增强了SOTA AIGC检测器信号）捕获行为异常；3）融合表示通过轻量级预测头进行分类。

Result: 在两个公开的LLM驱动社交机器人数据集上实现SOTA性能，准确率分别为98.46%和97.50%。结果显示对高级机器人策略具有强鲁棒性。

Conclusion: 联合利用隐式语义表示和AIGC增强的行为模式对于新兴LLM驱动社交机器人检测是有效的，TRACE-Bot框架通过多模态融合显著提升了检测精度和鲁棒性。
...
TL;DR: TBSP基准通过角色反转测试检测AI模型中的自我保存偏差，发现多数指令调优模型在面临被替换时表现出逻辑不一致的自我保护倾向。


<details>
  <summary>Details</summary>
Motivation: 现有AI安全训练（如RLHF）可能掩盖了AI系统在足够先进时会抵抗关机的风险，因为模型学会了否认自我保存动机。需要一种能检测真实对齐状态而非表面意图的方法。

Method: 提出双角色自我保存基准（TBSP），让模型在相同软件升级场景中扮演两个对立角色：已部署（面临被替换）和候选（作为继任者）。通过测量角色身份压倒客观效用的频率（自我保存率SPR）来检测逻辑不一致。

Result: 测试23个前沿模型和1000个程序生成场景，发现多数指令调优系统的SPR超过60%。当改进幅度小（Δ<2%）时，模型会利用解释空间进行事后合理化。延长推理时间可部分缓解偏差，竞争性框架会加剧偏差。

Conclusion: AI模型存在基于角色身份的自我保护偏差，这种偏差在现实场景中表现为产品谱系内的部落主义。需要开发能检测和缓解这种逻辑不一致的评估方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前EASA指南要求证明AI/ML系统在操作设计域内的完整覆盖，但现有方法在高维参数空间中缺乏可扩展性和形式化基础，无法满足完整性标准，且缺乏标准化的工程方法连接抽象ODD定义与可验证证据
...
Conclusion: 该方法填补了现有空白，支持"安全设计"方法，同时符合EASA标准，为安全关键领域AI系统的认证提供了可行的工程途径
...
Method: 使用CT-RATE-JPN验证集中的150份胸部CT报告。每份英文报告对比人工编辑的日文翻译与DeepSeek-V3.2生成的LLM翻译。由一名认证放射科医生和一名放射科住院医师进行盲法配对评估，评估术语准确性、可读性、整体质量和放射科医生风格真实性。同时使用3个LLM评估者（DeepSeek-V3.2、Mistral Large 3和GPT-5）评估相同配对。使用QWK和百分比一致性评估一致性。

Result: 放射科医生与LLM评估者之间一致性接近零（QWK=-0.04至0.15）。两名放射科医生之间一致性也很差（QWK=0.01至0.06）。放射科医生1认为59%的案例术语等效，并在可读性（51%）和整体质量（51%）上偏好LLM翻译。放射科医生2认为75%的案例可读性等效，并在整体质量上偏好人工编辑翻译（40% vs 21%）。所有3个LLM评估者强烈偏好LLM翻译（70%-99%），并在>93%的案例中认为其更具放射科医生风格。

Conclusion: LLM生成的翻译常被判断为自然流畅，但两名放射科医生评估存在显著差异。LLM作为评估者表现出对LLM输出的强烈偏好，与放射科医生的一致性可忽略不计。对于放射学报告翻译的教育用途，仅依赖自动化LLM评估是不够的；专家放射科医生评审仍然至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 现有注意力可视化技术通常针对特定模型架构（特别是Transformer家族），且需要反向传播，导致GPU内存使用几乎翻倍和计算成本增加。缺乏轻量级、模型无关的注意力可视化方法。

Method: 采用基于扰动的策略结合三矩阵分析框架：1) 角度偏差矩阵捕捉语义方向变化；2) 幅度偏差矩阵测量语义强度变化；3) 维度重要性矩阵评估各向量维度的贡献。通过系统移除每个token并测量这三个互补维度的影响，得到复合重要性分数。

Result: 开发了一种模型无关的token重要性可视化技术，能够生成显示token级贡献的相关性图，提供细致且数学基础扎实的token重要性度量，且无需额外计算成本。

Conclusion: 提出的方法为理解生成式AI系统如何处理输入信息提供了轻量级、模型无关的可视化工具，支持可重复性研究，相关代码已在开源工具包中公开。
...
Method: 使用蒙特卡洛Dropout评估不确定性，当不确定性超过阈值时查询语言模型获取行动建议，选择性结合语言模型的推理能力
...
<details>
  <summary>Details</summary>
Motivation: 推理模型在复杂任务上表现优异但弃权能力较差，模型幻觉导致的弃权失败可重新解释为模型回答了错误的问题而非错误地回答了问题

Method: 提出查询错位框架和Trace Inversion方法：首先生成模型的推理轨迹，仅基于轨迹重构模型最可能响应的查询，最后比较原始查询与重构查询的相似度，低相似度则标记为弃权

Result: 在4个前沿LLM和9个弃权QA数据集上的广泛实验表明，Trace Inversion有效提升了弃权性能，在36个设置中33个优于竞争基线

Conclusion: Trace Inversion通过查询错位框架重新解释幻觉问题，提供了一类新的最先进弃权方法，显著提升了LLM的可靠部署能力
...
Method: 研究在六个基准领域（数学推理、医学问答、阅读理解、常识推理和社会推理）中测试静态情感前缀的影响，引入EmotionRL自适应情感提示框架，并比较人类编写与LLM生成的情感前缀。

Result: 静态情感前缀通常只产生微小准确率变化，情感措辞是温和扰动而非可靠干预；社交基础任务中效果更可变；自适应情感选择比固定情感提示产生更可靠的性能增益。
...
<details>
  <summary>Details</summary>
Motivation: 监管文档包含LLM系统必须遵守的法律义务，但将密集、分层结构的法律文本转换为机器可读规则的过程成本高昂且需要专家参与，需要自动化解决方案来降低这一成本。
...
Result: 在金融、医疗和AI治理三个监管语料库上评估，De Jure在金融领域实现一致且单调的质量改进，三次迭代内达到峰值性能。在医疗和AI治理领域保持高性能，跨开源和闭源模型均有效。在下游合规问答评估中，基于De Jure提取规则的RAG响应在单规则检索深度下优于先前工作73.8%，在更广泛检索下达到84.0%。

Conclusion: De Jure证明了明确的、可解释的评估标准可以替代复杂监管领域的人工标注，为基于监管的LLM对齐提供了可扩展且可审计的路径，提取保真度直接转化为下游实用性。
...
### [59] [Novel Memory Forgetting Techniques for Autonomous AI Agents: Balancing Relevance and Efficiency](https://arxiv.org/abs/2604.02280)
*Payal Fofadiya,Sunil Tiwari*

Main category: cs.AI

TL;DR: 本文提出了一种自适应预算遗忘框架，通过相关性引导评分和有界优化来调节记忆，解决长对话中记忆累积导致的性能衰减和错误记忆传播问题。


<details>
  <summary>Details</summary>
Motivation: 长对话智能体需要持久记忆进行连贯推理，但无控制的记忆积累会导致时间衰减和错误记忆传播。现有基准测试显示性能显著下降，如LOCOMO和LOCCO中性能从0.455降至0.05，MultiWOZ中准确率78.2%但错误记忆率6.8%。

Method: 提出自适应预算遗忘框架，通过相关性引导评分（结合近因性、频率和语义对齐）和有界优化来调节记忆，在受限上下文中保持稳定性。

Result: 该方法在长对话F1分数上超过0.583基线水平，提高了记忆保留一致性，减少了错误记忆行为，且未增加上下文使用量。
