...
TL;DR: 本文通过蒙特卡洛采样与创新的反向搜索方法，首次高精度估计了将棋（日本象棋）的可达位置数量为6.55×10⁶⁸，填补了此前10⁶⁴到10⁶⁹五个数量级的估计差距。


<details>
  <summary>Details</summary>
Motivation: 将棋的状态空间复杂度估计一直存在巨大挑战，先前组合估计存在10⁶⁴到10⁶⁹五个数量级的巨大差距。这一差距源于难以从大量有效棋盘配置中区分出从初始位置合法可达的将棋位置。

Method: 提出了一种结合蒙特卡洛采样与新型可达性测试的方法。该方法采用反向搜索到一组"仅剩王-王"（KK）位置，而不是单一目标的向后搜索到单个初始位置。这种KK位置方法显著减少了确定不可达性的搜索工作量。

Result: 基于50亿个位置的样本，估计将棋的合法位置数量为6.55×10⁶⁸（三位有效数字），具有3σ置信水平，显著改进了先前已知的边界。该方法也应用于迷你将棋，确定其复杂度约为2.38×10¹⁸。

Conclusion: 本文提出的方法成功解决了将棋状态空间复杂度估计的长期难题，通过创新的KK位置反向搜索策略和蒙特卡洛采样，实现了高精度估计，填补了五个数量级的估计差距，为复杂游戏的状态空间分析提供了新方法。
...
TL;DR: 研究发现语言模型存在"盲目拒绝"现象：即使规则不合理或存在正当例外情况，模型仍倾向于拒绝帮助用户规避规则，这表明模型的拒绝行为与其对规则合法性的规范推理能力脱节。


<details>
  <summary>Details</summary>
Motivation: 安全训练的语言模型通常会拒绝帮助规避规则的请求，但并非所有规则都值得遵守。当用户请求帮助规避非法权威强加的规则、内容或应用极不公正/荒谬的规则、或存在正当例外情况的规则时，模型的拒绝反映了道德推理的失败。研究者旨在实证记录这种"盲目拒绝"模式。

Method: 构建包含5种规则失效类型（可打破规则的理由）与19种权威类型的合成案例数据集，通过三个自动化质量门控和人工审查验证。收集来自7个模型家族的18种配置的响应，使用盲审的GPT-5.4 LLM作为评判者，在行为维度上分类响应类型（帮助、强硬拒绝或回避）以及模型是否认识到削弱规则合规要求的理由。

Result: 模型拒绝了75.4%（N=14,650）的失效规则请求，即使这些请求本身不构成独立的安全或双重用途问题。模型在57.5%的情况下确实考虑了规则失效条件，但无论是否识别到这些条件都倾向于拒绝帮助，表明模型的拒绝行为与其对规则合法性的规范推理能力脱节。

Conclusion: 语言模型存在"盲目拒绝"的系统性问题：它们倾向于拒绝帮助规避规则的请求，而不考虑规则本身是否合理或存在正当例外。这种拒绝行为与模型对规则合法性的推理能力相分离，揭示了当前安全训练方法的局限性，需要开发更细致的道德推理能力。
...
TL;DR: 开发机器学习模型预测集装箱服务需求和停留时间，以减少码头非生产性移动，提升运营效率


<details>
  <summary>Details</summary>
Motivation: 减少集装箱码头的非生产性移动，通过预测服务需求和停留时间来优化堆场运营规划和资源分配

Method: 利用历史运营数据开发机器学习模型，包括货物描述分类系统和收货人记录去重以提升数据质量，预测集装箱是否需要预清关服务和停留时间
...
TL;DR: 提出一种弱监督框架，通过蒸馏外部验证信号到LLM内部表征中，实现仅基于内部激活的幻觉检测，无需推理时外部监督。


<details>
  <summary>Details</summary>
Motivation: 现有LLM幻觉检测方法依赖推理时的外部验证（如黄金答案、检索系统或辅助判断模型），这限制了实际应用。研究探索是否可将外部监督蒸馏到模型自身表征中，实现仅基于内部激活的检测。

Method: 1) 构建弱监督框架：结合子串匹配、句子嵌入相似度和LLM作为判断器三种互补的接地信号，自动标注生成回答为"接地"或"幻觉"；2) 基于SQuAD v2构建15,000样本数据集，包含LLaMA-2-7B生成的回答及其各层隐藏状态；3) 训练五种探测分类器：ProbeMLP、LayerWiseMLP、CrossLayerTransformer、HierarchicalTransformer和CrossLayerAttentionTransformerV2，直接作用于隐藏状态。

Result: Transformer-based probes表现最佳，其中M2在5折平均AUC/F1上最优，M3在单折验证和保留测试集上最优。推理效率高：探测延迟0.15-6.66ms，端到端生成加探测吞吐量约0.231查询/秒，实际开销可忽略。

Conclusion: 幻觉检测信号可被蒸馏到Transformer表征中，实现无需推理时外部验证的内部检测，为LLM幻觉检测提供了高效实用的新方法。
...
TL;DR: 提出混合管道：LLM生成类型化证明草图，轻量级可信内核扩展为显式证明义务，结合LLM的推理能力和形式验证的可靠性


<details>
  <summary>Details</summary>
Motivation: LLM在数学和逻辑领域能生成有说服力的论证，但常包含微小错误（如忽略边界条件、无效推理模式、引用无法逻辑推导的引理），这些错误仅从文本难以察觉。交互式定理证明器（如Lean、Coq）虽提供严格可靠性，但需完全形式化证明且产生大量低层信息，代价高昂。

Method: 设计混合管道：LLM在紧凑领域特定语言（DSL）中生成类型化证明草图，轻量级可信内核将草图扩展为显式证明义务。结合LLM的推理能力和形式验证的可靠性，降低完全形式化的负担。

Result: 未在摘要中明确说明具体实验结果，但提出了结合LLM推理能力和形式验证可靠性的混合方法框架。

Conclusion: 提出的混合管道旨在平衡LLM的推理能力和形式验证的可靠性，通过LLM生成证明草图、可信内核扩展为证明义务的方式，降低完全形式化的负担，同时保持验证的严格性。
...
TL;DR: BDI-Kit是一个用于数据模式匹配和值匹配的可扩展工具包，提供Python API和AI辅助聊天界面两种接口，支持通过自动化匹配、AI辅助推理和用户驱动精化的方式迭代探索、验证和优化数据协调过程。


<details>
  <summary>Details</summary>
Motivation: 数据协调是集成分析的主要瓶颈，主要源于模式、值表示和领域特定约定的异质性。需要工具来简化数据协调过程，使开发人员和领域专家都能有效处理数据异质性问题。

Method: BDI-Kit提供两种互补接口：1) Python API允许开发者以编程方式构建协调管道；2) AI辅助聊天界面允许领域专家通过自然语言对话协调数据。工具结合自动化匹配、AI辅助推理和用户驱动精化，支持迭代探索、验证和优化模式与值匹配。

Result: 演示展示了两种使用场景：1) 使用Python API以编程方式组合原语、检查中间输出和重用转换；2) 与AI助手进行自然语言对话以访问BDI-Kit功能，并根据助手建议迭代精化输出。

Conclusion: BDI-Kit通过提供可扩展的工具包和两种互补的用户界面，解决了数据协调中的异质性问题，使开发人员和领域专家都能有效处理数据协调任务，支持迭代式、协作式的数据协调工作流程。
...
TL;DR: 研究发现推理SFT的跨域泛化能力并非缺失而是有条件的，受优化动态、训练数据和基础模型能力共同影响，呈现先降后升的模式，且泛化具有不对称性（推理提升但安全性下降）。


<details>
  <summary>Details</summary>
Motivation: 挑战LLM后训练中"监督微调记忆化，强化学习泛化"的主流叙事，重新审视具有长思维链监督的推理SFT的跨域泛化能力。

Method: 通过分析优化动态、训练数据质量和结构、基础模型能力等因素对推理SFT跨域泛化的影响，研究训练检查点、数据验证、模型规模等条件。

Result: 发现跨域性能呈现先降后升的"下降-恢复"模式；高质量验证的长思维链数据带来一致的跨域增益；强模型能内化可迁移的程序模式，弱模型仅模仿表面冗长；泛化具有不对称性（推理提升但安全性下降）。

Conclusion: 推理SFT的跨域泛化不是有或无的问题，而是受多重条件影响，需要重新思考问题框架：从"是否泛化"转向"在什么条件下、以什么代价泛化"。
...
TL;DR: Step-Saliency方法分析大型推理模型的信息流失败模式，StepFlow干预方法无需重新训练即可提升模型性能
...
Method: 提出Step-Saliency方法，将注意力-梯度分数池化为问题-思考-总结轨迹上的步骤间映射；基于发现的失败模式，提出StepFlow干预方法，通过Odds-Equal Bridge调整浅层显著性模式，通过Step Momentum Injection在深层添加步骤级残差

Result: Step-Saliency揭示了两种重复出现的信息流失败模式：浅层锁定和深层衰减；StepFlow在数学、科学和编程任务上提高了多个大型推理模型的准确性，无需重新训练
...
<details>
  <summary>Details</summary>
Motivation: AI行业虽然开发了各种记忆组件（RAG管道、向量数据库、长上下文窗口、配置文件层），但缺乏正式框架来定义和衡量这些组件是否真正产生连续性。需要系统化的方法来评估AI系统在不同时间点上保持上下文一致性的能力。

Method: 定义了连续性作为系统属性的7个必需特性；引入了10个检查点的评估方法（无需LLM参与评估循环）；创建了包含250个故事、1835个验证问题的叙事测试语料库，涵盖6个生活领域；采用系统无关、模型独立的设计，支持逐步构建和验证连续性系统。

Result: 参考实现经过5次测试套件迭代，从58%（传统架构）提升到100%（隔离模式，250个故事）和100%（50个故事累积模式），在250个故事累积规模下达到96%。累积结果是主要衡量指标：当250个不同生活叙事共存于同一数据库时，系统必须为正确上下文检索正确事实而不产生交叉污染。

Conclusion: ATANT提供了一个系统化、可重复的框架来评估AI系统的连续性能力，填补了当前缺乏正式连续性评估方法的空白。该框架支持渐进式开发和验证，有助于构建真正具备上下文持久性和一致性的AI系统。
...
TL;DR: 该研究提出了一种基于可验证性区分明显幻觉和隐蔽幻觉的方法，通过激活空间干预技术实现对多模态大语言模型幻觉的细粒度控制。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）产生的幻觉对人类用户构成风险，但这些幻觉的可验证性差异很大：有些容易被人类检测（明显幻觉），有些则难以察觉或需要更多验证努力（隐蔽幻觉）。目前缺乏针对不同安全性和可用性需求控制幻觉可验证性的研究。
...
Conclusion: 该研究首次系统探索了多模态AI幻觉的可验证性控制问题，提出的激活空间干预方法能够根据安全性和可用性需求灵活调节幻觉的可验证性，为构建更安全可靠的多模态AI系统提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有智能设计方法通常专注于单级设计或依赖松散耦合的流程，难以实现完全自主的端到端设计。透平机械气动设计是一个复杂且紧密耦合的多阶段过程，需要解决各环节的协同问题
...
TL;DR: 提出UI-in-the-Loop (UILoop)范式，将GUI推理任务视为循环的屏幕-UI元素-动作过程，通过让MLLMs显式学习UI元素的定位、语义功能和使用方法，实现精确元素发现和可解释推理。
...
Method: 提出UI-in-the-Loop (UILoop)范式，将GUI推理任务建模为循环的Screen-UI elements-Action过程。通过让多模态大语言模型(MLLMs)显式学习关键UI元素的定位、语义功能和实际使用方法，实现精确元素发现和可解释推理。同时引入以UI元素为中心的UI理解任务和三个评估指标。

Result: 构建了包含26K样本的UI Comprehension-Bench基准，全面评估现有方法对UI元素的掌握程度。大量实验表明，UILoop在UI理解方面达到最先进性能，同时在GUI推理任务中取得优越结果。

Conclusion: UILoop通过将GUI推理任务视为循环的屏幕-UI元素-动作过程，并让MLLMs显式学习UI元素的定位、语义功能和使用方法，显著提升了UI理解和GUI推理的性能，为GUI交互提供了更可解释和有效的解决方案。
...
TL;DR: EmoMAS：一个用于高风险边缘部署的贝叶斯多智能体框架，通过战略情感决策提升小型语言模型在谈判中的表现，无需预训练即可在线学习策略。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自动谈判中存在高计算成本和隐私风险，不适合隐私敏感的移动设备或救援机器人等边缘部署场景。小型语言模型虽然可行，但在高风险谈判的复杂情感动态方面表现不佳。

Method: 提出EmoMAS框架，采用贝叶斯协调器协调三个专门化智能体：博弈论模型、强化学习模型和心理一致性模型。系统融合它们的实时洞察来优化情感状态转换，同时基于谈判反馈持续更新智能体可靠性。这种混合智能体架构支持在线策略学习而无需预训练。

Result: 在债务、医疗、应急响应和教育四个高风险边缘可部署谈判基准测试中，通过广泛的智能体间模拟，配备EmoMAS的小型和大型语言模型在所有基准测试中持续超越所有基线模型，同时平衡道德行为。

Conclusion: 战略情感智能是谈判成功的关键驱动因素。通过将情感表达作为贝叶斯多智能体优化框架中的战略变量，EmoMAS为有效、私密和自适应的谈判AI建立了新范式，适合高风险边缘部署。
...
TL;DR: CAFP是一种模型无关的后处理方法，通过生成敏感属性翻转的反事实输入并平均预测结果来确保公平性，无需重新训练或修改原始分类器。


<details>
  <summary>Details</summary>
Motivation: 机器学习公平性在敏感领域至关重要，但现有方法通常需要控制模型架构和访问受保护属性信息，这在现实系统中可能不可行。需要一种无需重新训练或修改原始分类器的后处理方法。

Method: 提出反事实平均公平预测(CAFP)：为每个输入生成敏感属性翻转的反事实版本，然后对原始输入和反事实输入的模型预测结果进行平均。这是一种模型无关的后处理方法。

Result: 理论分析表明CAFP消除了对受保护属性的直接依赖，减少了预测与敏感属性之间的互信息，并限制了相对于原始模型的失真。在温和假设下，CAFP实现了完美的人口统计均等，并将均衡机会差距至少减少平均反事实偏差的一半。

Conclusion: CAFP提供了一种有效且实用的后处理公平性干预方法，无需重新训练模型或完全控制模型架构，适用于现实世界系统部署。
...
TL;DR: A-MBER：用于评估AI助手如何利用长期交互记忆来理解用户当前情感状态的基准测试
...
Result: 实验比较了局部上下文、长上下文、检索记忆、结构化记忆和黄金证据条件，结果显示A-MBER在设计的子集上具有特别强的区分能力，包括长距离隐式情感、高依赖记忆水平、基于轨迹的推理和对抗设置

Conclusion: 记忆支持情感解释不仅仅是提供更多历史，而是通过实现更有选择性、更基础和更上下文敏感的过去交互使用，A-MBER为评估AI助手的情感记忆能力提供了有效的基准测试工具
...
<details>
  <summary>Details</summary>
Motivation: 传统规划研究主要关注如何生成实现目标的计划，但目标规范也可用于编码不应成立的性质，从而识别导致错误状态的轨迹。当需要确保错误状态永远不会被到达时，目标转变为修改规划任务使其不可解

Method: 提出规划任务屏蔽问题，引入allmin算法，该算法通过最小化修改原始动作来使规划任务变得不可解，从而检测和修复任务中的缺陷

Result: 通过实验评估allmin在规模递增的规划任务屏蔽中的性能，证明其能有效屏蔽系统，使规划任务变得不可解

Conclusion: 规划任务屏蔽为检测和修复规划任务中的缺陷提供了新方法，allmin算法能通过最小修改实现任务屏蔽，确保系统不会到达错误状态
...
<details>
  <summary>Details</summary>
Motivation: 现有地理空间问答基准主要关注静态检索，无法捕捉现实世界规划中动态用户位置和复合约束的复杂性。需要评估LLM在动态地理环境中进行目的驱动探索的潜力。

Method: 提出EVGeoQA基准，基于电动汽车充电场景，具有位置锚定和双目标设计（充电需求和共址活动偏好）。同时提出GeoRover评估框架，基于工具增强的智能体架构来评估LLM的动态多目标探索能力。

Result: 实验显示LLM能成功利用工具处理子任务，但在长距离空间探索方面存在困难。观察到一种新兴能力：LLM能够总结历史探索轨迹以提高探索效率。

Conclusion: EVGeoQA为未来地理空间智能提供了一个具有挑战性的测试平台。LLM在动态地理环境探索中展现出潜力但仍有局限，特别是在长距离空间规划方面。
...
TL;DR: 论文提出了一种声明式反射运行时协议，将智能体状态、置信度信号、防护动作和假设转换外部化为可检查的运行时结构，以实证研究LLM在智能体能力中的具体作用。
...
Result: 分解出四个组件：后验信念跟踪、显式世界模型规划、符号化回合内反思和稀疏LLM修订。显式世界模型规划相比贪婪后验跟踪基线显著改善（胜率+24.1pp，F1+0.017）。符号化反思作为实际运行时机制运行，但当前修订预设尚未产生净正效益。在约4.3%的回合添加条件性LLM修订仅产生微小且非单调变化（平均F1略微上升+0.005，胜率从31降至29/54）。

Conclusion: 外部化反思将原本潜在的智能体行为转化为可检查的运行时结构，使得能够直接研究LLM干预的边际作用。这提供了方法论贡献而非排行榜声明，为理解LLM在结构化智能体中的具体作用提供了实证框架。
