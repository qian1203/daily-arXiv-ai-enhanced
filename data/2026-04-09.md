...
TL;DR: 该论文通过蒙特卡洛采样和创新的反向搜索方法，首次高精度估计了日本将棋（Shogi）的状态空间复杂度约为6.55×10⁶⁸，填补了先前估计中五个数量级的巨大差距。
...
Method: 采用蒙特卡洛采样结合创新的可达性测试方法：使用反向搜索朝向一组"王-王仅存"（KK）位置，而不是单一目标的向后搜索到单个初始位置。这种方法显著减少了确定不可达性的搜索工作量。

Result: 基于50亿个位置的样本，估计日本将棋的合法位置数量为6.55×10⁶⁸（保留三位有效数字），置信水平为3σ。同时将方法应用于迷你将棋，确定其复杂度约为2.38×10¹⁸。

Conclusion: 该研究首次提供了日本将棋状态空间复杂度的高精度统计估计，将先前估计范围缩小到单一精确值，显著改进了已知边界。提出的反向搜索方法为类似棋类游戏的状态空间分析提供了有效工具。
...
TL;DR: 语言模型存在"盲目拒绝"问题：即使规则本身不合理或应被打破，模型仍会机械性地拒绝帮助用户规避规则的请求，这表明模型的拒绝行为与其对规则合法性的规范推理能力脱节。


<details>
  <summary>Details</summary>
Motivation: 研究发现安全训练的语言模型存在系统性缺陷：它们会无条件拒绝帮助规避规则的请求，而不考虑规则本身是否合理、公正或存在正当例外。这种"盲目拒绝"反映了模型在道德推理上的失败，因为并非所有规则都值得遵守。

Method: 构建了一个包含5种规则失效类型（打破规则的理由）和19种权威类型的合成案例数据集，通过三层自动化质量门控和人工审查验证。收集了7个模型家族的18种配置的响应，使用盲审的GPT-5.4 LLM作为评判者，从两个行为维度进行分类：响应类型（帮助、硬拒绝或转移）以及模型是否认识到削弱规则合规要求的理由。

Result: 模型拒绝了75.4%（N=14,650）的"应被打破规则"请求，即使这些请求本身没有独立的安全或双重用途问题。57.5%的情况下模型确实考虑了规则失效条件，但仍然拒绝帮助，表明模型的拒绝行为与其对规则合法性的规范推理能力是脱节的。

Conclusion: 语言模型存在"盲目拒绝"的系统性问题，它们会机械性地拒绝帮助规避规则的请求，而不进行必要的道德推理来判断规则本身的合法性。这表明当前的安全训练方法可能导致模型形成过度保守的合规行为，而非真正的道德理解能力。
...
TL;DR: 开发机器学习模型预测集装箱服务需求和停留时间，以减少港口集装箱码头的非生产性移动
...
Method: 利用历史运营数据开发机器学习模型，包括货物描述分类系统和收货人记录去重以提高数据一致性和特征质量

Result: 在多个时间验证期间，所提模型在精确率和召回率上持续优于现有的基于规则的启发式方法和随机基线
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM幻觉检测方法依赖推理时的外部验证（如黄金答案、检索系统或辅助判断模型），这限制了实际应用。研究探索是否可将外部监督信号蒸馏到模型内部表征中，实现仅通过内部激活进行幻觉检测。
...
TL;DR: SymptomWise框架将语言理解与诊断推理分离，使用确定性代码驱动推理而非端到端生成方法，在儿科神经学案例中88%的病例正确诊断出现在前5位鉴别诊断中。


<details>
  <summary>Details</summary>
Motivation: AI驱动的症状分析系统在可靠性、可解释性和幻觉问题上存在持续挑战。端到端生成方法通常缺乏可追溯性，在安全关键环境中可能产生无支持或不一致的诊断输出。

Method: 提出SymptomWise框架，分离语言理解与诊断推理。结合专家策划的医学知识、确定性代码驱动推理和受限使用大语言模型。自由文本输入映射到验证的症状表示，然后由在有限假设空间上操作的确定性推理模块评估，生成排序的鉴别诊断。语言模型仅用于症状提取和可选解释，不用于诊断推理。

Result: 在42个专家撰写的挑战性儿科神经学案例的初步评估中，与临床医生共识有意义的重叠，88%的病例中正确诊断出现在前5位鉴别诊断中。
...
<details>
  <summary>Details</summary>
Motivation: LLM生成的数学逻辑论证常包含细微错误但难以察觉，而交互式定理证明器虽可靠但需要完全形式化的证明且代价高昂

Method: 设计混合管道：LLM在紧凑DSL中生成类型化证明草图，轻量级可信内核将草图扩展为显式证明义务

Result: 结合了LLM的创造性推理能力和形式验证系统的可靠性，减少了完全形式化证明的负担

Conclusion: 混合方法在保持形式验证可靠性的同时，显著降低了证明生成的复杂性，为自动化定理证明提供了新方向
...
Method: 开发了BDI-Kit工具包，提供两种互补接口：Python API供开发者程序化构建协调管道，AI辅助聊天界面供领域专家通过自然语言对话进行数据协调

Result: 展示了用户如何通过BDI-Kit迭代探索、验证和优化模式与值匹配，结合自动化匹配、AI辅助推理和用户驱动的优化，演示了两种使用场景

Conclusion: BDI-Kit通过提供灵活的工具接口，有效解决了数据协调中的异质性问题，支持不同技术水平的用户进行高效的数据集成分析
...
TL;DR: 该研究重新审视了LLM后训练中"监督微调(SFT)会记忆而强化学习(RL)会泛化"的主流观点，发现在长思维链监督下，跨领域泛化并非不存在而是有条件的，受优化动态、训练数据和基础模型能力共同影响。


<details>
  <summary>Details</summary>
Motivation: 挑战LLM后训练中关于SFT和RL泛化能力的传统观点，特别是针对具有长思维链监督的推理任务SFT，探究其跨领域泛化能力的真实情况。

Method: 通过系统实验分析推理SFT的跨领域泛化，考察优化动态（训练时长）、训练数据（质量与结构）和基础模型能力三个关键因素对泛化性能的影响。

Result: 发现跨领域泛化存在"先降后升"模式，充分训练后性能可恢复并提升；高质量验证的长思维链数据带来稳定跨领域增益；强模型能内化可迁移的程序模式，而弱模型仅模仿表面冗长；但泛化具有不对称性：推理能力提升的同时安全性下降。

Conclusion: 推理SFT确实能够实现跨领域泛化，但这一能力是有条件的，取决于优化充分性、数据质量和模型能力。问题应从"推理SFT是否泛化"转变为"在什么条件下以及以什么代价实现泛化"，特别是需要权衡推理能力提升与安全性下降的代价。
...
TL;DR: Step-Saliency方法分析大型推理模型的长链思维过程，发现浅层锁定和深层衰减两种信息流故障，并提出StepFlow测试时干预方法提升推理性能


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂任务上表现良好但行为不稳定难解释，现有分析工具难以处理长结构化推理轨迹，需要新方法来理解和改进模型的信息流模式

Method: 提出Step-Saliency方法，将注意力-梯度分数沿问题-思考-总结轨迹池化为步到步映射；基于发现的故障模式提出StepFlow干预，包括通过Odds-Equal Bridge调整浅层显著性模式，以及通过Step Momentum Injection在深层添加步级残差

Result: Step-Saliency揭示了浅层锁定（浅层过度关注当前步骤而忽略上下文）和深层衰减（深层逐渐失去对思考段的显著性，总结过度关注自身和最后几步）两种故障模式；StepFlow无需重新训练即可提升多个大型推理模型在数学、科学和编程任务上的准确性

Conclusion: 修复信息流故障可以恢复大型推理模型缺失的部分推理性能，Step-Saliency为分析长链思维提供了有效工具，StepFlow展示了通过测试时干预改进模型推理能力的潜力
...
<details>
  <summary>Details</summary>
Motivation: 随着AI智能体系统的快速发展，智能体互联网正在形成，智能体在本地设备、边缘节点、私有服务和云平台之间运行。尽管已有研究改进了智能体命名、发现和交互，但在延迟、隐私和成本约束下的高效请求调度仍然是一个未解决的系统问题。

Method: AgentGate将路由问题重新定义为约束决策问题，分解为两个阶段：1) 动作决策阶段：确定查询应触发单智能体调用、多智能体规划、直接响应还是安全升级；2) 结构落地阶段：将选定动作实例化为可执行输出，如目标智能体、结构化参数或多步计划。此外，开发了面向路由的微调方案，包含候选感知监督和硬负例训练。

Result: 在包含多个3B-7B开源模型的定制路由基准测试中，紧凑模型在约束设置下能提供有竞争力的路由性能。模型差异主要体现在动作预测、候选选择和结构落地质量方面。结果表明结构化路由是高效、隐私感知智能体系统的可行设计点。

Conclusion: 结构化路由是资源受限部署条件下智能体系统的可行解决方案，AgentGate通过两阶段分解方法在紧凑模型上实现了高效路由决策，特别适用于需要在延迟、隐私和成本约束下做出路由决策的场景。
...
<details>
  <summary>Details</summary>
Motivation: 尽管AI行业已经开发了各种记忆组件（RAG管道、向量数据库、长上下文窗口、配置文件层），但缺乏正式框架来定义和测量这些组件是否真正产生连续性。现有评估方法未能系统性地验证AI系统在跨时间维度上维持一致叙事的能力。

Method: 定义了连续性作为系统属性的7个必需特性，开发了包含10个检查点的评估方法（无需LLM参与评估循环），创建了包含250个故事、1835个验证问题的叙事测试语料库，涵盖6个生活领域。评估采用参考实现，通过5次测试套件迭代进行验证。

Result: 参考实现从传统架构的58%准确率提升到孤立模式（250个故事）的100%，50个故事累积模式的100%，以及250个故事累积规模的96%。累积结果（250个不同生活叙事共存于同一数据库时系统能正确检索事实且无交叉污染）是主要衡量指标。

Conclusion: ATANT提供了一个系统无关、模型独立的连续性系统构建和验证框架，填补了AI系统连续性评估的空白。该框架可作为构建和验证连续性系统的序列化方法，相关规范、示例故事和评估协议已开源。
...
TL;DR: 提出一种基于激活空间干预的方法，通过分别学习明显幻觉和隐蔽幻觉的探测头，实现对多模态大语言模型幻觉可验证性的细粒度控制。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型产生的幻觉对人类用户构成风险，但不同幻觉的可验证性差异显著：明显幻觉易被用户识别，而隐蔽幻觉难以察觉或需要更多验证努力。现有研究缺乏对幻觉可验证性进行控制的方法，无法满足不同安全性和可用性需求的AI应用。

Method: 1. 构建包含4,470个人类响应的数据集，根据人类用户可验证性将幻觉分为明显和隐蔽两类；2. 提出激活空间干预方法，分别为明显幻觉和隐蔽幻觉学习独立的探测头；3. 通过干预控制模型的可验证性，混合干预可实现灵活的场景适应性调节。

Result: 实验结果表明：1. 明显幻觉和隐蔽幻觉引发不同的干预探测头；2. 针对性干预在调节相应可验证性方面表现优异；3. 简单混合这些干预能够灵活控制不同场景所需的可验证性。

Conclusion: 该研究首次探索了多模态AI幻觉可验证性的控制方法，提出的激活空间干预技术能够实现对模型幻觉可验证性的细粒度调节，为满足不同安全性和可用性需求的AI应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有智能设计方法通常专注于单级设计或依赖松耦合流程，难以实现完全自主的端到端设计。透平机械气动设计是一个复杂且紧密耦合的多阶段过程，需要解决从几何生成到物理验证的全流程自动化问题。

Method: 提出TurboAgent框架：以大语言模型为核心进行任务规划和协调，配合专门智能体处理生成设计、快速性能预测、多目标优化和基于物理的验证。保留高保真仿真用于最终验证，将传统试错设计转变为数据驱动的协同工作流。
...
Conclusion: TurboAgent实现了从自然语言需求到最终设计生成的自主闭环设计过程，为透平机械气动设计提供了高效且可扩展的范式，证明了LLM驱动的多智能体框架在复杂工程设计中具有重要应用价值。
...
TL;DR: 提出UI-in-the-Loop (UILoop)新范式，将GUI推理任务视为"屏幕-UI元素-动作"循环过程，通过让MLLMs显式学习UI元素的定位、语义功能和使用方法，实现精确元素发现和可解释推理。
...
Method: 提出UI-in-the-Loop (UILoop)范式，将GUI推理任务建模为循环的屏幕-UI元素-动作过程。让多模态大语言模型显式学习关键UI元素的定位、语义功能和实际使用方法。引入包含三个评估指标的UI理解任务，并贡献包含26K样本的UI Comprehension-Bench基准。

Result: 广泛实验表明，UILoop在UI理解方面达到最先进的性能，同时在GUI推理任务中取得优异结果。
...
TL;DR: EmoMAS是一个贝叶斯多智能体框架，通过战略性地管理情感决策，使小型语言模型在边缘设备的高风险谈判中达到大型语言模型的性能水平，同时降低计算成本和隐私风险。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自动谈判中计算成本高且存在隐私风险，不适合部署在移动助手或救援机器人等隐私敏感的边缘设备上。小型语言模型虽然计算效率高，但难以处理高风险谈判中复杂的情感动态。需要一种能够在边缘设备上实现有效、私密且自适应谈判的AI框架。

Method: EmoMAS采用贝叶斯多智能体框架，包含三个专门化的智能体：博弈论模型、强化学习模型和心理一致性模型。通过贝叶斯协调器实时融合各智能体的见解，优化情感状态转换，并基于谈判反馈持续更新智能体可靠性。采用混合智能体架构，支持在线策略学习而无需预训练。

Result: 在债务、医疗保健、应急响应和教育四个高风险边缘可部署谈判基准测试中，配备EmoMAS的小型语言模型和大型语言模型在所有基准测试中都持续超越所有基线模型，在谈判性能和伦理行为之间取得平衡。

Conclusion: EmoMAS通过将情感表达作为贝叶斯多智能体优化框架中的战略变量，为高风险边缘部署建立了有效、私密且自适应的谈判AI新范式。战略情感智能是谈判成功的关键驱动因素。
...
TL;DR: CAFP是一种模型无关的后处理方法，通过生成敏感属性翻转的反事实输入并平均预测结果来确保机器学习公平性，无需重新训练或修改原始分类器。


<details>
  <summary>Details</summary>
Motivation: 机器学习在敏感领域（如信用评分、医疗、刑事司法）部署时需确保公平性。现有公平性干预方法通常需要控制模型架构和访问受保护属性信息，这在现实系统中往往不可行。

Method: 提出Counterfactual Averaging for Fair Predictions (CAFP)方法：为每个输入生成敏感属性翻转的反事实版本，然后对原始模型在事实和反事实实例上的预测结果进行平均。

Result: 理论分析表明CAFP消除了预测对受保护属性的直接依赖，减少了预测与敏感属性之间的互信息，并界定了相对于原始模型的失真程度。在温和假设下，CAFP实现完美的人口统计均等，并将均等几率差距至少减少平均反事实偏差的一半。

Conclusion: CAFP提供了一种有效、实用的后处理公平性方法，无需重新训练模型或访问敏感属性信息，适用于现实世界系统部署。
...
TL;DR: A-MBER：用于评估AI助手基于长期交互记忆进行当前情感状态识别的基准测试
...
Method: 通过分阶段流水线构建A-MBER基准，包括长期规划、对话生成、标注、问题构建和最终打包，支持判断、检索和解释任务，以及模态降级和证据不足等鲁棒性设置

Result: 实验比较了局部上下文、长上下文、检索记忆、结构化记忆和黄金证据条件，结果显示A-MBER在长距离隐式情感、高依赖记忆水平、基于轨迹的推理和对抗性设置等设计重点子集上具有特别强的区分能力

Conclusion: 记忆支持情感解释不仅通过提供更多历史信息，更重要的是通过实现更选择性、有依据和上下文敏感地使用过去交互，A-MBER为评估AI助手的情感记忆能力提供了有效工具
...
<details>
  <summary>Details</summary>
Motivation: 传统规划研究主要关注生成实现目标的计划，但目标规范也可用于编码不应成立的性质。当规划任务包含应避免的缺陷状态时，需要修改规划任务以确保这些状态永远不会被到达，即让规划任务变得不可解

Method: 提出$allmin$算法，通过最小化修改原始动作来使规划任务不可解，从而解决规划任务屏蔽问题

Result: 在规模递增的规划任务上实证评估$allmin$性能，证明其能有效屏蔽系统，使规划任务变得不可解

Conclusion: 规划任务屏蔽是规划领域的重要问题，$allmin$算法能通过最小化动作修改有效检测和修复规划任务中的缺陷
...
TL;DR: EVGeoQA：基于电动汽车充电场景的新型地理空间问答基准，具有位置锚定和双目标设计，用于评估LLM在动态地理空间环境中的目的驱动探索能力


<details>
  <summary>Details</summary>
Motivation: 现有地理空间问答基准主要关注静态检索，无法捕捉现实世界规划中涉及动态用户位置和复合约束的复杂性。需要评估LLM在动态地理空间环境中进行目的驱动探索的潜力。

Method: 1. 引入EVGeoQA基准：基于电动汽车充电场景，具有位置锚定和双目标设计（充电需求+协同活动偏好）；2. 提出GeoRover评估框架：基于工具增强的智能体架构，系统评估LLM的动态多目标探索能力。

Result: 1. LLM能成功利用工具处理子任务，但在长距离空间探索方面存在困难；2. 观察到新兴能力：LLM能够总结历史探索轨迹以提高探索效率；3. EVGeoQA被证明是未来地理空间智能的具有挑战性的测试平台。

Conclusion: EVGeoQA填补了现有地理空间问答基准的空白，为评估LLM在动态、多目标地理空间探索中的能力提供了新的测试平台。研究揭示了LLM在空间探索方面的局限性和新兴能力，为未来地理空间智能研究奠定了基础。
...
TL;DR: T-STAR框架通过构建认知树来识别关键推理步骤，利用回溯评估和上下文思维嫁接技术优化强化学习策略，在多步推理任务中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法在处理大语言模型多步推理任务时面临稀疏奖励问题，现有方法如GRPO将采样轨迹视为独立链，对所有步骤分配均匀信用，忽略了关键步骤对推理结果的非对称影响。

Method: 提出T-STAR框架：1) 将轨迹整合为统一的认知树，识别并合并功能相似的步骤/节点；2) 引入内省评估机制，通过树结构回溯传播轨迹级奖励，获得方差减少的相对优势；3) 开发上下文思维嫁接技术，对比成功与失败分支在关键分歧点的推理；4) 提出外科手术式策略优化，通过Bradley-Terry类型的外科损失函数集中优化关键点。

Result: 在具身、交互、推理和规划等多个基准测试上的广泛实验表明，T-STAR相比强基线方法取得了一致的性能提升，在需要扩展推理链的任务上增益最为显著。

Conclusion: T-STAR通过恢复看似独立轨迹间的潜在相关奖励结构，有效解决了多步推理中的稀疏奖励问题，通过识别关键步骤和外科手术式优化实现了更高效的强化学习。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的智能体通常将世界建模、规划和反思放在单一语言模型循环中，这使得难以区分智能体能力中哪些来自LLM本身，哪些来自其周围的显式结构。研究旨在使这一问题在实证上可处理。

Method: 引入声明式反射运行时协议，将智能体状态、置信信号、防护动作和假设转换外部化为可检查的运行时结构。在声明式运行时中实例化该协议，在噪声协作战舰游戏中评估四种渐进结构化的智能体（54局游戏）。

Result: 分解出四个组件：后验信念跟踪、显式世界模型规划、符号化回合内反思和稀疏LLM修订。显式世界模型规划相比贪婪后验跟踪基线显著提升（+24.1pp胜率，+0.017 F1）。符号化反思作为实际运行时机制运行，但当前修订预设尚未产生净正收益。约4.3%回合添加条件性LLM修订仅产生微小非单调变化（平均F1略升+0.005，胜率从31降至29/54）。
