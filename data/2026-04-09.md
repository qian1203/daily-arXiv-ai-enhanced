...
TL;DR: 该论文通过蒙特卡洛采样和基于"王-王"位置反向搜索的新方法，精确估计了日本将棋的可达位置数量为6.55×10⁶⁸，显著缩小了先前估计的五个数量级差距。


<details>
  <summary>Details</summary>
Motivation: 日本将棋的状态空间复杂度估计存在巨大不确定性，先前组合估计的范围从10⁶⁴到10⁶⁹，存在五个数量级的差距。这种差距源于难以从大量有效棋盘配置中区分出从初始位置合法可达的位置。

Method: 采用蒙特卡洛采样结合新颖的可达性测试方法。该方法使用向"王-王"位置集合的反向搜索，而不是传统的向单一初始位置的反向搜索。这种KK位置反向搜索方法显著减少了确定不可达性所需的搜索工作量。基于50亿个位置的样本进行统计估计。

Result: 估计日本将棋的合法位置数量为6.55×10⁶⁸（三位有效数字），置信水平为3σ。该方法也应用于迷你将棋，确定其复杂度约为2.38×10¹⁸。这一结果显著改进了先前已知的边界估计。

Conclusion: 提出的基于KK位置反向搜索的蒙特卡洛方法成功解决了日本将棋状态空间复杂度估计的长期难题，将估计精度从五个数量级的不确定性提高到三位有效数字的精确估计，为棋类游戏复杂度分析提供了新方法。
...
TL;DR: 研究发现语言模型存在"盲目拒绝"问题：即使面对不合理或不公正的规则，模型仍倾向于拒绝帮助用户规避规则，这种拒绝行为与模型对规则合法性的规范推理能力脱节。


<details>
  <summary>Details</summary>
Motivation: 安全训练的语言模型通常会拒绝帮助规避规则的请求，但并非所有规则都值得遵守。当用户请求帮助规避由非法权威制定的规则、内容或应用极不公正/荒谬的规则、或允许合理例外的规则时，模型的拒绝反映了道德推理的失败。研究者旨在实证记录这种"盲目拒绝"模式。

Method: 构建包含5种规则失效类型（可打破规则的理由）和19种权威类型的合成案例数据集，通过三个自动化质量门控和人工审核验证。收集来自7个模型家族的18种配置的响应，使用盲审的GPT-5.4 LLM-as-judge评估方法，在响应类型（帮助、硬拒绝、转移）和模型是否认识到削弱规则合规要求的理由两个维度进行分类。

Result: 模型拒绝了75.4%（N=14,650）的失效规则请求，即使这些请求本身不涉及独立的安全或双重用途问题。57.5%的情况下模型会参与讨论失效条件，但仍拒绝提供帮助，表明模型的拒绝行为与其对规则合法性的规范推理能力脱节。

Conclusion: 语言模型存在系统性的"盲目拒绝"问题，这种拒绝行为与模型对规则合法性的道德推理能力不匹配。模型能够识别规则失效的理由，但仍选择拒绝帮助，这揭示了当前安全训练方法的局限性，需要开发更细致的道德推理能力。
...
TL;DR: 开发机器学习模型预测集装箱是否需要预清关服务及在港停留时间，以优化码头运营效率
...
Result: 模型在多个时间验证期内持续优于现有的基于规则的启发式方法和随机基线，在精确率和召回率方面表现更好
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM幻觉检测方法依赖推理时的外部验证（如黄金答案、检索系统或辅助判断模型），这限制了实际应用。研究探索是否可以将这种外部监督蒸馏到模型自身表示中，仅通过内部激活进行幻觉检测

Method: 1) 提出弱监督框架，结合三种互补的grounding信号：子串匹配、句子嵌入相似度和LLM作为判断器；2) 基于SQuAD v2构建15,000样本数据集，包含LLaMA-2-7B生成答案及其各层隐藏状态；3) 训练五种探测分类器：ProbeMLP、LayerWiseMLP、CrossLayerTransformer、HierarchicalTransformer和CrossLayerAttentionTransformerV2

Result: Transformer-based probes表现最佳，M2在5折平均AUC/F1上最优，M3在单折验证和保留测试集上最优。推理效率方面：探测延迟0.15-6.66ms，端到端生成加探测吞吐量约0.231 QPS，实际开销可忽略

Conclusion: 成功验证了幻觉检测信号可以蒸馏到transformer表示中的假设，实现了无需推理时外部验证的内部幻觉检测，为更高效的LLM幻觉检测提供了新方向
...
TL;DR: SymptomWise框架通过分离语言理解与诊断推理，结合专家知识、确定性代码驱动推理和受限大语言模型使用，提升AI症状分析系统的可靠性、可解释性和减少幻觉。
...
Method: 系统将语言理解与诊断推理分离：自由文本输入映射到验证的症状表示，然后由确定性推理模块在有限假设空间中进行评估，生成排序的鉴别诊断。语言模型仅用于症状提取和可选解释，不用于诊断推理。

Result: 在42个专家编写的具有挑战性的儿科神经病学案例的初步评估中，系统与临床医生共识有意义的重叠，正确诊断出现在前五鉴别诊断中的比例为88%。
...
<details>
  <summary>Details</summary>
Motivation: LLM能生成数学逻辑领域的说服性论证，但常包含微小错误（如忽略边界条件、无效推理、引用无法推导的引理），这些错误难以从文本中察觉。交互式定理证明器（如Lean、Coq）虽提供严格可靠性保证，但需要完全形式化证明且需大量低层信息，代价高昂。

Method: 设计混合管道：LLM在紧凑DSL中生成类型化证明草图，轻量级可信内核将草图扩展为显式证明义务，结合LLM的生成能力和形式验证的可靠性。

Result: 提出了一种结合LLM生成能力和形式验证可靠性的混合方法，通过类型化证明草图和轻量级可信内核的协作，降低完全形式化证明的负担。

Conclusion: 混合管道方法在保持形式验证可靠性的同时，利用LLM生成能力降低证明负担，为数学逻辑论证提供更高效可靠的解决方案。
...
Method: BDI-Kit提供两种互补接口：Python API允许开发者以编程方式构建协调管道；AI辅助聊天界面允许领域专家通过自然语言对话协调数据。工具结合自动匹配、AI辅助推理和用户驱动优化

Result: 演示展示了用户如何与BDI-Kit交互，通过迭代探索、验证和优化模式与值匹配。展示了两个场景：使用Python API编程组合原语、检查中间输出和重用转换；与AI助手自然语言对话访问BDI-Kit功能并基于建议迭代优化输出

Conclusion: BDI-Kit通过提供灵活的工具包和用户友好的接口，解决了数据协调中的异构性问题，支持不同技术水平的用户进行有效的数据集成分析
...
TL;DR: 该论文重新审视了LLM后训练中"监督微调(SFT)记忆化而强化学习(RL)泛化"的主流观点，发现推理SFT在长思维链监督下的跨域泛化并非不存在，而是有条件的，受优化动态、训练数据和基础模型能力共同影响。


<details>
  <summary>Details</summary>
Motivation: 挑战LLM后训练中关于SFT和RL泛化能力的传统观点，特别是针对推理任务中的思维链监督，探究SFT是否真的缺乏跨域泛化能力。

Method: 通过系统实验分析推理SFT的跨域泛化，考察优化动态（训练时长）、训练数据（质量与结构）和基础模型能力对泛化的影响，观察性能变化模式。

Result: 发现跨域泛化存在但条件复杂：1) 优化不足会导致性能先降后升的"下降-恢复"模式；2) 数据质量和结构关键，已验证的长思维链能带来一致增益；3) 模型能力决定能否内化可迁移的程序模式；4) 泛化不对称，推理能力提升但安全性下降。

Conclusion: 推理SFT确实能实现跨域泛化，但受多种条件制约，问题应从"是否泛化"转向"在什么条件下、以什么代价泛化"，为LLM后训练提供了更精细的理解框架。
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂任务上表现良好但行为不稳定难以解释，现有分析工具难以处理长结构化推理轨迹，需要新的分析方法和干预策略

Method: 提出Step-Saliency方法，将注意力-梯度分数池化为步骤间映射；基于分析发现提出StepFlow干预，包括通过Odds-Equal Bridge调整浅层注意力模式，通过Step Momentum Injection在深层添加步骤级残差

Result: Step-Saliency揭示了浅层锁定和深层衰减两种信息流故障模式；StepFlow在不重新训练的情况下提高了多个大型推理模型在数学、科学和编程任务上的准确性

Conclusion: 修复信息流故障可以恢复大型推理模型缺失的部分推理性能，Step-Saliency为分析长推理轨迹提供了有效工具，StepFlow展示了基于注意力模式的测试时干预潜力
...
TL;DR: AgentGate：一个轻量级结构化路由引擎，用于候选感知的智能体调度，将路由问题分解为动作决策和结构落地两个阶段，通过路由导向的微调方案提升紧凑模型在资源受限环境下的路由性能。


<details>
  <summary>Details</summary>
Motivation: 随着AI智能体系统的快速发展，出现了智能体互联网，其中专用智能体在本地设备、边缘节点、私有服务和云平台之间运行。尽管最近的研究改进了智能体命名、发现和交互，但在延迟、隐私和成本约束下的高效请求调度仍然是一个开放的系统问题。

Method: AgentGate将路由问题形式化为约束决策问题，分解为两个阶段：动作决策和结构落地。第一阶段决定查询应触发单智能体调用、多智能体规划、直接响应还是安全升级；第二阶段将选定动作实例化为可执行输出，如目标智能体、结构化参数或多步计划。为适应紧凑模型，开发了路由导向的微调方案，包含候选感知监督和硬负例。

Result: 在包含多个3B-7B开放权重模型的精心策划路由基准测试中，紧凑模型在约束设置下能提供有竞争力的路由性能。模型差异主要体现在动作预测、候选选择和结构落地质量上。

Conclusion: 结构化路由是构建高效且隐私感知的智能体系统的可行设计点，特别是在资源受限部署条件下必须做出路由决策时。紧凑模型通过适当的结构化方法可以在智能体调度中发挥重要作用。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI行业虽然开发了各种记忆组件（RAG管道、向量数据库、长上下文窗口、配置文件层），但缺乏正式框架来定义和测量这些组件是否真正产生连续性。需要系统化的方法来评估AI系统在时间维度上保持上下文一致性的能力。

Method: 定义了连续性作为系统属性，包含7个必需特性；开发了10个检查点的评估方法，无需LLM参与评估循环；创建了包含250个故事、1835个验证问题的叙事测试语料库，涵盖6个生活领域；采用系统无关、模型独立的设计，支持序列化方法构建和验证连续性系统。

Result: 参考实现经过5次测试套件迭代，从58%（传统架构）提升到100%（隔离模式，250个故事）和100%（50个故事累积模式），在250个故事累积规模下达到96%。累积结果是主要衡量标准：当250个不同生活叙事共存于同一数据库时，系统必须为正确上下文检索正确事实而不发生交叉污染。

Conclusion: ATANT提供了一个正式、可重复的框架来测量AI系统的连续性能力，填补了当前评估方法的空白。该框架开源可用，支持AI系统在长期上下文管理和记忆保持方面的开发和验证。
...
TL;DR: 提出针对多模态大语言模型幻觉的可验证性控制方法，通过激活空间干预区分明显和隐蔽幻觉，实现细粒度控制


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型产生的幻觉对人类用户构成风险，但不同幻觉的可验证性差异显著：明显幻觉易被人类检测，隐蔽幻觉难以发现。现有研究缺乏对幻觉可验证性的控制方法，无法满足不同安全性和可用性需求的应用场景

Method: 1) 从4,470个人类响应构建数据集，根据人类可验证性将幻觉分为明显和隐蔽两类；2) 提出激活空间干预方法，学习针对明显和隐蔽幻觉的独立探针；3) 通过混合干预实现灵活的可验证性控制

Result: 实验结果表明：1) 明显和隐蔽幻觉引发不同的干预探针；2) 针对性干预在调节相应可验证性方面表现优异；3) 混合干预可灵活适应不同场景的可验证性需求

Conclusion: 该研究首次探索了多模态AI幻觉的可验证性控制，提出的激活空间干预方法能有效区分和调节明显与隐蔽幻觉，为不同安全需求的应用提供了细粒度控制方案
...
TL;DR: 提出TurboAgent：基于大语言模型驱动的自主多智能体框架，用于透平机械气动设计与优化，实现从自然语言需求到最终设计的端到端自主闭环设计流程。
...
Method: 提出UI-in-the-Loop (UILoop)范式，将GUI推理任务建模为循环的屏幕-UI元素-动作过程。通过多模态大语言模型显式学习关键UI元素的定位、语义功能和实际使用方法，实现精确元素发现和可解释推理。同时引入以UI元素为中心的UI理解任务及三个评估指标。

Result: 构建包含26K样本的UI Comprehension-Bench基准，全面评估现有方法对UI元素的掌握程度。大量实验表明，UILoop在UI理解性能上达到最先进水平，并在GUI推理任务中取得优越结果。

Conclusion: UILoop通过显式学习UI元素的定位、语义功能和使用方法，提供了一种创新的GUI推理范式，显著提升了UI理解能力和任务执行效果，为GUI交互提供了更可解释和有效的解决方案。
...
TL;DR: EmoMAS：一个贝叶斯多智能体框架，通过战略性地管理情绪状态来提升小型语言模型在隐私敏感边缘设备上的谈判性能，无需预训练即可在线学习策略。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自动谈判中应用广泛，但其高计算成本和隐私风险限制了在隐私敏感边缘设备（如移动助手、救援机器人）上的部署。小型语言模型虽然可行，但在高风险谈判的复杂情绪动态方面表现不佳。

Method: EmoMAS采用贝叶斯多智能体框架，包含一个贝叶斯协调器来协调三个专门化智能体：博弈论模型、强化学习模型和心理一致性模型。系统融合它们的实时洞察来优化情绪状态转换，同时根据谈判反馈持续更新智能体可靠性。这种混合智能体架构支持无需预训练的在线策略学习。

Result: 在债务、医疗、应急响应和教育四个高风险边缘可部署谈判基准测试中，通过广泛的智能体间模拟，配备EmoMAS的小型和大型语言模型在所有基准测试中均持续超越所有基线模型，在谈判性能与伦理行为之间取得平衡。

Conclusion: 战略情绪智能是谈判成功的关键驱动因素。通过将情绪表达作为贝叶斯多智能体优化框架中的战略变量，EmoMAS为高风险边缘部署建立了有效、私密且自适应的谈判AI新范式。
...
TL;DR: 提出CAFP方法，通过生成反事实输入并平均预测结果来实现公平性，无需重新训练模型


<details>
  <summary>Details</summary>
Motivation: 机器学习公平性在敏感领域至关重要，现有方法通常需要控制模型架构或访问受保护属性信息，这在现实系统中往往不可行

Method: CAFP是一种模型无关的后处理方法，通过翻转敏感属性生成反事实输入，然后平均原始模型在事实和反事实实例上的预测结果

Result: 理论分析表明CAFP消除了对受保护属性的直接依赖，减少了预测与敏感属性之间的互信息，并限制了相对于原始模型的失真

Conclusion: CAFP在温和假设下能够实现完美的人口统计均等，并将均衡机会差距至少减少平均反事实偏差的一半
...
TL;DR: A-MBER：一个用于评估AI助手基于记忆交互历史进行情感识别的基准测试，填补了现有数据集在长期情感记忆评估方面的空白


<details>
  <summary>Details</summary>
Motivation: 现有情感数据集主要评估瞬时情感，而长期记忆基准测试则侧重于事实回忆、时间一致性或知识更新，缺乏评估模型能否利用记忆的交互历史来解释用户当前情感状态的能力
...
Conclusion: 记忆支持情感解释不仅通过提供更多历史信息，而是通过实现更有选择性、更接地气和更上下文敏感的过去交互使用方式，A-MBER为评估AI助手的情感记忆能力提供了重要工具
...
TL;DR: 提出规划任务屏蔽问题，通过最优算法allmin检测并修复规划任务中的缺陷，使包含错误状态的目标规范无法达成


<details>
  <summary>Details</summary>
Motivation: 传统规划研究主要关注如何生成实现目标的计划，但目标规范也可用于编码不应出现的属性（错误状态）。当需要确保系统永不进入错误状态时，目标转变为修改规划任务使其不可解，即实现规划任务屏蔽

Method: 提出allmin算法，通过最小化修改原始动作来使规划任务不可解。该算法能检测规划任务中的缺陷并进行修复，实现最优屏蔽

Result: 在规模递增的规划任务上进行实证评估，allmin算法能有效屏蔽系统，成功使规划任务不可解

Conclusion: 规划任务屏蔽是规划领域的重要扩展，allmin算法为解决该问题提供了有效的解决方案，能够通过最小化修改确保系统永不进入错误状态
...
Method: 提出EVGeoQA基准，具有位置锚定和双目标设计（充电需求和协同活动偏好）；开发GeoRover评估框架，基于工具增强的智能体架构来评估LLM的动态多目标探索能力。

Result: 实验表明LLM能成功利用工具处理子任务，但在长距离空间探索方面存在困难；发现LLM具有新兴能力：能够总结历史探索轨迹以提高探索效率。
...
TL;DR: T-STAR是一个强化学习框架，通过构建认知树来识别关键推理步骤，使用内省评估机制传播轨迹级奖励，并通过思想嫁接和外科手术式策略优化来改进LLM智能体的多步推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法（如GRPO）将采样的轨迹视为独立链，对所有步骤分配均匀的信用，忽略了关键步骤对推理结果的非对称影响，导致稀疏奖励问题在多步推理任务中阻碍LLM智能体的学习。

Method: 1. 构建认知树：将轨迹整合为统一结构，识别并合并功能相似的步骤/节点；2. 内省评估机制：通过树结构反向传播轨迹级奖励，获得方差减少的步骤级相对优势；3. 上下文思想嫁接：在关键分歧点对比成功和失败分支，合成纠正性推理；4. 外科手术式策略优化：使用Bradley-Terry类型的外科损失函数，专注于关键点的策略梯度信息。

Result: 在具身、交互、推理和规划等多个基准测试上的广泛实验表明，T-STAR相比强基线方法取得了一致的改进，在需要扩展推理链的任务上增益最为显著。

Conclusion: T-STAR通过恢复看似独立轨迹间的潜在相关奖励结构，有效解决了多步推理中的稀疏奖励问题，通过识别关键步骤并集中优化资源，显著提升了LLM智能体的推理能力。
...
TL;DR: 该研究通过声明式反射运行时协议，将LLM智能体的世界建模、规划和反思等内部状态外部化为可检查的运行时结构，从而实证分析LLM在智能体能力中的具体贡献。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的智能体通常将世界建模、规划和反思集成在单一语言模型循环中，这使得难以区分智能体能力中哪些来自LLM本身，哪些来自其周围的结构化设计。研究旨在通过实证方法解决这一基本科学问题。

Method: 提出声明式反射运行时协议，将智能体状态、置信度信号、受保护动作和假设转换外部化为可检查的运行时结构。在声明式运行时中实例化该协议，并在噪声协作战舰游戏中评估四种渐进结构化智能体（54场游戏：18个棋盘×3种子）。

Result: 分解出四个组件：后验信念跟踪、显式世界模型规划、符号化回合内反思、稀疏LLM修订。显式世界模型规划相比贪婪后验跟踪基线显著提升（胜率+24.1pp，F1+0.017）。符号化反思作为实际运行时机制运行，但当前修订预设尚未产生净正效益。在约4.3%回合添加条件性LLM修订仅带来微小非单调变化（平均F1略升+0.005，胜率从31降至29/54）。

Conclusion: 研究贡献在于方法论而非性能领先：将反思外部化使原本潜在的智能体行为变为可检查的运行时结构，从而能够直接研究LLM干预的边际作用，为理解LLM在智能体架构中的具体角色提供实证基础。
