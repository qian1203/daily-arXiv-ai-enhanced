...
TL;DR: 该研究将结构化LLM路由重新定义为系统级负载分配问题，而非单纯的提示工程问题，并通过跨后端基准测试发现路由性能高度依赖后端特性，没有通用最优方案。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型成为智能体AI系统的核心控制组件，可靠的结构化路由需要在正确性、延迟和实现成本之间取得平衡。当前研究多将结构化路由视为提示工程问题，但作者认为这本质上是一个系统级的负载分配问题，需要在实际部署约束下进行优化。

Method: 提出将结构化路由视为系统级负载分配问题的框架，通过全面的全因子基准测试评估48种部署配置和15,552个请求，覆盖OpenAI、Gemini和Llama三种后端。研究考察了结构工作在不同生成堆栈层级的分配方式：模型直接输出结构、传输过程中压缩、或生成后本地重建。

Result: 核心发现：不存在通用的最佳路由模式，后端特定的交互效应主导性能。在Gemini和OpenAI上保持高可靠性的模式，在Llama上可能出现显著的正确性下降；压缩实现的效率增益也高度依赖后端。性能表现呈现明显的后端依赖性。
...
<details>
  <summary>Details</summary>
Motivation: 传统因果推断方法（可忽略性、平行趋势、排除限制）都通过假设替代缺失数据，但都无法直接产生反事实结果本身。需要一种新框架来系统处理反事实的不可观测性问题。

Method: 提出数字孪生反事实框架：1) 将数字孪生模拟器形式化为潜在结果框架中的随机映射；2) 建立孪生保真度假设层次结构（边际保真度、联合保真度、结构保真度）；3) 设计五级验证架构将不可证伪主张转化为可检验测试；4) 形式化分解将因果量区分为边际可验证和copula依赖两类。

Result: 1) 建立了将不可证伪因果主张转化为可检验测试的验证架构；2) 形式化分解了边际可验证因果量（ATE、CATE、QTE）和copula依赖因果量（ITE分布、获益/伤害概率、处理效应方差）；3) 提供了边界、敏感性和不确定性量化工具使copula依赖性显式化。

Conclusion: DTCF不解决因果推断的根本问题，但提供了一个框架：使边际因果主张变得可检验，使联合因果主张变得假设索引化，并形式化表征了两者之间的差距。该框架使因果推断的假设依赖性更加透明和可量化。
...
TL;DR: IDEA2是一个结合大型语言模型与专家协作的半自动化工作流，用于解决本体工程中能力问题提取的资源密集型瓶颈问题。
...
Method: IDEA2采用半自动化工作流，核心是迭代循环：LLM从需求文档中初步提取能力问题，领域专家在协作平台上进行共同创建性评审和反馈，LLM根据反馈迭代重新表述被拒绝的问题直至达成共识，并使用溯源模型跟踪完整生命周期。

Result: 在科学数据和文化遗产两个真实场景中验证，IDEA2能够加速需求工程过程，提高生成能力问题的接受度和相关性，并在领域专家中展现出高可用性和有效性。

Conclusion: IDEA2通过整合LLM与专家协作流程，有效解决了本体工程中能力问题提取的瓶颈问题，提供了透明、可复现的解决方案，并开源了所有代码和实验。
...
Method: 引入世界中心多智能体系统概念，提出世界分类维度（本体显式性、规范性等），使用语义模型作为表示世界的数学形式化方法，并开发Ontobox平台作为WMAS的实现

Result: 建立了世界中心多智能体系统的理论框架，提供了世界分类方法，提出了语义模型作为形式化表示工具，并实现了Ontobox平台作为具体实现方案

Conclusion: 世界中心多智能体系统为结构化领域提供了更优的架构选择，通过共享世界模型实现了全局一致性、语义一致性和可验证的系统行为，为企业和制度系统的智能体系统设计提供了新范式
...
TL;DR: 该研究提出AI自动化是一个连续体，包含"崩溃浪潮"（AI能力在少量任务上突然激增）和"上升潮汐"（AI能力持续广泛提升）两种模式。基于对3000多个基于文本的任务评估，研究发现上升潮汐是主要的AI自动化形式，AI在广泛任务上表现良好且快速改进。


<details>
  <summary>Details</summary>
Motivation: 研究旨在理解AI自动化的发展模式，区分"崩溃浪潮"（能力突然激增）和"上升潮汐"（持续广泛提升）两种假设。通过实证检验哪种模式主导AI能力发展，为AI对经济和劳动力市场的影响提供预测基础。

Method: 使用美国劳工部O*NET分类系统，选取3000多个基于文本且LLM可处理的任务进行评估。通过来自这些工作岗位的17000多次人工评估，测量AI模型在不同任务上的表现。分析AI能力随时间的变化趋势，预测未来发展趋势。

Result: 研究发现几乎没有"崩溃浪潮"的证据（与METR的研究相反），但发现"上升潮汐"是主要的AI自动化形式。AI在广泛任务上表现良好且快速改进：2024年第二季度，AI模型能以约50%的成功率完成人类需要3-4小时的任务；到2025年第三季度，成功率预计提高到约65%。如果当前趋势持续，到2029年LLM在大多数文本相关任务上的平均成功率将达到80%-95%。

Conclusion: AI自动化主要通过"上升潮汐"模式发展，即能力持续广泛提升而非突然激增。AI能力改进速度很快，预计未来几年将在大多数文本任务上达到高成功率。虽然AI能力提升迅速，但其对经济和劳动力市场的实际影响可能因组织采用AI的时间线而滞后。
...
TL;DR: 该研究探讨了大型语言模型中的认知偏差，开发了LLM CogBias基准测试四种偏差类型，发现偏差在激活空间中线性可分，并通过激活导向技术有效减少了26-32%的偏差分数。


<details>
  <summary>Details</summary>
Motivation: LLMs越来越多地应用于高风险决策场景，虽然已有研究表明LLMs行为上表现出认知偏差，但这些偏差是否对应可识别的内部表征以及能否通过针对性干预来缓解，仍然是一个开放性问题。

Method: 1. 定义LLM认知偏差为在可计算基准任务中系统性、可复现的偏离正确答案；2. 引入LLM CogBias基准，围绕四种认知偏差家族组织：判断偏差、信息处理偏差、社会偏差和响应偏差；3. 评估三个LLMs；4. 使用对比设计下的线性探针分析偏差在激活空间中的表征；5. 应用激活导向技术调节偏差行为。

Result: 1. 认知偏差在所有四种家族中系统性出现，其程度和去偏响应强烈依赖于偏差家族：提示级去偏显著减少响应偏差，但对判断偏差产生反效果；2. 偏差在模型激活空间中编码为线性可分方向；3. 激活导向技术实现了26-32%的偏差分数减少，同时在25个基准测试中保持下游能力（Llama：可忽略的退化；Qwen：判断偏差最多下降19.0个百分点）；4. 尽管模型间偏差表征近乎正交（平均余弦相似度0.01），但导向技术在不同架构中减少偏差的速率相似，表明存在共享的功能组织。

Conclusion: LLMs中的认知偏差对应可识别的内部表征，可通过激活导向技术有效缓解。不同偏差家族需要不同的干预策略，而不同模型间偏差表征的差异并不妨碍共享的去偏机制。这为理解和减轻LLMs在关键应用中的偏差提供了理论基础和方法框架。
...
TL;DR: 提出RIFT分类法，系统诊断评分标准设计中的失败模式，包含8种失败模式分为3大类，通过人工标注验证一致性，并开发自动化指标进行规模化诊断。


<details>
  <summary>Details</summary>
Motivation: 当前LLM基准测试和训练流程广泛使用基于评分标准的评估，但现有方法仅通过下游信号（如强化学习结果）评估评分标准质量，缺乏从聚合或下游信号中诊断评分标准质量问题的系统方法。

Method: 提出RIFT（RubrIc Failure mode Taxonomy）分类法，包含8种失败模式，分为可靠性失败、内容效度失败和后果效度失败三大类。基于扎根理论，从五个多样化基准（通用指令遵循、代码生成、创意写作、专家级深度研究）中迭代标注评分标准直至饱和。评估分类法一致性，测量独立标注者间一致性（87%成对一致性和0.64平均Cohen's kappa）。开发自动化评分标准质量指标，验证其与人工失败模式标注的对齐程度。

Result: RIFT分类法包含8种失败模式，分为三大类。人工标注者间一致性达到87%成对一致性和0.64平均Cohen's kappa（公平一致性）。自动化指标与人工失败模式标注对齐良好，最高达到0.86 F1分数。

Conclusion: RIFT为系统诊断评分标准设计中的失败模式提供了原则性框架，填补了现有评估方法的空白。该分类法具有良好的一致性，且自动化指标支持规模化诊断，有助于提高LLM评估和训练中评分标准的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: POMDP在状态和转移不确定性下的规划具有理论优势，但维数灾难和历史诅咒使得在有限规划时间内获得高性能策略仍具挑战性。许多POMDP问题中，信息价值(VOI)在信念空间上变化，这为优化计算分配提供了机会。

Method: 提出动态规划框架，基于每个信念点的信息价值有条件地处理观测。在此基础上提出VOIMCP算法，这是一种蒙特卡洛树搜索算法，当VOI较低时选择性忽略观测信息，避免不必要的观测分支，从而更高效地分配计算资源。

Result: 提供了VOI推理框架的近似最优性理论保证，推导了VOIMCP的非渐近收敛界。仿真评估表明，VOIMCP在多个POMDP基准测试中优于基线方法。

Conclusion: 通过利用信息价值在信念空间中的变化特性，提出的VOIMCP框架能够更高效地分配计算资源，在有限规划时间内获得更好的策略性能，为大规模POMDP规划提供了有效的解决方案。
...
TL;DR: 该论文研究了AI辅助药物系统的可靠性，重点关注系统故障类型及其临床后果，强调需要超越传统性能指标，采用风险感知的评估方法。
...
Method: 通过一系列涉及药物相互作用和剂量决策的受控模拟场景，分析不同类型的系统故障，包括遗漏的相互作用、错误的风险标记和不适当的剂量推荐。研究重点从单纯的聚合指标评估转向错误如何发生以及AI系统产生错误输出时的后果。

Result: 研究发现，在药物相关背景下，AI错误可能导致药物不良反应、无效治疗或延迟护理，特别是在没有足够人工监督的情况下使用系统时。研究还揭示了过度依赖AI推荐的风险以及决策过程透明度有限带来的挑战。

Conclusion: 本文为医疗AI评估提供了一个以可靠性为中心的视角，强调理解故障行为和现实世界影响的重要性。研究强调需要将传统性能指标与风险感知评估方法相结合，特别是在药房实践等安全关键领域。
...
TL;DR: 提出了一种不可行性感知的LLM框架，用于组合优化问题，结合了可验证数据集构建、监督微调和LLM辅助下游搜索，在minor-embedding问题上实现了更好的不可行性检测和优化加速。


<details>
  <summary>Details</summary>
Motivation: 现有LLM方法主要关注可行实例的解决方案生成，而忽视了不可行性检测的重要性。对于NP难组合优化问题，识别不可行实例与生成可行解同样关键，但现有方法缺乏对此的显式处理。

Method: 提出了一个不可行性感知框架，包含三个核心组件：1）可验证数据集构建：通过新的数学规划公式和可证明的零阶段不可行性筛选，生成标注为可行（带结构化证书）或可证明不可行的训练实例；2）监督微调：使用精确优化流程生成的数据对8B参数LLM进行微调，使其能够同时执行解决方案生成和不可行性检测；3）LLM辅助下游搜索：将LLM输出作为下游局部搜索的初始解，即使LLM输出不完美也能加速优化过程。

Result: 实验表明，微调后的模型在整体准确率上比GPT-5.2提高了30%；同时，LLM引导的初始解在下游局部搜索中相比从零开始提供了2倍的加速效果。

Conclusion: 该研究证明了结合可验证数据生成、监督微调和LLM辅助搜索的框架能够有效提升LLM在组合优化问题中的性能，特别是在不可行性检测方面，同时通过LLM输出作为初始解可以显著加速下游优化过程。
...
TL;DR: 提出P-COD方法，利用文档间关系检测异常，减少LLM在科学文献摘要中的数据提取幻觉，精度达98%


<details>
  <summary>Details</summary>
Motivation: 当前LLM数据提取方法（如提示工程、思维链）仅关注单个文档，忽略了文档间的关联性，导致幻觉问题。科学文献中相似实验设置应得出相似结论，需要利用文档间关系提高提取准确性。

Method: 提出Peer Context Outlier Detection (P-COD)方法：通过比较提取数据与语料库中已验证的同行信息，调整置信度分数，标记低置信结果供专家审查。高置信度且经同行验证的结果被视为可靠。

Result: 在6个科学领域的实验中，异常检测精度高达98%。P-COD能有效减少幻觉，增强自动化系统的可信度，让研究人员专注于模糊案例，优化数据提取工作流程。
...
TL;DR: 提出一种基于技能演化的智能体框架，用于超表面逆向设计，通过积累可重用的求解器特定专业知识，减少工作流构建的专家依赖


<details>
  <summary>Details</summary>
Motivation: 超表面逆向设计需要计算电磁学和求解器特定软件工程的专业知识，现有语言驱动系统缺乏跨任务可重用工作流知识的保存，限制了自动化程度

Method: 结合编码智能体、演化技能构件和基于物理仿真的确定性评估器，通过迭代精炼求解器特定策略而不修改模型权重或底层物理求解器

Result: 在分布内任务上，成功率从38%提升至74%，标准通过率从0.510增至0.870，平均尝试次数从4.10降至2.30；在分布外任务上，成功率变化有限但误差组成和智能体行为显示部分工作流知识迁移
...
TL;DR: 提出基于角色分工的LLM框架，通过分配政策分析师、法律策略专家和食品系统专家等专门角色，自动化从非结构化健康食品政策数据中提取信息，解决传统LLM方法中的幻觉、误分类和遗漏问题。
...
Result: 使用608个健康食品政策（来自HFPP数据库）评估框架，与零样本、少样本和思维链基线（基于Llama-3.3-70B）进行比较。所提框架在复杂推理任务中表现出优越性能，为健康政策信息提取提供了可靠且透明的方法论。

Conclusion: 角色化LLM框架通过模拟专家工作流程和整合结构化领域知识，有效解决了健康食品政策信息提取中的挑战，提供了自动化政策分析的可靠方法。
...
TL;DR: 该论文研究将表示自编码器（RAE）整合到连续自回归（AR）模型中的挑战，提出分布归一化和高斯噪声注入方法来解决高维表示自编码器在AR范式中的建模困难，使RAE在AR模型中达到与VAE相当的性能。


<details>
  <summary>Details</summary>
Motivation: 虽然最近RAE方法显示表示自编码器在生成建模中具有竞争力，但将其整合到连续自回归（AR）模型中仍未被充分探索。传统上，生成建模的潜在空间由VAE编码器主导，而预训练表示编码器（如DINO、SigLIP、MAE）的潜在表示被认为不适合生成建模。需要研究高维表示自编码器在AR范式中的具体挑战。

Method: 提出RAE-AR框架，针对AR模型的独特性质解决两个主要障碍：1）通过分布归一化进行标记简化，降低复杂标记级分布建模难度；2）通过训练时注入高斯噪声来缓解高维度放大的训练-推理差距（暴露偏差），增强预测鲁棒性。

Result: 实验结果表明，这些修改显著缩小了性能差距，使表示自编码器在AR模型中能够达到与传统VAE相当的结果。这为视觉理解和生成建模的统一架构铺平了道路。

Conclusion: 该工作成功解决了将高维表示自编码器整合到连续自回归模型中的挑战，通过分布归一化和噪声注入技术，使RAE-AR框架能够实现与VAE-AR相当的性能，推动了视觉理解与生成建模架构的统一。
...
TL;DR: 研究发现LLM训练中归一化层与优化器之间存在显著交互效应：Derf归一化与Muon优化器组合时性能大幅下降，而DyT归一化则无此问题，表明需将归一化层与优化器作为耦合系统设计。


<details>
  <summary>Details</summary>
Motivation: 传统LLM训练中将归一化层和优化器视为独立设计选择，但本研究通过实验发现这种假设可能不成立，特别是在Derf归一化与Muon优化器组合时出现显著的负交互效应，需要重新审视这种独立性假设。

Method: 采用3x2因子设计实验，在10亿参数规模、1000训练步数下测试不同归一化层（Derf、DyT、RMSNorm）与优化器（AdamW、Muon）的组合。通过分析失效模式（饱和与尺度盲区），提出EMA混合方法和调整Derf的alpha参数来缓解问题。

Result: Derf与Muon组合时出现严重负交互效应：与RMSNorm的差距从AdamW下的+0.31 nats扩大到Muon下的+0.97 nats（约3倍）。DyT作为有界归一化对照组未显示类似惩罚。失效源于erf在Muon更快的谱范数增长下的饱和和尺度盲区问题。EMA混合恢复约84%差距，降低Derf的alpha从0.5到0.3恢复约80%差距。

Conclusion: 归一化层与优化器不应被视为独立设计选择，存在显著的交互效应。Derf与Muon组合的失效模式（饱和和尺度盲区）可通过EMA混合或调整alpha参数缓解。这种失效在短试运行中不易察觉，强调需要系统评估归一化-优化器组合。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在线性OR问题中表现良好，但在真实世界非线性场景中因数学公式与求解器代码间的语义错位以及信息提取不稳定而性能严重下降

Method: 提出NED-Tree框架：(a)采用逐句提取策略确保参数映射的鲁棒性和可追溯性；(b)使用递归树结构自适应地将复杂非线性项分解为求解器兼容的子元素

Result: 在10个基准测试中平均准确率达到72.51%，建立了新的最先进水平；同时创建了NEXTOR基准测试集专门用于复杂非线性、多约束的OR问题

Conclusion: NED-Tree是首个通过元素分解驱动LLM解决非线性建模困难的框架，实现了建模语义与代码语义的对齐，为OR问题自动化翻译提供了系统解决方案
...
TL;DR: ThinkTwice是一个两阶段框架，通过GRPO联合优化LLMs解决推理问题和自我精炼答案，无需正确性信号或批评标注，在数学推理基准上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习与验证推理（RLVR）方法通常需要额外的正确性信号或批评标注来训练自我精炼能力，这限制了方法的可扩展性和实用性。研究者希望开发一种无需额外监督信号就能联合优化推理和精炼能力的方法。

Method: ThinkTwice采用简单的两阶段训练框架：1）优化模型解决推理问题；2）优化模型精炼自己的解决方案。两个阶段使用相同的二元正确性奖励，无需额外正确性信号或批评标注。基于Group Relative Policy Optimization（GRPO）实现，在每对训练步骤中交替进行推理和精炼优化。

Result: 在五个数学推理基准和两个模型家族（Qwen3-4B和Olmo3-7B）上，ThinkTwice显著优于竞争性的在线策略优化基线。在Qwen3-4B上，ThinkTwice在AIME基准上比GRPO在精炼前提升5个百分点，一次自我精炼后提升11.5个百分点（pass@4）。训练动态分析显示隐含的"纠正-强化"课程：早期精炼主要纠正错误，随着模型改进自然转向保持已正确的解决方案。

Conclusion: ThinkTwice证明了推理和自我精炼的联合训练是RLVR的一种原则性且有效的方法论，无需额外监督信号就能显著提升性能，并揭示了训练过程中的隐含课程学习机制。
...
Method: 使用先前研究人类教学策略的受控任务，让LLMs作为模拟教师，在奖励标注的有向图中为学习者揭示单一边缘。采用与人类研究相同的认知模型（贝叶斯最优教师、弱贝叶斯变体、启发式基线、非心智化效用模型）拟合LLMs的逐试次选择，并进行模型比较（BIC）

Result: 在基线实验中，大多数LLMs表现良好，策略变化小，图间表现与人类相似。模型比较显示贝叶斯最优教学最能解释多数模型的决策。脚手架干预使模型遵循推理或奖励导向的提示，但这些干预不能可靠提升在启发式不一致测试图上的教学表现，有时反而降低性能

Conclusion: 认知模型拟合揭示了LLMs的教学策略本质上是贝叶斯最优推理，提示合规性不能保证更好的教学决策，LLMs的教学决策机制与人类相似但受干预影响有限
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs主要依赖监督微调学习代码模式，但缺乏代码执行环境暴露；同时即使最先进的MLLMs也难以有效进行自校正。需要一种能够通过环境交互和迭代自校正来生成更准确可执行代码的方法。
...
Method: 提出CRaFT框架，通过评估特征在拒绝边界附近提示上对模型拒绝-合规决策的影响来排序特征，使用电路引导的方法识别因果特征而非仅依赖激活强度。
...
<details>
  <summary>Details</summary>
Motivation: 多智能体系统(MAS)在处理复杂任务时存在协调开销大、上下文碎片化和脆弱的阶段排序问题。将MAS蒸馏为单智能体技能可以绕过这些成本，但缺乏关于何时、蒸馏什么的原则性指导，导致实证结果不一致（技能提升从28%改进到2%退化）。

Method: 1. 引入Metric Freedom (F)作为技能效用的先验预测指标，通过Mantel测试量化输出多样性与得分方差之间的耦合关系，衡量度量评分景观的拓扑刚性。2. 提出两阶段自适应蒸馏框架：阶段1作为选择性提取机制，在"自由"度量上提取工具和知识同时丢弃限制性结构以保持探索；阶段2专门针对"刚性"度量(F≲0.6)进行计算密集的迭代优化，消除轨迹局部过拟合。

Result: 在4个任务、11个数据集和6个度量上的评估显示，F强烈预测技能效用(ρ=-0.62，p<0.05)。相同的智能体轨迹在刚性与自由度量下产生截然相反的技能提升，证明技能效用本质上是度量级属性。自适应智能体匹配或超越原始MAS，同时将成本降低高达8倍，延迟降低高达15倍。

Conclusion: 技能效用不是由任务决定，而是由评估度量决定。Metric Freedom (F)作为第一个先验预测指标，能够准确预测技能效用。基于F信号的自适应蒸馏框架能够在显著降低计算成本和延迟的同时，保持或提升系统性能。
...
TL;DR: GraphWalk：无需训练的图导航框架，让LLM通过工具调用在大型知识图谱上进行多跳推理，解决上下文窗口限制问题


<details>
  <summary>Details</summary>
Motivation: 企业级知识图谱规模庞大，无法放入现有最大上下文窗口。传统基于提示或检索增强生成的方法在处理复杂多跳查询时面临严重限制，需要新的解决方案。

Method: 提出GraphWalk框架，为LLM提供一组最小正交图操作工具（如节点遍历、关系查询），让模型通过工具调用序列自主导航图结构，无需训练或领域特定知识。

Result: 在迷宫遍历和合成企业知识图谱基准测试中，GraphWalk相比上下文基线在所有模型家族上都取得显著提升。随着规模增大，优势更加明显，而上下文方法则完全失效。

Conclusion: 工具化图遍历方法能有效解决大规模知识图谱推理问题，提供透明执行轨迹，且不依赖语义知识，纯结构推理能力随模型规模提升而增强。
...
Method: 使用AWS Bedrock平台上的四种指令调优模型（Claude 3.7 Sonnet、Claude 4 Sonnet、Mixtral 8x7B Instruct、Mistral 7B Instruct），在收据项目分类任务上系统评估其准确性、响应稳定性和令牌级成本，比较零样本和少样本提示方法的效果

Result: 实验结果表明，Claude 3.7 Sonnet在分类准确性和成本效率之间达到了最有利的平衡，为收据项目分类任务提供了最优的性能-成本比

Conclusion: Claude 3.7 Sonnet是收据项目分类任务中最具成本效益的模型选择，该研究为生产环境中LLM的选型提供了基于准确性和成本的系统评估框架
...
Conclusion: 扩散语言模型具有识别事实不确定性的内在能力，这在自回归模型的顺序token承诺结构中不存在；OSCAR展示了利用DLMs轨迹进行推理时幻觉缓解的有效性
...
TL;DR: EnviSmart是一个用于环境FAIR数据管理的生产系统，通过三轨知识架构和角色分离的多智能体设计解决LLM驱动工作流的可靠性问题，在校园存储基础设施上部署，验证了效率和可靠性改进。


<details>
  <summary>Details</summary>
Motivation: 将LLM驱动的智能体嵌入环境FAIR数据管理具有吸引力，因为它们可以外部化操作知识并在异构数据和演化规范中扩展数据管理。但用概率性工作流替换确定性组件改变了故障模式：LLM管道可能生成看似合理但错误的输出，这些输出通过表面检查并传播到不可逆操作（如DOI注册和公开发布）中。
...
Result: 比较了两个生产部署：大学GIS中心生态档案（849个数据集）作为单智能体基线；SF2Bench复合洪水基准（2,452个监测站和8,557个文件，跨越39年）验证了多智能体工作流。多智能体方法提高了效率（单个操作员在两天内完成，跨部署重复使用工件）和可靠性：审计交接检测并阻止了影响所有2,452个站的坐标转换错误。代表性事件（ISS-004）展示了边界遏制，检测延迟10分钟，零用户暴露，80分钟解决。

Conclusion: EnviSmart通过架构机制解决了LLM驱动数据管理中的可靠性挑战，将可靠性作为系统属性而非事后检查。多智能体设计在信任边界恢复故障停止语义，有效防止错误传播到不可逆操作，同时提高操作效率。该系统已部署在生产环境中，验证了其有效性。
...
TL;DR: CORAL是一个基于大语言模型的多智能体自主进化框架，用于开放式发现问题，通过持久记忆、异步执行和心跳干预实现知识积累与协作，在多种任务上取得显著性能提升。
...
Method: CORAL框架采用长期运行的智能体，通过共享持久记忆、异步多智能体执行和基于心跳的干预实现探索、反思和协作。提供隔离工作空间、评估器分离、资源管理、智能体会话和健康管理等实用保障机制。

Result: 在多样化的数学、算法和系统优化任务上，CORAL在10个任务上创造了新的最先进结果，相比固定进化搜索基线，改进率提高了3-10倍且评估次数更少。在Anthropic的内核工程任务中，四个协同进化智能体将最佳得分从1363周期提升到1103周期。

Conclusion: 更大的智能体自主性和多智能体进化能够显著改善开放式发现。CORAL展示了通过知识重用和多智能体探索与通信实现持续进化的潜力。
...
Conclusion: 为构建抗本体扭曲的临床AI管道提供了理论基础和设计模式，但缺乏实证验证，需在真实系统中进一步评估
...
TL;DR: BACM-RL：一种基于强化学习的预算感知上下文管理框架，通过将上下文管理建模为带预算约束的序列决策问题，学习在有限上下文预算下优化信息保留与压缩的策略。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在长时程推理中面临上下文大小限制（内存、延迟、成本等），随着交互历史增长，需要在保留历史信息和遵守上下文预算之间进行权衡。现有方法缺乏对上下文预算的系统性管理策略。

Method: 提出预算感知上下文管理（BACM），将上下文管理建模为带预算约束的序列决策问题。开发BACM-RL，采用端到端课程式强化学习方法，学习在不同上下文预算下的压缩策略。智能体在纳入新观察前评估可用预算，决定何时以及压缩多少交互历史。

Result: 在组合多目标QA和长时程网页浏览基准测试中，BACM-RL在不同模型规模和任务复杂度下均优于现有方法，在高复杂度设置下比强基线提升超过1.6倍。在预算缩减时仍保持优势，而大多数方法性能下降。

Conclusion: BACM-RL通过将上下文管理形式化为预算约束的序列决策问题，提供了一种系统性的解决方案，能够有效管理有限上下文资源，在长时程推理任务中实现更优的性能和鲁棒性。
...
Method: 提出分层记忆编排（HMO）框架，将交互历史组织成三层目录结构：紧凑的主缓存（包含近期和关键记忆与动态用户画像）、高优先级次级层、全局完整历史档案。用户画像驱动记忆在层次间的重新分配，将符合长期模式的信息提升到更活跃层级，降低不相关信息的重要性

Result: 在多个基准测试中达到最先进性能。在OpenClaw等生态系统中的实际部署显示，HMO显著提升了智能体的流畅性和个性化程度

Conclusion: HMO框架通过用户中心化的分层记忆编排，在需要时精确呈现历史知识，同时保持精简高效的活动搜索空间，解决了长期记忆管理中的性能瓶颈问题
...
<details>
  <summary>Details</summary>
Motivation: 当前模型融合方法主要针对同源模型（共享相同预训练骨干），但在开放模型生态系统中，有用专家模型通常基于不同架构（如Llama、Qwen、Mistral），直接权重空间融合因架构不匹配、潜在基对齐问题和跨源冲突而变得不适定
...
Result: 在异构转移、多源融合、噪声源鲁棒性和跨族泛化等设置下，HeteroFusion始终优于强大的融合、合并和集成基线方法
...
TL;DR: 研究基于中国视频平台数据，发现AIGC与HGC在创作消费行为上存在差异：AIGC创作者通过高产量获得与HGC相当的总体参与度，但消费者明显偏好HGC；算法分发机制能调节AIGC相关利益冲突。
...
Method: 利用中国领先视频分享平台包含数千万用户的纵向数据集，分析AIGC与HGC在创作和消费行为上的差异，研究算法分发机制对AIGC相关利益冲突的调节作用。

Result: 发现AIGC创作者通过"规模优于偏好"策略（高产量）获得与HGC相当的总体参与度，但消费者明显偏好HGC；算法分发机制能有效调节AIGC相关的竞争利益。
...
TL;DR: LiteInception：面向边缘部署的轻量级可解释故障诊断框架，采用两阶段级联架构，通过多方法融合压缩传感器通道和模型参数，引入知识蒸馏作为精度-召回率调节机制，构建双层可解释性框架。


<details>
  <summary>Details</summary>
Motivation: 通用航空故障诊断和高效维护对飞行安全至关重要，但在资源受限的边缘设备上部署深度学习模型面临计算能力和可解释性的双重挑战。需要设计既轻量又具有良好可解释性的故障诊断框架。

Method: 提出LiteInception框架：1）采用两阶段级联架构（故障检测+细粒度分类）；2）基于互信息、梯度分析和SE注意力权重的多方法融合策略压缩传感器通道；3）设计1+1分支LiteInception架构压缩InceptionTime参数；4）引入知识蒸馏作为精度-召回率调节机制；5）构建集成四种归因方法的双层可解释性框架。

Result: 在NGAFID数据集上：传感器通道从23个压缩到15个；模型参数压缩70%；CPU推理加速超过8倍；F1损失小于3%；故障检测准确率81.92%，召回率83.24%；故障识别准确率77.00%。

Conclusion: LiteInception框架在效率、准确性和可解释性之间取得了良好平衡，验证了其适用于资源受限边缘设备的通用航空故障诊断场景，能够满足不同应用场景（安全关键型和辅助诊断）的需求。
...
Method: 采用专家参与循环方法，结合LLM辅助需求获取与协作本体工程开发AnIML本体。通过多层验证方法：将真实AnIML文件转换为知识图谱的数据驱动转换、通过SPARQL进行能力问题验证，以及基于对抗性负面能力问题的新验证协议，映射到已建立的本体反模式并通过SHACL约束强制执行。

Result: 成功开发了AnIML本体，形式化了AnIML的语义并与Allotrope数据格式对齐。通过多层验证方法证明了本体的有效性和实用性，包括成功转换真实AnIML文件、通过SPARQL验证能力问题，以及使用SHACL约束检测本体反模式。

Conclusion: AnIML本体通过形式化AnIML的语义并与其他标准对齐，为解决实验数据系统间的语义互操作性提供了有效解决方案。提出的多层验证方法，特别是基于对抗性负面能力问题的验证协议，为未来本体工程提供了有价值的验证框架。
...
TL;DR: SAT框架解决二维单规格板材切割问题，通过需求扩展、板材分配变量、不可行方向消除规则，在Cui-Zhao基准测试中优于OR-Tools、CPLEX和Gurobi


<details>
  <summary>Details</summary>
Motivation: 二维单规格板材切割问题(2D-CSSP)是制造业中的核心任务，需要从标准板材上切割矩形零件以满足需求并最小化浪费。该问题比装箱问题更复杂，因为每种零件类型需要多个副本，导致组合爆炸。现有求解器在处理此类问题时面临挑战，需要更高效的解决方案。

Method: 提出基于SAT的框架：1) 将零件类型按需求扩展为多个副本；2) 为每个副本分配板材变量；3) 仅对分配到同一板材的副本激活非重叠约束；4) 引入不可行方向消除规则，当零件只有一种方向能放入板材时固定旋转变量。比较三种最小化板材数量的方法：非增量SAT结合二分搜索、增量SAT跨迭代重用子句、加权部分MaxSAT。
...
Conclusion: SAT框架在解决2D-CSSP问题上表现出色，特别是在处理旋转约束时展现出不同的性能特征。该方法为制造业切割优化问题提供了有效的解决方案，显著优于现有商业求解器。
...
<details>
  <summary>Details</summary>
Motivation: 通用LLM将生成视为单次文本完成，无法满足安全关键工程工作流中固有的顺序、多门约束，导致生成可执行仿真工件时出现级联约束违反，限制了LLM在高超声速热防护系统设计中的应用。
...
Result: AeroTherm-GPT在HyTPS-Bench上达到88.7%端到端成功率（95%置信区间：87.5-89.9），比无CDG消融基线提高12.5个百分点，根因修复效率达4.16（扁平清单修复为1.76），且在科学推理和代码生成任务上无灾难性遗忘。

Conclusion: 约束闭环生成框架通过约束依赖图指导的迭代修复，有效解决了LLM在安全关键工程工作流中的级联约束违反问题，显著提高了TPS设计工件的生成成功率，为领域专用LLM智能体在复杂工程系统中的应用提供了可行方案。
...
TL;DR: 提出领域情境化概念图（DCG）框架，将领域信息作为知识表示的内在组成部分而非外部注释，通过模态世界约束解决概念在不同领域中意义漂移的问题。
...
Method: 引入领域情境化概念图（DCG）框架，将领域作为关系的一部分写入表示形式(C, R at D, C')，其中at D标识关系成立的世界。形式上，关系通过领域索引的必然性算子解释，使真值、推理和冲突检查都限定在相关世界中。开发了Kripke风格语义学、紧凑谓词系统、Prolog实现，并映射到RDF、OWL和关系数据库。

Result: DCG框架实现了三个主要效果：1) 模糊概念可以在表示点进行消歧；2) 无效断言可以根据其领域进行挑战；3) 跨领域关系可以通过显式谓词连接。为领域本身提供了结构化和可计算的角色。

Conclusion: 领域应被视为知识表示的一部分而非补充注释。DCG通过将领域作为表示内部的结构化和可计算元素，解决了知识系统中许多实际失败的根本原因，即领域被处理为断言外部信息的问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于可验证奖励的强化学习（RLVR）框架在多模态推理中存在方法学缺陷：通过在所有生成token上分配相同优势，稀释了视觉关键步骤的学习信号，阻碍了感知基础推理的优化
...
Conclusion: PGPO通过细粒度信用分配解决了多模态强化学习中的关键信号稀释问题，实现了更稳健、感知基础的多模态推理，为视觉语言模型的优化提供了新范式
...
TL;DR: 论文提出了AWARE框架，通过监督嵌入学习和轻量级适配器改进临床预测中的表格上下文学习，在极端不平衡数据下提升AUPRC达12.2%
...
Method: 提出了AWARE框架，包含监督嵌入学习和轻量级适配器，通过任务对齐的检索机制改进表格上下文学习。在多队列EHR基准上比较了经典方法、深度表格模型和TICL模型，评估了数据规模、特征维度、结果稀有性和跨队列泛化能力。

Result: PFN-based TICL模型在低数据场景下样本效率高，但在异质性和不平衡增加时，基于朴素距离的检索性能下降。AWARE框架在极端不平衡情况下将AUPRC提升高达12.2%，且随着数据复杂性增加，改进效果更明显。
...
<details>
  <summary>Details</summary>
Motivation: 传统SSP算法使用启发式函数指导状态选择，但Bellman备份仍需评估所有可用动作，即使启发式表明某些动作代价过高，导致计算资源浪费。

Method: 将启发式搜索重新表述为线性规划问题，引入针对SSP的高效约束生成实现，开发CG-iLAO*算法，在iLAO*基础上集成该技术以排除昂贵动作。

Result: CG-iLAO*仅考虑iLAO* 40%的动作（某些问题低至1%），平均减少3.5倍动作代价计算，比iLAO*和LRTDP分别快2.8倍和3.7倍。

Conclusion: 通过约束生成技术利用启发式信息排除昂贵动作，可显著提升SSP求解效率，为启发式搜索算法提供了新的优化方向。
...
<details>
  <summary>Details</summary>
Motivation: 传统监督学习通常使用硬标签或概率标签，但现实世界中存在不确定性信息，如可能性分布。需要一种能够处理可能性监督的方法，既能利用可能性分布中的分级信息，又能保持其定性结构约束。

Method: 1. 从归一化可能性分布构造非空闭凸可容许概率分布集，结合概率兼容性和线性形状约束；2. 使用Dykstra算法计算模型输出的严格正概率向量到可容许集的KL投影；3. 通过最小化预测与其投影之间的散度来训练模型，量化满足约束所需的最小调整。

Result: 在合成数据和基于ChaosNLI数据集的真实自然语言推理任务上的实验表明：1. 提出的投影算法足够高效，可用于实际应用；2. 基于投影的学习目标能够提升预测性能。

Conclusion: 该方法成功地将可能性监督融入多类分类学习，通过构造可容许概率集和KL投影机制，在保持可能性分布定性结构的同时有效利用了分级不确定性信息，为处理不确定性监督提供了新框架。
...
<details>
  <summary>Details</summary>
Motivation: 需要一种能够在特定上下文中进行推理的逻辑框架，能够表达如"每个人都知道Alice说的一切"这样的量化陈述，同时支持上下文中的矛盾（即允许超协调逻辑），并与现有一阶逻辑定理证明器兼容。

Method: 基于一阶逻辑构建逻辑框架，支持对公式和上下文进行量化，允许在上下文中使用超协调逻辑，设计为可有限公理化，确保与现有一阶逻辑定理证明器的兼容性。

Result: 展示了Qiana如何用于表示时间性、事件演算和模态逻辑，讨论了不同的设计替代方案，证明了该框架的表达能力和实用性。

Conclusion: Qiana提供了一个强大而灵活的逻辑框架，能够在特定上下文中进行推理，支持对公式和上下文的量化，允许上下文中的矛盾，并与现有一阶逻辑工具兼容，具有广泛的应用潜力。
...
TL;DR: 本研究提出了NeuroPose-AHM数据集，这是一个基于知识库的神经性异常头部运动数据集，包含57种神经疾病的2,756条患者组记录，通过多LLM框架从1,430篇文献中提取构建，并展示了在颈椎肌张力障碍研究中的应用价值。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏整合运动学测量、临床严重程度评分和患者人口统计学的多条件资源，这阻碍了AI驱动诊断工具的发展。为填补这一空白，需要构建一个结构化的神经性异常头部运动数据集。

Method: 采用多LLM提取框架从1,430篇同行评审文献中构建NeuroPose-AHM数据集，包含2,756条患者组记录，涵盖57种神经疾病。通过四任务框架应用于颈椎肌张力障碍：1) 多标签AHM类型分类；2) 构建头颈严重程度指数(HNSI)；3) 用真实患者数据验证HNSI；4) 运动类型概率与HNSI分数的桥接分析。

Result: 多LLM提取框架表现出强健的提取性能（kappa = 0.822）。颈椎肌张力障碍研究中，AHM类型分类F1分数达0.856；构建的HNSI指标与真实患者数据严重带比例一致（6.7%）；运动类型概率与HNSI分数存在显著相关性（p < 0.001）。

Conclusion: NeuroPose-AHM是一个结构化、基于知识的神经性异常头部运动研究资源，具有分析实用性。数据集已在Zenodo公开可用，为神经疾病诊断工具开发提供了重要基础。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型即使存在高效数值捷径也常默认逐步计算，这引发了一个基本问题：它们是否表现出类似人类的数字感知能力，即识别数字结构、在适当时应用捷径、在不适用时避免使用捷径的能力。

Method: 引入SenseMath基准测试，包含4,800个项目，涵盖八个捷径类别和四个数字规模，配有匹配的强捷径、弱捷径和控制变体。支持三个评估设置：捷径使用（模型能否在适用捷径的问题上应用捷径）、适用性判断（能否识别捷径何时适用或误导）、问题生成（能否生成正确允许给定类型捷径的新问题项）。评估了从GPT-4o-mini到Llama-3.1-8B的五个LLM。

Result: 当明确提示时，模型能采用捷径策略并在适用捷径的项目上获得显著准确率提升（高达15%），但在标准思维链提示下，它们自发使用此类策略的情况不到40%。这种能力仅限于使用层面；模型系统性地将捷径过度泛化到不适用的问题，并且无法从头生成有效的包含捷径的问题。

Conclusion: 当前LLMs表现出程序性捷径流畅性，但缺乏支撑人类数字感知的结构性理解，即理解捷径何时以及为何有效。它们能应用捷径但缺乏真正的数字感知。
...
TL;DR: 基于Transformer掩码自编码器的无标签步态异常检测与运动学校正框架，仅使用正常步态数据训练，通过两阶段推理实现异常关节定位与轨迹校正


<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的步态分析大多依赖有监督分类器，需要疾病标签数据，难以泛化到异质性病理表现。需要开发无需疾病标签的异常检测方法，提高对多样化病理步态的适应性。

Method: 提出基于Transformer掩码自编码器的无标签框架：1) 仅使用150名成年人的正常步态序列训练；2) 推理时采用两阶段流程：先通过遮挡单个关节计算不一致性分数，再屏蔽异常关节从剩余时空上下文重建完整骨架；3) 使用无标记多摄像头运动捕捉系统采集数据。

Result: 在10名模拟7种步态异常的受试者上验证：1) 准确定位生物力学不一致关节；2) 所有分析关节的角度偏差显著减少，效应量大；3) 保持正常运动学特征；4) 实现无需疾病标签的可解释、个体化步态损伤定位。

Conclusion: 该框架为步态异常检测提供了无需疾病标签的解决方案，能够准确定位异常关节并校正运动学轨迹，具有临床应用的潜力，特别是在异质性病理表现的步态分析中。
...
TL;DR: 提出ProCeedRL方法，通过过程级批评家和探索性演示解决多轮智能体任务中的累积错误反馈循环问题


<details>
  <summary>Details</summary>
Motivation: 强化学习虽然能增强大语言模型的推理能力，但在多轮智能体任务中面临挑战：长时程交互和环境反馈的随机性导致次优动作产生误导性观察，形成累积错误反馈循环，使标准探索策略失效

Method: 提出ProCeedRL方法：1）采用过程级批评家实时监控交互；2）引入基于反思的演示指导智能体停止错误累积；3）将探索从被动选择转变为主动干预；4）结合探索性演示和在线策略样本学习

Result: ProCeedRL显著超越了模型的饱和探索性能，展示了实质性的探索优势，提高了探索效率，在复杂深度搜索和具身任务上实现了优越性能

Conclusion: 通过主动干预和过程级监控，ProCeedRL有效解决了多轮智能体任务中的累积错误反馈循环问题，为强化学习在复杂智能体任务中的应用提供了新思路
...
TL;DR: ATBench是一个轨迹级基准测试，用于评估LLM智能体的安全性，通过三维风险分类、异构工具池和长上下文延迟触发协议来捕捉多阶段风险涌现。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹级基准测试存在交互多样性不足、安全故障可观测性粗糙和长视野真实性弱的问题，而现实部署中的风险往往在多步交互中涌现，而非孤立提示或最终响应。

Method: 基于三维风险分类（风险来源、故障模式、现实危害），构建包含异构工具池和长上下文延迟触发协议的轨迹，包含1000条轨迹（503安全/497不安全），平均9.01轮交互和3.95k tokens，使用基于规则和LLM的过滤加人工审核确保数据质量。

Result: ATBench对前沿LLM、开源模型和专用防护系统都具有挑战性，同时支持分类分层分析、跨基准比较和长视野故障模式诊断。

Conclusion: ATBench提供了一个结构化、多样化和真实性的轨迹级基准测试，能够更好地评估LLM智能体在多步交互中的安全性风险。
...
TL;DR: 该论文是一篇关于语言模型中潜在空间的系统性综述，旨在为这一新兴领域提供统一框架，涵盖基础概念、发展历程、技术机制、能力表现及未来展望五个维度。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型的发展，潜在空间逐渐成为语言处理的核心计算媒介。传统基于显式token生成的理解方式存在局限性，而越来越多的研究表明，许多关键内部过程在连续潜在空间中更为自然。本文旨在为这一快速发展的领域提供系统性的综述和统一框架。

Method: 采用五维框架组织调查：1) 基础：界定潜在空间范围，区分显式/语言空间和视觉生成模型的潜在空间；2) 演进：追溯从早期探索到当前大规模扩展的发展历程；3) 机制：从架构、表示、计算、优化四个技术维度分析现有工作；4) 能力：展示潜在空间如何支持推理、规划、建模、感知、记忆、协作、具身等广泛能力；5) 展望：讨论开放挑战和未来研究方向。

Result: 建立了语言模型潜在空间的系统性分类框架，识别了该领域的关键技术发展脉络和能力谱系，为理解潜在空间作为下一代智能系统的通用计算范式提供了理论基础。

Conclusion: 潜在空间正成为语言模型的本征计算基质，超越了传统显式token处理的局限性。该综述不仅为现有研究提供了参考框架，更重要的是为理解潜在空间作为下一代智能的通用计算和系统范式奠定了基础。未来研究需要解决该领域的关键开放挑战，推动潜在空间计算范式的发展。
...
Conclusion: Diff-KD通过将扩散生成精炼与知识蒸馏相结合，有效解决了多智能体协同感知中的损坏问题，实现了对底层清洁语义的主动恢复，显著提升了系统的鲁棒性。
...
Method: 研究将大语言模型同时作为时间序列解释的生成器和评估器，在无参考设置下工作。给定时间序列、问题和候选解释，评估器基于模式识别、数值准确性和答案忠实性分配三元正确性标签，从而实现原则性评分和比较。为此构建了包含350个时间序列案例的合成基准，涵盖七种查询类型，每种都配有正确、部分正确和错误的解释。评估模型在四个任务上的表现：解释生成、相对排序、独立评分和多异常检测。

Result: 结果显示明显的不对称性：生成任务高度依赖模式，在某些查询类型上表现出系统性失败，准确率范围从季节性下降和波动率变化的0.00-0.12，到结构突变的0.94-0.96；而评估任务更加稳定，即使模型自身输出不正确，也能正确排序和评分解释。这表明LLM能够可靠地评估基于时间序列数据的推理。

Conclusion: 研究证明了基于数据的大语言模型评估时间序列解释的可行性，并突显了它们作为时间序列领域中数据驱动推理可靠评估器的潜力。评估任务比生成任务更加稳定，为无参考设置下的时间序列解释评估提供了实用方法。
...
<details>
  <summary>Details</summary>
Motivation: LLM驱动的社交机器人通过生成类人内容规避传统检测，现有方法因过度依赖单模态信号、对AIGC生成模式敏感性不足、未能充分建模语言模式与行为动态的相互作用，导致检测准确率有限。

Method: 提出TRACE-Bot统一双通道框架：1）从个人信息、交互行为和推文数据构建细粒度表示；2）双通道架构通过预训练语言模型捕获语言表示，通过多维活动特征（增强SOTA AIGC检测器信号）捕获行为异常；3）融合表示通过轻量级预测头进行分类。
...
TL;DR: 提出了一种用于验证AI系统在安全关键领域（如航空）中操作设计域（ODD）覆盖完整性的结构化工程方法，以满足EASA认证标准。
...
Method: 提出了一种结构化多步骤ODD覆盖验证流程，整合了参数离散化、基于约束的过滤和基于关键性的维度约简。该方法基于先前AI空中防撞研究的仿真数据，通过系统化工程方法定义和实现覆盖度量。

Result: 该方法能够在更高维度上验证ODD覆盖，推进"安全设计"方法，同时符合EASA标准要求。

Conclusion: 该工作填补了抽象ODD定义与可验证证据之间的空白，为AI系统在安全关键领域的认证提供了系统化的工程方法，满足了监管机构对完整性的要求。
...
TL;DR: 评估LLM生成胸CT报告日文翻译的教育适用性，比较放射科医生评估与LLM作为评判者的表现，发现两者一致性极低，LLM评判者强烈偏好LLM输出，专家评审仍不可或缺。


<details>
  <summary>Details</summary>
Motivation: 放射学报告的多语言翻译对研究、临床沟通和教育至关重要，但LLM评估的有效性尚不明确，需要验证LLM生成翻译的教育适用性以及LLM作为评判者的可靠性。

Method: 使用CT-RATE-JPN验证集的150份胸CT报告，比较人工编辑日文翻译与DeepSeek-V3.2生成的翻译。由放射科认证医生和住院医师进行盲法配对评估（术语准确性、可读性、整体质量、放射科风格真实性），同时使用3个LLM评判者（DeepSeek-V3.2、Mistral Large 3、GPT-5）进行平行评估，通过QWK和百分比一致性分析一致性。

Result: 放射科医生与LLM评判者一致性接近零（QWK=-0.04至0.15），两位放射科医生间一致性也很差（QWK=0.01至0.06）。放射科医生1认为59%病例术语等效，在可读性（51%）和整体质量（51%）上偏好LLM翻译；放射科医生2认为75%病例可读性等效，在整体质量上偏好人工编辑翻译（40% vs 21%）。所有3个LLM评判者强烈偏好LLM翻译（70%-99%），并在>93%病例中认为其更具放射科风格。

Conclusion: LLM生成翻译常被判断为自然流畅，但放射科医生评估存在显著差异。LLM作为评判者表现出对LLM输出的强烈偏好，与放射科医生一致性可忽略。对于翻译放射学报告的教育用途，仅依赖自动化LLM评估不足，专家放射科医生评审仍然重要。
...
Result: 开发了模型无关的token重要性可视化技术，生成相关性图展示token级贡献，提供开源实现（Infosys Responsible AI Toolkit）
...
Result: 在FrozenLake环境中，域内性能无提升，但在迁移任务中实现稳健导航，获得0.95的奖励分数

Conclusion: 有效的神经符号集成需要精心编排而非简单组合，强调模型规模充足和有效混合机制对成功OOD泛化的重要性
...
Method: 提出查询错位框架和Trace Inversion方法：首先生成模型的推理轨迹，仅基于轨迹重构模型最可能回应的查询，然后比较原始查询与重构查询的相似度，低相似度表明模型可能错误回答问题并标记为不回答

Result: 在四个前沿LLM和九个不回答QA数据集上的广泛实验表明，Trace Inversion有效提升了不回答性能，在36个设置中33个击败了竞争基线

Conclusion: Trace Inversion通过查询错位框架提供了一种新的最先进的不回答方法，显著提升了推理模型的自我评估和可靠部署能力
...
Result: 静态情感前缀通常只产生小的准确率变化；情感效应在社会基础任务中更易变；更强的情感措辞仅引起适度额外变化；自适应情感选择比固定情感提示产生更可靠的增益
...
Method: De Jure采用四阶段流水线：1) 将源文档规范化为结构化Markdown；2) LLM驱动的语义分解为结构化规则单元；3) 基于LLM-as-a-judge的多维度评估（19个维度）；4) 在有限再生预算内迭代修复低质量提取，优先修复上游组件再评估规则单元。

Result: 在金融、医疗和AI治理三个监管领域评估显示：金融领域提取质量持续单调提升，三个迭代达到峰值；跨领域泛化能力强，在开源和闭源模型上均保持高性能；下游合规问答中，基于De Jure提取规则的RAG响应在单规则检索深度上优于先前工作73.8%，更广泛检索下达到84.0%。
