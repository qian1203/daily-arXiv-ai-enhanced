...
TL;DR: LOM-action提出事件驱动的本体模拟架构，通过业务事件触发本体条件，在沙箱中生成场景有效的模拟图，所有决策仅基于该图，实现可审计的企业AI决策。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的智能体系统存在架构缺陷：它们从无限制的知识空间回答问题，而未先模拟活跃业务场景如何为当前事件重塑该空间，导致决策流畅但无基础且缺乏审计追踪。

Method: LOM-action采用事件驱动的本体模拟：业务事件触发企业本体中编码的场景条件，驱动确定性图突变在隔离沙箱中，将工作子图副本演化为场景有效的模拟图G_sim。核心流程为"事件→模拟→决策"，通过双模式架构（技能模式和推理模式）实现。

Result: LOM-action达到93.82%准确率和98.74%工具链F1分数，而前沿基线模型Doubao-1.8和DeepSeek-V3.2仅达到24-36% F1分数，尽管有80%准确率，暴露了"虚幻准确率"现象。四倍的F1优势证实本体治理、事件驱动的模拟（而非模型规模）是可信企业决策智能的架构前提。

Conclusion: 事件驱动的本体模拟架构解决了现有LLM智能体系统的根本缺陷，通过将决策限制在场景有效的模拟图中，实现了可审计、可靠的企业决策智能，证明了架构设计比模型规模对可信企业AI更为关键。
...
TL;DR: RAMP是一种在线学习数值规划动作模型的策略，通过强化学习、动作模型学习和规划三者的协同，在环境交互中同时训练DRL策略、学习动作模型并利用模型进行规划。


<details>
  <summary>Details</summary>
Motivation: 传统规划算法需要动作模型，但获取这样的模型很困难。现有的数值域动作模型学习算法都是离线的，需要专家轨迹作为输入，无法在线学习。

Method: 提出RAMP策略：1) 训练深度强化学习策略；2) 从历史交互中学习数值动作模型；3) 在可能时使用学到的模型进行规划。三者形成正向反馈循环：RL策略收集数据优化动作模型，规划器生成计划继续训练RL策略。开发了Numeric PDDLGym框架，将数值规划问题转换为Gym环境。

Result: 在标准IPC数值域上的实验结果表明，RAMP在可解性和规划质量方面显著优于著名的DRL算法PPO。

Conclusion: RAMP通过集成强化学习、动作模型学习和规划，实现了数值规划动作模型的在线学习，为解决动作模型获取困难的问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: Courcelle定理表明MSO2公式的模型检查在树宽参数下是线性时间可解的，但未涉及如何表示这些模型。研究旨在将Courcelle定理扩展到模型表示，连接参数化复杂度与知识表示领域。

Method: 扩展Courcelle定理，证明MSO2公式的模型可以用参数化线性大小的决策图表示：对于树宽使用句子决策图(SDD)，对于路径宽使用有序二元决策图(OBDD)。同时基于Razgon(2014)的下界证明某些MSO2公式在树宽参数下无法用OBDD表示。

Result: 1) MSO2公式的模型可以用参数化线性大小的决策图表示；2) 树宽参数下SDD大小有参数化线性上界；3) 路径宽参数下OBDD大小有参数化线性上界；4) 存在MSO2公式和树宽有界图类，其OBDD大小无法被树宽参数化。

Conclusion: 该研究为Courcelle定理提供了新的视角，将参数化复杂度与知识表示连接起来，证明了MSO2模型可以用参数化线性大小的决策图表示，但不同决策图表示能力存在差异，为知识表示中的模型压缩提供了理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型和推理模型出现，但从自然语言描述生成规划领域仍是一个开放问题。现有研究表明LLMs虽能辅助领域生成，但距离生成可实际部署的高质量领域仍有差距

Method: 采用智能语言模型反馈框架，从添加少量符号信息的自然语言描述生成规划领域。评估不同符号反馈形式（包括地标和VAL计划验证器输出）下的生成质量，通过启发式搜索在模型空间中优化领域质量

Result: 论文评估了各种符号反馈机制对生成领域质量的影响，并通过启发式搜索优化了领域生成效果

Conclusion: 智能语言模型反馈框架结合符号反馈和启发式搜索能够有效提升从自然语言描述生成规划领域的质量
...
TL;DR: 该论文提出强化学习中环境作为外部记忆的数学框架，证明某些观测（称为"人工制品"）可以减少表示历史所需的信息量，并通过实验验证空间路径观测能降低学习高性能策略所需的内存。


<details>
  <summary>Details</summary>
Motivation: 基于情境认知观点，智能行为不仅依赖于内部记忆，还依赖于智能体对环境资源的主动利用。该研究旨在将这一直觉形式化到强化学习框架中，探索环境如何功能性地作为智能体的记忆。

Method: 引入数学框架形式化环境作为记忆的概念，定义"人工制品"（artifacts）作为能减少历史表示所需信息的观测。通过实验验证理论，展示当智能体观测空间路径时，学习高性能策略所需的内存会减少。

Result: 理论证明某些观测可以减少历史表示的信息需求。实验表明，当智能体观察空间路径时，学习高性能策略所需的内存确实减少。有趣的是，这种效果是无意中通过智能体的感知流隐式产生的。

Conclusion: 研究为环境作为外部记忆提供了形式化基础，满足先前用于支撑外部记忆账户的定性特性。展望未来，这一方向的研究可能揭示利用环境替代显式内部记忆的原则性方法。
...
TL;DR: SPPO是一种序列级PPO算法，通过将推理过程重构为序列级上下文赌博机问题，使用解耦的标量值函数获得低方差优势信号，解决了标准PPO在长链式思维推理任务中的不稳定性和内存成本问题。


<details>
  <summary>Details</summary>
Motivation: 标准token级PPO在可验证奖励的LLM推理对齐任务中存在两个主要问题：1）长链式思维时间跨度上的时间信用分配不稳定；2）值模型的内存成本过高。虽然GRPO等无评论家方法缓解了这些问题，但需要多个样本进行基线估计，导致计算开销巨大，严重限制了训练吞吐量。

Method: SPPO将推理过程重构为序列级上下文赌博机问题，采用解耦的标量值函数来推导低方差优势信号，无需多采样。该方法结合了PPO的样本效率和基于结果更新的稳定性，提供了资源高效的推理LLM对齐框架。

Result: 在数学基准测试上的广泛实验表明，SPPO显著超越了标准PPO，并与计算密集的基于组的方法性能相当，为对齐推理LLM提供了资源高效的解决方案。

Conclusion: SPPO通过序列级重构和解耦标量值函数，成功解决了PPO在长链式思维推理任务中的稳定性和效率问题，为LLM推理对齐提供了可扩展且资源高效的算法框架。
...
TL;DR: StaRPO是一个增强稳定性的强化学习框架，通过自相关函数和路径效率两个轻量级指标来评估推理过程的局部连贯性和全局目标导向性，从而提高大语言模型在复杂推理任务中的准确性和逻辑稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习策略优化框架主要依赖最终答案正确性作为反馈信号，很少捕捉推理过程的内部逻辑结构，导致模型生成流畅且语义相关的回答，但在逻辑上不一致、结构混乱或冗余。
...
Conclusion: StaRPO通过显式地将推理稳定性纳入优化目标，有效解决了现有RL框架在逻辑一致性方面的不足，为大语言模型的复杂推理任务提供了更全面的优化框架。
...
TL;DR: PETITE框架通过让同一LLM实例化为导师和学生两个角色进行结构化交互，在代码生成任务中实现了更高准确率和更低计算成本


<details>
  <summary>Details</summary>
Motivation: 受人类认知发展中导师-学生角色交互的启发，探索是否可以通过结构化多智能体交互让同一LLM超越现有框架的性能，而不依赖更强监督模型或异构集成

Method: 提出PETITE框架：同一LLM实例化为两个不对称角色——学生智能体生成并迭代优化解决方案，导师智能体提供结构化评估反馈（无需真实答案），通过角色分化的结构化交互提升问题解决能力

Result: 在APPS代码基准测试中，相比Self-Consistency、Self-Refine、Multi-Agent Debate和Multi-Agent Review等SOTA方法，PETITE达到相似或更高准确率，同时消耗显著更少的token
...
TL;DR: 该论文提出了SEA-Eval基准测试，用于评估自我进化智能体在跨任务连续进化中的性能，发现当前最先进框架存在进化瓶颈，相同成功率下令牌消耗差异可达31.2倍。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的智能体在单次任务执行中表现良好，但受限于静态工具集和任务间遗忘，无法跨任务积累经验或优化策略。需要建立科学评估框架来推动智能体从任务执行者向真正自我进化的数字实体发展。

Method: 1) 提出基于数字具身和跨任务连续进化的SEA形式化定义；2) 引入SEA-Eval基准测试，从两个维度评估：任务内执行可靠性和长期进化性能；3) 将任务组织成序列流，分析随时间变化的成功率和令牌消耗；4) 量化进化增益和结构稳定性。

Result: 实证评估揭示了当前最先进框架存在显著进化瓶颈：相同成功率下令牌消耗差异可达31.2倍，序列分析显示不同的进化轨迹。SEA-Eval能够捕捉现有单次基准测试无法检测的进化特征。

Conclusion: SEA-Eval为推进智能体从单纯任务执行者向真正自我进化的数字实体发展提供了严谨的科学基础，揭示了当前框架的进化局限性，为未来研究指明了方向。
...
TL;DR: AGD-MBRL提出优势引导的扩散世界模型，通过优势估计指导反向扩散过程，解决短视问题，提升基于模型的强化学习样本效率和最终回报。


<details>
  <summary>Details</summary>
Motivation: 基于模型的强化学习中，自回归世界模型存在误差累积问题，扩散世界模型通过联合生成轨迹段缓解此问题。但现有扩散引导方法要么只使用策略信息（丢弃价值信息），要么基于奖励（在扩散视野较短时变得短视）。需要一种能利用优势估计、关注长期回报的引导方法。

Method: 提出优势引导扩散（AGD-MBRL），使用智能体的优势估计指导反向扩散过程，使采样集中在预期能产生超出生成窗口的更高长期回报的轨迹上。开发两种引导方法：Sigmoid优势引导（SAG）和指数优势引导（EAG）。该方法与PolyGRAD风格架构无缝集成，指导状态组件同时保持动作生成基于策略，且无需改变扩散训练目标。

Result: 在MuJoCo控制任务（HalfCheetah、Hopper、Walker2D和Reacher）上，AGD-MBRL相比PolyGRAD、在线Diffuser风格奖励引导和模型无关基线（PPO/TRPO）提高了样本效率和最终回报，某些情况下提升幅度达2倍。理论证明表明，通过SAG或EAG引导的扩散模型允许执行重加权采样，权重随状态-动作优势增加而增加。

Conclusion: 优势感知引导是解决扩散模型MBRL中短视问题的简单有效方法。AGD-MBRL通过优势估计指导扩散过程，使采样集中在高长期回报轨迹上，实现了策略改进和性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 理解人类如何在认知资源受限的情况下进行序列物理规划，特别是物理预测机制（直觉物理引擎vs启发式）和规划策略（深度前瞻vs短视）如何共同适应资源约束。

Method: 使用"悬挑塔"建造任务，要求参与者在保持稳定性的同时最大化水平悬挑。通过操纵任务复杂性和时间压力，研究物理预测机制和规划策略的变化。

Result: 发现人类在资源压力下表现出双重转变：早期阶段以IPE模拟为主，随着复杂性增加转向CNN视觉启发式；同时时间压力缩短了前瞻规划，转向更浅的规划视野。

Conclusion: 揭示了一个分层的、资源理性的认知架构，能够灵活地在计算成本和预测保真度之间进行权衡。将两个长期争论（模拟vs启发式、短视vs前瞻规划）统一为受认知预算重新配置的动态机制库。
...
Method: 在现有智能体流程基础上引入专门的电影摄影镜头智能体，该智能体集成递归故事板生成以增强镜头间的叙事连续性，并通过电影语言注入产生更具表现力、面向电影的镜头设计。

Result: 广泛的定量和定性结果表明，该方法在叙事一致性、动态表现力和感知电影质量方面持续优于现有基线。

Conclusion: Camera Artist通过引入专门的电影摄影镜头智能体，有效解决了多智能体视频生成系统中的叙事连续性和电影语言表达问题，显著提升了生成视频的电影质量。
...
TL;DR: CACM框架通过精确的集合级诊断和简洁的记忆回写机制，解决基于语言的药物发现中规划与协议级要求不匹配的问题，显著提升成功率。


<details>
  <summary>Details</summary>
Motivation: 当前基于语言的药物发现系统面临根本性控制问题：智能体逐步规划，但任务有效性由整个候选集合的协议级要求（如集合大小、多样性、结合质量、可开发性）决定。现有系统依赖冗长的原始历史和模糊的自我反思，导致故障定位不精确，规划状态噪声增加。

Method: 提出CACM（约束感知纠正记忆）框架，包含协议审计和基于证据的诊断器，共同分析任务要求、口袋上下文和候选集合证据等多模态证据，定位协议违规，生成可操作的修复提示，并偏置下一步行动。通过静态、动态和纠正三个通道组织记忆，并在回写前压缩，保持持久任务信息同时仅暴露最相关的故障。

Result: 实验结果显示，CACM相比最先进的基线方法，将目标级成功率提高了36.4%。

Conclusion: 可靠的基于语言的药物发现不仅需要更强大的分子工具，还需要更精确的诊断和更经济的智能体状态。CACM通过精确的集合级诊断和简洁的记忆管理机制，有效解决了规划与协议要求不匹配的问题。
...
TL;DR: Spatial-Gym是一个用于评估模型空间推理能力的Gymnasium环境，通过2D网格寻路任务测试模型在三种设置下的表现，发现当前模型与人类基线存在巨大差距，且存在多种局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试在一次性设置中评估模型，要求单次响应生成完整解决方案，这与人类在交互环境中逐步工作的方式不同。需要开发一个能够隔离空间约束推理并支持逐步交互评估的环境。

Method: 引入Spatial-Gym环境，将2D网格谜题寻路作为顺序决策任务进行测试，支持可选回溯。在500个episode上评估8个模型在三种设置下（一次性、逐步、带回溯逐步）的表现，并与人类、随机和A*基线进行比较。

Result: 最佳模型GPT-OSS 120B解决率为16.0%，比人类基线（98.0%）低82个百分点。逐步格式有助于较弱模型（提升达5.4%），但限制了较强模型的全局规划（降低达5.6%）。回溯提高了episode完成率，但仅对较弱模型提升解决率。关键发现：模型未能根据难度扩展推理努力；接收空间环境图像的视觉模型解决率降低73%；扩展思维链推理在逐步设置中仍保持3-5倍准确率优势。

Conclusion: Spatial-Gym能够诊断模型局限性，并为通过强化学习改进空间推理提供了框架。当前模型在空间推理任务上仍远落后于人类表现，需要进一步研究改进模型的逐步推理和规划能力。
...
<details>
  <summary>Details</summary>
Motivation: 多目标优化中，随着目标数量增加，需要大量解来近似整个帕累托前沿，这在贝叶斯优化中尤为困难，因为评估预算有限。实际应用中决策者最终只选择一个解部署，因此应专注于寻找单个最高质量解

Method: 提出SPMO框架，采用ESPI采集函数，通过样本平均近似方法进行梯度优化，适用于无噪声和噪声场景，理论证明其收敛性
...
TL;DR: E3-TIR是一种用于工具集成推理的增强经验利用训练范式，通过动态整合专家前缀、专家引导和自主探索三种经验类型，有效解决现有训练方法的局限性，在减少数据需求的同时显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有工具集成推理的训练范式存在显著局限性：Zero-RL方法因缺乏先验指导而面临探索效率低下和模式退化问题；SFT-then-RL方法则受限于高数据成本和由低熵崩溃导致的能力瓶颈。需要一种能平衡探索多样性与训练效率的新训练范式。

Method: 提出E3-TIR训练范式，将训练过程动态整合为三种经验类型：专家前缀、专家引导和自主探索。通过围绕专家"锚点"执行多样化的分支探索，并采用混合策略优化机制，有效缓解分布偏移并解决共享前缀带来的优化冲突。该方法能动态调整模型的知识边界。

Result: 在工具使用任务上，E3-TIR相比传统范式实现了6%的性能提升，同时仅需不到10%的合成数据。在综合性能、数据成本和训练效率的ROI指标上，相比基线获得了1.46倍的增益。

Conclusion: E3-TIR通过动态整合多种经验类型，有效解决了现有训练范式的局限性，在减少数据需求的同时显著提升了工具集成推理的性能和训练效率，为智能体训练提供了新的有效范式。
