...
Motivation: 自动化规划算法需要动作模型来指定每个动作的前置条件和效果，但获取这样的模型通常很困难。现有数值领域的动作模型学习算法是离线的，需要专家轨迹作为输入，这限制了其实际应用。

Method: 提出RAMP策略，通过与环境在线交互学习数值规划动作模型。该方法同时训练深度强化学习策略、从过去交互中学习数值动作模型，并在可能时使用该模型规划未来动作。这些组件形成正反馈循环：RL策略收集数据来优化动作模型，而规划器生成计划来继续训练RL策略。开发了Numeric PDDLGym框架，用于将数值规划问题转换为Gym环境。

Result: 在标准IPC数值领域上的实验结果表明，RAMP在可解性和规划质量方面显著优于PPO（著名的深度强化学习算法）。

Conclusion: RAMP策略成功实现了数值规划动作模型的在线学习，通过整合强化学习和规划形成了有效的正反馈循环，为数值规划领域的动作模型学习提供了新的在线解决方案。
...
TL;DR: 论文扩展了Courcelle定理，证明了具有自由变量的MSO2公式模型可以用参数化线性大小的决策图表示，并建立了树宽与SDD、路宽与OBDD的对应关系，同时给出了OBDD大小的下界。


<details>
  <summary>Details</summary>
Motivation: Courcelle定理在参数化复杂性中很重要，但只处理MSO2公式的可满足性检查。本研究旨在扩展该定理，探索MSO2公式模型（含自由变量）能否用参数化线性大小的决策图表示，从而连接Courcelle定理与知识表示领域。

Method: 1. 证明MSO2公式模型可以用决策图表示，其大小在树宽和公式大小参数下是线性的；2. 具体建立树宽与sentential decision diagram (SDD)的对应关系；3. 建立路宽与ordered binary decision diagram (OBDD)的对应关系；4. 基于Razgon (2014)的下界结果，证明存在有界树宽图类不能获得树宽参数化的OBDD。

Result: 1. 证明了MSO2公式模型可以用参数化线性大小的决策图表示；2. 获得了树宽参数下SDD大小的参数化线性上界；3. 获得了路宽参数下OBDD大小的参数化线性上界；4. 证明了存在有界树宽图类不能获得树宽参数化的OBDD，表明路宽是OBDD表示的必要条件。

Conclusion: 本研究成功扩展了Courcelle定理，将MSO2公式模型与决策图表示联系起来，建立了树宽与SDD、路宽与OBDD的对应关系，为Courcelle定理提供了新的视角，并连接了参数化复杂性与知识表示两个领域。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型和推理模型出现，但从自然语言描述生成规划领域仍是一个开放问题。现有研究表明LLM虽能辅助领域生成，但距离产生可实际部署的高质量领域仍有差距。因此需要探索更有效的方法来提升生成领域质量。

Method: 采用智能语言模型反馈框架，从添加少量符号信息的自然语言描述生成规划领域。评估不同形式的符号反馈（包括地标和VAL规划验证器输出）对生成质量的影响。使用启发式搜索在模型空间中进行优化以提升领域质量。

Result: 论文评估了各种符号反馈机制对生成规划领域质量的影响，并通过启发式搜索优化方法改进了领域生成效果。具体结果未在摘要中详细说明，但表明该方法在提升领域质量方面具有潜力。
...
TL;DR: 该论文将情境认知理论形式化到强化学习中，证明环境可以作为智能体的记忆，某些观测（称为"人工制品"）可以减少表示历史所需的信息量。


<details>
  <summary>Details</summary>
Motivation: 情境认知理论认为智能行为不仅依赖于内部记忆，还依赖于智能体对环境资源的主动利用。该研究旨在将这一直觉形式化到强化学习框架中，探索环境如何功能性地作为智能体的记忆。

Method: 引入数学框架来形式化环境作为记忆的概念，定义"人工制品"（artifacts）作为能够减少历史信息表示的观测。通过实验验证理论，展示当智能体观测空间路径时，学习高性能策略所需的内存会减少。

Result: 实验证实了理论预测：当智能体观察空间路径时，学习高性能策略所需的内存确实减少。有趣的是，这种效果是无意中产生的，通过智能体的感知流隐式实现。研究还表明这些发现满足先前用于解释外部记忆的定性特性。

Conclusion: 环境可以作为智能体记忆的有效替代，某些环境观测（人工制品）能够减少内部记忆需求。这一发现为利用环境作为显式内部记忆的替代提供了原则性方法，未来研究可进一步探索如何系统性地利用环境资源。
...
<details>
  <summary>Details</summary>
Motivation: 标准token级PPO在具有可验证奖励的推理任务中对齐大型语言模型时存在两个主要问题：1）在长思维链上时间信用分配不稳定；2）价值模型内存成本过高。而像GRPO这样的无批评者替代方案虽然缓解了这些问题，但需要多重采样进行基线估计，导致显著的计算开销，严重限制了训练吞吐量。

Method: SPPO将推理过程重新定义为序列级上下文赌博机问题，采用解耦的标量价值函数来推导低方差优势信号，无需多重采样。这种方法结合了PPO的样本效率和基于结果更新的稳定性。

Result: 在数学基准测试上的广泛实验表明，SPPO显著超越了标准PPO，并与计算密集型的基于组的方法性能相当，为对齐推理LLMs提供了一个资源高效的框架。

Conclusion: SPPO通过序列级优化方法，有效解决了标准PPO在长思维链推理任务中的不稳定性和内存成本问题，同时避免了无批评者方法的多重采样计算开销，实现了样本效率与稳定性的平衡。
...
TL;DR: StaRPO：一种稳定性增强的强化学习框架，通过引入推理稳定性指标（ACF和PE）来提升大语言模型在复杂推理任务中的逻辑一致性和准确性。
...
Method: 提出StaRPO框架，将推理稳定性分解为两个可计算的轻量级指标：评估局部步骤间连贯性的自相关函数（ACF）和评估全局目标导向性的路径效率（PE）。这些稳定性奖励与任务奖励结合，提供互补且过程感知的反馈。

Result: 在两个骨干模型上验证了ACF和PE奖励与逻辑错误的相关性。在四个推理基准测试中，StaRPO始终优于基线方法，能够同时提升最终答案准确性和逻辑稳定性。

Conclusion: StaRPO通过显式地将推理稳定性纳入优化目标，有效解决了现有RL框架在复杂推理任务中忽视逻辑结构的问题，实现了更一致、更高效的推理过程。
...
TL;DR: PETITE框架通过让同一LLM实例化为导师和学生两个角色进行结构化交互，在代码生成任务中实现了比现有方法更高或相当的准确率，同时显著减少了token消耗。


<details>
  <summary>Details</summary>
Motivation: 受人类认知发展中导师-学生角色互动启发的协同效应，探索是否可以通过角色分化的多智能体系统让LLM超越现有框架的局限，而不依赖更强的监督模型或异构集成。

Method: 提出PETITE框架：将同一LLM实例化为两个不对称角色——学生智能体生成并迭代优化解决方案，导师智能体提供结构化评估反馈（无需真实答案）。在自主编码问题领域测试这种结构化交互。

Result: 在APPS编码基准测试中，PETITE相比Self-Consistency、Self-Refine、Multi-Agent Debate和Multi-Agent Review等方法，实现了相似或更高的准确率，同时显著减少了token消耗。

Conclusion: 基于发展心理学的角色分化交互结构为增强LLM问题解决能力提供了一个原则性且资源高效的范式，通过结构化同伴式交互实现协同效应。
...
TL;DR: AGD-MBRL提出优势引导扩散模型，通过优势估计指导反向扩散过程，解决扩散世界模型在短视界下的myopia问题，提升MBRL的样本效率和最终回报。


<details>
  <summary>Details</summary>
Motivation: 基于模型的强化学习中，自回归世界模型存在误差累积问题，扩散世界模型通过联合生成轨迹段缓解了这一问题。然而，现有的扩散引导方法要么仅使用策略信息（丢弃价值信息），要么基于奖励（在扩散视界较短时变得短视）。需要一种既能利用价值信息又能避免短视的引导方法。

Method: 提出优势引导扩散（AGD-MBRL），使用智能体的优势估计来引导反向扩散过程，使采样集中在预期能产生超出生成窗口的长期回报的轨迹上。开发了两种引导方法：Sigmoid优势引导（SAG）和指数优势引导（EAG）。该方法与PolyGRAD风格架构无缝集成，通过引导状态组件同时保持动作生成策略条件化，且不需要改变扩散训练目标。

Result: 在MuJoCo控制任务（HalfCheetah、Hopper、Walker2D和Reacher）上，AGD-MBRL相比PolyGRAD、在线Diffuser风格奖励引导和模型无关基线（PPO/TRPO），提高了样本效率和最终回报，在某些情况下提升幅度达到2倍。证明了优势感知引导是解决扩散模型MBRL中短视界myopia问题的简单有效方法。

Conclusion: 优势引导扩散（AGD-MBRL）通过使用优势估计指导扩散过程，有效解决了现有扩散引导方法的局限性，在保持与现有架构兼容性的同时，显著提升了基于模型的强化学习性能，为扩散世界模型在强化学习中的应用提供了新的有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 理解人类如何在认知资源约束下进行序列物理规划，解决直觉物理学中模拟与启发式之争以及决策科学中前瞻规划与短视策略之争的孤立状态，揭示物理预测机制与规划策略如何共同适应有限认知资源。

Method: 使用"悬挑塔"建造任务，要求参与者在保持稳定性的同时最大化水平悬挑。通过控制任务复杂性和时间压力，观察物理预测机制（直觉物理引擎模拟 vs CNN视觉启发式）和规划策略（深思熟虑前瞻 vs 浅层视野）的转变。

Result: 发现人类在资源压力下表现出双重转变：早期阶段以直觉物理引擎模拟为主，复杂性增加时转向CNN视觉启发式；时间压力截断深思熟虑前瞻，使规划转向更浅的视野。这种双重转变是先前单机制理论无法预测的。

Conclusion: 研究揭示了一个分层的、资源理性的认知架构，能够灵活地在计算成本与预测保真度之间进行权衡。结果将两个长期存在的争论（模拟vs启发式、短视vs深思熟虑规划）统一为受认知预算重新配置的动态机制库。
...
Method: 在现有智能体流程基础上，引入专门的电影摄影镜头智能体，该智能体整合了递归故事板生成以增强镜头间的叙事连续性，并通过电影语言注入来产生更具表现力、面向电影的镜头设计。

Result: 广泛的定量和定性结果表明，该方法在叙事一致性、动态表现力和感知电影质量方面持续优于现有基线方法。

Conclusion: Camera Artist通过专门的电影摄影镜头智能体和递归故事板生成，有效提升了叙事视频生成的连续性和电影质量，为自动化电影制作提供了更完善的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于语言的药物发现系统存在根本性控制问题：智能体逐步规划，但任务有效性由整个候选集合决定。这导致系统依赖冗长的原始历史和模糊的自我反思，使得故障定位不精确，规划状态噪声增加。

Method: 提出CACM框架，包含协议审计和基于证据的诊断器，通过多模态证据分析协议违规，生成可操作的修复提示。采用静态、动态和纠正三个记忆通道，并在回写前进行压缩，保持持久任务信息同时暴露最相关的故障。

Result: CACM将目标级成功率比最先进基线提高了36.4%，表明可靠的基于语言的药物发现不仅需要更强大的分子工具，还需要更精确的诊断和更经济的智能体状态。

Conclusion: CACM通过精确的集合级诊断和简洁的记忆管理机制，有效解决了药物发现中逐步规划与协议级要求之间的不匹配问题，显著提升了基于语言模型的自主药物发现系统的可靠性。
...
TL;DR: Spatial-Gym：用于评估AI模型空间推理能力的Gymnasium环境，通过2D网格路径寻找任务测试模型在三种设置下的表现，发现当前模型与人类基线存在显著差距。
...
Conclusion: 模型未能根据难度扩展推理努力；接收空间环境图像的视觉模型解决率降低73%；扩展思维链推理在逐步设置中仍保持3-5倍准确度优势。Spatial-Gym能够诊断模型局限性，并为通过强化学习改进空间推理提供框架。
...
<details>
  <summary>Details</summary>
Motivation: 传统多目标贝叶斯优化方法在目标数增多时难以有效近似整个帕累托前沿，而实际部署时决策者只需选择一个解，因此在有限评估预算下应专注于寻找单个最高质量解。

Method: 提出SPMO框架，包含ESPI采集函数，适用于无噪声和有噪声场景，通过SAA方法结合梯度优化，并提供了理论收敛保证。
...
TL;DR: E3-TIR提出了一种新的工具集成推理训练范式，通过动态整合专家前缀、专家引导和自主探索三种经验类型，有效解决了现有训练方法在探索效率和能力瓶颈方面的问题。


<details>
  <summary>Details</summary>
Motivation: 现有工具集成推理的训练范式存在显著局限性：Zero-RL方法由于缺乏先验指导导致探索效率低下和模式退化，而SFT-then-RL方法则受限于高数据成本和低熵崩溃导致的能力瓶颈。需要一种能够平衡探索多样性和训练效率的新训练范式。

Method: E3-TIR将训练过程形式化为三种经验类型的动态整合：专家前缀、专家引导和自主探索。通过在专家"锚点"周围执行多样化的分支探索，并采用混合策略优化机制，有效缓解分布偏移并解决共享前缀带来的优化冲突。该方法能够动态调整模型的知识边界，平衡探索多样性与训练效率。

Result: 实验结果表明，E3-TIR在工具使用任务上比传统范式提升了6%的性能，同时仅需要不到10%的合成数据。在综合考虑性能、数据成本和训练效率的ROI指标上，相比基线获得了1.46倍的增益。

Conclusion: E3-TIR通过动态整合多种经验类型，有效解决了现有训练范式的局限性，在保持高性能的同时显著降低了数据需求和训练成本，为工具集成推理的训练提供了新的有效范式。
